...
log using "log CovidLongitudinal Turkey 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Turkey 1 Process.do"
...
**** "do CovidLongitudinal Turkey 1 Process.do"
...
** output data files: "`model' Turkey.dta"
**                   "ALL MODLES Turkey.dta"

** output data dictionary files: "Turkey ALL MODELS update dates data dictionary.csv"
**                               "`model' Turkey data dictionary.csv"
**                               "ALL MODLES Turkey data dictionary.csv"

** graphs:
** "graph 02 Turkey `model' C19 daily deaths all updates.pdf"
** "graph 02 Turkey ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Turkey absent update_date_block"
	
	qui keep if loc_grand_name == "Turkey"
	...
	save "Turkey `model' update dates.dta", replace
...
use "Turkey DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Turkey `model' update dates.dta"
...
save "Turkey ALL MODELS update dates.dta", replace
	
export excel using "Turkey ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Turkey ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Turkey"

keep date DayDeaMeSmJOHNTUR
...
save "JOHN Turkey.dta", replace 




* graph Turkey JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNTUR date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Turkey, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Turkey JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Turkey.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Turkey.dta"
...
merge m:m date using "JOHN Turkey.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Turkey data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Turkey DELP daily deaths each update 
	...
	subtitle("Turkey, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Turkey DELP daily deaths update `update'.pdf", replace	
...
* graph Turkey DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Turkey, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Turkey DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Turkey IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Turkey, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Turkey IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Turkey.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Turkey.dta"
...
merge m:1 date using "JOHN Turkey.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Turkey data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Turkey IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Turkey, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Turkey IHME daily deaths update `update'.pdf", replace	
...
* graph Turkey IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Turkey, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Turkey IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Turkey.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Turkey.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Turkey data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Turkey IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Turkey, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Turkey IMPE daily deaths update `update'.pdf", replace	
...
* graph Turkey IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Turkey, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Turkey IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Turkey.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Turkey.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Turkey data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Turkey LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Turkey, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Turkey LANL daily deaths update `update'.pdf", replace	
...
* graph Turkey LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Turkey, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Turkey LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Turkey.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Turkey.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Turkey.dta"
...
preserve
    ...
    export delimited name varlab using "UCLA Turkey data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Turkey UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Turkey, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Turkey UCLA daily deaths update `update'.pdf", replace	

}
*/


* graph Turkey UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Turkey, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Turkey UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Turkey.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Turkey.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Turkey.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Turkey data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Turkey YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Turkey, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Turkey YYGU daily deaths update `update'.pdf", replace	
...
* graph Turkey YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Turkey, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Turkey YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Turkey.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Turkey.dta", force
...
save "ALL MODLES Turkey.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Turkey data dictionary.csv", replace 
...
* graph Turkey ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Turkey, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(886 "JOHN" 11 "DELP" 58 "IHME" 25 "IMPE" 22 "LANL" 405 "UCLA" 1 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Turkey ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Turkey 1 Process.smcl"
