...
log using "log CovidLongitudinal Malta 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Malta 1 Process.do"
...
**** "do CovidLongitudinal Malta 1 Process.do"
...
** output data files: "`model' Malta.dta"
**                   "ALL MODLES Malta.dta"

** output data dictionary files: "Malta ALL MODELS update dates data dictionary.csv"
**                               "`model' Malta data dictionary.csv"
**                               "ALL MODLES Malta data dictionary.csv"

** graphs:
** "graph 02 Malta `model' C19 daily deaths all updates.pdf"
** "graph 02 Malta ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Malta absent update_date_block"
	
	qui keep if loc_grand_name == "Malta"
	...
	save "Malta `model' update dates.dta", replace
...
use "Malta DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Malta `model' update dates.dta"
...
save "Malta ALL MODELS update dates.dta", replace
	
export excel using "Malta ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Malta ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Malta"

keep date DayDeaMeSmJOHNMLT
...
save "JOHN Malta.dta", replace 




* graph Malta JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMLT date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Malta, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malta JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Malta.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Malta.dta"
...
merge m:m date using "JOHN Malta.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Malta data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Malta DELP daily deaths each update 
	...
	subtitle("Malta, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Malta DELP daily deaths update `update'.pdf", replace	
...
* graph Malta DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malta, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malta DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Malta IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Malta, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Malta IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Malta.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Malta.dta"
...
merge m:1 date using "JOHN Malta.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Malta data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Malta IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Malta, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Malta IHME daily deaths update `update'.pdf", replace	
...
* graph Malta IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malta, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malta IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Malta.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Malta.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Malta data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Malta IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Malta, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Malta IMPE daily deaths update `update'.pdf", replace	
...
* graph Malta IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malta, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malta IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Malta.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Malta.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Malta data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Malta LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Malta, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Malta LANL daily deaths update `update'.pdf", replace	
...
* graph Malta LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malta, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malta LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Malta.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Malta.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Malta.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Malta data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Malta YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Malta, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Malta YYGU daily deaths update `update'.pdf", replace	
...
* graph Malta YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malta, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malta YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Malta.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Malta.dta", force
...
save "ALL MODLES Malta.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Malta data dictionary.csv", replace 
...
* graph Malta ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malta, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(778 "JOHN" 15 "DELP" 1 "IHME" 70 "IMPE" 240 "LANL" 2 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malta ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Malta 1 Process.smcl"
