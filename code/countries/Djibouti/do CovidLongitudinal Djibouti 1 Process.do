...
log using "log CovidLongitudinal Djibouti 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Djibouti 1 Process.do"
...
**** "do CovidLongitudinal Djibouti 1 Process.do"
...
** output data files: "`model' Djibouti.dta"
**                   "ALL MODLES Djibouti.dta"

** output data dictionary files: "Djibouti ALL MODELS update dates data dictionary.csv"
**                               "`model' Djibouti data dictionary.csv"
**                               "ALL MODLES Djibouti data dictionary.csv"

** graphs:
** "graph 02 Djibouti `model' C19 daily deaths all updates.pdf"
** "graph 02 Djibouti ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Djibouti absent update_date_block"
	
	qui keep if loc_grand_name == "Djibouti"
	...
	save "Djibouti `model' update dates.dta", replace
...
use "Djibouti DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Djibouti `model' update dates.dta"
...
save "Djibouti ALL MODELS update dates.dta", replace
	
export excel using "Djibouti ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Djibouti ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Djibouti"

keep date DayDeaMeSmJOHNDJI
...
save "JOHN Djibouti.dta", replace 




* graph Djibouti JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNDJI date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Djibouti, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Djibouti JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Djibouti.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Djibouti.dta"
...
save "DELP Djibouti.dta", replace
...
merge m:m date using "JOHN Djibouti.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Djibouti data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Djibouti DELP daily deaths each update 
	...
	subtitle("Djibouti, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Djibouti DELP daily deaths update `update'.pdf", replace	
...
* graph Djibouti DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Djibouti, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Djibouti DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Djibouti IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Djibouti, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Djibouti IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Djibouti.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Djibouti.dta"
...
save "IHME Djibouti.dta", replace
...
merge m:1 date using "JOHN Djibouti.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Djibouti data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Djibouti IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Djibouti, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Djibouti IHME daily deaths update `update'.pdf", replace	
...
* graph Djibouti IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Djibouti, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Djibouti IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Djibouti.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Djibouti.dta"
...
save "IMPE Djibouti.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Djibouti data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Djibouti IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Djibouti, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Djibouti IMPE daily deaths update `update'.pdf", replace	
...
* graph Djibouti IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Djibouti, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Djibouti IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Djibouti.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Djibouti.dta"
...
save "LANL Djibouti.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Djibouti data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Djibouti LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Djibouti, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Djibouti LANL daily deaths update `update'.pdf", replace	
...
* graph Djibouti LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Djibouti, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Djibouti LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Djibouti.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Djibouti.dta", force
...
save "ALL MODLES Djibouti.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Djibouti data dictionary.csv", replace 
...
* graph Djibouti ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Djibouti, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(658 "JOHN" 1 "DELP" 63 "IHME" 3 "IMPE" 41 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Djibouti ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Djibouti 1 Process.smcl"
