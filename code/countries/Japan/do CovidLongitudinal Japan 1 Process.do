...
log using "log CovidLongitudinal Japan 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Japan 1 Process.do"
...
**** "do CovidLongitudinal Japan 1 Process.do"
...
** output data files: "`model' Japan.dta"
**                   "ALL MODLES Japan.dta"

** output data dictionary files: "Japan ALL MODELS update dates data dictionary.csv"
**                               "`model' Japan data dictionary.csv"
**                               "ALL MODLES Japan data dictionary.csv"

** graphs:
** "graph 02 Japan `model' C19 daily deaths all updates.pdf"
** "graph 02 Japan ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Japan absent update_date_block"
	
	qui keep if loc_grand_name == "Japan"
	...
	save "Japan `model' update dates.dta", replace
...
use "Japan DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Japan `model' update dates.dta"
...
save "Japan ALL MODELS update dates.dta", replace
	
export excel using "Japan ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Japan ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Japan"

keep date DayDeaMeSmJOHNJPN
...
save "JOHN Japan.dta", replace 




* graph Japan JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNJPN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Japan, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Japan JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Japan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Japan.dta"
...
merge m:m date using "JOHN Japan.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Japan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Japan DELP daily deaths each update 
	...
	subtitle("Japan, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Japan DELP daily deaths update `update'.pdf", replace	
...
* graph Japan DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Japan, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Japan DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Japan IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Japan, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Japan IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Japan.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Japan.dta"
...
merge m:1 date using "JOHN Japan.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Japan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Japan IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Japan, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Japan IHME daily deaths update `update'.pdf", replace	
...
* graph Japan IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Japan, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Japan IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Japan.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Japan.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Japan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Japan IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Japan, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Japan IMPE daily deaths update `update'.pdf", replace	
...
* graph Japan IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Japan, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Japan IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Japan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Japan.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Japan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Japan LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Japan, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Japan LANL daily deaths update `update'.pdf", replace	
...
* graph Japan LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Japan, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Japan LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Japan.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Japan.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Japan.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Japan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Japan YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Japan, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Japan YYGU daily deaths update `update'.pdf", replace	
...
* graph Japan YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Japan, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Japan YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Japan.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Japan.dta", force
...
save "ALL MODLES Japan.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Japan data dictionary.csv", replace 
...
* graph Japan ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Japan, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(784 "JOHN" 1 "DELP" 30 "IHME" 41 "IMPE" 3 "LANL" 10 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Japan ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Japan 1 Process.smcl"
