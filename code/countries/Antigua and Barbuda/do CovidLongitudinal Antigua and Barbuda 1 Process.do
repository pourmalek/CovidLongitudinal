...
log using "log CovidLongitudinal Antigua and Barbuda 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Antigua and Barbuda 1 Process.do"
...
**** "do CovidLongitudinal Antigua and Barbuda 1 Process.do"
...
** output data files: "`model' Antigua and Barbuda.dta"
**                   "ALL MODLES Antigua and Barbuda.dta"

** output data dictionary files: "Antigua and Barbuda ALL MODELS update dates data dictionary.csv"
**                               "`model' Antigua and Barbuda data dictionary.csv"
**                               "ALL MODLES Antigua and Barbuda data dictionary.csv"

** graphs:
** "graph 02 Antigua and Barbuda `model' C19 daily deaths all updates.pdf"
** "graph 02 Antigua and Barbuda ALL MODELS C19 daily deaths all updates.pdf"
...
local list DELP IMPE LANL

foreach model of local list {

	use "$pathCovidLongitudinal/download/`model'/`model' countries long.dta", clear 
	
	di in red "`model'" " Antigua and Barbuda absent update_date_block"
	
	qui keep if loc_grand_name == "Antigua and Barbuda"
	...
	save "Antigua and Barbuda `model' update dates.dta", replace
...
use "Antigua and Barbuda DELP update dates.dta", clear 

local list DELP IMPE LANL

foreach model of local list {
	
	append using "Antigua and Barbuda `model' update dates.dta"
...
save "Antigua and Barbuda ALL MODELS update dates.dta", replace
	
export excel using "Antigua and Barbuda ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Antigua and Barbuda ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Antigua and Barbuda"

keep date DayDeaMeSmJOHNATG
...
save "JOHN Antigua and Barbuda.dta", replace 




* graph Antigua and Barbuda JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNATG date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Antigua and Barbuda, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Antigua and Barbuda JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Antigua and Barbuda.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Antigua and Barbuda.dta"
...
save "DELP Antigua and Barbuda.dta", replace
...
merge m:m date using "JOHN Antigua and Barbuda.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Antigua and Barbuda data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Antigua and Barbuda DELP daily deaths each update 
	...
	subtitle("Antigua and Barbuda, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Antigua and Barbuda DELP daily deaths update `update'.pdf", replace	
...
* graph Antigua and Barbuda DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Antigua and Barbuda, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Antigua and Barbuda DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Antigua and Barbuda.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Antigua and Barbuda.dta"
...
save "IMPE Antigua and Barbuda.dta", replace
...
merge m:1 date using "JOHN Antigua and Barbuda.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Antigua and Barbuda data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Antigua and Barbuda IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Antigua and Barbuda, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Antigua and Barbuda IMPE daily deaths update `update'.pdf", replace	
...
* graph Antigua and Barbuda IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Antigua and Barbuda, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Antigua and Barbuda IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Antigua and Barbuda.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Antigua and Barbuda.dta"
...
save "LANL Antigua and Barbuda.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Antigua and Barbuda data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Antigua and Barbuda LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Antigua and Barbuda, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Antigua and Barbuda LANL daily deaths update `update'.pdf", replace	
...
* graph Antigua and Barbuda LANL daily deaths all updates
...
ytitle(Daily deaths) title("C19 daily deaths, Antigua and Barbuda, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Antigua and Barbuda LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Antigua and Barbuda.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Antigua and Barbuda.dta", force
...
save "ALL MODLES Antigua and Barbuda.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Antigua and Barbuda data dictionary.csv", replace 
...
* graph Antigua and Barbuda ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Antigua and Barbuda, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(446 "JOHN" 41 "DELP" 1 "IMPE" 77 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Antigua and Barbuda ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Antigua and Barbuda 1 Process.smcl"
