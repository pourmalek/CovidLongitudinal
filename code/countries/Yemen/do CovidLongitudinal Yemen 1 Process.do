...
log using "log CovidLongitudinal Yemen 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Yemen 1 Process.do"
...
**** "do CovidLongitudinal Yemen 1 Process.do"
...
** output data files: "`model' Yemen.dta"
**                   "ALL MODLES Yemen.dta"

** output data dictionary files: "Yemen ALL MODELS update dates data dictionary.csv"
**                               "`model' Yemen data dictionary.csv"
**                               "ALL MODLES Yemen data dictionary.csv"

** graphs:
** "graph 02 Yemen `model' C19 daily deaths all updates.pdf"
** "graph 02 Yemen ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Yemen absent update_date_block"
	
	qui keep if loc_grand_name == "Yemen"
	...
	save "Yemen `model' update dates.dta", replace
...
use "Yemen DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Yemen `model' update dates.dta"
...
save "Yemen ALL MODELS update dates.dta", replace
	
export excel using "Yemen ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Yemen ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Yemen"

keep date DayDeaMeSmJOHNYEM
...
save "JOHN Yemen.dta", replace 




* graph Yemen JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNYEM date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Yemen, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Yemen JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Yemen.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Yemen.dta"
...
merge m:m date using "JOHN Yemen.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Yemen data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Yemen DELP daily deaths each update 
	...
	subtitle("Yemen, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Yemen DELP daily deaths update `update'.pdf", replace	
...
* graph Yemen DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Yemen, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Yemen DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Yemen IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Yemen, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Yemen IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Yemen.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Yemen.dta"
...
merge m:1 date using "JOHN Yemen.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Yemen data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Yemen IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Yemen, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Yemen IHME daily deaths update `update'.pdf", replace	
...
* graph Yemen IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Yemen, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Yemen IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Yemen.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Yemen.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Yemen data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Yemen IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Yemen, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Yemen IMPE daily deaths update `update'.pdf", replace	
...
* graph Yemen IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Yemen, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Yemen IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Yemen.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Yemen.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Yemen data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Yemen LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Yemen, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Yemen LANL daily deaths update `update'.pdf", replace	
...
* graph Yemen LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Yemen, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Yemen LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Yemen.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Yemen.dta", force
...
save "ALL MODLES Yemen.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Yemen data dictionary.csv", replace 
...
* graph Yemen ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Yemen, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(652 "JOHN" 25 "DELP" 59 "IHME" 1 "IMPE" 18 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Yemen ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Yemen 1 Process.smcl"
