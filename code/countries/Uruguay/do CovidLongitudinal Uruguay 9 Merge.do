...
log using "log CovidLongitudinal Uruguay 9 Merge.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Uruguay 9 Merge.do"
...
** input data files: "`model' Uruguay error.dta"

** output data files: "ALL MODELS Uruguay error.dta" with all date
*                     "ALL MODELS Uruguay error - only models across error measures.dta", replace
*                     "ALL MODELS Uruguay error - only error measures across models.dta", replace

* output data dictionary files: "ALL MODELS Uruguay error data dictionary.csv"
*                               "ALL MODELS Uruguay error - only models across error measures data dictionary.csv"
*                               "ALL MODELS Uruguay error - only error measures across models data dictionary.csv"


** Graphs:
* "graph 03 Uruguay ALL MODELS C19 daily deaths error.pdf"
* "graph 04 Uruguay ALL MODELS C19 daily deaths absolute error.pdf"
* "graph 05 Uruguay ALL MODELS C19 daily deaths percent error.pdf"
* "graph 06 Uruguay ALL MODELS C19 daily deaths absolute percent error.pdf"

* "graph 07a1 Uruguay ALL MODELS C19 daily deaths average median error.pdf"
* "graph 07a2 Uruguay ALL MODELS C19 daily deaths average median error smoothed.pdf"
* "graph 08a1 Uruguay ALL MODELS C19 daily deaths average median absolute error.pdf"
* "graph 08a2 Uruguay ALL MODELS C19 daily deaths average median absolute error smoothed.pdf"
* "graph 09a1 Uruguay ALL MODELS C19 daily deaths average median percent error.pdf"
* "graph 09a2 Uruguay ALL MODELS C19 daily deaths average median percent error smoothed.pdf"
* "graph 10a1 Uruguay ALL MODELS C19 daily deaths average median absolute percent error.pdf"
* "graph 10a2 Uruguay ALL MODELS C19 daily deaths average median absolute percent error smoothed.pdf"

****** graph each same error measure across models:
* "graph 12a Uruguay ALL MODELS C19 daily deaths Average Mean Error.pdf"
* "graph 12b Uruguay ALL MODELS C19 daily deaths Average Absolute Error.pdf"
* "graph 12c Uruguay ALL MODELS C19 daily deaths Average Percent Error.pdf"
* "graph 12d Uruguay ALL MODELS C19 daily deaths Average Absolute Percent Error.pdf"

****** graph all models across error measures:
* "graph 13a Uruguay ALL MODELS C19 daily deaths, models across error measures.pdf"

****** graph all error measures across models:
* "graph 13b Uruguay ALL MODELS C19 daily deaths, error measures across models.pdf"
...
use "DELP Uruguay error.dta", clear 

	merge m:m date using "IHME Uruguay error.dta"

	drop _merge
	
	merge m:m date using "IMPE Uruguay error.dta"
	
	drop _merge
	
	merge m:m date using "LANL Uruguay error.dta"	
...
subtitle("Uruguay, all Models, all updates, forecast only", size(small)) ///
legend(position(6) order(1 "DELP" 35 "IHME" 12 "IMPE" 6 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 03 Uruguay ALL MODELS C19 daily deaths error.pdf", replace
...
qui graph export "graph 04 Uruguay ALL MODELS C19 daily deaths absolute error.pdf", replace
...
qui graph export "graph 05 Uruguay ALL MODELS C19 daily deaths percent error.pdf", replace
...
qui graph export "graph 06 Uruguay ALL MODELS C19 daily deaths absolute percent error.pdf", replace
...
subtitle("Uruguay, all Models, forecast only", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) yscale(titlegap(3)) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 07a1 Uruguay ALL MODELS C19 daily deaths average median error.pdf", replace 
...
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths error measure) title("C19 daily deaths, average median error", size(medium) color(black)) /// 
subtitle("Uruguay, all Models, forecast only; smoothed for better viewing", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) yscale(titlegap(3)) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 07a2 Uruguay ALL MODELS C19 daily deaths average median error smoothed.pdf", replace
...
qui graph export "graph 08a1 Uruguay ALL MODELS C19 daily deaths average median absolute error.pdf", replace 
...
qui graph export "graph 08a2 Uruguay ALL MODELS C19 daily deaths average median absolute error smoothed.pdf", replace
...
qui graph export "graph 09a1 Uruguay ALL MODELS C19 daily deaths average median percent error.pdf", replace
...
qui graph export "graph 09a2 Uruguay ALL MODELS C19 daily deaths average median percent error smoothed.pdf", replace
...
qui graph export "graph 10a1 Uruguay ALL MODELS C19 daily deaths average median absolute percent error.pdf", replace 
...
qui graph export "graph 10a2 Uruguay ALL MODELS C19 daily deaths average median absolute percent error smoothed.pdf", replace
...
subtitle("Over updates and calendar months; forecast only; Uruguay", size(small)) /// 
legend(region(lcolor(none))) legend(bexpand) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL") size(small) row(1)) 

qui graph export "graph 12a Uruguay ALL MODELS C19 daily deaths Average Mean Error.pdf", replace
...
qui graph export "graph 12b Uruguay ALL MODELS C19 daily deaths Average Absolute Error.pdf", replace
...
qui graph export "graph 12c Uruguay ALL MODELS C19 daily deaths Average Percent Error.pdf", replace
...
qui graph export "graph 12d Uruguay ALL MODELS C19 daily deaths Average Absolute Percent Error.pdf", replace
...
save "ALL MODELS Uruguay error.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Uruguay error data dictionary.csv", replace 
...
use "ALL MODELS Uruguay error.dta", clear

save "ALL MODELS Uruguay error - only models across error measures.dta", replace
...
save "CovidLongitudinal Uruguay merge temp.dta", replace 
...
qui graph export "graph 13a Uruguay ALL MODELS C19 daily deaths, models across error measures.pdf", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Uruguay error - only models across error measures data dictionary.csv", replace 
...
use "CovidLongitudinal Uruguay merge temp.dta", clear 
...
qui graph export "graph 13b Uruguay ALL MODELS C19 daily deaths, error measures across models.pdf", replace
...
save "ALL MODELS Uruguay error - only error measures across models.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Uruguay error - only error measures across models data dictionary.csv", replace 
...
view "log CovidLongitudinal Uruguay 9 Merge.smcl"
