...
log using "log CovidLongitudinal Estonia 9 Merge.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Estonia 9 Merge.do"
...
** input data files: "`model' Estonia error.dta"

** output data files: "ALL MODELS Estonia error.dta" with all date
*                     "ALL MODELS Estonia error - only models across error measures.dta", replace
*                     "ALL MODELS Estonia error - only error measures across models.dta", replace

* output data dictionary files: "ALL MODELS Estonia error data dictionary.csv"
*                               "ALL MODELS Estonia error - only models across error measures data dictionary.csv"
*                               "ALL MODELS Estonia error - only error measures across models data dictionary.csv"


** Graphs:
* "graph 03 Estonia ALL MODELS C19 daily deaths error.pdf"
* "graph 04 Estonia ALL MODELS C19 daily deaths absolute error.pdf"
* "graph 05 Estonia ALL MODELS C19 daily deaths percent error.pdf"
* "graph 06 Estonia ALL MODELS C19 daily deaths absolute percent error.pdf"

* "graph 07a1 Estonia ALL MODELS C19 daily deaths average median error.pdf"
* "graph 07a2 Estonia ALL MODELS C19 daily deaths average median error smoothed.pdf"
* "graph 08a1 Estonia ALL MODELS C19 daily deaths average median absolute error.pdf"
* "graph 08a2 Estonia ALL MODELS C19 daily deaths average median absolute error smoothed.pdf"
* "graph 09a1 Estonia ALL MODELS C19 daily deaths average median percent error.pdf"
* "graph 09a2 Estonia ALL MODELS C19 daily deaths average median percent error smoothed.pdf"
* "graph 10a1 Estonia ALL MODELS C19 daily deaths average median absolute percent error.pdf"
* "graph 10a2 Estonia ALL MODELS C19 daily deaths average median absolute percent error smoothed.pdf"

****** graph each same error measure across models:
* "graph 12a Estonia ALL MODELS C19 daily deaths Average Mean Error.pdf"
* "graph 12b Estonia ALL MODELS C19 daily deaths Average Absolute Error.pdf"
* "graph 12c Estonia ALL MODELS C19 daily deaths Average Percent Error.pdf"
* "graph 12d Estonia ALL MODELS C19 daily deaths Average Absolute Percent Error.pdf"

****** graph all models across error measures:
* "graph 13a Estonia ALL MODELS C19 daily deaths, models across error measures.pdf"

****** graph all error measures across models:
* "graph 13b Estonia ALL MODELS C19 daily deaths, error measures across models.pdf"
...
use "DELP Estonia error.dta", clear 

	merge m:m date using "IHME Estonia error.dta"

	drop _merge
	
	merge m:m date using "IMPE Estonia error.dta"
	
	drop _merge
	
	merge m:m date using "LANL Estonia error.dta"	
	
	drop _merge
	
	merge m:m date using "YYGU Estonia error.dta", force	
...
subtitle("Estonia, all Models, all updates, forecast only", size(small)) ///
legend(position(6) order(15 "DELP" 1 "IHME" 91 "IMPE" 29 "LANL" 2 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 03 Estonia ALL MODELS C19 daily deaths error.pdf", replace
...
qui graph export "graph 04 Estonia ALL MODELS C19 daily deaths absolute error.pdf", replace
...
qui graph export "graph 05 Estonia ALL MODELS C19 daily deaths percent error.pdf", replace
...
qui graph export "graph 06 Estonia ALL MODELS C19 daily deaths absolute percent error.pdf", replace
...
subtitle("Estonia, all Models, forecast only", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) yscale(titlegap(3)) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL" 5 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 07a1 Estonia ALL MODELS C19 daily deaths average median error.pdf", replace 
...
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths error measure) title("C19 daily deaths, average median error", size(medium) color(black)) /// 
subtitle("Estonia, all Models, forecast only; smoothed for better viewing", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) yscale(titlegap(3)) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL" 5 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 07a2 Estonia ALL MODELS C19 daily deaths average median error smoothed.pdf", replace
...
qui graph export "graph 08a1 Estonia ALL MODELS C19 daily deaths average median absolute error.pdf", replace 
...
qui graph export "graph 08a2 Estonia ALL MODELS C19 daily deaths average median absolute error smoothed.pdf", replace
...
qui graph export "graph 09a1 Estonia ALL MODELS C19 daily deaths average median percent error.pdf", replace
...
qui graph export "graph 09a2 Estonia ALL MODELS C19 daily deaths average median percent error smoothed.pdf", replace
...
qui graph export "graph 10a1 Estonia ALL MODELS C19 daily deaths average median absolute percent error.pdf", replace 
...
qui graph export "graph 10a2 Estonia ALL MODELS C19 daily deaths average median absolute percent error smoothed.pdf", replace
...
subtitle("Over updates and calendar months; forecast only; Estonia", size(small)) /// 
legend(region(lcolor(none))) legend(bexpand) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL" 5 "YYGU") size(small) row(1)) 

qui graph export "graph 12a Estonia ALL MODELS C19 daily deaths Average Mean Error.pdf", replace
...
qui graph export "graph 12b Estonia ALL MODELS C19 daily deaths Average Absolute Error.pdf", replace
...
qui graph export "graph 12c Estonia ALL MODELS C19 daily deaths Average Percent Error.pdf", replace
...
qui graph export "graph 12d Estonia ALL MODELS C19 daily deaths Average Absolute Percent Error.pdf", replace
...
save "ALL MODELS Estonia error.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Estonia error data dictionary.csv", replace 
...
use "ALL MODELS Estonia error.dta", clear

save "ALL MODELS Estonia error - only models across error measures.dta", replace
...
save "CovidLongitudinal Estonia merge temp.dta", replace 
...
qui graph export "graph 13a Estonia ALL MODELS C19 daily deaths, models across error measures.pdf", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Estonia error - only models across error measures data dictionary.csv", replace 
...
use "CovidLongitudinal Estonia merge temp.dta", clear 
...
qui graph export "graph 13b Estonia ALL MODELS C19 daily deaths, error measures across models.pdf", replace
...
save "ALL MODELS Estonia error - only error measures across models.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Estonia error - only error measures across models data dictionary.csv", replace 
...
view "log CovidLongitudinal Estonia 9 Merge.smcl"
