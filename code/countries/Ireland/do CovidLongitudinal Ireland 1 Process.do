...
log using "log CovidLongitudinal Ireland 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Ireland 1 Process.do"
...
**** "do CovidLongitudinal Ireland 1 Process.do"
...
** output data files: "`model' Ireland.dta"
**                   "ALL MODLES Ireland.dta"

** output data dictionary files: "Ireland ALL MODELS update dates data dictionary.csv"
**                               "`model' Ireland data dictionary.csv"
**                               "ALL MODLES Ireland data dictionary.csv"

** graphs:
** "graph 02 Ireland `model' C19 daily deaths all updates.pdf"
** "graph 02 Ireland ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Ireland absent update_date_block"
	
	qui keep if loc_grand_name == "Ireland"
	...
	save "Ireland `model' update dates.dta", replace
...
use "Ireland DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Ireland `model' update dates.dta"
...
save "Ireland ALL MODELS update dates.dta", replace
	
export excel using "Ireland ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Ireland ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Ireland"

keep date DayDeaMeSmJOHNIRL
...
save "JOHN Ireland.dta", replace 




* graph Ireland JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNIRL date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Ireland, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ireland JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Ireland.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Ireland.dta"
...
merge m:m date using "JOHN Ireland.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Ireland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ireland DELP daily deaths each update 
	...
	subtitle("Ireland, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ireland DELP daily deaths update `update'.pdf", replace	
...
* graph Ireland DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ireland, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ireland DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Ireland IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Ireland, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Ireland IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Ireland.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Ireland.dta"
...
merge m:1 date using "JOHN Ireland.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Ireland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ireland IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ireland, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ireland IHME daily deaths update `update'.pdf", replace	
...
* graph Ireland IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ireland, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ireland IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Ireland.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Ireland.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Ireland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ireland IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ireland, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ireland IMPE daily deaths update `update'.pdf", replace	
...
* graph Ireland IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ireland, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ireland IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Ireland.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Ireland.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Ireland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ireland LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ireland, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ireland LANL daily deaths update `update'.pdf", replace	
...
* graph Ireland LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ireland, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ireland LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Ireland.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Ireland.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Ireland.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Ireland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ireland YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ireland, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ireland YYGU daily deaths update `update'.pdf", replace	
...
* graph Ireland YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ireland, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ireland YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Ireland.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Ireland.dta", force
...
save "ALL MODLES Ireland.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Ireland data dictionary.csv", replace 
...
* graph Ireland ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ireland, all models, all updates, forecast only", size(medium) color(black)) ///
...
qui graph export "graph 02 Ireland ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Ireland 1 Process.smcl"
