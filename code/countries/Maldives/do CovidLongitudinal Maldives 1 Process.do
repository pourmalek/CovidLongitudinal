...
log using "log CovidLongitudinal Maldives 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Maldives 1 Process.do"
...
**** "do CovidLongitudinal Maldives 1 Process.do"
...
** output data files: "`model' Maldives.dta"
**                   "ALL MODLES Maldives.dta"

** output data dictionary files: "Maldives ALL MODELS update dates data dictionary.csv"
**                               "`model' Maldives data dictionary.csv"
**                               "ALL MODLES Maldives data dictionary.csv"

** graphs:
** "graph 02 Maldives `model' C19 daily deaths all updates.pdf"
** "graph 02 Maldives ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Maldives absent update_date_block"
	
	qui keep if loc_grand_name == "Maldives"
	...
	save "Maldives `model' update dates.dta", replace
...
use "Maldives DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Maldives `model' update dates.dta"
...
save "Maldives ALL MODELS update dates.dta", replace
	
export excel using "Maldives ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Maldives ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Maldives"

keep date DayDeaMeSmJOHNMDV
...
save "JOHN Maldives.dta", replace 




* graph Maldives JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMDV date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Maldives, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Maldives JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Maldives.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Maldives.dta"
...
save "DELP Maldives.dta", replace
...
merge m:m date using "JOHN Maldives.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Maldives data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Maldives DELP daily deaths each update 
	...
	subtitle("Maldives, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Maldives DELP daily deaths update `update'.pdf", replace	
...
* graph Maldives DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Maldives, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Maldives DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Maldives IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Maldives, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Maldives IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Maldives.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Maldives.dta"
...
save "IHME Maldives.dta", replace
...
merge m:1 date using "JOHN Maldives.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Maldives data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Maldives IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Maldives, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Maldives IHME daily deaths update `update'.pdf", replace	
...
* graph Maldives IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Maldives, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Maldives IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Maldives.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Maldives.dta"
...
save "IMPE Maldives.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Maldives data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Maldives IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Maldives, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Maldives IMPE daily deaths update `update'.pdf", replace	
...
* graph Maldives IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Maldives, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Maldives IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Maldives.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Maldives.dta"
...
save "LANL Maldives.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Maldives data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Maldives LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Maldives, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Maldives LANL daily deaths update `update'.pdf", replace	
...
* graph Maldives LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Maldives, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Maldives LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Maldives.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Maldives.dta", force
...
save "ALL MODLES Maldives.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Maldives data dictionary.csv", replace 
...
* graph Maldives ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Maldives, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(631 "JOHN" 1 "DELP" 49 "IHME" 2 "IMPE" 79 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Maldives ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Maldives 1 Process.smcl"
