...
log using "log CovidLongitudinal Tanzania 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Tanzania 1 Process.do"
...
**** "do CovidLongitudinal Tanzania 1 Process.do"
...
** output data files: "`model' Tanzania.dta"
**                   "ALL MODLES Tanzania.dta"

** output data dictionary files: "Tanzania ALL MODELS update dates data dictionary.csv"
**                               "`model' Tanzania data dictionary.csv"
**                               "ALL MODLES Tanzania data dictionary.csv"

** graphs:
** "graph 02 Tanzania `model' C19 daily deaths all updates.pdf"
** "graph 02 Tanzania ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Tanzania absent update_date_block"
	
	qui keep if loc_grand_name == "Tanzania"
	...
	save "Tanzania `model' update dates.dta", replace
...
use "Tanzania DELP update dates.dta", clear 

local list DELP IMPE LANL

foreach model of local list {
	
	append using "Tanzania `model' update dates.dta"
...
save "Tanzania ALL MODELS update dates.dta", replace
	
export excel using "Tanzania ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Tanzania ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Tanzania"

keep date DayDeaMeSmJOHNTZA
...
save "JOHN Tanzania.dta", replace 




* graph Tanzania JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNTZA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Tanzania, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tanzania JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Tanzania.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Tanzania.dta"
...
save "DELP Tanzania.dta", replace
...
merge m:m date using "JOHN Tanzania.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Tanzania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Tanzania DELP daily deaths each update 
	...
	subtitle("Tanzania, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Tanzania DELP daily deaths update `update'.pdf", replace	
...
* graph Tanzania DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tanzania, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tanzania DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Tanzania.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Tanzania.dta"
...
save "IMPE Tanzania.dta", replace
...
merge m:1 date using "JOHN Tanzania.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Tanzania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Tanzania IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Tanzania, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Tanzania IMPE daily deaths update `update'.pdf", replace	
...
* graph Tanzania IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tanzania, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tanzania IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Tanzania.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Tanzania.dta"
...
save "LANL Tanzania.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Tanzania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Tanzania LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Tanzania, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Tanzania LANL daily deaths update `update'.pdf", replace	
...
* graph Tanzania LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tanzania, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tanzania LANL C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Tanzania.dta", force
...
save "ALL MODLES Tanzania.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Tanzania data dictionary.csv", replace 
...
* graph Tanzania ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tanzania, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(586 "JOHN" 4 "DELP" 1 "IMPE" 16 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tanzania ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Tanzania 1 Process.smcl"
