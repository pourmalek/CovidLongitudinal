...
log using "log CovidLongitudinal Brazil 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Brazil 1 Process.do"
...
**** "do CovidLongitudinal Brazil 1 Process.do"
...
** output data files: "`model' Brazil.dta"
**                   "ALL MODLES Brazil.dta"

** output data dictionary files: "Brazil ALL MODELS update dates data dictionary.csv"
**                               "`model' Brazil data dictionary.csv"
**                               "ALL MODLES Brazil data dictionary.csv"

** graphs:
** "graph 02 Brazil `model' C19 daily deaths all updates.pdf"
** "graph 02 Brazil ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Brazil absent update_date_block"
	
	qui keep if loc_grand_name == "Brazil"
	...
	save "Brazil `model' update dates.dta", replace
...
use "Brazil DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Brazil `model' update dates.dta"
...
save "Brazil ALL MODELS update dates.dta", replace
	
export excel using "Brazil ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Brazil ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Brazil"

keep date DayDeaMeSmJOHNBRA
...
save "JOHN Brazil.dta", replace 




* graph Brazil JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNBRA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Brazil, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Brazil JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Brazil.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Brazil.dta"
...
merge m:m date using "JOHN Brazil.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Brazil data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Brazil DELP daily deaths each update 
	...
	subtitle("Brazil, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Brazil DELP daily deaths update `update'.pdf", replace	
...
* graph Brazil DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Brazil, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Brazil DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Brazil IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Brazil, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Brazil IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Brazil.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Brazil.dta"
...
merge m:1 date using "JOHN Brazil.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Brazil data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Brazil IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Brazil, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Brazil IHME daily deaths update `update'.pdf", replace	
...
* graph Brazil IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Brazil, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Brazil IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Brazil.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Brazil.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Brazil data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Brazil IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Brazil, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Brazil IMPE daily deaths update `update'.pdf", replace	
...
* graph Brazil IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Brazil, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Brazil IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Brazil.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Brazil.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Brazil data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Brazil LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Brazil, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Brazil LANL daily deaths update `update'.pdf", replace	
...
* graph Brazil LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Brazil, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Brazil LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Brazil.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Brazil.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Brazil.dta"
...
preserve
    ...
    export delimited name varlab using "UCLA Brazil data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Brazil UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Brazil, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Brazil UCLA daily deaths update `update'.pdf", replace	

}
*/


* graph Brazil UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Brazil, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Brazil UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Brazil.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Brazil.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Brazil.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Brazil data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Brazil YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Brazil, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Brazil YYGU daily deaths update `update'.pdf", replace	
...
* graph Brazil YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Brazil, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Brazil YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Brazil.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Brazil.dta", force
...
save "ALL MODLES Brazil.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Brazil data dictionary.csv", replace 
...
* graph Brazil ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Brazil, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(887 "JOHN" 11 "DELP" 59 "IHME" 25 "IMPE" 22 "LANL" 406 "UCLA" 1 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Brazil ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Brazil 1 Process.smcl"
