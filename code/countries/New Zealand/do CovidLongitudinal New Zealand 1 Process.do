...
log using "log CovidLongitudinal New Zealand 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal New Zealand 1 Process.do"
...
**** "do CovidLongitudinal New Zealand 1 Process.do"
...
** output data files: "`model' New Zealand.dta"
**                   "ALL MODLES New Zealand.dta"

** output data dictionary files: "New Zealand ALL MODELS update dates data dictionary.csv"
**                               "`model' New Zealand data dictionary.csv"
**                               "ALL MODLES New Zealand data dictionary.csv"

** graphs:
** "graph 02 New Zealand `model' C19 daily deaths all updates.pdf"
** "graph 02 New Zealand ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " New Zealand absent update_date_block"
	
	qui keep if loc_grand_name == "New Zealand"
	...
	save "New Zealand `model' update dates.dta", replace
...
use "New Zealand DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "New Zealand `model' update dates.dta"
...
save "New Zealand ALL MODELS update dates.dta", replace
	
export excel using "New Zealand ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "New Zealand ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "New Zealand"

keep date DayDeaMeSmJOHNNZL
...
save "JOHN New Zealand.dta", replace 




* graph New Zealand JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNNZL date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, New Zealand, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 New Zealand JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' New Zealand.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' New Zealand.dta"
...
save "DELP New Zealand.dta", replace
...
merge m:m date using "JOHN New Zealand.dta"
...
preserve
    ...
    export delimited name varlab using "DELP New Zealand data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph New Zealand DELP daily deaths each update 
	...
	subtitle("New Zealand, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 New Zealand DELP daily deaths update `update'.pdf", replace	
...
* graph New Zealand DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, New Zealand, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 New Zealand DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph New Zealand IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, New Zealand, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 New Zealand IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' New Zealand.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' New Zealand.dta"
...
save "IHME New Zealand.dta", replace
...
merge m:1 date using "JOHN New Zealand.dta"
...
preserve
    ...
    export delimited name varlab using "IHME New Zealand data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph New Zealand IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("New Zealand, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 New Zealand IHME daily deaths update `update'.pdf", replace	
...
* graph New Zealand IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, New Zealand, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 New Zealand IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' New Zealand.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' New Zealand.dta"
...
save "IMPE New Zealand.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE New Zealand data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph New Zealand IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("New Zealand, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 New Zealand IMPE daily deaths update `update'.pdf", replace	
...
* graph New Zealand IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, New Zealand, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 New Zealand IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' New Zealand.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' New Zealand.dta"
...
save "LANL New Zealand.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL New Zealand data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph New Zealand LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("New Zealand, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 New Zealand LANL daily deaths update `update'.pdf", replace	
...
* graph New Zealand LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, New Zealand, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 New Zealand LANL C19 daily deaths all updates.pdf", replace
...
* UCLA
* New Zealand not included in UCLA
...
* YYGU
* New Zealand not included in YYGU
...
use "DELP New Zealand.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' New Zealand.dta", force
...
save "ALL MODLES New Zealand.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES New Zealand data dictionary.csv", replace 
...
* graph New Zealand ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, New Zealand, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(628 "JOHN" 1 "DELP" 33 "IHME" 16 "IMPE" 4 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 New Zealand ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal New Zealand 1 Process.smcl"
