...
log using "log CovidLongitudinal Belgium 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Belgium 1 Process.do"
...
**** "do CovidLongitudinal Belgium 1 Process.do"
...
** output data files: "`model' Belgium.dta"
**                   "ALL MODLES Belgium.dta"

** output data dictionary files: "Belgium ALL MODELS update dates data dictionary.csv"
**                               "`model' Belgium data dictionary.csv"
**                               "ALL MODLES Belgium data dictionary.csv"

** graphs:
** "graph 02 Belgium `model' C19 daily deaths all updates.pdf"
** "graph 02 Belgium ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Belgium absent update_date_block"
	
	qui keep if loc_grand_name == "Belgium"
	...
	save "Belgium `model' update dates.dta", replace
...
use "Belgium DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Belgium `model' update dates.dta"
...
save "Belgium ALL MODELS update dates.dta", replace
	
export excel using "Belgium ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Belgium ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Belgium"

keep date DayDeaMeSmJOHNBEL
...
save "JOHN Belgium.dta", replace 




* graph Belgium JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNBEL date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Belgium, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belgium JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Belgium.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Belgium.dta"
...
merge m:m date using "JOHN Belgium.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Belgium data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Belgium DELP daily deaths each update 
	...
	subtitle("Belgium, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Belgium DELP daily deaths update `update'.pdf", replace	
...
* graph Belgium DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belgium, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belgium DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Belgium IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Belgium, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Belgium IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Belgium.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Belgium.dta"
...
merge m:1 date using "JOHN Belgium.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Belgium data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Belgium IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Belgium, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Belgium IHME daily deaths update `update'.pdf", replace	
...
* graph Belgium IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belgium, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belgium IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Belgium.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Belgium.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Belgium data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Belgium IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Belgium, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Belgium IMPE daily deaths update `update'.pdf", replace	
...
* graph Belgium IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belgium, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belgium IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Belgium.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Belgium.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Belgium data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Belgium LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Belgium, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Belgium LANL daily deaths update `update'.pdf", replace	
...
* graph Belgium LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belgium, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belgium LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Belgium.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Belgium.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Belgium.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Belgium data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Belgium YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Belgium, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Belgium YYGU daily deaths update `update'.pdf", replace	
...
* graph Belgium YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belgium, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belgium YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Belgium.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Belgium.dta", force
...
save "ALL MODLES Belgium.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Belgium data dictionary.csv", replace 
...
* graph Belgium ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belgium, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(837 "JOHN" 19 "DELP" 5 "IHME" 95 "IMPE" 33 "LANL" 1 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belgium ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Belgium 1 Process.smcl"
