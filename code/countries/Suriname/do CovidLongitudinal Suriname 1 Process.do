...
log using "log CovidLongitudinal Suriname 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Suriname 1 Process.do"
...
**** "do CovidLongitudinal Suriname 1 Process.do"
...
** output data files: "`model' Suriname.dta"
**                   "ALL MODLES Suriname.dta"

** output data dictionary files: "Suriname ALL MODELS update dates data dictionary.csv"
**                               "`model' Suriname data dictionary.csv"
**                               "ALL MODLES Suriname data dictionary.csv"

** graphs:
** "graph 02 Suriname `model' C19 daily deaths all updates.pdf"
** "graph 02 Suriname ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Suriname absent update_date_block"
	
	qui keep if loc_grand_name == "Suriname"
	...
	save "Suriname `model' update dates.dta", replace
...
use "Suriname DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Suriname `model' update dates.dta"
...
save "Suriname ALL MODELS update dates.dta", replace
	
export excel using "Suriname ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Suriname ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Suriname"

keep date DayDeaMeSmJOHNSUR
...
save "JOHN Suriname.dta", replace 




* graph Suriname JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSUR date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Suriname, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Suriname JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Suriname.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Suriname.dta"
...
save "DELP Suriname.dta", replace
...
merge m:m date using "JOHN Suriname.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Suriname data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Suriname DELP daily deaths each update 
	...
	subtitle("Suriname, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Suriname DELP daily deaths update `update'.pdf", replace	
...
* graph Suriname DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Suriname, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Suriname DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Suriname IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Suriname, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Suriname IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Suriname.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Suriname.dta"
...
save "IHME Suriname.dta", replace
...
merge m:1 date using "JOHN Suriname.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Suriname data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Suriname IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Suriname, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Suriname IHME daily deaths update `update'.pdf", replace	
...
* graph Suriname IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Suriname, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Suriname IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Suriname.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Suriname.dta"
...
save "IMPE Suriname.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Suriname data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Suriname IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Suriname, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Suriname IMPE daily deaths update `update'.pdf", replace	
...
* graph Suriname IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Suriname, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Suriname IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Suriname.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Suriname.dta"
...
save "LANL Suriname.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Suriname data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Suriname LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Suriname, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Suriname LANL daily deaths update `update'.pdf", replace	
...
* graph Suriname LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Suriname, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Suriname LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Suriname.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Suriname.dta", force
...
save "ALL MODLES Suriname.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Suriname data dictionary.csv", replace 
...
* graph Suriname ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Suriname, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(637 "JOHN" 43 "DELP" 46 "IHME" 1 "IMPE" 63 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Suriname ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Suriname 1 Process.smcl"
