...
log using "log CovidLongitudinal Syria 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Syria 1 Process.do"
...
**** "do CovidLongitudinal Syria 1 Process.do"
...
** output data files: "`model' Syria.dta"
**                   "ALL MODLES Syria.dta"

** output data dictionary files: "Syria ALL MODELS update dates data dictionary.csv"
**                               "`model' Syria data dictionary.csv"
**                               "ALL MODLES Syria data dictionary.csv"

** graphs:
** "graph 02 Syria `model' C19 daily deaths all updates.pdf"
** "graph 02 Syria ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Syria absent update_date_block"
	
	qui keep if loc_grand_name == "Syria"
	...
	save "Syria `model' update dates.dta", replace
...
use "Syria IHME update dates.dta", clear 

local list IHME IMPE LANL

foreach model of local list {
	
	append using "Syria `model' update dates.dta"
...
save "Syria ALL MODELS update dates.dta", replace
	
export excel using "Syria ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Syria ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Syria"

keep date DayDeaMeSmJOHNSYR
...
save "JOHN Syria.dta", replace 




* graph Syria JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSYR date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Syria, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Syria JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* graph Syria IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Syria, IHME, update `update'", size(medium) color(black)) ///
	...
	*/
	...
	/* qui graph export "graph 00 Syria IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Syria.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Syria.dta"
...
merge m:1 date using "JOHN Syria.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Syria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Syria IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Syria, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Syria IHME daily deaths update `update'.pdf", replace	
...
* graph Syria IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Syria, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Syria IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Syria.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Syria.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Syria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Syria IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Syria, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Syria IMPE daily deaths update `update'.pdf", replace	
...
* graph Syria IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Syria, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Syria IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Syria.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Syria.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Syria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Syria LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Syria, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Syria LANL daily deaths update `update'.pdf", replace	
...
* graph Syria LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Syria, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Syria LANL C19 daily deaths all updates.pdf", replace
...
use "IHME Syria.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Syria.dta", force
...
save "ALL MODLES Syria.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Syria data dictionary.csv", replace 
...
* graph Syria ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Syria, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(330 "JOHN" 45 "IHME" 1 "IMPE" 56 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Syria ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Syria 1 Process.smcl"
