...
log using "log CovidLongitudinal Romania 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Romania 1 Process.do"
...
**** "do CovidLongitudinal Romania 1 Process.do"
...
** output data files: "`model' Romania.dta"
**                   "ALL MODLES Romania.dta"

** output data dictionary files: "Romania ALL MODELS update dates data dictionary.csv"
**                               "`model' Romania data dictionary.csv"
**                               "ALL MODLES Romania data dictionary.csv"

** graphs:
** "graph 02 Romania `model' C19 daily deaths all updates.pdf"
** "graph 02 Romania ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Romania absent update_date_block"
	
	qui keep if loc_grand_name == "Romania"
	...
	save "Romania `model' update dates.dta", replace
...
use "Romania DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Romania `model' update dates.dta"
...
save "Romania ALL MODELS update dates.dta", replace
	
export excel using "Romania ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Romania ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Romania"

keep date DayDeaMeSmJOHNROU
...
save "JOHN Romania.dta", replace 




* graph Romania JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNROU date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Romania, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Romania JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Romania.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Romania.dta"
...
merge m:m date using "JOHN Romania.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Romania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Romania DELP daily deaths each update 
	...
	subtitle("Romania, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Romania DELP daily deaths update `update'.pdf", replace	
...
* graph Romania DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Romania, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Romania DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Romania IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Romania, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Romania IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Romania.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Romania.dta"
...
merge m:1 date using "JOHN Romania.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Romania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Romania IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Romania, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Romania IHME daily deaths update `update'.pdf", replace	
...
* graph Romania IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Romania, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Romania IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Romania.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Romania.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Romania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Romania IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Romania, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Romania IMPE daily deaths update `update'.pdf", replace	
...
* graph Romania IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Romania, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Romania IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Romania.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Romania.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Romania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Romania LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Romania, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Romania LANL daily deaths update `update'.pdf", replace	
...
* graph Romania LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Romania, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Romania LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Romania.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Romania.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Romania.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Romania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Romania YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Romania, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Romania YYGU daily deaths update `update'.pdf", replace	
...
* graph Romania YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Romania, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Romania YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Romania.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Romania.dta", force
...
save "ALL MODLES Romania.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Romania data dictionary.csv", replace 
...
* graph Romania ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Romania, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(871 "JOHN" 15 "DELP" 1 "IHME" 37 "IMPE" 29 "LANL" 2 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Romania ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Romania 1 Process.smcl"
