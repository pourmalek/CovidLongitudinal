...
log using "log CovidLongitudinal Lithuania 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Lithuania 1 Process.do"
...
**** "do CovidLongitudinal Lithuania 1 Process.do"
...
** output data files: "`model' Lithuania.dta"
**                   "ALL MODLES Lithuania.dta"

** output data dictionary files: "Lithuania ALL MODELS update dates data dictionary.csv"
**                               "`model' Lithuania data dictionary.csv"
**                               "ALL MODLES Lithuania data dictionary.csv"

** graphs:
** "graph 02 Lithuania `model' C19 daily deaths all updates.pdf"
** "graph 02 Lithuania ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Lithuania absent update_date_block"
	
	qui keep if loc_grand_name == "Lithuania"
	...
	save "Lithuania `model' update dates.dta", replace
...
use "Lithuania DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Lithuania `model' update dates.dta"
...
save "Lithuania ALL MODELS update dates.dta", replace
	
export excel using "Lithuania ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Lithuania ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Lithuania"

keep date DayDeaMeSmJOHNLTU
...
save "JOHN Lithuania.dta", replace 




* graph Lithuania JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNLTU date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Lithuania, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lithuania JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Lithuania.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Lithuania.dta"
...
save "DELP Lithuania.dta", replace
...
merge m:m date using "JOHN Lithuania.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Lithuania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Lithuania DELP daily deaths each update 
	...
	subtitle("Lithuania, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Lithuania DELP daily deaths update `update'.pdf", replace	
...
* graph Lithuania DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lithuania, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lithuania DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Lithuania IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Lithuania, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Lithuania IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Lithuania.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Lithuania.dta"
...
save "IHME Lithuania.dta", replace
...
merge m:1 date using "JOHN Lithuania.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Lithuania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Lithuania IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Lithuania, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Lithuania IHME daily deaths update `update'.pdf", replace	
...
* graph Lithuania IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lithuania, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lithuania IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Lithuania.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Lithuania.dta"
...
save "IMPE Lithuania.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Lithuania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Lithuania IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Lithuania, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Lithuania IMPE daily deaths update `update'.pdf", replace	
...
* graph Lithuania IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lithuania, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lithuania IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Lithuania.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Lithuania.dta"
...
save "LANL Lithuania.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Lithuania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Lithuania LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Lithuania, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Lithuania LANL daily deaths update `update'.pdf", replace	
...
* graph Lithuania LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lithuania, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lithuania LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Lithuania.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Lithuania.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Lithuania.dta"
...
save "YYGU Lithuania.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Lithuania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Lithuania YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Lithuania, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Lithuania YYGU daily deaths update `update'.pdf", replace	
...
* graph Lithuania YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lithuania, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lithuania YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Lithuania.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Lithuania.dta", force
...
save "ALL MODLES Lithuania.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Lithuania data dictionary.csv", replace 
...
* graph Lithuania ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lithuania, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(833 "JOHN" 15 "DELP" 1 "IHME" 90 "IMPE" 29 "LANL" 2 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lithuania ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Lithuania 1 Process.smcl"
