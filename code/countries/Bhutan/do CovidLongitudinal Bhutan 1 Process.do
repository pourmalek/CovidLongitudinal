...
log using "log CovidLongitudinal Bhutan 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Bhutan 1 Process.do"
...
**** "do CovidLongitudinal Bhutan 1 Process.do"
...
** output data files: "`model' Bhutan.dta"
**                   "ALL MODLES Bhutan.dta"

** output data dictionary files: "Bhutan ALL MODELS update dates data dictionary.csv"
**                               "`model' Bhutan data dictionary.csv"
**                               "ALL MODLES Bhutan data dictionary.csv"

** graphs:
** "graph 02 Bhutan `model' C19 daily deaths all updates.pdf"
** "graph 02 Bhutan ALL MODELS C19 daily deaths all updates.pdf"
...
local list DELP IMPE

foreach model of local list {

	use "$pathCovidLongitudinal/download/`model'/`model' countries long.dta", clear 
	
	di in red "`model'" " Bhutan absent update_date_block"
	
	qui keep if loc_grand_name == "Bhutan"
	...
	save "Bhutan `model' update dates.dta", replace
...
use "Bhutan DELP update dates.dta", clear 

local list DELP IMPE

foreach model of local list {
	
	append using "Bhutan `model' update dates.dta"
...
save "Bhutan ALL MODELS update dates.dta", replace
	
export excel using "Bhutan ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Bhutan ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Bhutan"

keep date DayDeaMeSmJOHNBTN
...
save "JOHN Bhutan.dta", replace 




* graph Bhutan JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNBTN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Bhutan, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bhutan JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Bhutan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Bhutan.dta"
...
merge m:m date using "JOHN Bhutan.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Bhutan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bhutan DELP daily deaths each update 
	...
	subtitle("Bhutan, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bhutan DELP daily deaths update `update'.pdf", replace	
...
* graph Bhutan DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bhutan, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bhutan DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Bhutan.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Bhutan.dta"
...
merge m:1 date using "JOHN Bhutan.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Bhutan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bhutan IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bhutan, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bhutan IMPE daily deaths update `update'.pdf", replace	
...
* graph Bhutan IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bhutan, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bhutan IMPE C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Bhutan.dta", force
...
save "ALL MODLES Bhutan.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Bhutan data dictionary.csv", replace 
...
* graph Bhutan ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bhutan, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(367 "JOHN" 1 "DELP" 7 "IMPE") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bhutan ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Bhutan 1 Process.smcl"
