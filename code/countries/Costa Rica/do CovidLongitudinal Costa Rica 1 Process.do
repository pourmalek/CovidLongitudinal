...
log using "log CovidLongitudinal Costa Rica 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Costa Rica 1 Process.do"
...
**** "do CovidLongitudinal Costa Rica 1 Process.do"
...
** output data files: "`model' Costa Rica.dta"
**                   "ALL MODLES Costa Rica.dta"

** output data dictionary files: "Costa Rica ALL MODELS update dates data dictionary.csv"
**                               "`model' Costa Rica data dictionary.csv"
**                               "ALL MODLES Costa Rica data dictionary.csv"

** graphs:
** "graph 02 Costa Rica `model' C19 daily deaths all updates.pdf"
** "graph 02 Costa Rica ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Costa Rica absent update_date_block"
	
	qui keep if loc_grand_name == "Costa Rica"
	...
	save "Costa Rica `model' update dates.dta", replace
...
use "Costa Rica DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Costa Rica `model' update dates.dta"
...
save "Costa Rica ALL MODELS update dates.dta", replace
	
export excel using "Costa Rica ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Costa Rica ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Costa Rica"

keep date DayDeaMeSmJOHNCRI
...
save "JOHN Costa Rica.dta", replace 




* graph Costa Rica JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNCRI date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Costa Rica, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Costa Rica JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Costa Rica.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Costa Rica.dta"
...
save "DELP Costa Rica.dta", replace
...
merge m:m date using "JOHN Costa Rica.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Costa Rica data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Costa Rica DELP daily deaths each update 
	...
	subtitle("Costa Rica, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Costa Rica DELP daily deaths update `update'.pdf", replace	
...
* graph Costa Rica DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Costa Rica, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Costa Rica DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Costa Rica IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Costa Rica, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Costa Rica IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Costa Rica.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Costa Rica.dta"
...
save "IHME Costa Rica.dta", replace
...
merge m:1 date using "JOHN Costa Rica.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Costa Rica data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Costa Rica IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Costa Rica, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Costa Rica IHME daily deaths update `update'.pdf", replace	
...
* graph Costa Rica IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Costa Rica, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Costa Rica IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Costa Rica.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Costa Rica.dta"
...
save "IMPE Costa Rica.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Costa Rica data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Costa Rica IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Costa Rica, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Costa Rica IMPE daily deaths update `update'.pdf", replace	
...
* graph Costa Rica IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Costa Rica, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Costa Rica IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Costa Rica.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Costa Rica.dta"
...
save "LANL Costa Rica.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Costa Rica data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Costa Rica LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Costa Rica, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Costa Rica LANL daily deaths update `update'.pdf", replace	
...
* graph Costa Rica LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Costa Rica, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Costa Rica LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Costa Rica.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Costa Rica.dta", force
...
save "ALL MODLES Costa Rica.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Costa Rica data dictionary.csv", replace 
...
* graph Costa Rica ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Costa Rica, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(649 "JOHN" 1 "DELP" 55 "IHME" 3 "IMPE" 66 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Costa Rica ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Costa Rica 1 Process.smcl"
