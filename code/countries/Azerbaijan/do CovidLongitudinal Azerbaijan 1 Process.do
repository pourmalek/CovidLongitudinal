...
log using "log CovidLongitudinal Azerbaijan 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Azerbaijan 1 Process.do"
...
**** "do CovidLongitudinal Azerbaijan 1 Process.do"
...
** output data files: "`model' Azerbaijan.dta"
**                   "ALL MODLES Azerbaijan.dta"

** output data dictionary files: "Azerbaijan ALL MODELS update dates data dictionary.csv"
**                               "`model' Azerbaijan data dictionary.csv"
**                               "ALL MODLES Azerbaijan data dictionary.csv"

** graphs:
** "graph 02 Azerbaijan `model' C19 daily deaths all updates.pdf"
** "graph 02 Azerbaijan ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Azerbaijan absent update_date_block"
	
	qui keep if loc_grand_name == "Azerbaijan"
	...
	save "Azerbaijan `model' update dates.dta", replace
...
use "Azerbaijan DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Azerbaijan `model' update dates.dta"
...
save "Azerbaijan ALL MODELS update dates.dta", replace
	
export excel using "Azerbaijan ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Azerbaijan ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Azerbaijan"

keep date DayDeaMeSmJOHNAZE
...
save "JOHN Azerbaijan.dta", replace 




* graph Azerbaijan JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNAZE date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Azerbaijan, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Azerbaijan JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Azerbaijan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Azerbaijan.dta"
...
save "DELP Azerbaijan.dta", replace
...
merge m:m date using "JOHN Azerbaijan.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Azerbaijan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Azerbaijan DELP daily deaths each update 
	...
	subtitle("Azerbaijan, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Azerbaijan DELP daily deaths update `update'.pdf", replace	
...
* graph Azerbaijan DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Azerbaijan, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Azerbaijan DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Azerbaijan IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Azerbaijan, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Azerbaijan IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Azerbaijan.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Azerbaijan.dta"
...
save "IHME Azerbaijan.dta", replace
...
merge m:1 date using "JOHN Azerbaijan.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Azerbaijan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Azerbaijan IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Azerbaijan, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Azerbaijan IHME daily deaths update `update'.pdf", replace	
...
* graph Azerbaijan IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Azerbaijan, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Azerbaijan IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Azerbaijan.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Azerbaijan.dta"
...
save "IMPE Azerbaijan.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Azerbaijan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Azerbaijan IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Azerbaijan, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Azerbaijan IMPE daily deaths update `update'.pdf", replace	
...
* graph Azerbaijan IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Azerbaijan, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Azerbaijan IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Azerbaijan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Azerbaijan.dta"
...
save "LANL Azerbaijan.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Azerbaijan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Azerbaijan LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Azerbaijan, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Azerbaijan LANL daily deaths update `update'.pdf", replace	
...
* graph Azerbaijan LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Azerbaijan, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Azerbaijan LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Azerbaijan.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Azerbaijan.dta", force
...
save "ALL MODLES Azerbaijan.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Azerbaijan data dictionary.csv", replace 
...
* graph Azerbaijan ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Azerbaijan, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(669 "JOHN" 1 "DELP" 72 "IHME" 4 "IMPE" 3 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Azerbaijan ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Azerbaijan 1 Process.smcl"
