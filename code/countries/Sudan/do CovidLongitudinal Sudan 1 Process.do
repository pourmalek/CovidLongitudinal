...
log using "log CovidLongitudinal Sudan 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Sudan 1 Process.do"
...
**** "do CovidLongitudinal Sudan 1 Process.do"
...
** output data files: "`model' Sudan.dta"
**                   "ALL MODLES Sudan.dta"

** output data dictionary files: "Sudan ALL MODELS update dates data dictionary.csv"
**                               "`model' Sudan data dictionary.csv"
**                               "ALL MODLES Sudan data dictionary.csv"

** graphs:
** "graph 02 Sudan `model' C19 daily deaths all updates.pdf"
** "graph 02 Sudan ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Sudan absent update_date_block"
	
	qui keep if loc_grand_name == "Sudan"
	...
	save "Sudan `model' update dates.dta", replace
...
use "Sudan DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Sudan `model' update dates.dta"
...
save "Sudan ALL MODELS update dates.dta", replace
	
export excel using "Sudan ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Sudan ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Sudan"

keep date DayDeaMeSmJOHNSDN
...
save "JOHN Sudan.dta", replace 




* graph Sudan JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSDN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Sudan, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sudan JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Sudan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Sudan.dta"
...
merge m:m date using "JOHN Sudan.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Sudan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sudan DELP daily deaths each update 
	...
	subtitle("Sudan, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sudan DELP daily deaths update `update'.pdf", replace	
...
* graph Sudan DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sudan, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sudan DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Sudan IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Sudan, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Sudan IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Sudan.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Sudan.dta"
...
merge m:1 date using "JOHN Sudan.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Sudan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sudan IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Sudan, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sudan IHME daily deaths update `update'.pdf", replace	
...
* graph Sudan IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sudan, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sudan IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Sudan.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Sudan.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Sudan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sudan IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Sudan, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sudan IMPE daily deaths update `update'.pdf", replace	
...
* graph Sudan IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sudan, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sudan IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Sudan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Sudan.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Sudan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sudan LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Sudan, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sudan LANL daily deaths update `update'.pdf", replace	
...
* graph Sudan LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sudan, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sudan LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Sudan.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Sudan.dta", force
...
save "ALL MODLES Sudan.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Sudan data dictionary.csv", replace 
...
* graph Sudan ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sudan, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(667 "JOHN" 6 "DELP" 70 "IHME" 2 "IMPE" 1 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sudan ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Sudan 1 Process.smcl"
