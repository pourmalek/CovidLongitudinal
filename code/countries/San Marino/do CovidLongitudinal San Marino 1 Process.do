...
log using "log CovidLongitudinal San Marino 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal San Marino 1 Process.do"
...
**** "do CovidLongitudinal San Marino 1 Process.do"
...
** output data files: "`model' San Marino.dta"
**                   "ALL MODLES San Marino.dta"

** output data dictionary files: "San Marino ALL MODELS update dates data dictionary.csv"
**                               "`model' San Marino data dictionary.csv"
**                               "ALL MODLES San Marino data dictionary.csv"

** graphs:
** "graph 02 San Marino `model' C19 daily deaths all updates.pdf"
** "graph 02 San Marino ALL MODELS C19 daily deaths all updates.pdf"
...
local list DELP IHME LANL

foreach model of local list {

	use "$pathCovidLongitudinal/download/`model'/`model' countries long.dta", clear 
	
	di in red "`model'" " San Marino absent update_date_block"
	
	qui keep if loc_grand_name == "San Marino"
	...
	save "San Marino `model' update dates.dta", replace
...
use "San Marino DELP update dates.dta", clear 

local list DELP IHME LANL

foreach model of local list {
	
	append using "San Marino `model' update dates.dta"
...
save "San Marino ALL MODELS update dates.dta", replace
	
export excel using "San Marino ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "San Marino ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "San Marino"

keep date DayDeaMeSmJOHNSMR
...
save "JOHN San Marino.dta", replace 




* graph San Marino JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSMR date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, San Marino, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 San Marino JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' San Marino.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' San Marino.dta"
...
save "DELP San Marino.dta", replace
...
merge m:m date using "JOHN San Marino.dta"
...
preserve
    ...
    export delimited name varlab using "DELP San Marino data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph San Marino DELP daily deaths each update 
	...
	subtitle("San Marino, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 San Marino DELP daily deaths update `update'.pdf", replace	
...
* graph San Marino DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, San Marino, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 San Marino DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph San Marino IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, San Marino, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 San Marino IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' San Marino.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' San Marino.dta"
...
save "IHME San Marino.dta", replace
...
merge m:1 date using "JOHN San Marino.dta"
...
preserve
    ...
    export delimited name varlab using "IHME San Marino data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph San Marino IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("San Marino, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 San Marino IHME daily deaths update `update'.pdf", replace	
...
* graph San Marino IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, San Marino, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 San Marino IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' San Marino.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' San Marino.dta"
...
save "LANL San Marino.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL San Marino data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph San Marino LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("San Marino, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 San Marino LANL daily deaths update `update'.pdf", replace	
...
* graph San Marino LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, San Marino, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 San Marino LANL C19 daily deaths all updates.pdf", replace
...
local list IHME LANL

use "DELP San Marino.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' San Marino.dta", force
...
save "ALL MODLES San Marino.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES San Marino data dictionary.csv", replace 
...
* graph San Marino ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, San Marino, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(520 "JOHN" 1 "DELP" 29 "IHME" 3 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 San Marino ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal San Marino 1 Process.smcl"
