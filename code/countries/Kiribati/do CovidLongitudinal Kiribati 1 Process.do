...
log using "log CovidLongitudinal Kiribati 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Kiribati 1 Process.do"
...
**** "do CovidLongitudinal Kiribati 1 Process.do"
...
** output data files: "`model' Kiribati.dta"
**                   "ALL MODLES Kiribati.dta"

** output data dictionary files: "Kiribati ALL MODELS update dates data dictionary.csv"
**                               "`model' Kiribati data dictionary.csv"
**                               "ALL MODLES Kiribati data dictionary.csv"

** graphs:
** "graph 02 Kiribati `model' C19 daily deaths all updates.pdf"
** "graph 02 Kiribati ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Kiribati absent update_date_block"
	
	qui keep if loc_grand_name == "Kiribati"
	...
	save "Kiribati `model' update dates.dta", replace
...
use "Kiribati IMPE update dates.dta", clear 

local list IMPE

foreach model of local list {
	
	append using "Kiribati `model' update dates.dta"
...
save "Kiribati ALL MODELS update dates.dta", replace
	
export excel using "Kiribati ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Kiribati ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Kiribati"

keep date DayDeaMeSmJOHNKIR
...
save "JOHN Kiribati.dta", replace 




* graph Kiribati JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNKIR date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Kiribati, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kiribati JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Kiribati.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Kiribati.dta"
...
save "IMPE Kiribati.dta", replace
...
merge m:1 date using "JOHN Kiribati.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Kiribati data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kiribati IMPE daily deaths each update 
	...
	subtitle("Kiribati, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kiribati IMPE daily deaths update `update'.pdf", replace	
...
* graph Kiribati IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kiribati, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kiribati IMPE C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Kiribati.dta", force
...
save "ALL MODLES Kiribati.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Kiribati data dictionary.csv", replace 
...
* graph Kiribati ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kiribati, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(68 "JOHN" 1 "IMPE") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kiribati ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Kiribati 1 Process.smcl"
