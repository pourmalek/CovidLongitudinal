...
log using "log CovidLongitudinal Luxembourg 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Luxembourg 1 Process.do"
...
**** "do CovidLongitudinal Luxembourg 1 Process.do"
...
** output data files: "`model' Luxembourg.dta"
**                   "ALL MODLES Luxembourg.dta"

** output data dictionary files: "Luxembourg ALL MODELS update dates data dictionary.csv"
**                               "`model' Luxembourg data dictionary.csv"
**                               "ALL MODLES Luxembourg data dictionary.csv"

** graphs:
** "graph 02 Luxembourg `model' C19 daily deaths all updates.pdf"
** "graph 02 Luxembourg ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Luxembourg absent update_date_block"
	
	qui keep if loc_grand_name == "Luxembourg"
	...
	save "Luxembourg `model' update dates.dta", replace
...
use "Luxembourg DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Luxembourg `model' update dates.dta"
...
save "Luxembourg ALL MODELS update dates.dta", replace
	
export excel using "Luxembourg ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Luxembourg ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Luxembourg"

keep date DayDeaMeSmJOHNLUX
...
save "JOHN Luxembourg.dta", replace 




* graph Luxembourg JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNLUX date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Luxembourg, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Luxembourg JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Luxembourg.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Luxembourg.dta"
...
save "DELP Luxembourg.dta", replace
...
merge m:m date using "JOHN Luxembourg.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Luxembourg data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Luxembourg DELP daily deaths each update 
	...
	subtitle("Luxembourg, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Luxembourg DELP daily deaths update `update'.pdf", replace	
...
* graph Luxembourg DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Luxembourg, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Luxembourg DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Luxembourg IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Luxembourg, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Luxembourg IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Luxembourg.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Luxembourg.dta"
...
save "IHME Luxembourg.dta", replace
...
merge m:1 date using "JOHN Luxembourg.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Luxembourg data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Luxembourg IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Luxembourg, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Luxembourg IHME daily deaths update `update'.pdf", replace	
...
* graph Luxembourg IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Luxembourg, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Luxembourg IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Luxembourg.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Luxembourg.dta"
...
save "IMPE Luxembourg.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Luxembourg data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Luxembourg IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Luxembourg, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Luxembourg IMPE daily deaths update `update'.pdf", replace	
...
* graph Luxembourg IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Luxembourg, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Luxembourg IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Luxembourg.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Luxembourg.dta"
...
save "LANL Luxembourg.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Luxembourg data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Luxembourg LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Luxembourg, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Luxembourg LANL daily deaths update `update'.pdf", replace	
...
* graph Luxembourg LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Luxembourg, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Luxembourg LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Luxembourg.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Luxembourg.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Luxembourg.dta"
...
save "YYGU Luxembourg.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Luxembourg data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Luxembourg YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Luxembourg, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Luxembourg YYGU daily deaths update `update'.pdf", replace	
...
* graph Luxembourg YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Luxembourg, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Luxembourg YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Luxembourg.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Luxembourg.dta", force
...
save "ALL MODLES Luxembourg.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Luxembourg data dictionary.csv", replace 
...
* graph Luxembourg ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Luxembourg, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(932 "JOHN" 19 "DELP" 1 "IHME" 107 "IMPE" 36 "LANL" 3 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Luxembourg ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Luxembourg 1 Process.smcl"
