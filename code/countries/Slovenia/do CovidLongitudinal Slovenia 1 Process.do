...
log using "log CovidLongitudinal Slovenia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Slovenia 1 Process.do"
...
**** "do CovidLongitudinal Slovenia 1 Process.do"
...
** output data files: "`model' Slovenia.dta"
**                   "ALL MODLES Slovenia.dta"

** output data dictionary files: "Slovenia ALL MODELS update dates data dictionary.csv"
**                               "`model' Slovenia data dictionary.csv"
**                               "ALL MODLES Slovenia data dictionary.csv"

** graphs:
** "graph 02 Slovenia `model' C19 daily deaths all updates.pdf"
** "graph 02 Slovenia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Slovenia absent update_date_block"
	
	qui keep if loc_grand_name == "Slovenia"
	...
	save "Slovenia `model' update dates.dta", replace
...
use "Slovenia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Slovenia `model' update dates.dta"
...
save "Slovenia ALL MODELS update dates.dta", replace
	
export excel using "Slovenia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Slovenia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Slovenia"

keep date DayDeaMeSmJOHNSVN
...
save "JOHN Slovenia.dta", replace 




* graph Slovenia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSVN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Slovenia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Slovenia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Slovenia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Slovenia.dta"
...
save "DELP Slovenia.dta", replace
...
merge m:m date using "JOHN Slovenia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Slovenia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Slovenia DELP daily deaths each update 
	...
	subtitle("Slovenia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Slovenia DELP daily deaths update `update'.pdf", replace	
...
* graph Slovenia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Slovenia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Slovenia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Slovenia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Slovenia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Slovenia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Slovenia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Slovenia.dta"
...
save "IHME Slovenia.dta", replace
...
merge m:1 date using "JOHN Slovenia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Slovenia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Slovenia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Slovenia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Slovenia IHME daily deaths update `update'.pdf", replace	
...
* graph Slovenia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Slovenia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Slovenia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Slovenia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Slovenia.dta"
...
save "IMPE Slovenia.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Slovenia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Slovenia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Slovenia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Slovenia IMPE daily deaths update `update'.pdf", replace	
...
* graph Slovenia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Slovenia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Slovenia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Slovenia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Slovenia.dta"
...
save "LANL Slovenia.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Slovenia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Slovenia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Slovenia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Slovenia LANL daily deaths update `update'.pdf", replace	
...
* graph Slovenia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Slovenia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Slovenia LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Slovenia.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Slovenia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Slovenia.dta"
...
save "YYGU Slovenia.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Slovenia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Slovenia YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Slovenia, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Slovenia YYGU daily deaths update `update'.pdf", replace	
...
* graph Slovenia YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Slovenia, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Slovenia YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Slovenia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Slovenia.dta", force
...
save "ALL MODLES Slovenia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Slovenia data dictionary.csv", replace 
...
* graph Slovenia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Slovenia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(832 "JOHN" 15 "DELP" 1 "IHME" 91 "IMPE" 29 "LANL" 2 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Slovenia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Slovenia 1 Process.smcl"
