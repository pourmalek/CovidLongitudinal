...
log using "log CovidLongitudinal Rwanda 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Rwanda 1 Process.do"
...
**** "do CovidLongitudinal Rwanda 1 Process.do"
...
** output data files: "`model' Rwanda.dta"
**                   "ALL MODLES Rwanda.dta"

** output data dictionary files: "Rwanda ALL MODELS update dates data dictionary.csv"
**                               "`model' Rwanda data dictionary.csv"
**                               "ALL MODLES Rwanda data dictionary.csv"

** graphs:
** "graph 02 Rwanda `model' C19 daily deaths all updates.pdf"
** "graph 02 Rwanda ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Rwanda absent update_date_block"
	
	qui keep if loc_grand_name == "Rwanda"
	...
	save "Rwanda `model' update dates.dta", replace
...
use "Rwanda DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Rwanda `model' update dates.dta"
...
save "Rwanda ALL MODELS update dates.dta", replace
	
export excel using "Rwanda ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Rwanda ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Rwanda"

keep date DayDeaMeSmJOHNRWA
...
save "JOHN Rwanda.dta", replace 




* graph Rwanda JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNRWA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Rwanda, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Rwanda JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Rwanda.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Rwanda.dta"
...
merge m:m date using "JOHN Rwanda.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Rwanda data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Rwanda DELP daily deaths each update 
	...
	subtitle("Rwanda, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Rwanda DELP daily deaths update `update'.pdf", replace	
...
* graph Rwanda DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Rwanda, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Rwanda DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Rwanda IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Rwanda, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Rwanda IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Rwanda.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Rwanda.dta"
...
merge m:1 date using "JOHN Rwanda.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Rwanda data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Rwanda IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Rwanda, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Rwanda IHME daily deaths update `update'.pdf", replace	
...
* graph Rwanda IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Rwanda, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Rwanda IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Rwanda.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Rwanda.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Rwanda data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Rwanda IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Rwanda, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Rwanda IMPE daily deaths update `update'.pdf", replace	
...
* graph Rwanda IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Rwanda, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Rwanda IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Rwanda.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Rwanda.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Rwanda data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Rwanda LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Rwanda, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Rwanda LANL daily deaths update `update'.pdf", replace	
...
* graph Rwanda LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Rwanda, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Rwanda LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Rwanda.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Rwanda.dta", force
...
save "ALL MODLES Rwanda.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Rwanda data dictionary.csv", replace 
...
* graph Rwanda ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Rwanda, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(599 "JOHN" 1 "DELP" 23 "IHME" 8 "IMPE" 68 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Rwanda ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Rwanda 1 Process.smcl"
