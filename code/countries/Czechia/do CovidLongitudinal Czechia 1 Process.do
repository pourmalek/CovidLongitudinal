...
log using "log CovidLongitudinal Czechia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Czechia 1 Process.do"
...
**** "do CovidLongitudinal Czechia 1 Process.do"
...
** output data files: "`model' Czechia.dta"
**                   "ALL MODLES Czechia.dta"

** output data dictionary files: "Czechia ALL MODELS update dates data dictionary.csv"
**                               "`model' Czechia data dictionary.csv"
**                               "ALL MODLES Czechia data dictionary.csv"

** graphs:
** "graph 02 Czechia `model' C19 daily deaths all updates.pdf"
** "graph 02 Czechia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Czechia absent update_date_block"
	
	qui keep if loc_grand_name == "Czechia"
	...
	save "Czechia `model' update dates.dta", replace
...
use "Czechia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Czechia `model' update dates.dta"
...
save "Czechia ALL MODELS update dates.dta", replace
	
export excel using "Czechia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Czechia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Czechia"

keep date DayDeaMeSmJOHNCZE
...
save "JOHN Czechia.dta", replace 




* graph Czechia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNCZE date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Czechia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Czechia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Czechia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Czechia.dta"
...
merge m:m date using "JOHN Czechia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Czechia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Czechia DELP daily deaths each update 
	...
	subtitle("Czechia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Czechia DELP daily deaths update `update'.pdf", replace	
...
* graph Czechia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Czechia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Czechia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Czechia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Czechia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Czechia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Czechia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Czechia.dta"
...
merge m:1 date using "JOHN Czechia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Czechia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Czechia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Czechia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Czechia IHME daily deaths update `update'.pdf", replace	
...
* graph Czechia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Czechia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Czechia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Czechia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Czechia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Czechia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Czechia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Czechia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Czechia IMPE daily deaths update `update'.pdf", replace	
...
* graph Czechia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Czechia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Czechia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Czechia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Czechia.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Czechia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Czechia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Czechia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Czechia LANL daily deaths update `update'.pdf", replace	
...
* graph Czechia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Czechia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Czechia LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Czechia.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Czechia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Czechia.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Czechia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Czechia YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Czechia, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Czechia YYGU daily deaths update `update'.pdf", replace	
...
* graph Czechia YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Czechia, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Czechia YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Czechia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Czechia.dta", force
...
save "ALL MODLES Czechia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Czechia data dictionary.csv", replace 
...
* graph Czechia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Czechia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(831 "JOHN" 25 "DELP" 1 "IHME" 88 "IMPE" 28 "LANL" 2 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Czechia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Czechia 1 Process.smcl"
