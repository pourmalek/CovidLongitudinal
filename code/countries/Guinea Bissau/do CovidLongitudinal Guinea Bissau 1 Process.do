...
log using "log CovidLongitudinal Guinea Bissau 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Guinea Bissau 1 Process.do"
...
**** "do CovidLongitudinal Guinea Bissau 1 Process.do"
...
** output data files: "`model' Guinea Bissau.dta"
**                   "ALL MODLES Guinea Bissau.dta"

** output data dictionary files: "Guinea Bissau ALL MODELS update dates data dictionary.csv"
**                               "`model' Guinea Bissau data dictionary.csv"
**                               "ALL MODLES Guinea Bissau data dictionary.csv"

** graphs:
** "graph 02 Guinea Bissau `model' C19 daily deaths all updates.pdf"
** "graph 02 Guinea Bissau ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Guinea Bissau absent update_date_block"
	
	qui keep if loc_grand_name == "Guinea Bissau"
	...
	save "Guinea Bissau `model' update dates.dta", replace
...
use "Guinea Bissau DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Guinea Bissau `model' update dates.dta"
...
save "Guinea Bissau ALL MODELS update dates.dta", replace
	
export excel using "Guinea Bissau ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Guinea Bissau ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Guinea Bissau"

keep date DayDeaMeSmJOHNGNB
...
save "JOHN Guinea Bissau.dta", replace 




* graph Guinea Bissau JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNGNB date, sort lwidth(thick) lcolor(cyan)) ///
if date >= td(01jan2020) & date <= td(01jan2023) ///
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%3.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guinea Bissau, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guinea Bissau JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Guinea Bissau.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Guinea Bissau.dta"
...
save "DELP Guinea Bissau.dta", replace
...
merge m:m date using "JOHN Guinea Bissau.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Guinea Bissau data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guinea Bissau DELP daily deaths each update 
	...
	subtitle("Guinea Bissau, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guinea Bissau DELP daily deaths update `update'.pdf", replace	
...
* graph Guinea Bissau DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guinea Bissau, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guinea Bissau DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Guinea Bissau IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Guinea Bissau, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Guinea Bissau IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Guinea Bissau.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Guinea Bissau.dta"
...
save "IHME Guinea Bissau.dta", replace
...
merge m:1 date using "JOHN Guinea Bissau.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Guinea Bissau data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guinea Bissau IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Guinea Bissau, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guinea Bissau IHME daily deaths update `update'.pdf", replace	
...
* graph Guinea Bissau IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guinea Bissau, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guinea Bissau IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Guinea Bissau.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Guinea Bissau.dta"
...
save "IMPE Guinea Bissau.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Guinea Bissau data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guinea Bissau IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Guinea Bissau, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guinea Bissau IMPE daily deaths update `update'.pdf", replace	
...
* graph Guinea Bissau IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guinea Bissau, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guinea Bissau IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Guinea Bissau.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Guinea Bissau.dta"
...
save "LANL Guinea Bissau.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Guinea Bissau data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guinea Bissau LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Guinea Bissau, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guinea Bissau LANL daily deaths update `update'.pdf", replace	
...
* graph Guinea Bissau LANL daily deaths all updates
...
ytitle(Daily deaths) title("C19 daily deaths, Guinea Bissau, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guinea Bissau LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Guinea Bissau.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Guinea Bissau.dta", force
...
save "ALL MODLES Guinea Bissau.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Guinea Bissau data dictionary.csv", replace 
...
* graph Guinea Bissau ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guinea Bissau, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(647 "JOHN" 12 "DELP" 51 "IHME" 1 "IMPE" 55 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guinea Bissau ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Guinea Bissau 1 Process.smcl"
