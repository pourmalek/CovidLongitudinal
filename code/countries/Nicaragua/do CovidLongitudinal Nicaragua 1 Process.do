...
log using "log CovidLongitudinal Nicaragua 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Nicaragua 1 Process.do"
...
**** "do CovidLongitudinal Nicaragua 1 Process.do"
...
** output data files: "`model' Nicaragua.dta"
**                   "ALL MODLES Nicaragua.dta"

** output data dictionary files: "Nicaragua ALL MODELS update dates data dictionary.csv"
**                               "`model' Nicaragua data dictionary.csv"
**                               "ALL MODLES Nicaragua data dictionary.csv"

** graphs:
** "graph 02 Nicaragua `model' C19 daily deaths all updates.pdf"
** "graph 02 Nicaragua ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Nicaragua absent update_date_block"
	
	qui keep if loc_grand_name == "Nicaragua"
	...
	save "Nicaragua `model' update dates.dta", replace
...
use "Nicaragua DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Nicaragua `model' update dates.dta"
...
save "Nicaragua ALL MODELS update dates.dta", replace
	
export excel using "Nicaragua ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Nicaragua ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Nicaragua"

keep date DayDeaMeSmJOHNNIC
...
save "JOHN Nicaragua.dta", replace 




* graph Nicaragua JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNNIC date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Nicaragua, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nicaragua JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Nicaragua.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Nicaragua.dta"
...
save "DELP Nicaragua.dta", replace
...
merge m:m date using "JOHN Nicaragua.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Nicaragua data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Nicaragua DELP daily deaths each update 
	...
	subtitle("Nicaragua, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Nicaragua DELP daily deaths update `update'.pdf", replace	
...
* graph Nicaragua DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nicaragua, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nicaragua DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Nicaragua IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Nicaragua, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Nicaragua IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Nicaragua.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Nicaragua.dta"
...
save "IHME Nicaragua.dta", replace
...
merge m:1 date using "JOHN Nicaragua.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Nicaragua data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Nicaragua IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Nicaragua, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Nicaragua IHME daily deaths update `update'.pdf", replace	
...
* graph Nicaragua IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nicaragua, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nicaragua IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Nicaragua.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Nicaragua.dta"
...
save "IMPE Nicaragua.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Nicaragua data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Nicaragua IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Nicaragua, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Nicaragua IMPE daily deaths update `update'.pdf", replace	
...
* graph Nicaragua IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nicaragua, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nicaragua IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Nicaragua.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Nicaragua.dta"
...
save "LANL Nicaragua.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Nicaragua data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Nicaragua LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Nicaragua, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Nicaragua LANL daily deaths update `update'.pdf", replace	
...
* graph Nicaragua LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nicaragua, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nicaragua LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Nicaragua.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Nicaragua.dta", force
...
save "ALL MODLES Nicaragua.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Nicaragua data dictionary.csv", replace 
...
* graph Nicaragua ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nicaragua, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(645 "JOHN" 33 "DELP" 57 "IHME" 1 "IMPE" 29 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nicaragua ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Nicaragua 1 Process.smcl"
