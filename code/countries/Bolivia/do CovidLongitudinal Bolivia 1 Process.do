...
log using "log CovidLongitudinal Bolivia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Bolivia 1 Process.do"
...
**** "do CovidLongitudinal Bolivia 1 Process.do"
...
** output data files: "`model' Bolivia.dta"
**                   "ALL MODLES Bolivia.dta"

** output data dictionary files: "Bolivia ALL MODELS update dates data dictionary.csv"
**                               "`model' Bolivia data dictionary.csv"
**                               "ALL MODLES Bolivia data dictionary.csv"

** graphs:
** "graph 02 Bolivia `model' C19 daily deaths all updates.pdf"
** "graph 02 Bolivia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Bolivia absent update_date_block"
	
	qui keep if loc_grand_name == "Bolivia"
	...
	save "Bolivia `model' update dates.dta", replace
...
use "Bolivia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Bolivia `model' update dates.dta"
...
save "Bolivia ALL MODELS update dates.dta", replace
	
export excel using "Bolivia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Bolivia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Bolivia"

keep date DayDeaMeSmJOHNBOL
...
save "JOHN Bolivia.dta", replace 




* graph Bolivia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNBOL date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Bolivia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bolivia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Bolivia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Bolivia.dta"
...
merge m:m date using "JOHN Bolivia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Bolivia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bolivia DELP daily deaths each update 
	...
	subtitle("Bolivia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bolivia DELP daily deaths update `update'.pdf", replace	
...
* graph Bolivia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bolivia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bolivia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Bolivia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Bolivia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Bolivia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Bolivia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Bolivia.dta"
...
merge m:1 date using "JOHN Bolivia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Bolivia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bolivia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bolivia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bolivia IHME daily deaths update `update'.pdf", replace	
...
* graph Bolivia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bolivia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bolivia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Bolivia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Bolivia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Bolivia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bolivia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bolivia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bolivia IMPE daily deaths update `update'.pdf", replace	
...
* graph Bolivia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bolivia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bolivia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Bolivia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Bolivia.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Bolivia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bolivia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bolivia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bolivia LANL daily deaths update `update'.pdf", replace	
...
* graph Bolivia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bolivia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bolivia LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Bolivia.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Bolivia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Bolivia.dta"
...
preserve
    ...
    export delimited name varlab using "UCLA Bolivia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bolivia UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bolivia, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bolivia UCLA daily deaths update `update'.pdf", replace	

}
*/


* graph Bolivia UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bolivia, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bolivia UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Bolivia.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Bolivia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Bolivia.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Bolivia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bolivia YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bolivia, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bolivia YYGU daily deaths update `update'.pdf", replace	
...
* graph Bolivia YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bolivia, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bolivia YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Bolivia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Bolivia.dta", force
...
save "ALL MODLES Bolivia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Bolivia data dictionary.csv", replace 
...
* graph Bolivia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bolivia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(841 "JOHN" 1 "DELP" 34 "IHME" 4 "IMPE" 3 "LANL" 356 "UCLA" 47 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bolivia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Bolivia 1 Process.smcl"
