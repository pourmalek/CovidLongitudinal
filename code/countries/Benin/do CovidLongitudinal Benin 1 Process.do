...
log using "log CovidLongitudinal Benin 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Benin 1 Process.do"
...
**** "do CovidLongitudinal Benin 1 Process.do"
...
** output data files: "`model' Benin.dta"
**                   "ALL MODLES Benin.dta"

** output data dictionary files: "Benin ALL MODELS update dates data dictionary.csv"
**                               "`model' Benin data dictionary.csv"
**                               "ALL MODLES Benin data dictionary.csv"

** graphs:
** "graph 02 Benin `model' C19 daily deaths all updates.pdf"
** "graph 02 Benin ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Benin absent update_date_block"
	
	qui keep if loc_grand_name == "Benin"
	...
	save "Benin `model' update dates.dta", replace
...
use "Benin DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Benin `model' update dates.dta"
...
save "Benin ALL MODELS update dates.dta", replace
	
export excel using "Benin ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Benin ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Benin"

keep date DayDeaMeSmJOHNBEN
...
save "JOHN Benin.dta", replace 




* graph Benin JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNBEN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Benin, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Benin JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Benin.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Benin.dta"
...
merge m:m date using "JOHN Benin.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Benin data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Benin DELP daily deaths each update 
	...
	subtitle("Benin, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Benin DELP daily deaths update `update'.pdf", replace	
...
* graph Benin DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Benin, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Benin DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Benin IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Benin, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Benin IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Benin.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Benin.dta"
...
merge m:1 date using "JOHN Benin.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Benin data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Benin IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Benin, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Benin IHME daily deaths update `update'.pdf", replace	
...
* graph Benin IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Benin, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Benin IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Benin.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Benin.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Benin data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Benin IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Benin, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Benin IMPE daily deaths update `update'.pdf", replace	
...
* graph Benin IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Benin, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Benin IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Benin.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Benin.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Benin data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Benin LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Benin, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Benin LANL daily deaths update `update'.pdf", replace	
...
* graph Benin LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Benin, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Benin LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Benin.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Benin.dta", force
...
save "ALL MODLES Benin.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Benin data dictionary.csv", replace 
...
* graph Benin ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Benin, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(647 "JOHN" 20 "DELP" 51 "IHME" 1 "IMPE" 56 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Benin ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Benin 1 Process.smcl"
