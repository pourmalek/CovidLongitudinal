...
log using "log CovidLongitudinal Mexico 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Mexico 1 Process.do"
...
**** "do CovidLongitudinal Mexico 1 Process.do"
...
** output data files: "`model' Mexico.dta"
**                   "ALL MODLES Mexico.dta"

** output data dictionary files: "Mexico ALL MODELS update dates data dictionary.csv"
**                               "`model' Mexico data dictionary.csv"
**                               "ALL MODLES Mexico data dictionary.csv"

** graphs:
** "graph 02 Mexico `model' C19 daily deaths all updates.pdf"
** "graph 02 Mexico ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Mexico absent update_date_block"
	
	qui keep if loc_grand_name == "Mexico"
	...
	save "Mexico `model' update dates.dta", replace
...
use "Mexico DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Mexico `model' update dates.dta"
...
save "Mexico ALL MODELS update dates.dta", replace
	
export excel using "Mexico ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Mexico ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Mexico"

keep date DayDeaMeSmJOHNMEX
...
save "JOHN Mexico.dta", replace 




* graph Mexico JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMEX date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Mexico, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mexico JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Mexico.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Mexico.dta"
...
merge m:m date using "JOHN Mexico.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Mexico data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mexico DELP daily deaths each update 
	...
	subtitle("Mexico, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mexico DELP daily deaths update `update'.pdf", replace	
...
* graph Mexico DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mexico, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mexico DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Mexico IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Mexico, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Mexico IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Mexico.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Mexico.dta"
...
merge m:1 date using "JOHN Mexico.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Mexico data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mexico IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mexico, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mexico IHME daily deaths update `update'.pdf", replace	
...
* graph Mexico IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mexico, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mexico IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Mexico.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Mexico.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Mexico data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mexico IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mexico, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mexico IMPE daily deaths update `update'.pdf", replace	
...
* graph Mexico IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mexico, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mexico IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Mexico.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Mexico.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Mexico data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mexico LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mexico, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mexico LANL daily deaths update `update'.pdf", replace	
...
* graph Mexico LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mexico, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mexico LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Mexico.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Mexico.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Mexico.dta"
...
preserve
    ...
    export delimited name varlab using "UCLA Mexico data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mexico UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mexico, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mexico UCLA daily deaths update `update'.pdf", replace	

}
*/


* graph Mexico UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mexico, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mexico UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Mexico.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Mexico.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Mexico.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Mexico data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mexico YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mexico, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mexico YYGU daily deaths update `update'.pdf", replace	
...
* graph Mexico YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mexico, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mexico YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Mexico.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Mexico.dta", force
...
save "ALL MODLES Mexico.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Mexico data dictionary.csv", replace 
...
* graph Mexico ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mexico, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(884 "JOHN" 11 "DELP" 59 "IHME" 25 "IMPE" 22 "LANL" 403 "UCLA" 1 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mexico ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Mexico 1 Process.smcl"
