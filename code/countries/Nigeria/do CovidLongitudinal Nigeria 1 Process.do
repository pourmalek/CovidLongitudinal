...
log using "log CovidLongitudinal Nigeria 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Nigeria 1 Process.do"
...
**** "do CovidLongitudinal Nigeria 1 Process.do"
...
** output data files: "`model' Nigeria.dta"
**                   "ALL MODLES Nigeria.dta"

** output data dictionary files: "Nigeria ALL MODELS update dates data dictionary.csv"
**                               "`model' Nigeria data dictionary.csv"
**                               "ALL MODLES Nigeria data dictionary.csv"

** graphs:
** "graph 02 Nigeria `model' C19 daily deaths all updates.pdf"
** "graph 02 Nigeria ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Nigeria absent update_date_block"
	
	qui keep if loc_grand_name == "Nigeria"
	...
	save "Nigeria `model' update dates.dta", replace
...
use "Nigeria DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Nigeria `model' update dates.dta"
...
save "Nigeria ALL MODELS update dates.dta", replace
	
export excel using "Nigeria ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Nigeria ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Nigeria"

keep date DayDeaMeSmJOHNNGA
...
save "JOHN Nigeria.dta", replace 




* graph Nigeria JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNNGA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Nigeria, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nigeria JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Nigeria.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Nigeria.dta"
...
merge m:m date using "JOHN Nigeria.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Nigeria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Nigeria DELP daily deaths each update 
	...
	subtitle("Nigeria, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Nigeria DELP daily deaths update `update'.pdf", replace	
...
* graph Nigeria DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nigeria, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nigeria DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Nigeria IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Nigeria, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Nigeria IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Nigeria.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Nigeria.dta"
...
merge m:1 date using "JOHN Nigeria.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Nigeria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Nigeria IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Nigeria, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Nigeria IHME daily deaths update `update'.pdf", replace	
...
* graph Nigeria IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nigeria, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nigeria IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Nigeria.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Nigeria.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Nigeria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Nigeria IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Nigeria, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Nigeria IMPE daily deaths update `update'.pdf", replace	
...
* graph Nigeria IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nigeria, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nigeria IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Nigeria.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Nigeria.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Nigeria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Nigeria LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Nigeria, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Nigeria LANL daily deaths update `update'.pdf", replace	
...
* graph Nigeria LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nigeria, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nigeria LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Nigeria.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Nigeria.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Nigeria.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Nigeria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Nigeria YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Nigeria, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Nigeria YYGU daily deaths update `update'.pdf", replace	
...
* graph Nigeria YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nigeria, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nigeria YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Nigeria.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Nigeria.dta", force
...
save "ALL MODLES Nigeria.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Nigeria data dictionary.csv", replace 
...
* graph Nigeria ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nigeria, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(815 "JOHN" 1 "DELP" 139 "IHME" 4 "IMPE" 3 "LANL" 25 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nigeria ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Nigeria 1 Process.smcl"
