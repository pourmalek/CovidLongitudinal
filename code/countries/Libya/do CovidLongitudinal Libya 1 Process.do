...
log using "log CovidLongitudinal Libya 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Libya 1 Process.do"
...
**** "do CovidLongitudinal Libya 1 Process.do"
...
** output data files: "`model' Libya.dta"
**                   "ALL MODLES Libya.dta"

** output data dictionary files: "Libya ALL MODELS update dates data dictionary.csv"
**                               "`model' Libya data dictionary.csv"
**                               "ALL MODLES Libya data dictionary.csv"

** graphs:
** "graph 02 Libya `model' C19 daily deaths all updates.pdf"
** "graph 02 Libya ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Libya absent update_date_block"
	
	qui keep if loc_grand_name == "Libya"
	...
	save "Libya `model' update dates.dta", replace
...
use "Libya DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Libya `model' update dates.dta"
...
save "Libya ALL MODELS update dates.dta", replace
	
export excel using "Libya ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Libya ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Libya"

keep date DayDeaMeSmJOHNLBY
...
save "JOHN Libya.dta", replace 




* graph Libya JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNLBY date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Libya, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Libya JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Libya.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Libya.dta"
...
merge m:m date using "JOHN Libya.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Libya data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Libya DELP daily deaths each update 
	...
	subtitle("Libya, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Libya DELP daily deaths update `update'.pdf", replace	
...
* graph Libya DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Libya, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Libya DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Libya IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Libya, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Libya IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Libya.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Libya.dta"
...
merge m:1 date using "JOHN Libya.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Libya data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Libya IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Libya, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Libya IHME daily deaths update `update'.pdf", replace	
...
* graph Libya IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Libya, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Libya IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Libya.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Libya.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Libya data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Libya IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Libya, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Libya IMPE daily deaths update `update'.pdf", replace	
...
* graph Libya IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Libya, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Libya IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Libya.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Libya.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Libya data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Libya LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Libya, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Libya LANL daily deaths update `update'.pdf", replace	
...
* graph Libya LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Libya, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Libya LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Libya.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Libya.dta", force
...
save "ALL MODLES Libya.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Libya data dictionary.csv", replace 
...
* graph Libya ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Libya, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(645 "JOHN" 39 "DELP" 48 "IHME" 1 "IMPE" 52 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Libya ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Libya 1 Process.smcl"
