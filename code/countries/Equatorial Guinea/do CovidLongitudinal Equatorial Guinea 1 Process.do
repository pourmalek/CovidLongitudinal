...
log using "log CovidLongitudinal Equatorial Guinea 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Equatorial Guinea 1 Process.do"
...
**** "do CovidLongitudinal Equatorial Guinea 1 Process.do"
...
** output data files: "`model' Equatorial Guinea.dta"
**                   "ALL MODLES Equatorial Guinea.dta"

** output data dictionary files: "Equatorial Guinea ALL MODELS update dates data dictionary.csv"
**                               "`model' Equatorial Guinea data dictionary.csv"
**                               "ALL MODLES Equatorial Guinea data dictionary.csv"

** graphs:
** "graph 02 Equatorial Guinea `model' C19 daily deaths all updates.pdf"
** "graph 02 Equatorial Guinea ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Equatorial Guinea absent update_date_block"
	
	qui keep if loc_grand_name == "Equatorial Guinea"
	...
	save "Equatorial Guinea `model' update dates.dta", replace
...
use "Equatorial Guinea DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Equatorial Guinea `model' update dates.dta"
...
save "Equatorial Guinea ALL MODELS update dates.dta", replace
	
export excel using "Equatorial Guinea ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Equatorial Guinea ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Equatorial Guinea"

keep date DayDeaMeSmJOHNGNQ
...
save "JOHN Equatorial Guinea.dta", replace 




* graph Equatorial Guinea JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNGNQ date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Equatorial Guinea, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Equatorial Guinea JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Equatorial Guinea.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Equatorial Guinea.dta"
...
save "DELP Equatorial Guinea.dta", replace
...
merge m:m date using "JOHN Equatorial Guinea.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Equatorial Guinea data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Equatorial Guinea DELP daily deaths each update 
	...
	subtitle("Equatorial Guinea, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Equatorial Guinea DELP daily deaths update `update'.pdf", replace	
...
* graph Equatorial Guinea DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Equatorial Guinea, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Equatorial Guinea DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	keep if country == "Equatorial Guinea" 
	...
	* graph Equatorial Guinea IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Equatorial Guinea, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Equatorial Guinea IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Equatorial Guinea.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Equatorial Guinea.dta"
...
save "IHME Equatorial Guinea.dta", replace
...
merge m:1 date using "JOHN Equatorial Guinea.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Equatorial Guinea data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Equatorial Guinea IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Equatorial Guinea, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Equatorial Guinea IHME daily deaths update `update'.pdf", replace	
...
* graph Equatorial Guinea IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Equatorial Guinea, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Equatorial Guinea IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Equatorial Guinea.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Equatorial Guinea.dta"
...
save "IMPE Equatorial Guinea.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Equatorial Guinea data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Equatorial Guinea IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Equatorial Guinea, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Equatorial Guinea IMPE daily deaths update `update'.pdf", replace	
...
* graph Equatorial Guinea IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Equatorial Guinea, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Equatorial Guinea IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Equatorial Guinea.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Equatorial Guinea.dta"
...
save "LANL Equatorial Guinea.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Equatorial Guinea data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Equatorial Guinea LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Equatorial Guinea, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Equatorial Guinea LANL daily deaths update `update'.pdf", replace	
...
* graph Equatorial Guinea LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Equatorial Guinea, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Equatorial Guinea LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Equatorial Guinea.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Equatorial Guinea.dta", force
...
save "ALL MODLES Equatorial Guinea.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Equatorial Guinea data dictionary.csv", replace 
...
* graph Equatorial Guinea ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Equatorial Guinea, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(650 "JOHN" 12 "DELP" 54 "IHME" 1 "IMPE" 47 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Equatorial Guinea ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Equatorial Guinea 1 Process.smcl"
