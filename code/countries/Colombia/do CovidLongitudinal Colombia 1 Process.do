...
log using "log CovidLongitudinal Colombia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Colombia 1 Process.do"
...
**** "do CovidLongitudinal Colombia 1 Process.do"
...
** output data files: "`model' Colombia.dta"
**                   "ALL MODLES Colombia.dta"

** output data dictionary files: "Colombia ALL MODELS update dates data dictionary.csv"
**                               "`model' Colombia data dictionary.csv"
**                               "ALL MODLES Colombia data dictionary.csv"

** graphs:
** "graph 02 Colombia `model' C19 daily deaths all updates.pdf"
** "graph 02 Colombia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Colombia absent update_date_block"
	
	qui keep if loc_grand_name == "Colombia"
	...
	save "Colombia `model' update dates.dta", replace
...
use "Colombia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Colombia `model' update dates.dta"
...
save "Colombia ALL MODELS update dates.dta", replace
	
export excel using "Colombia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Colombia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Colombia"

keep date DayDeaMeSmJOHNCOL
...
save "JOHN Colombia.dta", replace 




* graph Colombia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNCOL date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Colombia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Colombia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Colombia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Colombia.dta"
...
save "DELP Colombia.dta", replace
...
merge m:m date using "JOHN Colombia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Colombia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Colombia DELP daily deaths each update 
	...
	subtitle("Colombia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Colombia DELP daily deaths update `update'.pdf", replace	
...
* graph Colombia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Colombia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Colombia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Colombia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Colombia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Colombia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Colombia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Colombia.dta"
...
save "IHME Colombia.dta", replace
...
merge m:1 date using "JOHN Colombia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Colombia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Colombia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Colombia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Colombia IHME daily deaths update `update'.pdf", replace	
...
* graph Colombia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Colombia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Colombia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Colombia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Colombia.dta"
...
save "IMPE Colombia.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Colombia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Colombia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Colombia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Colombia IMPE daily deaths update `update'.pdf", replace	
...
* graph Colombia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Colombia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Colombia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Colombia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Colombia.dta"
...
save "LANL Colombia.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Colombia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Colombia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Colombia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Colombia LANL daily deaths update `update'.pdf", replace	
...
* graph Colombia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Colombia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Colombia LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Colombia.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Colombia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Colombia.dta"
...
save "UCLA Colombia.dta", replace
...
preserve
    ...
    export delimited name varlab using "UCLA Colombia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Colombia UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Colombia, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Colombia UCLA daily deaths update `update'.pdf", replace	

}
*/


* graph Colombia UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Colombia, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Colombia UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Colombia.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Colombia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Colombia.dta"
...
save "YYGU Colombia.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Colombia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Colombia YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Colombia, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Colombia YYGU daily deaths update `update'.pdf", replace	
...
* graph Colombia YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Colombia, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Colombia YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Colombia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Colombia.dta", force
...
save "ALL MODLES Colombia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Colombia data dictionary.csv", replace 
...
* graph Colombia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Colombia, all models, all updates, forecast only", size(medium) color(black)) ///
...
qui graph export "graph 02 Colombia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Colombia 1 Process.smcl"
