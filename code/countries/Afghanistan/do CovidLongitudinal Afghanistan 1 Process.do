...
log using "log CovidLongitudinal Afghanistan 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Afghanistan 1 Process.do"
...
**** "do CovidLongitudinal Afghanistan 1 Process.do"
...
** output data files: "`model' Afghanistan.dta"
**                   "ALL MODLES Afghanistan.dta"

** output data dictionary files: "Afghanistan ALL MODELS update dates data dictionary.csv"
**                               "`model' Afghanistan data dictionary.csv"
**                               "ALL MODLES Afghanistan data dictionary.csv"

** graphs:
** "graph 02 Afghanistan `model' C19 daily deaths all updates.pdf"
** "graph 02 Afghanistan ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Afghanistan absent update_date_block"
	
	qui keep if loc_grand_name == "Afghanistan"
	...
	save "Afghanistan `model' update dates.dta", replace
...
use "Afghanistan DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Afghanistan `model' update dates.dta"
...
save "Afghanistan ALL MODELS update dates.dta", replace
	
export excel using "Afghanistan ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Afghanistan ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Afghanistan"

keep date DayDeaMeSmJOHNAFG
...
save "JOHN Afghanistan.dta", replace 




* graph Afghanistan JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNAFG date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Afghanistan, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Afghanistan JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Afghanistan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Afghanistan.dta"
...
save "DELP Afghanistan.dta", replace
...
merge m:m date using "JOHN Afghanistan.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Afghanistan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Afghanistan DELP daily deaths each update 
	...
	subtitle("Afghanistan, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Afghanistan DELP daily deaths update `update'.pdf", replace	
...
* graph Afghanistan DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Afghanistan, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Afghanistan DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Afghanistan IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Afghanistan, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Afghanistan IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Afghanistan.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Afghanistan.dta"
...
save "IHME Afghanistan.dta", replace
...
merge m:1 date using "JOHN Afghanistan.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Afghanistan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Afghanistan IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Afghanistan, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Afghanistan IHME daily deaths update `update'.pdf", replace	
...
* graph Afghanistan IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Afghanistan, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Afghanistan IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Afghanistan.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Afghanistan.dta"
...
save "IMPE Afghanistan.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Afghanistan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Afghanistan IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Afghanistan, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Afghanistan IMPE daily deaths update `update'.pdf", replace	
...
* graph Afghanistan IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Afghanistan, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Afghanistan IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Afghanistan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Afghanistan.dta"
...
save "LANL Afghanistan.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Afghanistan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Afghanistan LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Afghanistan, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Afghanistan LANL daily deaths update `update'.pdf", replace	
...
* graph Afghanistan LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Afghanistan, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Afghanistan LANL C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Afghanistan.dta", force
...
save "ALL MODLES Afghanistan.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Afghanistan data dictionary.csv", replace 
...
* graph Afghanistan ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Afghanistan, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(668 "JOHN" 1 "DELP" 72 "IHME" 4 "IMPE" 3 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Afghanistan ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Afghanistan 1 Process.smcl"
