...
log using "log CovidLongitudinal Kuwait 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Kuwait 1 Process.do"
...
**** "do CovidLongitudinal Kuwait 1 Process.do"
...
** output data files: "`model' Kuwait.dta"
**                   "ALL MODLES Kuwait.dta"

** output data dictionary files: "Kuwait ALL MODELS update dates data dictionary.csv"
**                               "`model' Kuwait data dictionary.csv"
**                               "ALL MODLES Kuwait data dictionary.csv"

** graphs:
** "graph 02 Kuwait `model' C19 daily deaths all updates.pdf"
** "graph 02 Kuwait ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Kuwait absent update_date_block"
	
	qui keep if loc_grand_name == "Kuwait"
	...
	save "Kuwait `model' update dates.dta", replace
...
use "Kuwait DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Kuwait `model' update dates.dta"
...
save "Kuwait ALL MODELS update dates.dta", replace
	
export excel using "Kuwait ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Kuwait ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Kuwait"

keep date DayDeaMeSmJOHNKWT
...
save "JOHN Kuwait.dta", replace 




* graph Kuwait JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNKWT date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Kuwait, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kuwait JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Kuwait.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Kuwait.dta"
...
merge m:m date using "JOHN Kuwait.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Kuwait data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kuwait DELP daily deaths each update 
	...
	subtitle("Kuwait, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kuwait DELP daily deaths update `update'.pdf", replace	

}
*/


* graph Kuwait DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kuwait, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kuwait DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Kuwait IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Kuwait, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Kuwait IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Kuwait.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Kuwait.dta"
...
merge m:1 date using "JOHN Kuwait.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Kuwait data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kuwait IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Kuwait, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kuwait IHME daily deaths update `update'.pdf", replace	
...
* graph Kuwait IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kuwait, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kuwait IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Kuwait.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Kuwait.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Kuwait data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kuwait IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Kuwait, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kuwait IMPE daily deaths update `update'.pdf", replace	
...
* graph Kuwait IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kuwait, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kuwait IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Kuwait.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Kuwait.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Kuwait data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kuwait LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Kuwait, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kuwait LANL daily deaths update `update'.pdf", replace	
...
* graph Kuwait LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kuwait, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kuwait LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Kuwait.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Kuwait.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Kuwait.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Kuwait data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kuwait YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Kuwait, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kuwait YYGU daily deaths update `update'.pdf", replace	
...
* graph Kuwait YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kuwait, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kuwait YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Kuwait.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Kuwait.dta", force
...
save "ALL MODLES Kuwait.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Kuwait data dictionary.csv", replace 
...
* graph Kuwait ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kuwait, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(762 "JOHN" 1 "DELP" 65 "IHME" 24 "IMPE" 3 "LANL" 15 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kuwait ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Kuwait 1 Process.smcl"
