...
log using "log CovidLongitudinal Israel 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Israel 1 Process.do"
...
**** "do CovidLongitudinal Israel 1 Process.do"
...
** output data files: "`model' Israel.dta"
**                   "ALL MODLES Israel.dta"

** output data dictionary files: "Israel ALL MODELS update dates data dictionary.csv"
**                               "`model' Israel data dictionary.csv"
**                               "ALL MODLES Israel data dictionary.csv"

** graphs:
** "graph 02 Israel `model' C19 daily deaths all updates.pdf"
** "graph 02 Israel ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Israel absent update_date_block"
	
	qui keep if loc_grand_name == "Israel"
	...
	save "Israel `model' update dates.dta", replace
...
use "Israel DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Israel `model' update dates.dta"
...
save "Israel ALL MODELS update dates.dta", replace
	
export excel using "Israel ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Israel ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Israel"

keep date DayDeaMeSmJOHNISR
...
save "JOHN Israel.dta", replace 




* graph Israel JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNISR date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Israel, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Israel JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Israel.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Israel.dta"
...
merge m:m date using "JOHN Israel.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Israel data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Israel DELP daily deaths each update 
	...
	subtitle("Israel, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Israel DELP daily deaths update `update'.pdf", replace	
...
* graph Israel DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Israel, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Israel DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Israel IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Israel, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Israel IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Israel.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Israel.dta"
...
merge m:1 date using "JOHN Israel.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Israel data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Israel IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Israel, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Israel IHME daily deaths update `update'.pdf", replace	
...
* graph Israel IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Israel, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Israel IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Israel.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Israel.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Israel data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Israel IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Israel, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Israel IMPE daily deaths update `update'.pdf", replace	
...
* graph Israel IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Israel, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Israel IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Israel.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Israel.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Israel data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Israel LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Israel, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Israel LANL daily deaths update `update'.pdf", replace	
...
* graph Israel LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Israel, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Israel LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Israel.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Israel.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Israel.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Israel data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Israel YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Israel, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Israel YYGU daily deaths update `update'.pdf", replace	
...
* graph Israel YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Israel, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Israel YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Israel.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Israel.dta", force
...
save "ALL MODLES Israel.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Israel data dictionary.csv", replace 
...
* graph Israel ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Israel, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(784 "JOHN" 1 "DELP" 9 "IHME" 43 "IMPE" 3 "LANL" 10 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Israel ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Israel 1 Process.smcl"
