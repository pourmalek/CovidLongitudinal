...
log using "log CovidLongitudinal Nepal 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Nepal 1 Process.do"
...
**** "do CovidLongitudinal Nepal 1 Process.do"
...
** output data files: "`model' Nepal.dta"
**                   "ALL MODLES Nepal.dta"

** output data dictionary files: "Nepal ALL MODELS update dates data dictionary.csv"
**                               "`model' Nepal data dictionary.csv"
**                               "ALL MODLES Nepal data dictionary.csv"

** graphs:
** "graph 02 Nepal `model' C19 daily deaths all updates.pdf"
** "graph 02 Nepal ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Nepal absent update_date_block"
	
	qui keep if loc_grand_name == "Nepal"
	...
	save "Nepal `model' update dates.dta", replace
...
use "Nepal DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Nepal `model' update dates.dta"
...
save "Nepal ALL MODELS update dates.dta", replace
	
export excel using "Nepal ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Nepal ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Nepal"

keep date DayDeaMeSmJOHNNPL
...
save "JOHN Nepal.dta", replace 




* graph Nepal JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNNPL date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Nepal, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nepal JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Nepal.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Nepal.dta"
...
merge m:m date using "JOHN Nepal.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Nepal data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Nepal DELP daily deaths each update 
	...
	subtitle("Nepal, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Nepal DELP daily deaths update `update'.pdf", replace	
...
* graph Nepal DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nepal, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nepal DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Nepal IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Nepal, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Nepal IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Nepal.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Nepal.dta"
...
merge m:1 date using "JOHN Nepal.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Nepal data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Nepal IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Nepal, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Nepal IHME daily deaths update `update'.pdf", replace	
...
* graph Nepal IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nepal, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nepal IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Nepal.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Nepal.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Nepal data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Nepal IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Nepal, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Nepal IMPE daily deaths update `update'.pdf", replace	
...
* graph Nepal IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nepal, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nepal IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Nepal.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Nepal.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Nepal data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Nepal LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Nepal, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Nepal LANL daily deaths update `update'.pdf", replace	
...
* graph Nepal LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nepal, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nepal LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Nepal.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Nepal.dta", force
...
save "ALL MODLES Nepal.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Nepal data dictionary.csv", replace 
...
* graph Nepal ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Nepal, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(632 "JOHN" 1 "DELP" 35 "IHME" 2 "IMPE" 28 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Nepal ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Nepal 1 Process.smcl"
