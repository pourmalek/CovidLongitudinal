...
log using "log CovidLongitudinal Venezuela 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Venezuela 1 Process.do"
...
**** "do CovidLongitudinal Venezuela 1 Process.do"
...
** output data files: "`model' Venezuela.dta"
**                   "ALL MODLES Venezuela.dta"

** output data dictionary files: "Venezuela ALL MODELS update dates data dictionary.csv"
**                               "`model' Venezuela data dictionary.csv"
**                               "ALL MODLES Venezuela data dictionary.csv"

** graphs:
** "graph 02 Venezuela `model' C19 daily deaths all updates.pdf"
** "graph 02 Venezuela ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Venezuela absent update_date_block"
	
	qui keep if loc_grand_name == "Venezuela"
	...
	save "Venezuela `model' update dates.dta", replace
...
use "Venezuela DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Venezuela `model' update dates.dta"
...
save "Venezuela ALL MODELS update dates.dta", replace
	
export excel using "Venezuela ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Venezuela ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Venezuela"

keep date DayDeaMeSmJOHNVEN
...
save "JOHN Venezuela.dta", replace 




* graph Venezuela JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNVEN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Venezuela, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Venezuela JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Venezuela.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Venezuela.dta"
...
save "DELP Venezuela.dta", replace
...
merge m:m date using "JOHN Venezuela.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Venezuela data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Venezuela DELP daily deaths each update 
	...
	subtitle("Venezuela, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Venezuela DELP daily deaths update `update'.pdf", replace	
...
* graph Venezuela DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Venezuela, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Venezuela DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Venezuela IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Venezuela, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Venezuela IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Venezuela.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Venezuela.dta"
...
save "IHME Venezuela.dta", replace
...
merge m:1 date using "JOHN Venezuela.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Venezuela data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Venezuela IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Venezuela, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Venezuela IHME daily deaths update `update'.pdf", replace	
...
* graph Venezuela IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Venezuela, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Venezuela IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Venezuela.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Venezuela.dta"
...
save "IMPE Venezuela.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Venezuela data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Venezuela IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Venezuela, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Venezuela IMPE daily deaths update `update'.pdf", replace	
...
* graph Venezuela IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Venezuela, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Venezuela IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Venezuela.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Venezuela.dta"
...
save "LANL Venezuela.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Venezuela data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Venezuela LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Venezuela, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Venezuela LANL daily deaths update `update'.pdf", replace	
...
* graph Venezuela LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Venezuela, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Venezuela LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Venezuela.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Venezuela.dta", force
...
save "ALL MODLES Venezuela.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Venezuela data dictionary.csv", replace 
...
* graph Venezuela ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Venezuela, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(656 "JOHN" 1 "DELP" 61 "IHME" 2 "IMPE" 43 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Venezuela ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Venezuela 1 Process.smcl"
