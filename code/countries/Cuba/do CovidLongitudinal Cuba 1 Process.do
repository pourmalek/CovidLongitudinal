...
log using "log CovidLongitudinal Cuba 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Cuba 1 Process.do"
...
**** "do CovidLongitudinal Cuba 1 Process.do"
...
** output data files: "`model' Cuba.dta"
**                   "ALL MODLES Cuba.dta"

** output data dictionary files: "Cuba ALL MODELS update dates data dictionary.csv"
**                               "`model' Cuba data dictionary.csv"
**                               "ALL MODLES Cuba data dictionary.csv"

** graphs:
** "graph 02 Cuba `model' C19 daily deaths all updates.pdf"
** "graph 02 Cuba ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Cuba absent update_date_block"
	
	qui keep if loc_grand_name == "Cuba"
	...
	save "Cuba `model' update dates.dta", replace
...
use "Cuba DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Cuba `model' update dates.dta"
...
save "Cuba ALL MODELS update dates.dta", replace
	
export excel using "Cuba ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Cuba ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Cuba"

keep date DayDeaMeSmJOHNCUB
...
save "JOHN Cuba.dta", replace 




* graph Cuba JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNCUB date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Cuba, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cuba JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Cuba.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Cuba.dta"
...
merge m:m date using "JOHN Cuba.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Cuba data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cuba DELP daily deaths each update 
	...
	subtitle("Cuba, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cuba DELP daily deaths update `update'.pdf", replace	
...
* graph Cuba DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cuba, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cuba DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Cuba IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Cuba, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Cuba IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Cuba.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Cuba.dta"
...
merge m:1 date using "JOHN Cuba.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Cuba data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cuba IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cuba, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cuba IHME daily deaths update `update'.pdf", replace	
...
* graph Cuba IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cuba, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cuba IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Cuba.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Cuba.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Cuba data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cuba IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cuba, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cuba IMPE daily deaths update `update'.pdf", replace	
...
* graph Cuba IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cuba, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cuba IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Cuba.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Cuba.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Cuba data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cuba LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cuba, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cuba LANL daily deaths update `update'.pdf", replace	
...
* graph Cuba LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cuba, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cuba LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Cuba.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Cuba.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Cuba.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Cuba data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cuba YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cuba, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cuba YYGU daily deaths update `update'.pdf", replace	
...
* graph Cuba YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cuba, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cuba YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Cuba.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Cuba.dta", force
...
save "ALL MODLES Cuba.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Cuba data dictionary.csv", replace 
...
* graph Cuba ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cuba, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(811 "JOHN" 1 "DELP" 34 "IHME" 4 "IMPE" 3 "LANL" 47 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cuba ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Cuba 1 Process.smcl"
