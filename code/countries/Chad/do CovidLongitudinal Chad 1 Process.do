...
log using "log CovidLongitudinal Chad 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Chad 1 Process.do"
...
**** "do CovidLongitudinal Chad 1 Process.do"
...
** output data files: "`model' Chad.dta"
**                   "ALL MODLES Chad.dta"

** output data dictionary files: "Chad ALL MODELS update dates data dictionary.csv"
**                               "`model' Chad data dictionary.csv"
**                               "ALL MODLES Chad data dictionary.csv"

** graphs:
** "graph 02 Chad `model' C19 daily deaths all updates.pdf"
** "graph 02 Chad ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Chad absent update_date_block"
	
	qui keep if loc_grand_name == "Chad"
	...
	save "Chad `model' update dates.dta", replace
...
use "Chad DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Chad `model' update dates.dta"
...
save "Chad ALL MODELS update dates.dta", replace
	
export excel using "Chad ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Chad ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Chad"

keep date DayDeaMeSmJOHNTCD
...
save "JOHN Chad.dta", replace 




* graph Chad JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNTCD date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Chad, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Chad JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Chad.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Chad.dta"
...
merge m:m date using "JOHN Chad.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Chad data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Chad DELP daily deaths each update 
	...
	subtitle("Chad, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Chad DELP daily deaths update `update'.pdf", replace	
...
* graph Chad DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Chad, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Chad DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Chad IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Chad, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Chad IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Chad.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Chad.dta"
...
merge m:1 date using "JOHN Chad.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Chad data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Chad IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Chad, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Chad IHME daily deaths update `update'.pdf", replace	
...
* graph Chad IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Chad, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Chad IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Chad.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Chad.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Chad data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Chad IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Chad, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Chad IMPE daily deaths update `update'.pdf", replace	
...
* graph Chad IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Chad, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Chad IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Chad.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Chad.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Chad data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Chad LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Chad, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Chad LANL daily deaths update `update'.pdf", replace	
...
* graph Chad LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Chad, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Chad LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Chad.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Chad.dta", force
...
save "ALL MODLES Chad.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Chad data dictionary.csv", replace 
...
* graph Chad ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Chad, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(657 "JOHN" 21 "DELP" 64 "IHME" 1 "IMPE" 13 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Chad ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Chad 1 Process.smcl"
