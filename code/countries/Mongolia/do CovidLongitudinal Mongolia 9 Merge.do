...
log using "log CovidLongitudinal Mongolia 9 Merge.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Mongolia 9 Merge.do"
...
** input data files: "`model' Mongolia error.dta"

** output data files: "ALL MODELS Mongolia error.dta" with all date
*                     "ALL MODELS Mongolia error - only models across error measures.dta", replace
*                     "ALL MODELS Mongolia error - only error measures across models.dta", replace

* output data dictionary files: "ALL MODELS Mongolia error data dictionary.csv"
*                               "ALL MODELS Mongolia error - only models across error measures data dictionary.csv"
*                               "ALL MODELS Mongolia error - only error measures across models data dictionary.csv"


** Graphs:
* "graph 03 Mongolia ALL MODELS C19 daily deaths error.pdf"
* "graph 04 Mongolia ALL MODELS C19 daily deaths absolute error.pdf"
* "graph 05 Mongolia ALL MODELS C19 daily deaths percent error.pdf"
* "graph 06 Mongolia ALL MODELS C19 daily deaths absolute percent error.pdf"

* "graph 07a1 Mongolia ALL MODELS C19 daily deaths average median error.pdf"
* "graph 07a2 Mongolia ALL MODELS C19 daily deaths average median error smoothed.pdf"
* "graph 08a1 Mongolia ALL MODELS C19 daily deaths average median absolute error.pdf"
* "graph 08a2 Mongolia ALL MODELS C19 daily deaths average median absolute error smoothed.pdf"
* "graph 09a1 Mongolia ALL MODELS C19 daily deaths average median percent error.pdf"
* "graph 09a2 Mongolia ALL MODELS C19 daily deaths average median percent error smoothed.pdf"
* "graph 10a1 Mongolia ALL MODELS C19 daily deaths average median absolute percent error.pdf"
* "graph 10a2 Mongolia ALL MODELS C19 daily deaths average median absolute percent error smoothed.pdf"

****** graph each same error measure across models:
* "graph 12a Mongolia ALL MODELS C19 daily deaths Average Mean Error.pdf"
* "graph 12b Mongolia ALL MODELS C19 daily deaths Average Absolute Error.pdf"
* "graph 12c Mongolia ALL MODELS C19 daily deaths Average Percent Error.pdf"
* "graph 12d Mongolia ALL MODELS C19 daily deaths Average Absolute Percent Error.pdf"

****** graph all models across error measures:
* "graph 13a Mongolia ALL MODELS C19 daily deaths, models across error measures.pdf"

****** graph all error measures across models:
* "graph 13b Mongolia ALL MODELS C19 daily deaths, error measures across models.pdf"
...
use "DELP Mongolia error.dta", clear 

	merge m:m date using "IHME Mongolia error.dta"

	drop _merge
	
	merge m:m date using "IMPE Mongolia error.dta"
	
	drop _merge
	
	merge m:m date using "LANL Mongolia error.dta"	
...
subtitle("Mongolia, all Models, all updates, forecast only", size(small)) ///
legend(position(6) order(1 "DELP" 191 "IHME" 17 "IMPE" 56 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 03 Mongolia ALL MODELS C19 daily deaths error.pdf", replace
...
qui graph export "graph 04 Mongolia ALL MODELS C19 daily deaths absolute error.pdf", replace
...
qui graph export "graph 05 Mongolia ALL MODELS C19 daily deaths percent error.pdf", replace
...
qui graph export "graph 06 Mongolia ALL MODELS C19 daily deaths absolute percent error.pdf", replace
...
subtitle("Mongolia, all Models, forecast only", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) yscale(titlegap(3)) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 07a1 Mongolia ALL MODELS C19 daily deaths average median error.pdf", replace 
...
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths error measure) title("C19 daily deaths, average median error", size(medium) color(black)) /// 
subtitle("Mongolia, all Models, forecast only; smoothed for better viewing", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) yscale(titlegap(3)) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 07a2 Mongolia ALL MODELS C19 daily deaths average median error smoothed.pdf", replace
...
qui graph export "graph 08a1 Mongolia ALL MODELS C19 daily deaths average median absolute error.pdf", replace 
...
qui graph export "graph 08a2 Mongolia ALL MODELS C19 daily deaths average median absolute error smoothed.pdf", replace
...
qui graph export "graph 09a1 Mongolia ALL MODELS C19 daily deaths average median percent error.pdf", replace
...
qui graph export "graph 09a2 Mongolia ALL MODELS C19 daily deaths average median percent error smoothed.pdf", replace
...
qui graph export "graph 10a1 Mongolia ALL MODELS C19 daily deaths average median absolute percent error.pdf", replace 
...
qui graph export "graph 10a2 Mongolia ALL MODELS C19 daily deaths average median absolute percent error smoothed.pdf", replace
...
subtitle("Over updates and calendar months; forecast only; Mongolia", size(small)) /// 
legend(region(lcolor(none))) legend(bexpand) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL") size(small) row(1)) 

qui graph export "graph 12a Mongolia ALL MODELS C19 daily deaths Average Mean Error.pdf", replace
...
qui graph export "graph 12b Mongolia ALL MODELS C19 daily deaths Average Absolute Error.pdf", replace
...
qui graph export "graph 12c Mongolia ALL MODELS C19 daily deaths Average Percent Error.pdf", replace
...
qui graph export "graph 12d Mongolia ALL MODELS C19 daily deaths Average Absolute Percent Error.pdf", replace
...
save "ALL MODELS Mongolia error.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Mongolia error data dictionary.csv", replace 
...
use "ALL MODELS Mongolia error.dta", clear

save "ALL MODELS Mongolia error - only models across error measures.dta", replace
...
save "CovidLongitudinal Mongolia merge temp.dta", replace 
...
qui graph export "graph 13a Mongolia ALL MODELS C19 daily deaths, models across error measures.pdf", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Mongolia error - only models across error measures data dictionary.csv", replace 
...
use "CovidLongitudinal Mongolia merge temp.dta", clear 
...
qui graph export "graph 13b Mongolia ALL MODELS C19 daily deaths, error measures across models.pdf", replace
...
save "ALL MODELS Mongolia error - only error measures across models.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Mongolia error - only error measures across models data dictionary.csv", replace 
...
view "log CovidLongitudinal Mongolia 9 Merge.smcl"
