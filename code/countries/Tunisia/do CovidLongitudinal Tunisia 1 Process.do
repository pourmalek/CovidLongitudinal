...
log using "log CovidLongitudinal Tunisia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Tunisia 1 Process.do"
...
**** "do CovidLongitudinal Tunisia 1 Process.do"
...
** output data files: "`model' Tunisia.dta"
**                   "ALL MODLES Tunisia.dta"

** output data dictionary files: "Tunisia ALL MODELS update dates data dictionary.csv"
**                               "`model' Tunisia data dictionary.csv"
**                               "ALL MODLES Tunisia data dictionary.csv"

** graphs:
** "graph 02 Tunisia `model' C19 daily deaths all updates.pdf"
** "graph 02 Tunisia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Tunisia absent update_date_block"
	
	qui keep if loc_grand_name == "Tunisia"
	...
	save "Tunisia `model' update dates.dta", replace
...
use "Tunisia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Tunisia `model' update dates.dta"
...
save "Tunisia ALL MODELS update dates.dta", replace
	
export excel using "Tunisia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Tunisia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Tunisia"

keep date DayDeaMeSmJOHNTUN
...
save "JOHN Tunisia.dta", replace 




* graph Tunisia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNTUN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Tunisia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tunisia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Tunisia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Tunisia.dta"
...
merge m:m date using "JOHN Tunisia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Tunisia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Tunisia DELP daily deaths each update 
	...
	subtitle("Tunisia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Tunisia DELP daily deaths update `update'.pdf", replace	
...
* graph Tunisia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tunisia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tunisia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Tunisia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Tunisia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Tunisia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Tunisia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Tunisia.dta"
...
merge m:1 date using "JOHN Tunisia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Tunisia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Tunisia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Tunisia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Tunisia IHME daily deaths update `update'.pdf", replace	
...
* graph Tunisia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tunisia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tunisia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Tunisia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Tunisia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Tunisia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Tunisia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Tunisia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Tunisia IMPE daily deaths update `update'.pdf", replace	
...
* graph Tunisia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tunisia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tunisia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Tunisia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Tunisia.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Tunisia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Tunisia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Tunisia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Tunisia LANL daily deaths update `update'.pdf", replace	
...
* graph Tunisia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tunisia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tunisia LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Tunisia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Tunisia.dta", force
...
save "ALL MODLES Tunisia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Tunisia data dictionary.csv", replace 
...
* graph Tunisia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tunisia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(665 "JOHN" 1 "DELP" 93 "IHME" 4 "IMPE" 3 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tunisia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Tunisia 1 Process.smcl"
