...
log using "log CovidLongitudinal Papua New Guinea 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Papua New Guinea 1 Process.do"
...
**** "do CovidLongitudinal Papua New Guinea 1 Process.do"
...
** output data files: "`model' Papua New Guinea.dta"
**                   "ALL MODLES Papua New Guinea.dta"

** output data dictionary files: "Papua New Guinea ALL MODELS update dates data dictionary.csv"
**                               "`model' Papua New Guinea data dictionary.csv"
**                               "ALL MODLES Papua New Guinea data dictionary.csv"

** graphs:
** "graph 02 Papua New Guinea `model' C19 daily deaths all updates.pdf"
** "graph 02 Papua New Guinea ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Papua New Guinea absent update_date_block"
	
	qui keep if loc_grand_name == "Papua New Guinea"
	...
	save "Papua New Guinea `model' update dates.dta", replace
...
use "Papua New Guinea DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Papua New Guinea `model' update dates.dta"
...
save "Papua New Guinea ALL MODELS update dates.dta", replace
	
export excel using "Papua New Guinea ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Papua New Guinea ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Papua New Guinea"

keep date DayDeaMeSmJOHNPNG
...
save "JOHN Papua New Guinea.dta", replace 




* graph Papua New Guinea JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNPNG date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Papua New Guinea, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Papua New Guinea JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Papua New Guinea.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Papua New Guinea.dta"
...
save "DELP Papua New Guinea.dta", replace
...
merge m:m date using "JOHN Papua New Guinea.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Papua New Guinea data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Papua New Guinea DELP daily deaths each update 
	...
	subtitle("Papua New Guinea, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Papua New Guinea DELP daily deaths update `update'.pdf", replace	
...
* graph Papua New Guinea DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Papua New Guinea, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Papua New Guinea DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	keep if country == "Papua New Guinea" 
	...
	* graph Papua New Guinea IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Papua New Guinea, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Papua New Guinea IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Papua New Guinea.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Papua New Guinea.dta"
...
save "IHME Papua New Guinea.dta", replace
...
merge m:1 date using "JOHN Papua New Guinea.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Papua New Guinea data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Papua New Guinea IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Papua New Guinea, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Papua New Guinea IHME daily deaths update `update'.pdf", replace	
...
* graph Papua New Guinea IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Papua New Guinea, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Papua New Guinea IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Papua New Guinea.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Papua New Guinea.dta"
...
save "IMPE Papua New Guinea.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Papua New Guinea data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Papua New Guinea IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Papua New Guinea, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Papua New Guinea IMPE daily deaths update `update'.pdf", replace	
...
* graph Papua New Guinea IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Papua New Guinea, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Papua New Guinea IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Papua New Guinea.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Papua New Guinea.dta"
...
save "LANL Papua New Guinea.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Papua New Guinea data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Papua New Guinea LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Papua New Guinea, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Papua New Guinea LANL daily deaths update `update'.pdf", replace	
...
* graph Papua New Guinea LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Papua New Guinea, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Papua New Guinea LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Papua New Guinea.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Papua New Guinea.dta", force
...
save "ALL MODLES Papua New Guinea.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Papua New Guinea data dictionary.csv", replace 
...
* graph Papua New Guinea ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Papua New Guinea, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(441 "JOHN" 9 "DELP" 1 "IHME" 3 "IMPE" 66 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Papua New Guinea ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Papua New Guinea 1 Process.smcl"
