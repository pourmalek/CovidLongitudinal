...
log using "log CovidLongitudinal Liberia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Liberia 1 Process.do"
...
**** "do CovidLongitudinal Liberia 1 Process.do"
...
** output data files: "`model' Liberia.dta"
**                   "ALL MODLES Liberia.dta"

** output data dictionary files: "Liberia ALL MODELS update dates data dictionary.csv"
**                               "`model' Liberia data dictionary.csv"
**                               "ALL MODLES Liberia data dictionary.csv"

** graphs:
** "graph 02 Liberia `model' C19 daily deaths all updates.pdf"
** "graph 02 Liberia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Liberia absent update_date_block"
	
	qui keep if loc_grand_name == "Liberia"
	...
	save "Liberia `model' update dates.dta", replace
...
use "Liberia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Liberia `model' update dates.dta"
...
save "Liberia ALL MODELS update dates.dta", replace
	
export excel using "Liberia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Liberia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Liberia"

keep date DayDeaMeSmJOHNLBR
...
save "JOHN Liberia.dta", replace 




* graph Liberia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNLBR date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Liberia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Liberia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Liberia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Liberia.dta"
...
merge m:m date using "JOHN Liberia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Liberia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Liberia DELP daily deaths each update 
	...
	subtitle("Liberia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Liberia DELP daily deaths update `update'.pdf", replace	
...
* graph Liberia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Liberia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Liberia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Liberia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Liberia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Liberia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Liberia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Liberia.dta"
...
merge m:1 date using "JOHN Liberia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Liberia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Liberia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Liberia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Liberia IHME daily deaths update `update'.pdf", replace	
...
* graph Liberia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Liberia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Liberia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Liberia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Liberia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Liberia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Liberia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Liberia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Liberia IMPE daily deaths update `update'.pdf", replace	
...
* graph Liberia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Liberia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Liberia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Liberia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Liberia.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Liberia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Liberia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Liberia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Liberia LANL daily deaths update `update'.pdf", replace	
...
* graph Liberia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Liberia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Liberia LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Liberia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Liberia.dta", force
...
save "ALL MODLES Liberia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Liberia data dictionary.csv", replace 
...
* graph Liberia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Liberia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(665 "JOHN" 4 "DELP" 68 "IHME" 1 "IMPE" 11 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Liberia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Liberia 1 Process.smcl"
