...
log using "log CovidLongitudinal Malaysia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Malaysia 1 Process.do"
...
**** "do CovidLongitudinal Malaysia 1 Process.do"
...
** output data files: "`model' Malaysia.dta"
**                   "ALL MODLES Malaysia.dta"

** output data dictionary files: "Malaysia ALL MODELS update dates data dictionary.csv"
**                               "`model' Malaysia data dictionary.csv"
**                               "ALL MODLES Malaysia data dictionary.csv"

** graphs:
** "graph 02 Malaysia `model' C19 daily deaths all updates.pdf"
** "graph 02 Malaysia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Malaysia absent update_date_block"
	
	qui keep if loc_grand_name == "Malaysia"
	...
	save "Malaysia `model' update dates.dta", replace
...
use "Malaysia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Malaysia `model' update dates.dta"
...
save "Malaysia ALL MODELS update dates.dta", replace
	
export excel using "Malaysia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Malaysia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Malaysia"

keep date DayDeaMeSmJOHNMYS
...
save "JOHN Malaysia.dta", replace 




* graph Malaysia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMYS date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Malaysia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malaysia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Malaysia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Malaysia.dta"
...
save "DELP Malaysia.dta", replace
...
merge m:m date using "JOHN Malaysia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Malaysia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Malaysia DELP daily deaths each update 
	...
	subtitle("Malaysia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Malaysia DELP daily deaths update `update'.pdf", replace	
...
* graph Malaysia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malaysia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malaysia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Malaysia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Malaysia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Malaysia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Malaysia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Malaysia.dta"
...
save "IHME Malaysia.dta", replace
...
merge m:1 date using "JOHN Malaysia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Malaysia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Malaysia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Malaysia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Malaysia IHME daily deaths update `update'.pdf", replace	
...
* graph Malaysia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malaysia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malaysia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Malaysia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Malaysia.dta"
...
save "IMPE Malaysia.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Malaysia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Malaysia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Malaysia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Malaysia IMPE daily deaths update `update'.pdf", replace	
...
* graph Malaysia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malaysia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malaysia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Malaysia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Malaysia.dta"
...
save "LANL Malaysia.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Malaysia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Malaysia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Malaysia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Malaysia LANL daily deaths update `update'.pdf", replace	
...
* graph Malaysia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malaysia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malaysia LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Malaysia.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Malaysia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Malaysia.dta"
...
save "YYGU Malaysia.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Malaysia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Malaysia YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Malaysia, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Malaysia YYGU daily deaths update `update'.pdf", replace	
...
* graph Malaysia YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malaysia, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malaysia YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Malaysia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Malaysia.dta", force
...
save "ALL MODLES Malaysia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Malaysia data dictionary.csv", replace 
...
* graph Malaysia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malaysia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(824 "JOHN" 1 "DELP" 24 "IHME" 4 "IMPE" 3 "LANL" 26 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malaysia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Malaysia 1 Process.smcl"
