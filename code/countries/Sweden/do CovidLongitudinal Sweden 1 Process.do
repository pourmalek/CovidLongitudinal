...
log using "log CovidLongitudinal Sweden 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Sweden 1 Process.do"
...
**** "do CovidLongitudinal Sweden 1 Process.do"
...
** output data files: "`model' Sweden.dta"
**                   "ALL MODLES Sweden.dta"

** output data dictionary files: "Sweden ALL MODELS update dates data dictionary.csv"
**                               "`model' Sweden data dictionary.csv"
**                               "ALL MODLES Sweden data dictionary.csv"

** graphs:
** "graph 02 Sweden `model' C19 daily deaths all updates.pdf"
** "graph 02 Sweden ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Sweden absent update_date_block"
	
	qui keep if loc_grand_name == "Sweden"
	...
	save "Sweden `model' update dates.dta", replace
...
use "Sweden DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Sweden `model' update dates.dta"
...
save "Sweden ALL MODELS update dates.dta", replace
	
export excel using "Sweden ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Sweden ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Sweden"

keep date DayDeaMeSmJOHNSWE
...
save "JOHN Sweden.dta", replace 




* graph Sweden JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSWE date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Sweden, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sweden JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	duplicates drop		
	 ...
	 duplicates drop date loc_grand_name, force
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Sweden.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Sweden.dta"
...
merge m:m date using "JOHN Sweden.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Sweden data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sweden DELP daily deaths each update 
	...
	subtitle("Sweden, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sweden DELP daily deaths update `update'.pdf", replace	
...
* graph Sweden DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sweden, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sweden DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Sweden IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Sweden, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Sweden IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Sweden.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Sweden.dta"
...
merge m:1 date using "JOHN Sweden.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Sweden data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sweden IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Sweden, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sweden IHME daily deaths update `update'.pdf", replace	
...
* graph Sweden IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sweden, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sweden IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Sweden.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Sweden.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Sweden data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sweden IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Sweden, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sweden IMPE daily deaths update `update'.pdf", replace	
...
* graph Sweden IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sweden, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sweden IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Sweden.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Sweden.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Sweden data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sweden LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Sweden, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sweden LANL daily deaths update `update'.pdf", replace	
...
* graph Sweden LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sweden, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sweden LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Sweden.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Sweden.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Sweden.dta"
...
preserve
    ...
    export delimited name varlab using "UCLA Sweden data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sweden UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Sweden, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sweden UCLA daily deaths update `update'.pdf", replace	

}
*/


* graph Sweden UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sweden, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sweden UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Sweden.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Sweden.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Sweden.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Sweden data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sweden YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Sweden, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sweden YYGU daily deaths update `update'.pdf", replace	
...
* graph Sweden YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sweden, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sweden YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Sweden.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Sweden.dta", force
...
save "ALL MODLES Sweden.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Sweden data dictionary.csv", replace 
...
* graph Sweden ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sweden, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(862 "JOHN" 15 "DELP" 1 "IHME" 91 "IMPE" 29 "LANL" 379 "UCLA" 2 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sweden ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Sweden 1 Process.smcl"
