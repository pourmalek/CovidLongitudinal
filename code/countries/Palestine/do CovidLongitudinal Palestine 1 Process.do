...
log using "log CovidLongitudinal Palestine 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Palestine 1 Process.do"
...
**** "do CovidLongitudinal Palestine 1 Process.do"
...
** output data files: "`model' Palestine.dta"
**                   "ALL MODLES Palestine.dta"

** output data dictionary files: "Palestine ALL MODELS update dates data dictionary.csv"
**                               "`model' Palestine data dictionary.csv"
**                               "ALL MODLES Palestine data dictionary.csv"

** graphs:
** "graph 02 Palestine `model' C19 daily deaths all updates.pdf"
** "graph 02 Palestine ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Palestine absent update_date_block"
	
	qui keep if loc_grand_name == "Palestine"
	...
	save "Palestine `model' update dates.dta", replace
...
use "Palestine IHME update dates.dta", clear 

local list IHME IMPE LANL

foreach model of local list {
	
	append using "Palestine `model' update dates.dta"
...
save "Palestine ALL MODELS update dates.dta", replace
	
export excel using "Palestine ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Palestine ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Palestine"

keep date DayDeaMeSmJOHNPSE
...
save "JOHN Palestine.dta", replace 




* graph Palestine JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNPSE date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Palestine, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Palestine JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* graph Palestine IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Palestine, IHME, update `update'", size(medium) color(black)) ///
	...
	*/
	...
	/* qui graph export "graph 00 Palestine IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Palestine.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Palestine.dta"
...
save "IHME Palestine.dta", replace
...
merge m:1 date using "JOHN Palestine.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Palestine data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Palestine IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Palestine, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Palestine IHME daily deaths update `update'.pdf", replace	
...
* graph Palestine IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Palestine, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Palestine IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Palestine.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Palestine.dta"
...
save "IMPE Palestine.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Palestine data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Palestine IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Palestine, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Palestine IMPE daily deaths update `update'.pdf", replace	
...
* graph Palestine IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Palestine, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Palestine IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Palestine.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Palestine.dta"
...
save "LANL Palestine.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Palestine data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Palestine LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Palestine, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Palestine LANL daily deaths update `update'.pdf", replace	
...
* graph Palestine LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Palestine, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Palestine LANL C19 daily deaths all updates.pdf", replace
...
use "IHME Palestine.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Palestine.dta", force
...
save "ALL MODLES Palestine.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Palestine data dictionary.csv", replace 
...
* graph Palestine ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Palestine, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(333 "JOHN" 45 "IHME" 1 "IMPE" 53 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Palestine ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Palestine 1 Process.smcl"
