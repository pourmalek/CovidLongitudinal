...
log using "log CovidLongitudinal Panama 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Panama 1 Process.do"
...
**** "do CovidLongitudinal Panama 1 Process.do"
...
** output data files: "`model' Panama.dta"
**                   "ALL MODLES Panama.dta"

** output data dictionary files: "Panama ALL MODELS update dates data dictionary.csv"
**                               "`model' Panama data dictionary.csv"
**                               "ALL MODLES Panama data dictionary.csv"

** graphs:
** "graph 02 Panama `model' C19 daily deaths all updates.pdf"
** "graph 02 Panama ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Panama absent update_date_block"
	
	qui keep if loc_grand_name == "Panama"
	...
	save "Panama `model' update dates.dta", replace
...
use "Panama DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Panama `model' update dates.dta"
...
save "Panama ALL MODELS update dates.dta", replace
	
export excel using "Panama ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Panama ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Panama"

keep date DayDeaMeSmJOHNPAN
...
save "JOHN Panama.dta", replace 




* graph Panama JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNPAN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Panama, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Panama JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Panama.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Panama.dta"
...
merge m:m date using "JOHN Panama.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Panama data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Panama DELP daily deaths each update 
	...
	subtitle("Panama, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Panama DELP daily deaths update `update'.pdf", replace	
...
* graph Panama DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Panama, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Panama DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Panama IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Panama, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Panama IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Panama.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Panama.dta"
...
merge m:1 date using "JOHN Panama.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Panama data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Panama IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Panama, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Panama IHME daily deaths update `update'.pdf", replace	
...
* graph Panama IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Panama, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Panama IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Panama.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Panama.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Panama data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Panama IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Panama, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Panama IMPE daily deaths update `update'.pdf", replace	
...
* graph Panama IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Panama, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Panama IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Panama.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Panama.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Panama data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Panama LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Panama, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Panama LANL daily deaths update `update'.pdf", replace	
...
* graph Panama LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Panama, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Panama LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Panama.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Panama.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Panama.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Panama data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Panama YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Panama, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Panama YYGU daily deaths update `update'.pdf", replace	
...
* graph Panama YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Panama, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Panama YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Panama.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Panama.dta", force
...
save "ALL MODLES Panama.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Panama data dictionary.csv", replace 
...
* graph Panama ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Panama, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(791 "JOHN" 1 "DELP" 9 "IHME" 41 "IMPE" 3 "LANL" 10 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Panama ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Panama 1 Process.smcl"
