...
log using "log CovidLongitudinal Thailand 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Thailand 1 Process.do"
...
**** "do CovidLongitudinal Thailand 1 Process.do"
...
** output data files: "`model' Thailand.dta"
**                   "ALL MODLES Thailand.dta"

** output data dictionary files: "Thailand ALL MODELS update dates data dictionary.csv"
**                               "`model' Thailand data dictionary.csv"
**                               "ALL MODLES Thailand data dictionary.csv"

** graphs:
** "graph 02 Thailand `model' C19 daily deaths all updates.pdf"
** "graph 02 Thailand ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Thailand absent update_date_block"
	
	qui keep if loc_grand_name == "Thailand"
	...
	save "Thailand `model' update dates.dta", replace
...
use "Thailand DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Thailand `model' update dates.dta"
...
save "Thailand ALL MODELS update dates.dta", replace
	
export excel using "Thailand ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Thailand ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Thailand"

keep date DayDeaMeSmJOHNTHA
...
save "JOHN Thailand.dta", replace 




* graph Thailand JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNTHA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Thailand, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Thailand JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Thailand.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Thailand.dta"
...
save "DELP Thailand.dta", replace
...
merge m:m date using "JOHN Thailand.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Thailand data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Thailand DELP daily deaths each update 
	...
	subtitle("Thailand, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Thailand DELP daily deaths update `update'.pdf", replace	
...
* graph Thailand DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Thailand, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Thailand DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Thailand IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Thailand, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Thailand IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Thailand.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Thailand.dta"
...
save "IHME Thailand.dta", replace
...
merge m:1 date using "JOHN Thailand.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Thailand data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Thailand IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Thailand, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Thailand IHME daily deaths update `update'.pdf", replace	
...
* graph Thailand IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Thailand, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Thailand IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Thailand.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Thailand.dta"
...
save "IMPE Thailand.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Thailand data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Thailand IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Thailand, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Thailand IMPE daily deaths update `update'.pdf", replace	
...
* graph Thailand IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Thailand, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Thailand IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Thailand.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Thailand.dta"
...
save "LANL Thailand.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Thailand data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Thailand LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Thailand, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Thailand LANL daily deaths update `update'.pdf", replace	
...
* graph Thailand LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Thailand, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Thailand LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Thailand.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Thailand.dta", force
...
save "ALL MODLES Thailand.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Thailand data dictionary.csv", replace 
...
* graph Thailand ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Thailand, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(670 "JOHN" 1 "DELP" 73 "IHME" 4 "IMPE" 3 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Thailand ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Thailand 1 Process.smcl"
