...
log using "log CovidLongitudinal Chile 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Chile 1 Process.do"
...
**** "do CovidLongitudinal Chile 1 Process.do"
...
** output data files: "`model' Chile.dta"
**                   "ALL MODLES Chile.dta"

** output data dictionary files: "Chile ALL MODELS update dates data dictionary.csv"
**                               "`model' Chile data dictionary.csv"
**                               "ALL MODLES Chile data dictionary.csv"

** graphs:
** "graph 02 Chile `model' C19 daily deaths all updates.pdf"
** "graph 02 Chile ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Chile absent update_date_block"
	
	qui keep if loc_grand_name == "Chile"
	...
	save "Chile `model' update dates.dta", replace
...
use "Chile DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Chile `model' update dates.dta"
...
save "Chile ALL MODELS update dates.dta", replace
	
export excel using "Chile ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Chile ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Chile"

keep date DayDeaMeSmJOHNCHL
...
save "JOHN Chile.dta", replace 




* graph Chile JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNCHL date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Chile, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Chile JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Chile.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Chile.dta"
...
merge m:m date using "JOHN Chile.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Chile data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Chile DELP daily deaths each update 
	...
	subtitle("Chile, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Chile DELP daily deaths update `update'.pdf", replace	
...
* graph Chile DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Chile, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Chile DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Chile IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Chile, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Chile IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Chile.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Chile.dta"
...
merge m:1 date using "JOHN Chile.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Chile data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Chile IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Chile, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Chile IHME daily deaths update `update'.pdf", replace	
...
* graph Chile IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Chile, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Chile IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Chile.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Chile.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Chile data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Chile IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Chile, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Chile IMPE daily deaths update `update'.pdf", replace	
...
* graph Chile IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Chile, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Chile IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Chile.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Chile.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Chile data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Chile LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Chile, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Chile LANL daily deaths update `update'.pdf", replace	
...
* graph Chile LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Chile, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Chile LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Chile.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Chile.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Chile.dta"
...
preserve
    ...
    export delimited name varlab using "UCLA Chile data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Chile UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Chile, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Chile UCLA daily deaths update `update'.pdf", replace	

}
*/


* graph Chile UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Chile, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Chile UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Chile.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Chile.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Chile.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Chile data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Chile YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Chile, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Chile YYGU daily deaths update `update'.pdf", replace	
...
* graph Chile YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Chile, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Chile YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Chile.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Chile.dta", force
...
save "ALL MODLES Chile.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Chile data dictionary.csv", replace 
...
* graph Chile ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Chile, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(823 "JOHN" 1 "DELP" 9 "IHME" 41 "IMPE" 3 "LANL" 338 "UCLA" 10 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Chile ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Chile 1 Process.smcl"
