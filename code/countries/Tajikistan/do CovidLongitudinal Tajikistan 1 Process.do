...
log using "log CovidLongitudinal Tajikistan 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Tajikistan 1 Process.do"
...
**** "do CovidLongitudinal Tajikistan 1 Process.do"
...
** output data files: "`model' Tajikistan.dta"
**                   "ALL MODLES Tajikistan.dta"

** output data dictionary files: "Tajikistan ALL MODELS update dates data dictionary.csv"
**                               "`model' Tajikistan data dictionary.csv"
**                               "ALL MODLES Tajikistan data dictionary.csv"

** graphs:
** "graph 02 Tajikistan `model' C19 daily deaths all updates.pdf"
** "graph 02 Tajikistan ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Tajikistan absent update_date_block"
	
	qui keep if loc_grand_name == "Tajikistan"
	...
	save "Tajikistan `model' update dates.dta", replace
...
use "Tajikistan DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Tajikistan `model' update dates.dta"
...
save "Tajikistan ALL MODELS update dates.dta", replace
	
export excel using "Tajikistan ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Tajikistan ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Tajikistan"

keep date DayDeaMeSmJOHNTJK
...
save "JOHN Tajikistan.dta", replace 




* graph Tajikistan JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNTJK date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Tajikistan, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tajikistan JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Tajikistan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Tajikistan.dta"
...
save "DELP Tajikistan.dta", replace
...
merge m:m date using "JOHN Tajikistan.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Tajikistan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Tajikistan DELP daily deaths each update 
	...
	subtitle("Tajikistan, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Tajikistan DELP daily deaths update `update'.pdf", replace	
...
* graph Tajikistan DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tajikistan, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tajikistan DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Tajikistan IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Tajikistan, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Tajikistan IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Tajikistan.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Tajikistan.dta"
...
save "IHME Tajikistan.dta", replace
...
merge m:1 date using "JOHN Tajikistan.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Tajikistan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Tajikistan IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Tajikistan, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Tajikistan IHME daily deaths update `update'.pdf", replace	
...
* graph Tajikistan IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tajikistan, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tajikistan IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Tajikistan.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Tajikistan.dta"
...
save "IMPE Tajikistan.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Tajikistan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Tajikistan IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Tajikistan, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Tajikistan IMPE daily deaths update `update'.pdf", replace	
...
* graph Tajikistan IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tajikistan, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tajikistan IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Tajikistan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Tajikistan.dta"
...
save "LANL Tajikistan.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Tajikistan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Tajikistan LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Tajikistan, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Tajikistan LANL daily deaths update `update'.pdf", replace	
...
* graph Tajikistan LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tajikistan, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tajikistan LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Tajikistan.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Tajikistan.dta", force
...
save "ALL MODLES Tajikistan.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Tajikistan data dictionary.csv", replace 
...
* graph Tajikistan ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tajikistan, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(645 "JOHN" 15 "DELP" 58 "IHME" 1 "IMPE" 8 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tajikistan ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Tajikistan 1 Process.smcl"
