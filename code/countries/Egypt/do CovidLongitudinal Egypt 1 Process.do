...
log using "log CovidLongitudinal Egypt 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Egypt 1 Process.do"
...
**** "do CovidLongitudinal Egypt 1 Process.do"
...
** output data files: "`model' Egypt.dta"
**                   "ALL MODLES Egypt.dta"

** output data dictionary files: "Egypt ALL MODELS update dates data dictionary.csv"
**                               "`model' Egypt data dictionary.csv"
**                               "ALL MODLES Egypt data dictionary.csv"

** graphs:
** "graph 02 Egypt `model' C19 daily deaths all updates.pdf"
** "graph 02 Egypt ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Egypt absent update_date_block"
	
	qui keep if loc_grand_name == "Egypt"
	...
	save "Egypt `model' update dates.dta", replace
...
use "Egypt DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Egypt `model' update dates.dta"
...
save "Egypt ALL MODELS update dates.dta", replace
	
export excel using "Egypt ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Egypt ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Egypt"

keep date DayDeaMeSmJOHNEGY
...
save "JOHN Egypt.dta", replace 




* graph Egypt JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNEGY date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Egypt, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Egypt JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Egypt.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Egypt.dta"
...
merge m:m date using "JOHN Egypt.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Egypt data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Egypt DELP daily deaths each update 
	...
	subtitle("Egypt, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Egypt DELP daily deaths update `update'.pdf", replace	
...
* graph Egypt DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Egypt, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Egypt DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Egypt IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Egypt, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Egypt IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Egypt.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Egypt.dta"
...
merge m:1 date using "JOHN Egypt.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Egypt data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Egypt IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Egypt, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Egypt IHME daily deaths update `update'.pdf", replace	
...
* graph Egypt IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Egypt, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Egypt IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Egypt.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Egypt.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Egypt data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Egypt IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Egypt, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Egypt IMPE daily deaths update `update'.pdf", replace	
...
* graph Egypt IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Egypt, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Egypt IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Egypt.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Egypt.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Egypt data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Egypt LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Egypt, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Egypt LANL daily deaths update `update'.pdf", replace	
...
* graph Egypt LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Egypt, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Egypt LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Egypt.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Egypt.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Egypt.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Egypt data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Egypt YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Egypt, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Egypt YYGU daily deaths update `update'.pdf", replace	
...
* graph Egypt YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Egypt, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Egypt YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Egypt.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Egypt.dta", force
...
save "ALL MODLES Egypt.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Egypt data dictionary.csv", replace 
...
* graph Egypt ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Egypt, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(824 "JOHN" 1 "DELP" 23 "IHME" 4 "IMPE" 3 "LANL" 25 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Egypt ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Egypt 1 Process.smcl"
