...
log using "log CovidLongitudinal Bahamas 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Bahamas 1 Process.do"
...
**** "do CovidLongitudinal Bahamas 1 Process.do"
...
** output data files: "`model' Bahamas.dta"
**                   "ALL MODLES Bahamas.dta"

** output data dictionary files: "Bahamas ALL MODELS update dates data dictionary.csv"
**                               "`model' Bahamas data dictionary.csv"
**                               "ALL MODLES Bahamas data dictionary.csv"

** graphs:
** "graph 02 Bahamas `model' C19 daily deaths all updates.pdf"
** "graph 02 Bahamas ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Bahamas absent update_date_block"
	
	qui keep if loc_grand_name == "Bahamas"
	...
	save "Bahamas `model' update dates.dta", replace
...
use "Bahamas IHME update dates.dta", clear 

local list IHME IMPE LANL

foreach model of local list {
	
	append using "Bahamas `model' update dates.dta"
...
save "Bahamas ALL MODELS update dates.dta", replace
	
export excel using "Bahamas ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Bahamas ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Bahamas"

keep date DayDeaMeSmJOHNBHS
...
save "JOHN Bahamas.dta", replace 




* graph Bahamas JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNBHS date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Bahamas, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bahamas JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* graph Bahamas IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Bahamas, IHME, update `update'", size(medium) color(black)) ///
	...
	*/
	...
	/* qui graph export "graph 00 Bahamas IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Bahamas.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Bahamas.dta"
...
merge m:1 date using "JOHN Bahamas.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Bahamas data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bahamas IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bahamas, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bahamas IHME daily deaths update `update'.pdf", replace	
...
* graph Bahamas IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bahamas, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bahamas IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Bahamas.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Bahamas.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Bahamas data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bahamas IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bahamas, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bahamas IMPE daily deaths update `update'.pdf", replace	
...
* graph Bahamas IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bahamas, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bahamas IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Bahamas.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Bahamas.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Bahamas data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bahamas LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bahamas, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bahamas LANL daily deaths update `update'.pdf", replace	
...
* graph Bahamas LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bahamas, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bahamas LANL C19 daily deaths all updates.pdf", replace
...
use "IHME Bahamas.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Bahamas.dta", force
...
save "ALL MODLES Bahamas.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Bahamas data dictionary.csv", replace 
...
* graph Bahamas ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bahamas, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(284 "JOHN" 8 "IHME" 1 "IMPE" 34 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bahamas ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Bahamas 1 Process.smcl"
