...
log using "log CovidLongitudinal Mozambique 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Mozambique 1 Process.do"
...
**** "do CovidLongitudinal Mozambique 1 Process.do"
...
** output data files: "`model' Mozambique.dta"
**                   "ALL MODLES Mozambique.dta"

** output data dictionary files: "Mozambique ALL MODELS update dates data dictionary.csv"
**                               "`model' Mozambique data dictionary.csv"
**                               "ALL MODLES Mozambique data dictionary.csv"

** graphs:
** "graph 02 Mozambique `model' C19 daily deaths all updates.pdf"
** "graph 02 Mozambique ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Mozambique absent update_date_block"
	
	qui keep if loc_grand_name == "Mozambique"
	...
	save "Mozambique `model' update dates.dta", replace
...
use "Mozambique DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Mozambique `model' update dates.dta"
...
save "Mozambique ALL MODELS update dates.dta", replace
	
export excel using "Mozambique ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Mozambique ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Mozambique"

keep date DayDeaMeSmJOHNMOZ
...
save "JOHN Mozambique.dta", replace 




* graph Mozambique JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMOZ date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Mozambique, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mozambique JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Mozambique.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Mozambique.dta"
...
save "DELP Mozambique.dta", replace
...
merge m:m date using "JOHN Mozambique.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Mozambique data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mozambique DELP daily deaths each update 
	...
	subtitle("Mozambique, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mozambique DELP daily deaths update `update'.pdf", replace	
...
* graph Mozambique DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mozambique, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mozambique DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Mozambique IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Mozambique, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Mozambique IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Mozambique.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Mozambique.dta"
...
save "IHME Mozambique.dta", replace
...
merge m:1 date using "JOHN Mozambique.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Mozambique data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mozambique IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mozambique, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mozambique IHME daily deaths update `update'.pdf", replace	
...
* graph Mozambique IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mozambique, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mozambique IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Mozambique.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Mozambique.dta"
...
save "IMPE Mozambique.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Mozambique data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mozambique IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mozambique, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mozambique IMPE daily deaths update `update'.pdf", replace	
...
* graph Mozambique IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mozambique, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mozambique IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Mozambique.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Mozambique.dta"
...
save "LANL Mozambique.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Mozambique data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mozambique LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mozambique, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mozambique LANL daily deaths update `update'.pdf", replace	
...
* graph Mozambique LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mozambique, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mozambique LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Mozambique.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Mozambique.dta", force
...
save "ALL MODLES Mozambique.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Mozambique data dictionary.csv", replace 
...
* graph Mozambique ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mozambique, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(604 "JOHN" 1 "DELP" 23 "IHME" 2 "IMPE" 59 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mozambique ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Mozambique 1 Process.smcl"
