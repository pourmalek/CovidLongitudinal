...
log using "log CovidLongitudinal Bulgaria 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Bulgaria 1 Process.do"
...
**** "do CovidLongitudinal Bulgaria 1 Process.do"
...
** output data files: "`model' Bulgaria.dta"
**                   "ALL MODLES Bulgaria.dta"

** output data dictionary files: "Bulgaria ALL MODELS update dates data dictionary.csv"
**                               "`model' Bulgaria data dictionary.csv"
**                               "ALL MODLES Bulgaria data dictionary.csv"

** graphs:
** "graph 02 Bulgaria `model' C19 daily deaths all updates.pdf"
** "graph 02 Bulgaria ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Bulgaria absent update_date_block"
	
	qui keep if loc_grand_name == "Bulgaria"
	...
	save "Bulgaria `model' update dates.dta", replace
...
use "Bulgaria DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Bulgaria `model' update dates.dta"
...
save "Bulgaria ALL MODELS update dates.dta", replace
	
export excel using "Bulgaria ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Bulgaria ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Bulgaria"

keep date DayDeaMeSmJOHNBGR
...
save "JOHN Bulgaria.dta", replace 




* graph Bulgaria JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNBGR date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Bulgaria, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bulgaria JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Bulgaria.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Bulgaria.dta"
...
save "DELP Bulgaria.dta", replace
...
merge m:m date using "JOHN Bulgaria.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Bulgaria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bulgaria DELP daily deaths each update 
	...
	subtitle("Bulgaria, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bulgaria DELP daily deaths update `update'.pdf", replace	
...
* graph Bulgaria DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bulgaria, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bulgaria DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Bulgaria IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Bulgaria, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Bulgaria IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Bulgaria.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Bulgaria.dta"
...
save "IHME Bulgaria.dta", replace
...
merge m:1 date using "JOHN Bulgaria.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Bulgaria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bulgaria IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bulgaria, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bulgaria IHME daily deaths update `update'.pdf", replace	
...
* graph Bulgaria IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bulgaria, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bulgaria IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Bulgaria.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Bulgaria.dta"
...
save "IMPE Bulgaria.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Bulgaria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bulgaria IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bulgaria, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bulgaria IMPE daily deaths update `update'.pdf", replace	
...
* graph Bulgaria IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bulgaria, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bulgaria IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Bulgaria.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Bulgaria.dta"
...
save "LANL Bulgaria.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Bulgaria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bulgaria LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bulgaria, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bulgaria LANL daily deaths update `update'.pdf", replace	
...
* graph Bulgaria LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bulgaria, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bulgaria LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Bulgaria.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Bulgaria.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Bulgaria.dta"
...
save "YYGU Bulgaria.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Bulgaria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bulgaria YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bulgaria, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bulgaria YYGU daily deaths update `update'.pdf", replace	
...
* graph Bulgaria YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bulgaria, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bulgaria YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Bulgaria.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Bulgaria.dta", force
...
save "ALL MODLES Bulgaria.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Bulgaria data dictionary.csv", replace 
...
* graph Bulgaria ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bulgaria, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(873 "JOHN" 15 "DELP" 1 "IHME" 34 "IMPE" 29 "LANL" 2 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bulgaria ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Bulgaria 1 Process.smcl"
