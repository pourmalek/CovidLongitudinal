...
log using "log CovidLongitudinal Cote d'Ivoire 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Cote d'Ivoire 1 Process.do"
...
**** "do CovidLongitudinal Cote d'Ivoire 1 Process.do"
...
** output data files: "`model' Cote d'Ivoire.dta"
**                   "ALL MODLES Cote d'Ivoire.dta"

** output data dictionary files: "Cote d'Ivoire ALL MODELS update dates data dictionary.csv"
**                               "`model' Cote d'Ivoire data dictionary.csv"
**                               "ALL MODLES Cote d'Ivoire data dictionary.csv"

** graphs:
** "graph 02 Cote d'Ivoire `model' C19 daily deaths all updates.pdf"
** "graph 02 Cote d'Ivoire ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Cote d'Ivoire absent update_date_block"
	
	qui keep if loc_grand_name == "Cote d'Ivoire"
	...
	save "Cote d'Ivoire `model' update dates.dta", replace
...
use "Cote d'Ivoire DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Cote d'Ivoire `model' update dates.dta"
...
save "Cote d'Ivoire ALL MODELS update dates.dta", replace
	
export excel using "Cote d'Ivoire ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Cote d'Ivoire ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Cote d'Ivoire"

keep date DayDeaMeSmJOHNCIV
...
save "JOHN Cote d'Ivoire.dta", replace 




* graph Cote d'Ivoire JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNCIV date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Cote d'Ivoire, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cote d'Ivoire JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Cote d'Ivoire.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Cote d'Ivoire.dta"
...
save "DELP Cote d'Ivoire.dta", replace
...
merge m:m date using "JOHN Cote d'Ivoire.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Cote d'Ivoire data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cote d'Ivoire DELP daily deaths each update 
	...
	subtitle("Cote d'Ivoire, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cote d'Ivoire DELP daily deaths update `update'.pdf", replace	
...
* graph Cote d'Ivoire DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cote d'Ivoire, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cote d'Ivoire DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Cote d'Ivoire IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Cote d'Ivoire, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Cote d'Ivoire IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Cote d'Ivoire.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Cote d'Ivoire.dta"
...
save "IHME Cote d'Ivoire.dta", replace
...
merge m:1 date using "JOHN Cote d'Ivoire.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Cote d'Ivoire data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cote d'Ivoire IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cote d'Ivoire, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cote d'Ivoire IHME daily deaths update `update'.pdf", replace	
...
* graph Cote d'Ivoire IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cote d'Ivoire, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cote d'Ivoire IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Cote d'Ivoire.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Cote d'Ivoire.dta"
...
save "IMPE Cote d'Ivoire.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Cote d'Ivoire data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cote d'Ivoire IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cote d'Ivoire, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cote d'Ivoire IMPE daily deaths update `update'.pdf", replace	
...
* graph Cote d'Ivoire IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cote d'Ivoire, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cote d'Ivoire IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Cote d'Ivoire.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Cote d'Ivoire.dta"
...
save "LANL Cote d'Ivoire.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Cote d'Ivoire data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cote d'Ivoire LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cote d'Ivoire, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cote d'Ivoire LANL daily deaths update `update'.pdf", replace	
...
* graph Cote d'Ivoire LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cote d'Ivoire, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cote d'Ivoire LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Cote d'Ivoire.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Cote d'Ivoire.dta", force
...
save "ALL MODLES Cote d'Ivoire.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Cote d'Ivoire data dictionary.csv", replace 
...
* graph Cote d'Ivoire ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cote d'Ivoire, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(657 "JOHN" 1 "DELP" 122 "IHME" 3 "IMPE" 18 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cote d'Ivoire ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Cote d'Ivoire 1 Process.smcl"
