...
log using "log CovidLongitudinal Monaco 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Monaco 1 Process.do"
...
**** "do CovidLongitudinal Monaco 1 Process.do"
...
** output data files: "`model' Monaco.dta"
**                   "ALL MODLES Monaco.dta"

** output data dictionary files: "Monaco ALL MODELS update dates data dictionary.csv"
**                               "`model' Monaco data dictionary.csv"
**                               "ALL MODLES Monaco data dictionary.csv"

** graphs:
** "graph 02 Monaco `model' C19 daily deaths all updates.pdf"
** "graph 02 Monaco ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Monaco absent update_date_block"
	
	qui keep if loc_grand_name == "Monaco"
	...
	save "Monaco `model' update dates.dta", replace
...
use "Monaco DELP update dates.dta", clear 

local list DELP LANL

foreach model of local list {
	
	append using "Monaco `model' update dates.dta"
...
save "Monaco ALL MODELS update dates.dta", replace
	
export excel using "Monaco ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Monaco ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Monaco"

keep date DayDeaMeSmJOHNMCO
...
save "JOHN Monaco.dta", replace 




* graph Monaco JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMCO date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Monaco, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Monaco JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Monaco.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Monaco.dta"
...
merge m:m date using "JOHN Monaco.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Monaco data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Monaco DELP daily deaths each update 
	...
	subtitle("Monaco, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Monaco DELP daily deaths update `update'.pdf", replace	
...
* graph Monaco DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Monaco, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Monaco DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Monaco.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Monaco.dta"
...
merge m:1 date using "JOHN Monaco.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Monaco data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Monaco LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Monaco, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Monaco LANL daily deaths update `update'.pdf", replace	
...
* graph Monaco LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Monaco, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Monaco LANL C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Monaco.dta", force
...
save "ALL MODLES Monaco.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Monaco data dictionary.csv", replace 
...
* graph Monaco ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Monaco, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(351 "JOHN" 1 "DELP" 19 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Monaco ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Monaco 1 Process.smcl"
