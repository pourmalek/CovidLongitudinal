...
log using "log CovidLongitudinal Senegal 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Senegal 1 Process.do"
...
**** "do CovidLongitudinal Senegal 1 Process.do"
...
** output data files: "`model' Senegal.dta"
**                   "ALL MODLES Senegal.dta"

** output data dictionary files: "Senegal ALL MODELS update dates data dictionary.csv"
**                               "`model' Senegal data dictionary.csv"
**                               "ALL MODLES Senegal data dictionary.csv"

** graphs:
** "graph 02 Senegal `model' C19 daily deaths all updates.pdf"
** "graph 02 Senegal ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Senegal absent update_date_block"
	
	qui keep if loc_grand_name == "Senegal"
	...
	save "Senegal `model' update dates.dta", replace
...
use "Senegal DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Senegal `model' update dates.dta"
...
save "Senegal ALL MODELS update dates.dta", replace
	
export excel using "Senegal ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Senegal ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Senegal"

keep date DayDeaMeSmJOHNSEN
...
save "JOHN Senegal.dta", replace 




* graph Senegal JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSEN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Senegal, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Senegal JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Senegal.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Senegal.dta"
...
merge m:m date using "JOHN Senegal.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Senegal data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Senegal DELP daily deaths each update 
	...
	subtitle("Senegal, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
...
* graph Senegal DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Senegal, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Senegal DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Senegal IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Senegal, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Senegal IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Senegal.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Senegal.dta"
...
merge m:1 date using "JOHN Senegal.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Senegal data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Senegal IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Senegal, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
...
* graph Senegal IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Senegal, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Senegal IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Senegal.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Senegal.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Senegal data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Senegal IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Senegal, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
...
* graph Senegal IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Senegal, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Senegal IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Senegal.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Senegal.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Senegal data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Senegal LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Senegal, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
...
* graph Senegal LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Senegal, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Senegal LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Senegal.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Senegal.dta", force
...
save "ALL MODLES Senegal.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Senegal data dictionary.csv", replace 
...
* graph Senegal ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Senegal, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(665 "JOHN" 1 "DELP" 68 "IHME" 3 "IMPE" 21 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Senegal ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Senegal 1 Process.smcl"
