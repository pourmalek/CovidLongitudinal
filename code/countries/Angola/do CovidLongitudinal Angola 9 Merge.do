...
log using "log CovidLongitudinal Angola 9 Merge.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Angola 9 Merge.do"
...
** input data files: "`model' Angola error.dta"

** output data files: "ALL MODELS Angola error.dta" with all date
*                     "ALL MODELS Angola error - only models across error measures.dta", replace
*                     "ALL MODELS Angola error - only error measures across models.dta", replace

* output data dictionary files: "ALL MODELS Angola error data dictionary.csv"
*                               "ALL MODELS Angola error - only models across error measures data dictionary.csv"
*                               "ALL MODELS Angola error - only error measures across models data dictionary.csv"


** Graphs:
* "graph 03 Angola ALL MODELS C19 daily deaths error.pdf"
* "graph 04 Angola ALL MODELS C19 daily deaths absolute error.pdf"
* "graph 05 Angola ALL MODELS C19 daily deaths percent error.pdf"
* "graph 06 Angola ALL MODELS C19 daily deaths absolute percent error.pdf"

* "graph 07a1 Angola ALL MODELS C19 daily deaths average median error.pdf"
* "graph 07a2 Angola ALL MODELS C19 daily deaths average median error smoothed.pdf"
* "graph 08a1 Angola ALL MODELS C19 daily deaths average median absolute error.pdf"
* "graph 08a2 Angola ALL MODELS C19 daily deaths average median absolute error smoothed.pdf"
* "graph 09a1 Angola ALL MODELS C19 daily deaths average median percent error.pdf"
* "graph 09a2 Angola ALL MODELS C19 daily deaths average median percent error smoothed.pdf"
* "graph 10a1 Angola ALL MODELS C19 daily deaths average median absolute percent error.pdf"
* "graph 10a2 Angola ALL MODELS C19 daily deaths average median absolute percent error smoothed.pdf"

****** graph each same error measure across models:
* "graph 12a Angola ALL MODELS C19 daily deaths Average Mean Error.pdf"
* "graph 12b Angola ALL MODELS C19 daily deaths Average Absolute Error.pdf"
* "graph 12c Angola ALL MODELS C19 daily deaths Average Percent Error.pdf"
* "graph 12d Angola ALL MODELS C19 daily deaths Average Absolute Percent Error.pdf"

****** graph all models across error measures:
* "graph 13a Angola ALL MODELS C19 daily deaths, models across error measures.pdf"

****** graph all error measures across models:
* "graph 13b Angola ALL MODELS C19 daily deaths, error measures across models.pdf"
...
use "DELP Angola error.dta", clear 

	merge m:m date using "IHME Angola error.dta"

	drop _merge
	
	merge m:m date using "IMPE Angola error.dta"
	
	drop _merge
	
	merge m:m date using "LANL Angola error.dta"	
...
subtitle("Angola, all Models, all updates, forecast only", size(small)) ///
legend(position(6) order(43 "DELP" 46 "IHME" 1 "IMPE" 57 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 03 Angola ALL MODELS C19 daily deaths error.pdf", replace
...
qui graph export "graph 04 Angola ALL MODELS C19 daily deaths absolute error.pdf", replace
...
qui graph export "graph 05 Angola ALL MODELS C19 daily deaths percent error.pdf", replace
...
qui graph export "graph 06 Angola ALL MODELS C19 daily deaths absolute percent error.pdf", replace
...
subtitle("Angola, all Models, forecast only", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) yscale(titlegap(3)) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 07a1 Angola ALL MODELS C19 daily deaths average median error.pdf", replace 
...
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths error measure) title("C19 daily deaths, average median error", size(medium) color(black)) /// 
subtitle("Angola, all Models, forecast only; smoothed for better viewing", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) yscale(titlegap(3)) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 07a2 Angola ALL MODELS C19 daily deaths average median error smoothed.pdf", replace
...
qui graph export "graph 08a1 Angola ALL MODELS C19 daily deaths average median absolute error.pdf", replace 
...
qui graph export "graph 08a2 Angola ALL MODELS C19 daily deaths average median absolute error smoothed.pdf", replace
...
qui graph export "graph 09a1 Angola ALL MODELS C19 daily deaths average median percent error.pdf", replace
...
qui graph export "graph 09a2 Angola ALL MODELS C19 daily deaths average median percent error smoothed.pdf", replace
...
qui graph export "graph 10a1 Angola ALL MODELS C19 daily deaths average median absolute percent error.pdf", replace 
...
qui graph export "graph 10a2 Angola ALL MODELS C19 daily deaths average median absolute percent error smoothed.pdf", replace
...
subtitle("Over updates and calendar months; forecast only; Angola", size(small)) /// 
legend(region(lcolor(none))) legend(bexpand) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL") size(small) row(1)) 

qui graph export "graph 12a Angola ALL MODELS C19 daily deaths Average Mean Error.pdf", replace
...
qui graph export "graph 12b Angola ALL MODELS C19 daily deaths Average Absolute Error.pdf", replace
...
qui graph export "graph 12c Angola ALL MODELS C19 daily deaths Average Percent Error.pdf", replace
...
qui graph export "graph 12d Angola ALL MODELS C19 daily deaths Average Absolute Percent Error.pdf", replace
...
save "ALL MODELS Angola error.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Angola error data dictionary.csv", replace 
...
use "ALL MODELS Angola error.dta", clear

save "ALL MODELS Angola error - only models across error measures.dta", replace
...
save "CovidLongitudinal Angola merge temp.dta", replace 
...
qui graph export "graph 13a Angola ALL MODELS C19 daily deaths, models across error measures.pdf", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Angola error - only models across error measures data dictionary.csv", replace 
...
use "CovidLongitudinal Angola merge temp.dta", clear 
...
qui graph export "graph 13b Angola ALL MODELS C19 daily deaths, error measures across models.pdf", replace
...
save "ALL MODELS Angola error - only error measures across models.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Angola error - only error measures across models data dictionary.csv", replace 
...
view "log CovidLongitudinal Angola 9 Merge.smcl"
