...
log using "log CovidLongitudinal Morocco 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Morocco 1 Process.do"
...
**** "do CovidLongitudinal Morocco 1 Process.do"
...
** output data files: "`model' Morocco.dta"
**                   "ALL MODLES Morocco.dta"

** output data dictionary files: "Morocco ALL MODELS update dates data dictionary.csv"
**                               "`model' Morocco data dictionary.csv"
**                               "ALL MODLES Morocco data dictionary.csv"

** graphs:
** "graph 02 Morocco `model' C19 daily deaths all updates.pdf"
** "graph 02 Morocco ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Morocco absent update_date_block"
	
	qui keep if loc_grand_name == "Morocco"
	...
	save "Morocco `model' update dates.dta", replace
...
use "Morocco DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Morocco `model' update dates.dta"
...
save "Morocco ALL MODELS update dates.dta", replace
	
export excel using "Morocco ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Morocco ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Morocco"

keep date DayDeaMeSmJOHNMAR
...
save "JOHN Morocco.dta", replace 




* graph Morocco JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMAR date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Morocco, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Morocco JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Morocco.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Morocco.dta"
...
merge m:m date using "JOHN Morocco.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Morocco data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Morocco DELP daily deaths each update 
	...
	subtitle("Morocco, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Morocco DELP daily deaths update `update'.pdf", replace	
...
* graph Morocco DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Morocco, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Morocco DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Morocco IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Morocco, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Morocco IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Morocco.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Morocco.dta"
...
merge m:1 date using "JOHN Morocco.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Morocco data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Morocco IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Morocco, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Morocco IHME daily deaths update `update'.pdf", replace	
...
* graph Morocco IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Morocco, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Morocco IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Morocco.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Morocco.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Morocco data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Morocco IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Morocco, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Morocco IMPE daily deaths update `update'.pdf", replace	
...
* graph Morocco IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Morocco, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Morocco IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Morocco.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Morocco.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Morocco data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Morocco LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Morocco, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Morocco LANL daily deaths update `update'.pdf", replace	
...
* graph Morocco LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Morocco, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Morocco LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Morocco.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Morocco.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Morocco.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Morocco data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Morocco YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Morocco, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Morocco YYGU daily deaths update `update'.pdf", replace	
...
* graph Morocco YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Morocco, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Morocco YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Morocco.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Morocco.dta", force
...
save "ALL MODLES Morocco.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Morocco data dictionary.csv", replace 
...
* graph Morocco ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Morocco, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(813 "JOHN" 1 "DELP" 116 "IHME" 4 "IMPE" 3 "LANL" 24 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Morocco ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Morocco 1 Process.smcl"
