...
log using "log CovidLongitudinal Cape Verde 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Cape Verde 1 Process.do"
...
**** "do CovidLongitudinal Cape Verde 1 Process.do"
...
** output data files: "`model' Cape Verde.dta"
**                   "ALL MODLES Cape Verde.dta"

** output data dictionary files: "Cape Verde ALL MODELS update dates data dictionary.csv"
**                               "`model' Cape Verde data dictionary.csv"
**                               "ALL MODLES Cape Verde data dictionary.csv"

** graphs:
** "graph 02 Cape Verde `model' C19 daily deaths all updates.pdf"
** "graph 02 Cape Verde ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Cape Verde absent update_date_block"
	
	qui keep if loc_grand_name == "Cape Verde"
	...
	save "Cape Verde `model' update dates.dta", replace
...
use "Cape Verde DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Cape Verde `model' update dates.dta"
...
save "Cape Verde ALL MODELS update dates.dta", replace
	
export excel using "Cape Verde ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Cape Verde ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Cape Verde"

keep date DayDeaMeSmJOHNCPV
...
save "JOHN Cape Verde.dta", replace 




* graph Cape Verde JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNCPV date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Cape Verde, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cape Verde JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Cape Verde.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Cape Verde.dta"
...
save "DELP Cape Verde.dta", replace
...
merge m:m date using "JOHN Cape Verde.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Cape Verde data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cape Verde DELP daily deaths each update 
	...
	subtitle("Cape Verde, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cape Verde DELP daily deaths update `update'.pdf", replace	
...
* graph Cape Verde DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cape Verde, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cape Verde DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Cape Verde IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Cape Verde, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Cape Verde IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Cape Verde.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Cape Verde.dta"
...
save "IHME Cape Verde.dta", replace
...
merge m:1 date using "JOHN Cape Verde.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Cape Verde data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cape Verde IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cape Verde, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cape Verde IHME daily deaths update `update'.pdf", replace	
...
* graph Cape Verde IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cape Verde, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cape Verde IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Cape Verde.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Cape Verde.dta"
...
save "IMPE Cape Verde.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Cape Verde data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cape Verde IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cape Verde, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cape Verde IMPE daily deaths update `update'.pdf", replace	
...
* graph Cape Verde IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cape Verde, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cape Verde IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Cape Verde.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Cape Verde.dta"
...
save "LANL Cape Verde.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Cape Verde data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cape Verde LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cape Verde, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cape Verde LANL daily deaths update `update'.pdf", replace	
...
* graph Cape Verde LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cape Verde, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cape Verde LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Cape Verde.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Cape Verde.dta", force
...
save "ALL MODLES Cape Verde.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Cape Verde data dictionary.csv", replace 
...
* graph Cape Verde ALL MODELS daily deaths all updates
...
legend(position(6) order(642 "JOHN" 12 "DELP" 52 "IHME" 1 "IMPE" 69 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cape Verde ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Cape Verde 1 Process.smcl"
