...
log using "log CovidLongitudinal Lesotho 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Lesotho 1 Process.do"
...
**** "do CovidLongitudinal Lesotho 1 Process.do"
...
** output data files: "`model' Lesotho.dta"
**                   "ALL MODLES Lesotho.dta"

** output data dictionary files: "Lesotho ALL MODELS update dates data dictionary.csv"
**                               "`model' Lesotho data dictionary.csv"
**                               "ALL MODLES Lesotho data dictionary.csv"

** graphs:
** "graph 02 Lesotho `model' C19 daily deaths all updates.pdf"
** "graph 02 Lesotho ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Lesotho absent update_date_block"
	
	qui keep if loc_grand_name == "Lesotho"
	...
	save "Lesotho `model' update dates.dta", replace
...
use "Lesotho DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Lesotho `model' update dates.dta"
...
save "Lesotho ALL MODELS update dates.dta", replace
	
export excel using "Lesotho ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Lesotho ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Lesotho"

keep date DayDeaMeSmJOHNLSO
...
save "JOHN Lesotho.dta", replace 




* graph Lesotho JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNLSO date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Lesotho, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lesotho JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Lesotho.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Lesotho.dta"
...
merge m:m date using "JOHN Lesotho.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Lesotho data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Lesotho DELP daily deaths each update 
	...
	subtitle("Lesotho, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Lesotho DELP daily deaths update `update'.pdf", replace	
...
* graph Lesotho DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lesotho, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lesotho DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Lesotho IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Lesotho, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Lesotho IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Lesotho.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Lesotho.dta"
...
merge m:1 date using "JOHN Lesotho.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Lesotho data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Lesotho IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Lesotho, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Lesotho IHME daily deaths update `update'.pdf", replace	
...
* graph Lesotho IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lesotho, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lesotho IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Lesotho.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Lesotho.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Lesotho data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Lesotho IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Lesotho, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Lesotho IMPE daily deaths update `update'.pdf", replace	
...
* graph Lesotho IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lesotho, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lesotho IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Lesotho.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Lesotho.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Lesotho data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Lesotho LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Lesotho, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Lesotho LANL daily deaths update `update'.pdf", replace	
...
* graph Lesotho LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lesotho, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lesotho LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Lesotho.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Lesotho.dta", force
...
save "ALL MODLES Lesotho.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Lesotho data dictionary.csv", replace 
...
* graph Lesotho ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lesotho, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(510 "JOHN" 4 "DELP" 282 "IHME" 1 "IMPE" 12 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lesotho ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Lesotho 1 Process.smcl"
