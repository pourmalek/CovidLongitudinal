...
log using "log CovidLongitudinal Argentina 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Argentina 1 Process.do"
...
**** "do CovidLongitudinal Argentina 1 Process.do"
...
** output data files: "`model' Argentina.dta"
**                   "ALL MODLES Argentina.dta"

** output data dictionary files: "Argentina ALL MODELS update dates data dictionary.csv"
**                               "`model' Argentina data dictionary.csv"
**                               "ALL MODLES Argentina data dictionary.csv"

** graphs:
** "graph 02 Argentina `model' C19 daily deaths all updates.pdf"
** "graph 02 Argentina ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Argentina absent update_date_block"
	
	qui keep if loc_grand_name == "Argentina"
	...
	save "Argentina `model' update dates.dta", replace
...
use "Argentina DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Argentina `model' update dates.dta"
...
save "Argentina ALL MODELS update dates.dta", replace
	
export excel using "Argentina ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Argentina ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Argentina"

keep date DayDeaMeSmJOHNARG
...
save "JOHN Argentina.dta", replace 




* graph Argentina JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNARG date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Argentina, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Argentina JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Argentina.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Argentina.dta"
...
save "DELP Argentina.dta", replace
...
merge m:m date using "JOHN Argentina.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Argentina data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Argentina DELP daily deaths each update 
	...
	subtitle("Argentina, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Argentina DELP daily deaths update `update'.pdf", replace	
...
* graph Argentina DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Argentina, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Argentina DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Argentina IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Argentina, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Argentina IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Argentina.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Argentina.dta"
...
save "IHME Argentina.dta", replace
...
merge m:1 date using "JOHN Argentina.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Argentina data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Argentina IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Argentina, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Argentina IHME daily deaths update `update'.pdf", replace	
...
* graph Argentina IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Argentina, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Argentina IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Argentina.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Argentina.dta"
...
save "IMPE Argentina.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Argentina data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Argentina IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Argentina, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Argentina IMPE daily deaths update `update'.pdf", replace	
...
* graph Argentina IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Argentina, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Argentina IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Argentina.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Argentina.dta"
...
save "LANL Argentina.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Argentina data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Argentina LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Argentina, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Argentina LANL daily deaths update `update'.pdf", replace	
...
* graph Argentina LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Argentina, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Argentina LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Argentina.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Argentina.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Argentina.dta"
...
save "UCLA Argentina.dta", replace
...
preserve
    ...
    export delimited name varlab using "UCLA Argentina data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Argentina UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Argentina, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Argentina UCLA daily deaths update `update'.pdf", replace	

}
*


* graph Argentina UCLA daily deaths all updates

twoway ///
(line DayDeaMeSmJOHNARG date, sort lwidth(thick) lcolor(cyan)) /// 
(line DayDeaMeFoUCLA20200103 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200110 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200206 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200307 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200315 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200328 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200404 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200712 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200719 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200726 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200802 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200809 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200816 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200823 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200830 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200906 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200913 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200920 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20200927 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20201004 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20201011 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20201018 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20201025 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20201101 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20201108 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20201122 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20201129 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20201206 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20201213 date, sort lwidth(thin) lcolor(blue)) ///
(line DayDeaMeFoUCLA20201220 date, sort lwidth(thin) lcolor(blue)) ///
if date >= td(01jan2020) & date <= td(01jan2023) ///
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Argentina, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Argentina UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Argentina.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Argentina.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Argentina.dta"
...
save "YYGU Argentina.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Argentina data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Argentina YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Argentina, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Argentina YYGU daily deaths update `update'.pdf", replace	
...
* graph Argentina YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Argentina, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Argentina YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Argentina.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Argentina.dta", force
...
save "ALL MODLES Argentina.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Argentina data dictionary.csv", replace 
...
* graph Argentina ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Argentina, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(856 "JOHN" 8 "DELP" 31 "IHME" 11 "IMPE" 10 "LANL" 1 "UCLA" 33 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Argentina ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Argentina 1 Process.smcl"
