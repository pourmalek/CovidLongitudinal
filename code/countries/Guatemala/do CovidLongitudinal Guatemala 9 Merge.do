...
log using "log CovidLongitudinal Guatemala 9 Merge.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Guatemala 9 Merge.do"
...
** input data files: "`model' Guatemala error.dta"

** output data files: "ALL MODELS Guatemala error.dta" with all date
*                     "ALL MODELS Guatemala error - only models across error measures.dta", replace
*                     "ALL MODELS Guatemala error - only error measures across models.dta", replace

* output data dictionary files: "ALL MODELS Guatemala error data dictionary.csv"
*                               "ALL MODELS Guatemala error - only models across error measures data dictionary.csv"
*                               "ALL MODELS Guatemala error - only error measures across models data dictionary.csv"


** Graphs:
* "graph 03 Guatemala ALL MODELS C19 daily deaths error.pdf"
* "graph 04 Guatemala ALL MODELS C19 daily deaths absolute error.pdf"
* "graph 05 Guatemala ALL MODELS C19 daily deaths percent error.pdf"
* "graph 06 Guatemala ALL MODELS C19 daily deaths absolute percent error.pdf"

* "graph 07a1 Guatemala ALL MODELS C19 daily deaths average median error.pdf"
* "graph 07a2 Guatemala ALL MODELS C19 daily deaths average median error smoothed.pdf"
* "graph 08a1 Guatemala ALL MODELS C19 daily deaths average median absolute error.pdf"
* "graph 08a2 Guatemala ALL MODELS C19 daily deaths average median absolute error smoothed.pdf"
* "graph 09a1 Guatemala ALL MODELS C19 daily deaths average median percent error.pdf"
* "graph 09a2 Guatemala ALL MODELS C19 daily deaths average median percent error smoothed.pdf"
* "graph 10a1 Guatemala ALL MODELS C19 daily deaths average median absolute percent error.pdf"
* "graph 10a2 Guatemala ALL MODELS C19 daily deaths average median absolute percent error smoothed.pdf"

****** graph each same error measure across models:
* "graph 12a Guatemala ALL MODELS C19 daily deaths Average Mean Error.pdf"
* "graph 12b Guatemala ALL MODELS C19 daily deaths Average Absolute Error.pdf"
* "graph 12c Guatemala ALL MODELS C19 daily deaths Average Percent Error.pdf"
* "graph 12d Guatemala ALL MODELS C19 daily deaths Average Absolute Percent Error.pdf"

****** graph all models across error measures:
* "graph 13a Guatemala ALL MODELS C19 daily deaths, models across error measures.pdf"

****** graph all error measures across models:
* "graph 13b Guatemala ALL MODELS C19 daily deaths, error measures across models.pdf"
...
use "DELP Guatemala error.dta", clear 

	merge m:m date using "IHME Guatemala error.dta"

	drop _merge
	
	merge m:m date using "IMPE Guatemala error.dta"
	
	drop _merge
	
	merge m:m date using "LANL Guatemala error.dta"	
...
subtitle("Guatemala, all Models, all updates, forecast only", size(small)) ///
legend(position(6) order(1 "DELP" 69 "IHME" 2 "IMPE" 12 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 03 Guatemala ALL MODELS C19 daily deaths error.pdf", replace
...
qui graph export "graph 04 Guatemala ALL MODELS C19 daily deaths absolute error.pdf", replace
...
qui graph export "graph 05 Guatemala ALL MODELS C19 daily deaths percent error.pdf", replace
...
qui graph export "graph 06 Guatemala ALL MODELS C19 daily deaths absolute percent error.pdf", replace
...
subtitle("Guatemala, all Models, forecast only", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) yscale(titlegap(3)) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 07a1 Guatemala ALL MODELS C19 daily deaths average median error.pdf", replace 
...
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths error measure) title("C19 daily deaths, average median error", size(medium) color(black)) /// 
subtitle("Guatemala, all Models, forecast only; smoothed for better viewing", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) yscale(titlegap(3)) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 07a2 Guatemala ALL MODELS C19 daily deaths average median error smoothed.pdf", replace
...
qui graph export "graph 08a1 Guatemala ALL MODELS C19 daily deaths average median absolute error.pdf", replace 
...
qui graph export "graph 08a2 Guatemala ALL MODELS C19 daily deaths average median absolute error smoothed.pdf", replace
...
qui graph export "graph 09a1 Guatemala ALL MODELS C19 daily deaths average median percent error.pdf", replace
...
qui graph export "graph 09a2 Guatemala ALL MODELS C19 daily deaths average median percent error smoothed.pdf", replace
...
qui graph export "graph 10a1 Guatemala ALL MODELS C19 daily deaths average median absolute percent error.pdf", replace 
...
qui graph export "graph 10a2 Guatemala ALL MODELS C19 daily deaths average median absolute percent error smoothed.pdf", replace
...
subtitle("Over updates and calendar months; forecast only; Guatemala", size(small)) /// 
legend(region(lcolor(none))) legend(bexpand) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL") size(small) row(1)) 

qui graph export "graph 12a Guatemala ALL MODELS C19 daily deaths Average Mean Error.pdf", replace
...
qui graph export "graph 12b Guatemala ALL MODELS C19 daily deaths Average Absolute Error.pdf", replace
...
qui graph export "graph 12c Guatemala ALL MODELS C19 daily deaths Average Percent Error.pdf", replace
...
qui graph export "graph 12d Guatemala ALL MODELS C19 daily deaths Average Absolute Percent Error.pdf", replace
...
save "ALL MODELS Guatemala error.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Guatemala error data dictionary.csv", replace 
...
use "ALL MODELS Guatemala error.dta", clear

save "ALL MODELS Guatemala error - only models across error measures.dta", replace
...
save "CovidLongitudinal Guatemala merge temp.dta", replace 
...
qui graph export "graph 13a Guatemala ALL MODELS C19 daily deaths, models across error measures.pdf", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Guatemala error - only models across error measures data dictionary.csv", replace 
...
use "CovidLongitudinal Guatemala merge temp.dta", clear 
...
qui graph export "graph 13b Guatemala ALL MODELS C19 daily deaths, error measures across models.pdf", replace
...
save "ALL MODELS Guatemala error - only error measures across models.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Guatemala error - only error measures across models data dictionary.csv", replace 
...
view "log CovidLongitudinal Guatemala 9 Merge.smcl"
