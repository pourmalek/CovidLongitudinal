...
log using "log CovidLongitudinal Georgia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Georgia 1 Process.do"
...
**** "do CovidLongitudinal Georgia 1 Process.do"
...
** output data files: "`model' Georgia.dta"
**                   "ALL MODLES Georgia.dta"

** output data dictionary files: "Georgia ALL MODELS update dates data dictionary.csv"
**                               "`model' Georgia data dictionary.csv"
**                               "ALL MODLES Georgia data dictionary.csv"

** graphs:
** "graph 02 Georgia `model' C19 daily deaths all updates.pdf"
** "graph 02 Georgia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Georgia absent update_date_block"
	
	qui keep if loc_grand_name == "Georgia"
	...
	save "Georgia `model' update dates.dta", replace
...
use "Georgia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Georgia `model' update dates.dta"
...
save "Georgia ALL MODELS update dates.dta", replace
	
export excel using "Georgia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Georgia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Georgia"

keep date DayDeaMeSmJOHNGEO
...
save "JOHN Georgia.dta", replace 




* graph Georgia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNGEO date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Georgia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Georgia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Georgia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Georgia.dta"
...
merge m:m date using "JOHN Georgia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Georgia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Georgia DELP daily deaths each update 
	...
	subtitle("Georgia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Georgia DELP daily deaths update `update'.pdf", replace	
...
* graph Georgia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Georgia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Georgia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Georgia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Georgia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Georgia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Georgia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Georgia.dta"
...
merge m:1 date using "JOHN Georgia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Georgia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Georgia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Georgia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Georgia IHME daily deaths update `update'.pdf", replace	
...
* graph Georgia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Georgia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Georgia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Georgia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Georgia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Georgia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Georgia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Georgia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Georgia IMPE daily deaths update `update'.pdf", replace	
...
* graph Georgia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Georgia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Georgia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Georgia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Georgia.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Georgia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Georgia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Georgia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Georgia LANL daily deaths update `update'.pdf", replace	
...
* graph Georgia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Georgia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Georgia LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Georgia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Georgia.dta", force
...
save "ALL MODLES Georgia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Georgia data dictionary.csv", replace 
...
* graph Georgia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Georgia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(626 "JOHN" 1 "DELP" 55 "IHME" 3 "IMPE" 106 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Georgia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Georgia 1 Process.smcl"
