...
log using "log CovidLongitudinal Canada 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Canada 1 Process.do"
...
**** "do CovidLongitudinal Canada 1 Process.do"
...
** output data files: "`model' Canada.dta"
**                   "ALL MODLES Canada.dta"

** output data dictionary files: "Canada ALL MODELS update dates data dictionary.csv"
**                               "`model' Canada data dictionary.csv"
**                               "ALL MODLES Canada data dictionary.csv"

** graphs:
** "graph 02 Canada `model' C19 daily deaths all updates.pdf"
** "graph 02 Canada ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Canada absent update_date_block"
	
	qui keep if loc_grand_name == "Canada"
	...
	save "Canada `model' update dates.dta", replace
...
use "Canada DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Canada `model' update dates.dta"
...
save "Canada ALL MODELS update dates.dta", replace
	
export excel using "Canada ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Canada ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Canada"

keep date DayDeaMeSmJOHNCAN
...
save "JOHN Canada.dta", replace 




* graph Canada JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNCAN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Canada, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Canada JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	keep if loc_grand_name == "Canada" & provincestate == "Canada"
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Canada.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Canada.dta"
...
merge m:m date using "JOHN Canada.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Canada data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Canada DELP daily deaths each update 
	...
	subtitle("Canada, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Canada DELP daily deaths update `update'.pdf", replace	
...
* graph Canada DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Canada, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Canada DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Canada IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Canada, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Canada IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Canada.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Canada.dta"
...
merge m:1 date using "JOHN Canada.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Canada data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Canada IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Canada, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
...
* graph Canada IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Canada, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Canada IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Canada.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Canada.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Canada data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Canada IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Canada, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Canada IMPE daily deaths update `update'.pdf", replace	
...
* graph Canada IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Canada, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Canada IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Canada.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Canada.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Canada data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Canada LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Canada, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Canada LANL daily deaths update `update'.pdf", replace	
...
* graph Canada LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Canada, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Canada LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Canada.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Canada.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Canada.dta"
...
preserve
    ...
    export delimited name varlab using "UCLA Canada data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Canada UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Canada, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Canada UCLA daily deaths update `update'.pdf", replace	

}



* graph Canada UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Canada, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Canada UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Canada.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Canada.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Canada.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Canada data dictionary.csv", replace 
...
foreach update of local list {
	...
	* graph Canada YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Canada, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Canada YYGU daily deaths update `update'.pdf", replace	
...
* graph Canada YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Canada, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Canada YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Canada.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Canada.dta", force
...
save "ALL MODLES Canada.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Canada data dictionary.csv", replace 
...
* graph Canada ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Canada, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(858 "JOHN" 11 "DELP" 17 "IHME" 85 "IMPE" 23 "LANL" 373 "UCLA" 1 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Canada ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Canada 1 Process.smcl"
