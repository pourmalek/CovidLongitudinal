...
log using "log CovidLongitudinal Uruguay 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Uruguay 1 Process.do"
...
**** "do CovidLongitudinal Uruguay 1 Process.do"
...
** output data files: "`model' Uruguay.dta"
**                   "ALL MODLES Uruguay.dta"

** output data dictionary files: "Uruguay ALL MODELS update dates data dictionary.csv"
**                               "`model' Uruguay data dictionary.csv"
**                               "ALL MODLES Uruguay data dictionary.csv"

** graphs:
** "graph 02 Uruguay `model' C19 daily deaths all updates.pdf"
** "graph 02 Uruguay ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Uruguay absent update_date_block"
	
	qui keep if loc_grand_name == "Uruguay"
	...
	save "Uruguay `model' update dates.dta", replace
...
use "Uruguay DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Uruguay `model' update dates.dta"
...
save "Uruguay ALL MODELS update dates.dta", replace
	
export excel using "Uruguay ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Uruguay ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Uruguay"

keep date DayDeaMeSmJOHNURY
...
save "JOHN Uruguay.dta", replace 




* graph Uruguay JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNURY date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Uruguay, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uruguay JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Uruguay.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Uruguay.dta"
...
merge m:m date using "JOHN Uruguay.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Uruguay data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Uruguay DELP daily deaths each update 
	...
	subtitle("Uruguay, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Uruguay DELP daily deaths update `update'.pdf", replace	
...
* graph Uruguay DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Uruguay, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uruguay DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Uruguay IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Uruguay, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Uruguay IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Uruguay.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Uruguay.dta"
...
merge m:1 date using "JOHN Uruguay.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Uruguay data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Uruguay IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Uruguay, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Uruguay IHME daily deaths update `update'.pdf", replace	
...
* graph Uruguay IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Uruguay, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uruguay IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Uruguay.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Uruguay.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Uruguay data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Uruguay IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Uruguay, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Uruguay IMPE daily deaths update `update'.pdf", replace	
...
* graph Uruguay IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Uruguay, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uruguay IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Uruguay.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Uruguay.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Uruguay data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Uruguay LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Uruguay, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Uruguay LANL daily deaths update `update'.pdf", replace	
...
* graph Uruguay LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Uruguay, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uruguay LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Uruguay.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Uruguay.dta", force
...
save "ALL MODLES Uruguay.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Uruguay data dictionary.csv", replace 
...
* graph Uruguay ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Uruguay, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(629 "JOHN" 1 "DELP" 35 "IHME" 12 "IMPE" 6 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uruguay ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Uruguay 1 Process.smcl"
