...
log using "log CovidLongitudinal Croatia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Croatia 1 Process.do"
...
**** "do CovidLongitudinal Croatia 1 Process.do"
...
** output data files: "`model' Croatia.dta"
**                   "ALL MODLES Croatia.dta"

** output data dictionary files: "Croatia ALL MODELS update dates data dictionary.csv"
**                               "`model' Croatia data dictionary.csv"
**                               "ALL MODLES Croatia data dictionary.csv"

** graphs:
** "graph 02 Croatia `model' C19 daily deaths all updates.pdf"
** "graph 02 Croatia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Croatia absent update_date_block"
	
	qui keep if loc_grand_name == "Croatia"
	...
	save "Croatia `model' update dates.dta", replace
...
use "Croatia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Croatia `model' update dates.dta"
...
save "Croatia ALL MODELS update dates.dta", replace
	
export excel using "Croatia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Croatia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Croatia"

keep date DayDeaMeSmJOHNHRV
...
save "JOHN Croatia.dta", replace 




* graph Croatia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNHRV date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Croatia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Croatia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Croatia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Croatia.dta"
...
merge m:m date using "JOHN Croatia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Croatia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Croatia DELP daily deaths each update 
	...
	subtitle("Croatia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Croatia DELP daily deaths update `update'.pdf", replace	
...
* graph Croatia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Croatia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Croatia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Croatia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Croatia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Croatia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Croatia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Croatia.dta"
...
merge m:1 date using "JOHN Croatia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Croatia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Croatia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Croatia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Croatia IHME daily deaths update `update'.pdf", replace	
...
* graph Croatia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Croatia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Croatia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Croatia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Croatia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Croatia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Croatia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Croatia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Croatia IMPE daily deaths update `update'.pdf", replace	
...
* graph Croatia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Croatia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Croatia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Croatia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Croatia.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Croatia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Croatia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Croatia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Croatia LANL daily deaths update `update'.pdf", replace	
...
* graph Croatia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Croatia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Croatia LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Croatia.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Croatia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Croatia.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Croatia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Croatia YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Croatia, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Croatia YYGU daily deaths update `update'.pdf", replace	
...
* graph Croatia YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Croatia, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Croatia YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Croatia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Croatia.dta", force
...
save "ALL MODLES Croatia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Croatia data dictionary.csv", replace 
...
* graph Croatia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Croatia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(834 "JOHN" 15 "DELP" 1 "IHME" 91 "IMPE" 29 "LANL" 2 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Croatia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Croatia 1 Process.smcl"
