...
log using "log CovidLongitudinal Iraq 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Iraq 1 Process.do"
...
**** "do CovidLongitudinal Iraq 1 Process.do"
...
** output data files: "`model' Iraq.dta"
**                   "ALL MODLES Iraq.dta"

** output data dictionary files: "Iraq ALL MODELS update dates data dictionary.csv"
**                               "`model' Iraq data dictionary.csv"
**                               "ALL MODLES Iraq data dictionary.csv"

** graphs:
** "graph 02 Iraq `model' C19 daily deaths all updates.pdf"
** "graph 02 Iraq ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Iraq absent update_date_block"
	
	qui keep if loc_grand_name == "Iraq"
	...
	save "Iraq `model' update dates.dta", replace
...
use "Iraq DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Iraq `model' update dates.dta"
...
save "Iraq ALL MODELS update dates.dta", replace
	
export excel using "Iraq ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Iraq ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Iraq"

keep date DayDeaMeSmJOHNIRQ
...
save "JOHN Iraq.dta", replace 




* graph Iraq JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNIRQ date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Iraq, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Iraq JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Iraq.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Iraq.dta"
...
merge m:m date using "JOHN Iraq.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Iraq data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Iraq DELP daily deaths each update 
	...
	subtitle("Iraq, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Iraq DELP daily deaths update `update'.pdf", replace	
...
* graph Iraq DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Iraq, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Iraq DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Iraq IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Iraq, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Iraq IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Iraq.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Iraq.dta"
...
merge m:1 date using "JOHN Iraq.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Iraq data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Iraq IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Iraq, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Iraq IHME daily deaths update `update'.pdf", replace	
...
* graph Iraq IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Iraq, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Iraq IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Iraq.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Iraq.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Iraq data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Iraq IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Iraq, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Iraq IMPE daily deaths update `update'.pdf", replace	
...
* graph Iraq IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Iraq, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Iraq IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Iraq.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Iraq.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Iraq data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Iraq LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Iraq, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Iraq LANL daily deaths update `update'.pdf", replace	
...
* graph Iraq LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Iraq, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Iraq LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Iraq.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Iraq.dta", force
...
save "ALL MODLES Iraq.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Iraq data dictionary.csv", replace 
...
* graph Iraq ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Iraq, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(667 "JOHN" 1 "DELP" 72 "IHME" 4 "IMPE" 3 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Iraq ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Iraq 1 Process.smcl"
