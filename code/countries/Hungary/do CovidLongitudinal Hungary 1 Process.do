...
log using "log CovidLongitudinal Hungary 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Hungary 1 Process.do"
...
**** "do CovidLongitudinal Hungary 1 Process.do"
...
** output data files: "`model' Hungary.dta"
**                   "ALL MODLES Hungary.dta"

** output data dictionary files: "Hungary ALL MODELS update dates data dictionary.csv"
**                               "`model' Hungary data dictionary.csv"
**                               "ALL MODLES Hungary data dictionary.csv"

** graphs:
** "graph 02 Hungary `model' C19 daily deaths all updates.pdf"
** "graph 02 Hungary ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Hungary absent update_date_block"
	
	qui keep if loc_grand_name == "Hungary"
	...
	save "Hungary `model' update dates.dta", replace
...
use "Hungary DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Hungary `model' update dates.dta"
...
save "Hungary ALL MODELS update dates.dta", replace
	
export excel using "Hungary ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Hungary ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Hungary"

keep date DayDeaMeSmJOHNHUN
...
save "JOHN Hungary.dta", replace 




* graph Hungary JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNHUN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Hungary, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Hungary JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Hungary.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Hungary.dta"
...
merge m:m date using "JOHN Hungary.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Hungary data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Hungary DELP daily deaths each update 
	...
	subtitle("Hungary, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Hungary DELP daily deaths update `update'.pdf", replace	
...
* graph Hungary DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Hungary, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Hungary DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Hungary IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Hungary, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Hungary IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Hungary.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Hungary.dta"
...
merge m:1 date using "JOHN Hungary.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Hungary data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Hungary IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Hungary, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Hungary IHME daily deaths update `update'.pdf", replace	
...
* graph Hungary IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Hungary, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Hungary IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Hungary.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Hungary.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Hungary data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Hungary IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Hungary, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Hungary IMPE daily deaths update `update'.pdf", replace	
...
* graph Hungary IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Hungary, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Hungary IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Hungary.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Hungary.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Hungary data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Hungary LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Hungary, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Hungary LANL daily deaths update `update'.pdf", replace	
...
* graph Hungary LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Hungary, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Hungary LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Hungary.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Hungary.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Hungary.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Hungary data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Hungary YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Hungary, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Hungary YYGU daily deaths update `update'.pdf", replace	
...
* graph Hungary YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Hungary, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Hungary YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Hungary.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Hungary.dta", force
...
save "ALL MODLES Hungary.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Hungary data dictionary.csv", replace 
...
* graph Hungary ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Hungary, all models, all updates, forecast only", size(medium) color(black)) ///
...
qui graph export "graph 02 Hungary ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Hungary 1 Process.smcl"
