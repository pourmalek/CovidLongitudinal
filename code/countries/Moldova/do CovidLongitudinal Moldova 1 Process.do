...
log using "log CovidLongitudinal Moldova 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Moldova 1 Process.do"
...
**** "do CovidLongitudinal Moldova 1 Process.do"
...
** output data files: "`model' Moldova.dta"
**                   "ALL MODLES Moldova.dta"

** output data dictionary files: "Moldova ALL MODELS update dates data dictionary.csv"
**                               "`model' Moldova data dictionary.csv"
**                               "ALL MODLES Moldova data dictionary.csv"

** graphs:
** "graph 02 Moldova `model' C19 daily deaths all updates.pdf"
** "graph 02 Moldova ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Moldova absent update_date_block"
	
	qui keep if loc_grand_name == "Moldova"
	...
	save "Moldova `model' update dates.dta", replace
...
use "Moldova DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Moldova `model' update dates.dta"
...
save "Moldova ALL MODELS update dates.dta", replace
	
export excel using "Moldova ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Moldova ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Moldova"

keep date DayDeaMeSmJOHNMDA
...
save "JOHN Moldova.dta", replace 




* graph Moldova JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMDA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Moldova, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Moldova JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Moldova.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Moldova.dta"
...
merge m:m date using "JOHN Moldova.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Moldova data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Moldova DELP daily deaths each update 
	...
	subtitle("Moldova, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Moldova DELP daily deaths update `update'.pdf", replace	
...
* graph Moldova DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Moldova, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Moldova DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Moldova IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Moldova, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Moldova IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Moldova.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Moldova.dta"
...
merge m:1 date using "JOHN Moldova.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Moldova data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Moldova IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Moldova, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Moldova IHME daily deaths update `update'.pdf", replace	
...
* graph Moldova IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Moldova, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Moldova IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Moldova.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Moldova.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Moldova data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Moldova IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Moldova, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Moldova IMPE daily deaths update `update'.pdf", replace	
...
* graph Moldova IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Moldova, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Moldova IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Moldova.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Moldova.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Moldova data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Moldova LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Moldova, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Moldova LANL daily deaths update `update'.pdf", replace	
...
* graph Moldova LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Moldova, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Moldova LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Moldova.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Moldova.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Moldova.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Moldova data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Moldova YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Moldova, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Moldova YYGU daily deaths update `update'.pdf", replace	
...
* graph Moldova YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Moldova, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Moldova YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Moldova.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Moldova.dta", force
...
save "ALL MODLES Moldova.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Moldova data dictionary.csv", replace 
...
* graph Moldova ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Moldova, all models, all updates, forecast only", size(medium) color(black)) ///
...
qui graph export "graph 02 Moldova ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Moldova 1 Process.smcl"
