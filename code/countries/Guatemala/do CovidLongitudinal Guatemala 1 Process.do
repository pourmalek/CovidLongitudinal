...
log using "log CovidLongitudinal Guatemala 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Guatemala 1 Process.do"
...
**** "do CovidLongitudinal Guatemala 1 Process.do"
...
** output data files: "`model' Guatemala.dta"
**                   "ALL MODLES Guatemala.dta"

** output data dictionary files: "Guatemala ALL MODELS update dates data dictionary.csv"
**                               "`model' Guatemala data dictionary.csv"
**                               "ALL MODLES Guatemala data dictionary.csv"

** graphs:
** "graph 02 Guatemala `model' C19 daily deaths all updates.pdf"
** "graph 02 Guatemala ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Guatemala absent update_date_block"
	
	qui keep if loc_grand_name == "Guatemala"
	...
	save "Guatemala `model' update dates.dta", replace
...
use "Guatemala DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Guatemala `model' update dates.dta"
...
save "Guatemala ALL MODELS update dates.dta", replace
	
export excel using "Guatemala ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Guatemala ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Guatemala"

keep date DayDeaMeSmJOHNGTM
...
save "JOHN Guatemala.dta", replace 




* graph Guatemala JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNGTM date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Guatemala, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guatemala JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Guatemala.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Guatemala.dta"
...
save "DELP Guatemala.dta", replace
...
merge m:m date using "JOHN Guatemala.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Guatemala data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guatemala DELP daily deaths each update 
	...
	subtitle("Guatemala, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guatemala DELP daily deaths update `update'.pdf", replace	
...
* graph Guatemala DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guatemala, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guatemala DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Guatemala IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Guatemala, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Guatemala IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Guatemala.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Guatemala.dta"
...
save "IHME Guatemala.dta", replace
...
merge m:1 date using "JOHN Guatemala.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Guatemala data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guatemala IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Guatemala, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guatemala IHME daily deaths update `update'.pdf", replace	
...
* graph Guatemala IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guatemala, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guatemala IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Guatemala.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Guatemala.dta"
...
save "IMPE Guatemala.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Guatemala data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guatemala IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Guatemala, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guatemala IMPE daily deaths update `update'.pdf", replace	
...
* graph Guatemala IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guatemala, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guatemala IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Guatemala.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Guatemala.dta"
...
save "LANL Guatemala.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Guatemala data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guatemala LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Guatemala, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guatemala LANL daily deaths update `update'.pdf", replace	
...
* graph Guatemala LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guatemala, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guatemala LANL C19 daily deaths all updates.pdf", replace
...
* UCLA
* Guatemala not included in UCLA
...
* YYGU
* Guatemala not included in YYGU
...
use "DELP Guatemala.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Guatemala.dta", force
...
save "ALL MODLES Guatemala.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Guatemala data dictionary.csv", replace 
...
* graph Guatemala ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guatemala, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(665 "JOHN" 1 "DELP" 69 "IHME" 2 "IMPE" 12 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guatemala ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Guatemala 1 Process.smcl"
