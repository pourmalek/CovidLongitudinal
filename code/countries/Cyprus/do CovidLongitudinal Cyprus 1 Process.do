...
log using "log CovidLongitudinal Cyprus 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Cyprus 1 Process.do"
...
**** "do CovidLongitudinal Cyprus 1 Process.do"
...
** output data files: "`model' Cyprus.dta"
**                   "ALL MODLES Cyprus.dta"

** output data dictionary files: "Cyprus ALL MODELS update dates data dictionary.csv"
**                               "`model' Cyprus data dictionary.csv"
**                               "ALL MODLES Cyprus data dictionary.csv"

** graphs:
** "graph 02 Cyprus `model' C19 daily deaths all updates.pdf"
** "graph 02 Cyprus ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Cyprus absent update_date_block"
	
	qui keep if loc_grand_name == "Cyprus"
	...
	save "Cyprus `model' update dates.dta", replace
...
use "Cyprus DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Cyprus `model' update dates.dta"
...
save "Cyprus ALL MODELS update dates.dta", replace
	
export excel using "Cyprus ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Cyprus ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Cyprus"

keep date DayDeaMeSmJOHNCYP
...
save "JOHN Cyprus.dta", replace 




* graph Cyprus JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNCYP date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Cyprus, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cyprus JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Cyprus.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Cyprus.dta"
...
merge m:m date using "JOHN Cyprus.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Cyprus data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cyprus DELP daily deaths each update 
	...
	subtitle("Cyprus, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cyprus DELP daily deaths update `update'.pdf", replace	
...
* graph Cyprus DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cyprus, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cyprus DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Cyprus IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Cyprus, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Cyprus IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Cyprus.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Cyprus.dta"
...
merge m:1 date using "JOHN Cyprus.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Cyprus data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cyprus IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cyprus, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cyprus IHME daily deaths update `update'.pdf", replace	
...
* graph Cyprus IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cyprus, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cyprus IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Cyprus.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Cyprus.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Cyprus data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cyprus IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cyprus, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cyprus IMPE daily deaths update `update'.pdf", replace	
...
* graph Cyprus IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cyprus, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cyprus IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Cyprus.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Cyprus.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Cyprus data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cyprus LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cyprus, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cyprus LANL daily deaths update `update'.pdf", replace	
...
* graph Cyprus LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cyprus, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cyprus LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Cyprus.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Cyprus.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Cyprus.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Cyprus data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Cyprus YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Cyprus, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Cyprus YYGU daily deaths update `update'.pdf", replace	
...
* graph Cyprus YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cyprus, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cyprus YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Cyprus.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Cyprus.dta", force
...
save "ALL MODLES Cyprus.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Cyprus data dictionary.csv", replace 
...
* graph Cyprus ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Cyprus, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(803 "JOHN" 15 "DELP" 1 "IHME" 80 "IMPE" 194 "LANL" 2 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Cyprus ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Cyprus 1 Process.smcl"
