...
log using "log CovidLongitudinal Micronesia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Micronesia 1 Process.do"
...
**** "do CovidLongitudinal Micronesia 1 Process.do"
...
** output data files: "`model' Micronesia.dta"
**                   "ALL MODLES Micronesia.dta"

** output data dictionary files: "Micronesia ALL MODELS update dates data dictionary.csv"
**                               "`model' Micronesia data dictionary.csv"
**                               "ALL MODLES Micronesia data dictionary.csv"

** graphs:
** "graph 02 Micronesia `model' C19 daily deaths all updates.pdf"
** "graph 02 Micronesia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Micronesia absent update_date_block"
	
	qui keep if loc_grand_name == "Micronesia"
	...
	save "Micronesia `model' update dates.dta", replace
...
use "Micronesia IMPE update dates.dta", clear 

local list IMPE

foreach model of local list {
	
	append using "Micronesia `model' update dates.dta"
...
save "Micronesia ALL MODELS update dates.dta", replace
	
export excel using "Micronesia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Micronesia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Micronesia"

keep date DayDeaMeSmJOHNFSM
...
save "JOHN Micronesia.dta", replace 




* graph Micronesia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNFSM date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Micronesia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Micronesia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Micronesia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Micronesia.dta"
...
save "IMPE Micronesia.dta", replace
...
merge m:1 date using "JOHN Micronesia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Micronesia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Micronesia IMPE daily deaths each update 
	...
	subtitle("Micronesia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Micronesia IMPE daily deaths update `update'.pdf", replace	
...
* graph Micronesia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Micronesia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Micronesia IMPE C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Micronesia.dta", force
...
save "ALL MODLES Micronesia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Micronesia data dictionary.csv", replace 
...
* graph Micronesia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Micronesia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(67 "JOHN" 1 "IMPE") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Micronesia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Micronesia 1 Process.smcl"
