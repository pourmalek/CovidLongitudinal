...
log using "log CovidLongitudinal Germany 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Germany 1 Process.do"
...
**** "do CovidLongitudinal Germany 1 Process.do"
...
** output data files: "`model' Germany.dta"
**                   "ALL MODLES Germany.dta"

** output data dictionary files: "Germany ALL MODELS update dates data dictionary.csv"
**                               "`model' Germany data dictionary.csv"
**                               "ALL MODLES Germany data dictionary.csv"

** graphs:
** "graph 02 Germany `model' C19 daily deaths all updates.pdf"
** "graph 02 Germany ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Germany absent update_date_block"
	
	qui keep if loc_grand_name == "Germany"
	...
	save "Germany `model' update dates.dta", replace
...
use "Germany DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Germany `model' update dates.dta"
...
save "Germany ALL MODELS update dates.dta", replace
	
export excel using "Germany ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Germany ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Germany"

keep date DayDeaMeSmJOHNDEU
...
save "JOHN Germany.dta", replace 




* graph Germany JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNDEU date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Germany, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Germany JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Germany.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Germany.dta"
...
merge m:m date using "JOHN Germany.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Germany data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Germany DELP daily deaths each update 
	...
	subtitle("Germany, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Germany DELP daily deaths update `update'.pdf", replace	
...
* graph Germany DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Germany, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Germany DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Germany IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Germany, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Germany IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Germany.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Germany.dta"
...
merge m:1 date using "JOHN Germany.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Germany data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Germany IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Germany, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Germany IHME daily deaths update `update'.pdf", replace	
...
* graph Germany IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Germany, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Germany IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Germany.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Germany.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Germany data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Germany IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Germany, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Germany IMPE daily deaths update `update'.pdf", replace	
...
* graph Germany IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Germany, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Germany IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Germany.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Germany.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Germany data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Germany LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Germany, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Germany LANL daily deaths update `update'.pdf", replace	
...
* graph Germany LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Germany, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Germany LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Germany.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Germany.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Germany.dta"
...
preserve
    ...
    export delimited name varlab using "UCLA Germany data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Germany UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Germany, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Germany UCLA daily deaths update `update'.pdf", replace	

}
*/


* graph Germany UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Germany, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Germany UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Germany.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Germany.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Germany.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Germany data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Germany YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Germany, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Germany YYGU daily deaths update `update'.pdf", replace	
...
* graph Germany YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Germany, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Germany YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Germany.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Germany.dta", force
...
save "ALL MODLES Germany.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Germany data dictionary.csv", replace 
...
* graph Germany ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Germany, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(866 "JOHN" 19 "DELP" 5 "IHME" 95 "IMPE" 33 "LANL" 383 "UCLA" 1 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Germany ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Germany 1 Process.smcl"
