...
log using "log CovidLongitudinal Australia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Australia 1 Process.do"
...
**** "do CovidLongitudinal Australia 1 Process.do"
...
** output data files: "`model' Australia.dta"
**                   "ALL MODLES Australia.dta"

** output data dictionary files: "Australia ALL MODELS update dates data dictionary.csv"
**                               "`model' Australia data dictionary.csv"
**                               "ALL MODLES Australia data dictionary.csv"

** graphs:
** "graph 02 Australia `model' C19 daily deaths all updates.pdf"
** "graph 02 Australia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Australia absent update_date_block"
	
	qui keep if loc_grand_name == "Australia"
	...
	save "Australia `model' update dates.dta", replace
...
use "Australia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Australia `model' update dates.dta"
...
save "Australia ALL MODELS update dates.dta", replace
	
export excel using "Australia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Australia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Australia"

keep date DayDeaMeSmJOHNAUS
...
save "JOHN Australia.dta", replace 




* graph Australia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNAUS date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Australia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Australia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	keep if loc_grand_name == "Australia" & provincestate == "Australia"
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Australia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Australia.dta"
...
save "DELP Australia.dta", replace
...
merge m:m date using "JOHN Australia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Australia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Australia DELP daily deaths each update 
	...
	subtitle("Australia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Australia DELP daily deaths update `update'.pdf", replace	
...
* graph Australia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Australia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Australia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Australia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Australia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Australia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Australia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Australia.dta"
...
save "IHME Australia.dta", replace
...
merge m:1 date using "JOHN Australia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Australia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Australia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Australia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Australia IHME daily deaths update `update'.pdf", replace	
...
* graph Australia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Australia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Australia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Australia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Australia.dta"
...
save "IMPE Australia.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Australia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Australia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Australia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Australia IMPE daily deaths update `update'.pdf", replace	
...
* graph Australia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Australia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Australia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Australia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Australia.dta"
...
save "LANL Australia.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Australia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Australia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Australia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Australia LANL daily deaths update `update'.pdf", replace	
...
* graph Australia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Australia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Australia LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Australia.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Australia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Australia.dta"
...
save "YYGU Australia.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Australia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Australia YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Australia, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Australia YYGU daily deaths update `update'.pdf", replace	
...
* graph Australia YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Australia, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Australia YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Australia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Australia.dta", force
...
save "ALL MODLES Australia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Australia data dictionary.csv", replace 
...
* graph Australia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Australia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(761 "JOHN" 1 "DELP" 65 "IHME" 25 "IMPE" 3 "LANL" 16 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Australia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Australia 1 Process.smcl"
