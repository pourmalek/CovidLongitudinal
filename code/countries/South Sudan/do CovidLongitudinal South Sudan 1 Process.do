...
log using "log CovidLongitudinal South Sudan 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal South Sudan 1 Process.do"
...
**** "do CovidLongitudinal South Sudan 1 Process.do"
...
** output data files: "`model' South Sudan.dta"
**                   "ALL MODLES South Sudan.dta"

** output data dictionary files: "South Sudan ALL MODELS update dates data dictionary.csv"
**                               "`model' South Sudan data dictionary.csv"
**                               "ALL MODLES South Sudan data dictionary.csv"

** graphs:
** "graph 02 South Sudan `model' C19 daily deaths all updates.pdf"
** "graph 02 South Sudan ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " South Sudan absent update_date_block"
	
	qui keep if loc_grand_name == "South Sudan"
	...
	save "South Sudan `model' update dates.dta", replace
...
use "South Sudan DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "South Sudan `model' update dates.dta"
...
save "South Sudan ALL MODELS update dates.dta", replace
	
export excel using "South Sudan ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "South Sudan ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "South Sudan"

keep date DayDeaMeSmJOHNSSD
...
save "JOHN South Sudan.dta", replace 




* graph South Sudan JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSSD date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, South Sudan, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 South Sudan JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' South Sudan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' South Sudan.dta"
...
save "DELP South Sudan.dta", replace
...
merge m:m date using "JOHN South Sudan.dta"
...
preserve
    ...
    export delimited name varlab using "DELP South Sudan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph South Sudan DELP daily deaths each update 
	...
	subtitle("South Sudan, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 South Sudan DELP daily deaths update `update'.pdf", replace	
...
* graph South Sudan DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, South Sudan, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 South Sudan DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph South Sudan IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, South Sudan, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 South Sudan IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' South Sudan.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' South Sudan.dta"
...
save "IHME South Sudan.dta", replace
...
merge m:1 date using "JOHN South Sudan.dta"
...
preserve
    ...
    export delimited name varlab using "IHME South Sudan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph South Sudan IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("South Sudan, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 South Sudan IHME daily deaths update `update'.pdf", replace	
...
* graph South Sudan IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, South Sudan, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 South Sudan IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' South Sudan.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' South Sudan.dta"
...
save "IMPE South Sudan.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE South Sudan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph South Sudan IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("South Sudan, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 South Sudan IMPE daily deaths update `update'.pdf", replace	
...
* graph South Sudan IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, South Sudan, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 South Sudan IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' South Sudan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' South Sudan.dta"
...
save "LANL South Sudan.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL South Sudan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph South Sudan LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("South Sudan, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 South Sudan LANL daily deaths update `update'.pdf", replace	
...
* graph South Sudan LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, South Sudan, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 South Sudan LANL C19 daily deaths all updates.pdf", replace
...
use "DELP South Sudan.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' South Sudan.dta", force
...
save "ALL MODLES South Sudan.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES South Sudan data dictionary.csv", replace 
...
* graph South Sudan ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, South Sudan, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(635 "JOHN" 3 "DELP" 38 "IHME" 1 "IMPE" 27 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 South Sudan ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal South Sudan 1 Process.smcl"
