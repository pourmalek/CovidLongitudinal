...
log using "log CovidLongitudinal Kazakhstan 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Kazakhstan 1 Process.do"
...
**** "do CovidLongitudinal Kazakhstan 1 Process.do"
...
** output data files: "`model' Kazakhstan.dta"
**                   "ALL MODLES Kazakhstan.dta"

** output data dictionary files: "Kazakhstan ALL MODELS update dates data dictionary.csv"
**                               "`model' Kazakhstan data dictionary.csv"
**                               "ALL MODLES Kazakhstan data dictionary.csv"

** graphs:
** "graph 02 Kazakhstan `model' C19 daily deaths all updates.pdf"
** "graph 02 Kazakhstan ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Kazakhstan absent update_date_block"
	
	qui keep if loc_grand_name == "Kazakhstan"
	...
	save "Kazakhstan `model' update dates.dta", replace
...
use "Kazakhstan DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Kazakhstan `model' update dates.dta"
...
save "Kazakhstan ALL MODELS update dates.dta", replace
	
export excel using "Kazakhstan ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Kazakhstan ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Kazakhstan"

keep date DayDeaMeSmJOHNKAZ
...
save "JOHN Kazakhstan.dta", replace 




* graph Kazakhstan JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNKAZ date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Kazakhstan, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kazakhstan JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Kazakhstan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Kazakhstan.dta"
...
save "DELP Kazakhstan.dta", replace
...
merge m:m date using "JOHN Kazakhstan.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Kazakhstan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kazakhstan DELP daily deaths each update 
	...
	subtitle("Kazakhstan, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kazakhstan DELP daily deaths update `update'.pdf", replace	
...
* graph Kazakhstan DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kazakhstan, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kazakhstan DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Kazakhstan IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Kazakhstan, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Kazakhstan IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Kazakhstan.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Kazakhstan.dta"
...
save "IHME Kazakhstan.dta", replace
...
merge m:1 date using "JOHN Kazakhstan.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Kazakhstan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kazakhstan IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Kazakhstan, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kazakhstan IHME daily deaths update `update'.pdf", replace	
...
* graph Kazakhstan IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kazakhstan, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kazakhstan IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Kazakhstan.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Kazakhstan.dta"
...
save "IMPE Kazakhstan.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Kazakhstan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kazakhstan IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Kazakhstan, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kazakhstan IMPE daily deaths update `update'.pdf", replace	
...
* graph Kazakhstan IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kazakhstan, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kazakhstan IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Kazakhstan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Kazakhstan.dta"
...
save "LANL Kazakhstan.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Kazakhstan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kazakhstan LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Kazakhstan, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kazakhstan LANL daily deaths update `update'.pdf", replace	
...
* graph Kazakhstan LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kazakhstan, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kazakhstan LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Kazakhstan.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Kazakhstan.dta", force
...
save "ALL MODLES Kazakhstan.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Kazakhstan data dictionary.csv", replace 
...
* graph Kazakhstan ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kazakhstan, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(669 "JOHN" 1 "DELP" 72 "IHME" 4 "IMPE" 3 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kazakhstan ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Kazakhstan 1 Process.smcl"
