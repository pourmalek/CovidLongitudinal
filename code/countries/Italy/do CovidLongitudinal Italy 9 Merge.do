...
log using "log CovidLongitudinal Italy 9 Merge.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Italy 9 Merge.do"
...
** input data files: "`model' Italy error.dta"

** output data files: "ALL MODELS Italy error.dta" with all date
*                     "ALL MODELS Italy error - only models across error measures.dta", replace
*                     "ALL MODELS Italy error - only error measures across models.dta", replace

* output data dictionary files: "ALL MODELS Italy error data dictionary.csv"
*                               "ALL MODELS Italy error - only models across error measures data dictionary.csv"
*                               "ALL MODELS Italy error - only error measures across models data dictionary.csv"


** Graphs:
* "graph 03 Italy ALL MODELS C19 daily deaths error.pdf"
* "graph 04 Italy ALL MODELS C19 daily deaths absolute error.pdf"
* "graph 05 Italy ALL MODELS C19 daily deaths percent error.pdf"
* "graph 06 Italy ALL MODELS C19 daily deaths absolute percent error.pdf"

* "graph 07a1 Italy ALL MODELS C19 daily deaths average median error.pdf"
* "graph 07a2 Italy ALL MODELS C19 daily deaths average median error smoothed.pdf"
* "graph 08a1 Italy ALL MODELS C19 daily deaths average median absolute error.pdf"
* "graph 08a2 Italy ALL MODELS C19 daily deaths average median absolute error smoothed.pdf"
* "graph 09a1 Italy ALL MODELS C19 daily deaths average median percent error.pdf"
* "graph 09a2 Italy ALL MODELS C19 daily deaths average median percent error smoothed.pdf"
* "graph 10a1 Italy ALL MODELS C19 daily deaths average median absolute percent error.pdf"
* "graph 10a2 Italy ALL MODELS C19 daily deaths average median absolute percent error smoothed.pdf"

****** graph each same error measure across models:
* "graph 12a Italy ALL MODELS C19 daily deaths Average Mean Error.pdf"
* "graph 12b Italy ALL MODELS C19 daily deaths Average Absolute Error.pdf"
* "graph 12c Italy ALL MODELS C19 daily deaths Average Percent Error.pdf"
* "graph 12d Italy ALL MODELS C19 daily deaths Average Absolute Percent Error.pdf"

****** graph all models across error measures:
* "graph 13a Italy ALL MODELS C19 daily deaths, models across error measures.pdf"

****** graph all error measures across models:
* "graph 13b Italy ALL MODELS C19 daily deaths, error measures across models.pdf"
...
use "DELP Italy error.dta", clear 

	merge m:m date using "IHME Italy error.dta"

	drop _merge
	
	merge m:m date using "IMPE Italy error.dta"
	
	drop _merge
	
	merge m:m date using "LANL Italy error.dta"	
	
	drop _merge
	
	merge m:m date using "UCLA Italy error.dta", force
	
	drop _merge
	
	merge m:m date using "YYGU Italy error.dta", force	
...
subtitle("Italy, all Models, all updates, forecast only", size(small)) ///
legend(position(6) order(20 "DELP" 6 "IHME" 96 "IMPE" 34 "LANL" 384 "UCLA" 1 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 03 Italy ALL MODELS C19 daily deaths error.pdf", replace
...
qui graph export "graph 04 Italy ALL MODELS C19 daily deaths absolute error.pdf", replace
...
qui graph export "graph 05 Italy ALL MODELS C19 daily deaths percent error.pdf", replace
...
qui graph export "graph 06 Italy ALL MODELS C19 daily deaths absolute percent error.pdf", replace
...
subtitle("Italy, all Models, forecast only", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) yscale(titlegap(3)) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL" 5 "UCLA" 6 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 07a1 Italy ALL MODELS C19 daily deaths average median error.pdf", replace 
...
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths error measure) title("C19 daily deaths, average median error", size(medium) color(black)) /// 
subtitle("Italy, all Models, forecast only; smoothed for better viewing", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) yscale(titlegap(3)) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL" 5 "UCLA" 6 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 07a2 Italy ALL MODELS C19 daily deaths average median error smoothed.pdf", replace
...
qui graph export "graph 08a1 Italy ALL MODELS C19 daily deaths average median absolute error.pdf", replace 
...
qui graph export "graph 08a2 Italy ALL MODELS C19 daily deaths average median absolute error smoothed.pdf", replace
...
qui graph export "graph 09a1 Italy ALL MODELS C19 daily deaths average median percent error.pdf", replace
...
qui graph export "graph 09a2 Italy ALL MODELS C19 daily deaths average median percent error smoothed.pdf", replace
...
qui graph export "graph 10a1 Italy ALL MODELS C19 daily deaths average median absolute percent error.pdf", replace 
...
qui graph export "graph 10a2 Italy ALL MODELS C19 daily deaths average median absolute percent error smoothed.pdf", replace
...
subtitle("Over updates and calendar months; forecast only; Italy", size(small)) /// 
...
qui graph export "graph 12a Italy ALL MODELS C19 daily deaths Average Mean Error.pdf", replace
...
qui graph export "graph 12b Italy ALL MODELS C19 daily deaths Average Absolute Error.pdf", replace
...
qui graph export "graph 12c Italy ALL MODELS C19 daily deaths Average Percent Error.pdf", replace
...
qui graph export "graph 12d Italy ALL MODELS C19 daily deaths Average Absolute Percent Error.pdf", replace
...
save "ALL MODELS Italy error.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Italy error data dictionary.csv", replace 
...
use "ALL MODELS Italy error.dta", clear

save "ALL MODELS Italy error - only models across error measures.dta", replace
...
save "CovidLongitudinal Italy merge temp.dta", replace 
...
qui graph export "graph 13a Italy ALL MODELS C19 daily deaths, models across error measures.pdf", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Italy error - only models across error measures data dictionary.csv", replace 
...
use "CovidLongitudinal Italy merge temp.dta", clear 
...
qui graph export "graph 13b Italy ALL MODELS C19 daily deaths, error measures across models.pdf", replace
...
save "ALL MODELS Italy error - only error measures across models.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Italy error - only error measures across models data dictionary.csv", replace 
...
view "log CovidLongitudinal Italy 9 Merge.smcl"
