...
log using "log CovidLongitudinal Serbia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Serbia 1 Process.do"
...
**** "do CovidLongitudinal Serbia 1 Process.do"
...
** output data files: "`model' Serbia.dta"
**                   "ALL MODLES Serbia.dta"

** output data dictionary files: "Serbia ALL MODELS update dates data dictionary.csv"
**                               "`model' Serbia data dictionary.csv"
**                               "ALL MODLES Serbia data dictionary.csv"

** graphs:
** "graph 02 Serbia `model' C19 daily deaths all updates.pdf"
** "graph 02 Serbia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Serbia absent update_date_block"
	
	qui keep if loc_grand_name == "Serbia"
	...
	save "Serbia `model' update dates.dta", replace
...
use "Serbia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Serbia `model' update dates.dta"
...
save "Serbia ALL MODELS update dates.dta", replace
	
export excel using "Serbia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Serbia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Serbia"

keep date DayDeaMeSmJOHNSRB
...
save "JOHN Serbia.dta", replace 




* graph Serbia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSRB date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Serbia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Serbia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Serbia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Serbia.dta"
...
merge m:m date using "JOHN Serbia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Serbia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Serbia DELP daily deaths each update 
	...
	subtitle("Serbia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Serbia DELP daily deaths update `update'.pdf", replace	
...
* graph Serbia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Serbia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Serbia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Serbia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Serbia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Serbia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Serbia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Serbia.dta"
...
merge m:1 date using "JOHN Serbia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Serbia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Serbia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Serbia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Serbia IHME daily deaths update `update'.pdf", replace	
...
* graph Serbia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Serbia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Serbia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Serbia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Serbia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Serbia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Serbia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Serbia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Serbia IMPE daily deaths update `update'.pdf", replace	
...
* graph Serbia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Serbia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Serbia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Serbia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Serbia.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Serbia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Serbia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Serbia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Serbia LANL daily deaths update `update'.pdf", replace	
...
* graph Serbia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Serbia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Serbia LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Serbia.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Serbia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Serbia.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Serbia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Serbia YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Serbia, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Serbia YYGU daily deaths update `update'.pdf", replace	
...
* graph Serbia YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Serbia, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Serbia YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Serbia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Serbia.dta", force
...
save "ALL MODLES Serbia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Serbia data dictionary.csv", replace 
...
* graph Serbia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Serbia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(821 "JOHN" 1 "DELP" 55 "IHME" 4 "IMPE" 3 "LANL" 25 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Serbia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Serbia 1 Process.smcl"
