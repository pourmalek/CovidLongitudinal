...
log using "log CovidLongitudinal Tonga 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Tonga 1 Process.do"
...
**** "do CovidLongitudinal Tonga 1 Process.do"
...
** output data files: "`model' Tonga.dta"
**                   "ALL MODLES Tonga.dta"

** output data dictionary files: "Tonga ALL MODELS update dates data dictionary.csv"
**                               "`model' Tonga data dictionary.csv"
**                               "ALL MODLES Tonga data dictionary.csv"

** graphs:
** "graph 02 Tonga `model' C19 daily deaths all updates.pdf"
** "graph 02 Tonga ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Tonga absent update_date_block"
	
	qui keep if loc_grand_name == "Tonga"
	...
	save "Tonga `model' update dates.dta", replace
...
use "Tonga IMPE update dates.dta", clear 

local list IMPE

foreach model of local list {
	
	append using "Tonga `model' update dates.dta"
...
save "Tonga ALL MODELS update dates.dta", replace
	
export excel using "Tonga ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Tonga ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Tonga"

keep date DayDeaMeSmJOHNTON
...
save "JOHN Tonga.dta", replace 




* graph Tonga JOHN daily deaths
...
ytitle(Daily deaths) title("C19 daily deaths, Tonga, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tonga JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Tonga.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Tonga.dta"
...
merge m:1 date using "JOHN Tonga.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Tonga data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Tonga IMPE daily deaths each update 
	...
	subtitle("Tonga, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Tonga IMPE daily deaths update `update'.pdf", replace	
...
* graph Tonga IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tonga, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tonga IMPE C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Tonga.dta", force
...
save "ALL MODLES Tonga.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Tonga data dictionary.csv", replace 
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Tonga, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(68 "JOHN" 1 "IMPE") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Tonga ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Tonga 1 Process.smcl"
