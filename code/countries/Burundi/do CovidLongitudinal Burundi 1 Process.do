...
log using "log CovidLongitudinal Burundi 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Burundi 1 Process.do"
...
**** "do CovidLongitudinal Burundi 1 Process.do"
...
** output data files: "`model' Burundi.dta"
**                   "ALL MODLES Burundi.dta"

** output data dictionary files: "Burundi ALL MODELS update dates data dictionary.csv"
**                               "`model' Burundi data dictionary.csv"
**                               "ALL MODLES Burundi data dictionary.csv"

** graphs:
** "graph 02 Burundi `model' C19 daily deaths all updates.pdf"
** "graph 02 Burundi ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Burundi absent update_date_block"
	
	qui keep if loc_grand_name == "Burundi"
	...
	save "Burundi `model' update dates.dta", replace
...
use "Burundi DELP update dates.dta", clear 

local list DELP IMPE LANL

foreach model of local list {
	
	append using "Burundi `model' update dates.dta"
...
save "Burundi ALL MODELS update dates.dta", replace
	
export excel using "Burundi ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Burundi ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Burundi"

keep date DayDeaMeSmJOHNBDI
...
save "JOHN Burundi.dta", replace 




* graph Burundi JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNBDI date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Burundi, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Burundi JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Burundi.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Burundi.dta"
...
merge m:m date using "JOHN Burundi.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Burundi data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Burundi DELP daily deaths each update 
	...
	subtitle("Burundi, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Burundi DELP daily deaths update `update'.pdf", replace	
...
* graph Burundi DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Burundi, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Burundi DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Burundi.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Burundi.dta"
...
merge m:1 date using "JOHN Burundi.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Burundi data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Burundi IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Burundi, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Burundi IMPE daily deaths update `update'.pdf", replace	
...
* graph Burundi IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Burundi, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Burundi IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Burundi.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Burundi.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Burundi data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Burundi LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Burundi, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Burundi LANL daily deaths update `update'.pdf", replace	
...
* graph Burundi LANL daily deaths all updates
...
ytitle(Daily deaths) title("C19 daily deaths, Burundi, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Burundi LANL C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Burundi.dta", force
...
save "ALL MODLES Burundi.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Burundi data dictionary.csv", replace 
...
* graph Burundi ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Burundi, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(460 "JOHN" 45 "DELP" 1 "IMPE" 158 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Burundi ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Burundi 1 Process.smcl"
