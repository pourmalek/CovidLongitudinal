...
log using "log CovidLongitudinal Gabon 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Gabon 1 Process.do"
...
**** "do CovidLongitudinal Gabon 1 Process.do"
...
** output data files: "`model' Gabon.dta"
**                   "ALL MODLES Gabon.dta"

** output data dictionary files: "Gabon ALL MODELS update dates data dictionary.csv"
**                               "`model' Gabon data dictionary.csv"
**                               "ALL MODLES Gabon data dictionary.csv"

** graphs:
** "graph 02 Gabon `model' C19 daily deaths all updates.pdf"
** "graph 02 Gabon ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Gabon absent update_date_block"
	
	qui keep if loc_grand_name == "Gabon"
	...
	save "Gabon `model' update dates.dta", replace
...
use "Gabon DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Gabon `model' update dates.dta"
...
save "Gabon ALL MODELS update dates.dta", replace
	
export excel using "Gabon ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Gabon ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Gabon"

keep date DayDeaMeSmJOHNGAB
...
save "JOHN Gabon.dta", replace 




* graph Gabon JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNGAB date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Gabon, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Gabon JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Gabon.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Gabon.dta"
...
merge m:m date using "JOHN Gabon.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Gabon data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Gabon DELP daily deaths each update 
	...
	subtitle("Gabon, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Gabon DELP daily deaths update `update'.pdf", replace	
...
* graph Gabon DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Gabon, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Gabon DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Gabon IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Gabon, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Gabon IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Gabon.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Gabon.dta"
...
merge m:1 date using "JOHN Gabon.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Gabon data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Gabon IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Gabon, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Gabon IHME daily deaths update `update'.pdf", replace	
...
* graph Gabon IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Gabon, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Gabon IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Gabon.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Gabon.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Gabon data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Gabon IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Gabon, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Gabon IMPE daily deaths update `update'.pdf", replace	
...
* graph Gabon IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Gabon, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Gabon IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Gabon.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Gabon.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Gabon data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Gabon LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Gabon, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Gabon LANL daily deaths update `update'.pdf", replace	
...
* graph Gabon LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Gabon, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Gabon LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Gabon.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Gabon.dta", force
...
save "ALL MODLES Gabon.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Gabon data dictionary.csv", replace 
...
* graph Gabon ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Gabon, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(656 "JOHN" 4 "DELP" 60 "IHME" 1 "IMPE" 42 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Gabon ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Gabon 1 Process.smcl"
