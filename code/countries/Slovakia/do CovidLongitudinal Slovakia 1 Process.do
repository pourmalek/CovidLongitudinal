...
log using "log CovidLongitudinal Slovakia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Slovakia 1 Process.do"
...
**** "do CovidLongitudinal Slovakia 1 Process.do"
...
** output data files: "`model' Slovakia.dta"
**                   "ALL MODLES Slovakia.dta"

** output data dictionary files: "Slovakia ALL MODELS update dates data dictionary.csv"
**                               "`model' Slovakia data dictionary.csv"
**                               "ALL MODLES Slovakia data dictionary.csv"

** graphs:
** "graph 02 Slovakia `model' C19 daily deaths all updates.pdf"
** "graph 02 Slovakia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Slovakia absent update_date_block"
	
	qui keep if loc_grand_name == "Slovakia"
	...
	save "Slovakia `model' update dates.dta", replace
...
use "Slovakia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Slovakia `model' update dates.dta"
...
save "Slovakia ALL MODELS update dates.dta", replace
	
export excel using "Slovakia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Slovakia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Slovakia"

keep date DayDeaMeSmJOHNSVK
...
save "JOHN Slovakia.dta", replace 




* graph Slovakia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSVK date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Slovakia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Slovakia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Slovakia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Slovakia.dta"
...
save "DELP Slovakia.dta", replace
...
merge m:m date using "JOHN Slovakia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Slovakia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Slovakia DELP daily deaths each update 
	...
	subtitle("Slovakia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Slovakia DELP daily deaths update `update'.pdf", replace	
...
* graph Slovakia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Slovakia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Slovakia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Slovakia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Slovakia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Slovakia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Slovakia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Slovakia.dta"
...
save "IHME Slovakia.dta", replace
...
merge m:1 date using "JOHN Slovakia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Slovakia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Slovakia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Slovakia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Slovakia IHME daily deaths update `update'.pdf", replace	
...
* graph Slovakia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Slovakia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Slovakia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Slovakia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Slovakia.dta"
...
save "IMPE Slovakia.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Slovakia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Slovakia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Slovakia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Slovakia IMPE daily deaths update `update'.pdf", replace	
...
* graph Slovakia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Slovakia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Slovakia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Slovakia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Slovakia.dta"
...
save "LANL Slovakia.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Slovakia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Slovakia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Slovakia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Slovakia LANL daily deaths update `update'.pdf", replace	
...
* graph Slovakia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Slovakia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Slovakia LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Slovakia.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Slovakia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Slovakia.dta"
...
save "YYGU Slovakia.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Slovakia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Slovakia YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Slovakia, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Slovakia YYGU daily deaths update `update'.pdf", replace	
...
* graph Slovakia YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Slovakia, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Slovakia YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Slovakia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Slovakia.dta", force
...
save "ALL MODLES Slovakia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Slovakia data dictionary.csv", replace 
...
* graph Slovakia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Slovakia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(832 "JOHN" 25 "DELP" 1 "IHME" 89 "IMPE" 34 "LANL" 2 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Slovakia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Slovakia 1 Process.smcl"
