...
log using "log CovidLongitudinal Sri Lanka 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Sri Lanka 1 Process.do"
...
**** "do CovidLongitudinal Sri Lanka 1 Process.do"
...
** output data files: "`model' Sri Lanka.dta"
**                   "ALL MODLES Sri Lanka.dta"

** output data dictionary files: "Sri Lanka ALL MODELS update dates data dictionary.csv"
**                               "`model' Sri Lanka data dictionary.csv"
**                               "ALL MODLES Sri Lanka data dictionary.csv"

** graphs:
** "graph 02 Sri Lanka `model' C19 daily deaths all updates.pdf"
** "graph 02 Sri Lanka ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Sri Lanka absent update_date_block"
	
	qui keep if loc_grand_name == "Sri Lanka"
	...
	save "Sri Lanka `model' update dates.dta", replace
...
use "Sri Lanka DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Sri Lanka `model' update dates.dta"
...
save "Sri Lanka ALL MODELS update dates.dta", replace
	
export excel using "Sri Lanka ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Sri Lanka ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Sri Lanka"

keep date DayDeaMeSmJOHNLKA
...
save "JOHN Sri Lanka.dta", replace 




* graph Sri Lanka JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNLKA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Sri Lanka, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sri Lanka JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Sri Lanka.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Sri Lanka.dta"
...
save "DELP Sri Lanka.dta", replace
...
merge m:m date using "JOHN Sri Lanka.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Sri Lanka data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sri Lanka DELP daily deaths each update 
	...
	subtitle("Sri Lanka, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sri Lanka DELP daily deaths update `update'.pdf", replace	
...
* graph Sri Lanka DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sri Lanka, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sri Lanka DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Sri Lanka IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Sri Lanka, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Sri Lanka IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Sri Lanka.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Sri Lanka.dta"
...
save "IHME Sri Lanka.dta", replace
...
merge m:1 date using "JOHN Sri Lanka.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Sri Lanka data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sri Lanka IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Sri Lanka, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sri Lanka IHME daily deaths update `update'.pdf", replace	
...
* graph Sri Lanka IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sri Lanka, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sri Lanka IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Sri Lanka.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Sri Lanka.dta"
...
save "IMPE Sri Lanka.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Sri Lanka data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sri Lanka IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Sri Lanka, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sri Lanka IMPE daily deaths update `update'.pdf", replace	
...
* graph Sri Lanka IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sri Lanka, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sri Lanka IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Sri Lanka.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Sri Lanka.dta"
...
save "LANL Sri Lanka.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Sri Lanka data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sri Lanka LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Sri Lanka, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sri Lanka LANL daily deaths update `update'.pdf", replace	
...
* graph Sri Lanka LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sri Lanka, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sri Lanka LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Sri Lanka.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Sri Lanka.dta", force
...
save "ALL MODLES Sri Lanka.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Sri Lanka data dictionary.csv", replace 
...
* graph Sri Lanka ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sri Lanka, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(616 "JOHN" 1 "DELP" 55 "IHME" 3 "IMPE" 124 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sri Lanka ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Sri Lanka 1 Process.smcl"
