...
log using "log CovidLongitudinal Sierra Leone 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Sierra Leone 1 Process.do"
...
**** "do CovidLongitudinal Sierra Leone 1 Process.do"
...
** output data files: "`model' Sierra Leone.dta"
**                   "ALL MODLES Sierra Leone.dta"

** output data dictionary files: "Sierra Leone ALL MODELS update dates data dictionary.csv"
**                               "`model' Sierra Leone data dictionary.csv"
**                               "ALL MODLES Sierra Leone data dictionary.csv"

** graphs:
** "graph 02 Sierra Leone `model' C19 daily deaths all updates.pdf"
** "graph 02 Sierra Leone ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Sierra Leone absent update_date_block"
	
	qui keep if loc_grand_name == "Sierra Leone"
	...
	save "Sierra Leone `model' update dates.dta", replace
...
use "Sierra Leone DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Sierra Leone `model' update dates.dta"
...
save "Sierra Leone ALL MODELS update dates.dta", replace
	
export excel using "Sierra Leone ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Sierra Leone ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Sierra Leone"

keep date DayDeaMeSmJOHNSLE
...
save "JOHN Sierra Leone.dta", replace 




* graph Sierra Leone JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSLE date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Sierra Leone, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sierra Leone JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Sierra Leone.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Sierra Leone.dta"
...
save "DELP Sierra Leone.dta", replace
...
merge m:m date using "JOHN Sierra Leone.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Sierra Leone data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sierra Leone DELP daily deaths each update 
	...
	subtitle("Sierra Leone, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sierra Leone DELP daily deaths update `update'.pdf", replace	
...
* graph Sierra Leone DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sierra Leone, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sierra Leone DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Sierra Leone IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Sierra Leone, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Sierra Leone IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Sierra Leone.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Sierra Leone.dta"
...
save "IHME Sierra Leone.dta", replace
...
merge m:1 date using "JOHN Sierra Leone.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Sierra Leone data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sierra Leone IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Sierra Leone, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sierra Leone IHME daily deaths update `update'.pdf", replace	
...
* graph Sierra Leone IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sierra Leone, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sierra Leone IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Sierra Leone.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Sierra Leone.dta"
...
save "IMPE Sierra Leone.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Sierra Leone data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sierra Leone IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Sierra Leone, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sierra Leone IMPE daily deaths update `update'.pdf", replace	
...
* graph Sierra Leone IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sierra Leone, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sierra Leone IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Sierra Leone.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Sierra Leone.dta"
...
save "LANL Sierra Leone.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Sierra Leone data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Sierra Leone LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Sierra Leone, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Sierra Leone LANL daily deaths update `update'.pdf", replace	
...
* graph Sierra Leone LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sierra Leone, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sierra Leone LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Sierra Leone.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Sierra Leone.dta", force
...
save "ALL MODLES Sierra Leone.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Sierra Leone data dictionary.csv", replace 
...
* graph Sierra Leone ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Sierra Leone, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(660 "JOHN" 12 "DELP" 65 "IHME" 1 "IMPE" 18 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Sierra Leone ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Sierra Leone 1 Process.smcl"
