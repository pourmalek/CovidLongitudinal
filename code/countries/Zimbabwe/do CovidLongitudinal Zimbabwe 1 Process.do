...
log using "log CovidLongitudinal Zimbabwe 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Zimbabwe 1 Process.do"
...
**** "do CovidLongitudinal Zimbabwe 1 Process.do"
...
** output data files: "`model' Zimbabwe.dta"
**                   "ALL MODLES Zimbabwe.dta"

** output data dictionary files: "Zimbabwe ALL MODELS update dates data dictionary.csv"
**                               "`model' Zimbabwe data dictionary.csv"
**                               "ALL MODLES Zimbabwe data dictionary.csv"

** graphs:
** "graph 02 Zimbabwe `model' C19 daily deaths all updates.pdf"
** "graph 02 Zimbabwe ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Zimbabwe absent update_date_block"
	
	qui keep if loc_grand_name == "Zimbabwe"
	...
	save "Zimbabwe `model' update dates.dta", replace
...
use "Zimbabwe DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Zimbabwe `model' update dates.dta"
...
save "Zimbabwe ALL MODELS update dates.dta", replace
	
export excel using "Zimbabwe ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Zimbabwe ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Zimbabwe"

keep date DayDeaMeSmJOHNZWE
...
save "JOHN Zimbabwe.dta", replace 




* graph Zimbabwe JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNZWE date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Zimbabwe, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Zimbabwe JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Zimbabwe.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Zimbabwe.dta"
...
save "DELP Zimbabwe.dta", replace
...
merge m:m date using "JOHN Zimbabwe.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Zimbabwe data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Zimbabwe DELP daily deaths each update 
	...
	subtitle("Zimbabwe, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Zimbabwe DELP daily deaths update `update'.pdf", replace	
...
* graph Zimbabwe DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Zimbabwe, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Zimbabwe DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Zimbabwe IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Zimbabwe, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Zimbabwe IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Zimbabwe.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Zimbabwe.dta"
...
save "IHME Zimbabwe.dta", replace
...
merge m:1 date using "JOHN Zimbabwe.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Zimbabwe data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Zimbabwe IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Zimbabwe, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Zimbabwe IHME daily deaths update `update'.pdf", replace	
...
* graph Zimbabwe IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Zimbabwe, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Zimbabwe IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Zimbabwe.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Zimbabwe.dta"
...
save "IMPE Zimbabwe.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Zimbabwe data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Zimbabwe IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Zimbabwe, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Zimbabwe IMPE daily deaths update `update'.pdf", replace	
...
* graph Zimbabwe IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Zimbabwe, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Zimbabwe IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Zimbabwe.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Zimbabwe.dta"
...
save "LANL Zimbabwe.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Zimbabwe data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Zimbabwe LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Zimbabwe, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Zimbabwe LANL daily deaths update `update'.pdf", replace	
...
* graph Zimbabwe LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Zimbabwe, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Zimbabwe LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Zimbabwe.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Zimbabwe.dta", force
...
save "ALL MODLES Zimbabwe.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Zimbabwe data dictionary.csv", replace 
...
* graph Zimbabwe ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Zimbabwe, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(638 "JOHN" 39 "DELP" 48 "IHME" 1 "IMPE" 61 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Zimbabwe ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Zimbabwe 1 Process.smcl"
