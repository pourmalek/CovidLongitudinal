...
log using "log CovidLongitudinal Barbados 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Barbados 1 Process.do"
...
**** "do CovidLongitudinal Barbados 1 Process.do"
...
** output data files: "`model' Barbados.dta"
**                   "ALL MODLES Barbados.dta"

** output data dictionary files: "Barbados ALL MODELS update dates data dictionary.csv"
**                               "`model' Barbados data dictionary.csv"
**                               "ALL MODLES Barbados data dictionary.csv"

** graphs:
** "graph 02 Barbados `model' C19 daily deaths all updates.pdf"
** "graph 02 Barbados ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Barbados absent update_date_block"
	
	qui keep if loc_grand_name == "Barbados"
	...
	save "Barbados `model' update dates.dta", replace
...
use "Barbados DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Barbados `model' update dates.dta"
...
save "Barbados ALL MODELS update dates.dta", replace
	
export excel using "Barbados ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Barbados ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Barbados"

keep date DayDeaMeSmJOHNBRB
...
save "JOHN Barbados.dta", replace 




* graph Barbados JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNBRB date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Barbados, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Barbados JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Barbados.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Barbados.dta"
...
save "DELP Barbados.dta", replace
...
merge m:m date using "JOHN Barbados.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Barbados data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Barbados DELP daily deaths each update 
	...
	subtitle("Barbados, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Barbados DELP daily deaths update `update'.pdf", replace	
...
* graph Barbados DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Barbados, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Barbados DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Barbados IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Barbados, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Barbados IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Barbados.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Barbados.dta"
...
save "IHME Barbados.dta", replace
...
merge m:1 date using "JOHN Barbados.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Barbados data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Barbados IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Barbados, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Barbados IHME daily deaths update `update'.pdf", replace	
...
* graph Barbados IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Barbados, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Barbados IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Barbados.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Barbados.dta"
...
save "IMPE Barbados.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Barbados data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Barbados IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Barbados, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Barbados IMPE daily deaths update `update'.pdf", replace	
...
* graph Barbados IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Barbados, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Barbados IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Barbados.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Barbados.dta"
...
save "LANL Barbados.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Barbados data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Barbados LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Barbados, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Barbados LANL daily deaths update `update'.pdf", replace	
...
* graph Barbados LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Barbados, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Barbados LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Barbados.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Barbados.dta", force
...
save "ALL MODLES Barbados.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Barbados data dictionary.csv", replace 
...
* graph Barbados ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Barbados, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(536 "JOHN" 27 "DELP" 8 "IHME" 1 "IMPE" 105 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Barbados ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Barbados 1 Process.smcl"
