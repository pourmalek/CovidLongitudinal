...
log using "log CovidLongitudinal Zambia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Zambia 1 Process.do"
...
**** "do CovidLongitudinal Zambia 1 Process.do"
...
** output data files: "`model' Zambia.dta"
**                   "ALL MODLES Zambia.dta"

** output data dictionary files: "Zambia ALL MODELS update dates data dictionary.csv"
**                               "`model' Zambia data dictionary.csv"
**                               "ALL MODLES Zambia data dictionary.csv"

** graphs:
** "graph 02 Zambia `model' C19 daily deaths all updates.pdf"
** "graph 02 Zambia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Zambia absent update_date_block"
	
	qui keep if loc_grand_name == "Zambia"
	...
	save "Zambia `model' update dates.dta", replace
...
use "Zambia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Zambia `model' update dates.dta"
...
save "Zambia ALL MODELS update dates.dta", replace
	
export excel using "Zambia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Zambia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Zambia"

keep date DayDeaMeSmJOHNZMB
...
save "JOHN Zambia.dta", replace 




* graph Zambia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNZMB date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Zambia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Zambia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Zambia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Zambia.dta"
...
merge m:m date using "JOHN Zambia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Zambia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Zambia DELP daily deaths each update 
	...
	subtitle("Zambia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Zambia DELP daily deaths update `update'.pdf", replace	
...
* graph Zambia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Zambia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Zambia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Zambia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Zambia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Zambia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Zambia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Zambia.dta"
...
merge m:1 date using "JOHN Zambia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Zambia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Zambia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Zambia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Zambia IHME daily deaths update `update'.pdf", replace	
...
* graph Zambia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Zambia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Zambia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Zambia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Zambia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Zambia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Zambia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Zambia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Zambia IMPE daily deaths update `update'.pdf", replace	
...
* graph Zambia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Zambia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Zambia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Zambia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Zambia.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Zambia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Zambia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Zambia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Zambia LANL daily deaths update `update'.pdf", replace	
...
* graph Zambia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Zambia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Zambia LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Zambia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Zambia.dta", force
...
save "ALL MODLES Zambia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Zambia data dictionary.csv", replace 
...
* graph Zambia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Zambia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(648 "JOHN" 12 "DELP" 52 "IHME" 1 "IMPE" 56 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Zambia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Zambia 1 Process.smcl"
