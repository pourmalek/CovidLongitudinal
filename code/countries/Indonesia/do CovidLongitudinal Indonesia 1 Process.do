...
log using "log CovidLongitudinal Indonesia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Indonesia 1 Process.do"
...
**** "do CovidLongitudinal Indonesia 1 Process.do"
...
** output data files: "`model' Indonesia.dta"
**                   "ALL MODLES Indonesia.dta"

** output data dictionary files: "Indonesia ALL MODELS update dates data dictionary.csv"
**                               "`model' Indonesia data dictionary.csv"
**                               "ALL MODLES Indonesia data dictionary.csv"

** graphs:
** "graph 02 Indonesia `model' C19 daily deaths all updates.pdf"
** "graph 02 Indonesia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Indonesia absent update_date_block"
	
	qui keep if loc_grand_name == "Indonesia"
	...
	save "Indonesia `model' update dates.dta", replace
...
use "Indonesia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Indonesia `model' update dates.dta"
...
save "Indonesia ALL MODELS update dates.dta", replace
	
export excel using "Indonesia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Indonesia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Indonesia"

keep date DayDeaMeSmJOHNIDN
...
save "JOHN Indonesia.dta", replace 




* graph Indonesia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNIDN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Indonesia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Indonesia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Indonesia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Indonesia.dta"
...
save "DELP Indonesia.dta", replace
...
merge m:m date using "JOHN Indonesia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Indonesia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Indonesia DELP daily deaths each update 
	...
	subtitle("Indonesia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Indonesia DELP daily deaths update `update'.pdf", replace	
...
* graph Indonesia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Indonesia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Indonesia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Indonesia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Indonesia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Indonesia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Indonesia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Indonesia.dta"
...
save "IHME Indonesia.dta", replace
...
merge m:1 date using "JOHN Indonesia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Indonesia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Indonesia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Indonesia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Indonesia IHME daily deaths update `update'.pdf", replace	
...
* graph Indonesia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Indonesia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Indonesia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Indonesia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Indonesia.dta"
...
save "IMPE Indonesia.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Indonesia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Indonesia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Indonesia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Indonesia IMPE daily deaths update `update'.pdf", replace	
...
* graph Indonesia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Indonesia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Indonesia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Indonesia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Indonesia.dta"
...
save "LANL Indonesia.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Indonesia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Indonesia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Indonesia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Indonesia LANL daily deaths update `update'.pdf", replace	
...
* graph Indonesia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Indonesia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Indonesia LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Indonesia.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Indonesia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Indonesia.dta"
...
save "UCLA Indonesia.dta", replace
...
preserve
    ...
    export delimited name varlab using "UCLA Indonesia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Indonesia UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Indonesia, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Indonesia UCLA daily deaths update `update'.pdf", replace	

}
*/


* graph Indonesia UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Indonesia, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Indonesia UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Indonesia.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Indonesia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Indonesia.dta"
...
save "YYGU Indonesia.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Indonesia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Indonesia YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Indonesia, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Indonesia YYGU daily deaths update `update'.pdf", replace	
...
* graph Indonesia YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Indonesia, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Indonesia YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Indonesia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Indonesia.dta", force
...
save "ALL MODLES Indonesia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Indonesia data dictionary.csv", replace 
...
* graph Indonesia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Indonesia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(880 "JOHN" 11 "DELP" 174 "IHME" 25 "IMPE" 22 "LANL" 395 "UCLA" 1 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Indonesia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Indonesia 1 Process.smcl"
