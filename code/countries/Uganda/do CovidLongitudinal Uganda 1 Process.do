...
log using "log CovidLongitudinal Uganda 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Uganda 1 Process.do"
...
**** "do CovidLongitudinal Uganda 1 Process.do"
...
** output data files: "`model' Uganda.dta"
**                   "ALL MODLES Uganda.dta"

** output data dictionary files: "Uganda ALL MODELS update dates data dictionary.csv"
**                               "`model' Uganda data dictionary.csv"
**                               "ALL MODLES Uganda data dictionary.csv"

** graphs:
** "graph 02 Uganda `model' C19 daily deaths all updates.pdf"
** "graph 02 Uganda ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Uganda absent update_date_block"
	
	qui keep if loc_grand_name == "Uganda"
	...
	save "Uganda `model' update dates.dta", replace
...
use "Uganda DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Uganda `model' update dates.dta"
...
save "Uganda ALL MODELS update dates.dta", replace
	
export excel using "Uganda ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Uganda ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Uganda"

keep date DayDeaMeSmJOHNUGA
...
save "JOHN Uganda.dta", replace 




* graph Uganda JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNUGA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Uganda, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uganda JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Uganda.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Uganda.dta"
...
merge m:m date using "JOHN Uganda.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Uganda data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Uganda DELP daily deaths each update 
	...
	subtitle("Uganda, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Uganda DELP daily deaths update `update'.pdf", replace	
...
* graph Uganda DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Uganda, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uganda DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Uganda IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Uganda, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Uganda IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Uganda.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Uganda.dta"
...
merge m:1 date using "JOHN Uganda.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Uganda data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Uganda IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Uganda, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Uganda IHME daily deaths update `update'.pdf", replace	
...
* graph Uganda IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Uganda, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uganda IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Uganda.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Uganda.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Uganda data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Uganda IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Uganda, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Uganda IMPE daily deaths update `update'.pdf", replace	
...
* graph Uganda IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Uganda, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uganda IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Uganda.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Uganda.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Uganda data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Uganda LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Uganda, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Uganda LANL daily deaths update `update'.pdf", replace	
...
* graph Uganda LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Uganda, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uganda LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Uganda.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Uganda.dta", force
...
save "ALL MODLES Uganda.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Uganda data dictionary.csv", replace 
...
* graph Uganda ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Uganda, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(570 "JOHN" 1 "DELP" 7 "IHME" 13 "IMPE" 25 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uganda ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Uganda 1 Process.smcl"
