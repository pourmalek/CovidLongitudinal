...
log using "log CovidLongitudinal Oman 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Oman 1 Process.do"
...
**** "do CovidLongitudinal Oman 1 Process.do"
...
** output data files: "`model' Oman.dta"
**                   "ALL MODLES Oman.dta"

** output data dictionary files: "Oman ALL MODELS update dates data dictionary.csv"
**                               "`model' Oman data dictionary.csv"
**                               "ALL MODLES Oman data dictionary.csv"

** graphs:
** "graph 02 Oman `model' C19 daily deaths all updates.pdf"
** "graph 02 Oman ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Oman absent update_date_block"
	
	qui keep if loc_grand_name == "Oman"
	...
	save "Oman `model' update dates.dta", replace
...
use "Oman DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Oman `model' update dates.dta"
...
save "Oman ALL MODELS update dates.dta", replace
	
export excel using "Oman ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Oman ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Oman"

keep date DayDeaMeSmJOHNOMN
...
save "JOHN Oman.dta", replace 




* graph Oman JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNOMN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Oman, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Oman JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Oman.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Oman.dta"
...
merge m:m date using "JOHN Oman.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Oman data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Oman DELP daily deaths each update 
	...
	subtitle("Oman, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Oman DELP daily deaths update `update'.pdf", replace	
...
* graph Oman DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Oman, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Oman DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Oman IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Oman, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Oman IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Oman.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Oman.dta"
...
merge m:1 date using "JOHN Oman.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Oman data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Oman IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Oman, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Oman IHME daily deaths update `update'.pdf", replace	
...
* graph Oman IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Oman, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Oman IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Oman.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Oman.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Oman data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Oman IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Oman, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Oman IMPE daily deaths update `update'.pdf", replace	
...
* graph Oman IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Oman, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Oman IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Oman.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Oman.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Oman data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Oman LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Oman, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Oman LANL daily deaths update `update'.pdf", replace	
...
* graph Oman LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Oman, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Oman LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Oman.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Oman.dta", force
...
save "ALL MODLES Oman.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Oman data dictionary.csv", replace 
...
* graph Oman ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Oman, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(625 "JOHN" 1 "DELP" 30 "IHME" 13 "IMPE" 6 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Oman ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Oman 1 Process.smcl"
