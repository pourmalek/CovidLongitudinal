...
log using "log CovidLongitudinal Guyana 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Guyana 1 Process.do"
...
**** "do CovidLongitudinal Guyana 1 Process.do"
...
** output data files: "`model' Guyana.dta"
**                   "ALL MODLES Guyana.dta"

** output data dictionary files: "Guyana ALL MODELS update dates data dictionary.csv"
**                               "`model' Guyana data dictionary.csv"
**                               "ALL MODLES Guyana data dictionary.csv"

** graphs:
** "graph 02 Guyana `model' C19 daily deaths all updates.pdf"
** "graph 02 Guyana ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Guyana absent update_date_block"
	
	qui keep if loc_grand_name == "Guyana"
	...
	save "Guyana `model' update dates.dta", replace
...
use "Guyana DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Guyana `model' update dates.dta"
...
save "Guyana ALL MODELS update dates.dta", replace
	
export excel using "Guyana ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Guyana ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Guyana"

keep date DayDeaMeSmJOHNGUY
...
save "JOHN Guyana.dta", replace 




* graph Guyana JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNGUY date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Guyana, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guyana JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Guyana.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Guyana.dta"
...
merge m:m date using "JOHN Guyana.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Guyana data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guyana DELP daily deaths each update 
	...
	subtitle("Guyana, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guyana DELP daily deaths update `update'.pdf", replace	
...
* graph Guyana DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guyana, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guyana DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Guyana IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Guyana, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Guyana IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Guyana.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Guyana.dta"
...
merge m:1 date using "JOHN Guyana.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Guyana data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guyana IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Guyana, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guyana IHME daily deaths update `update'.pdf", replace	
...
* graph Guyana IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guyana, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guyana IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Guyana.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Guyana.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Guyana data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guyana IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Guyana, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guyana IMPE daily deaths update `update'.pdf", replace	
...
* graph Guyana IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guyana, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guyana IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Guyana.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Guyana.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Guyana data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guyana LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Guyana, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guyana LANL daily deaths update `update'.pdf", replace	
...
* graph Guyana LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guyana, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guyana LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Guyana.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Guyana.dta", force
...
save "ALL MODLES Guyana.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Guyana data dictionary.csv", replace 
...
* graph Guyana ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guyana, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(639 "JOHN" 26 "DELP" 50 "IHME" 1 "IMPE" 72 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guyana ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Guyana 1 Process.smcl"
