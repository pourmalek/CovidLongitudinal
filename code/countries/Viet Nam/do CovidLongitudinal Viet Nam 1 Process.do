...
log using "log CovidLongitudinal Viet Nam 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Viet Nam 1 Process.do"
...
**** "do CovidLongitudinal Viet Nam 1 Process.do"
...
** output data files: "`model' Viet Nam.dta"
**                   "ALL MODLES Viet Nam.dta"

** output data dictionary files: "Viet Nam ALL MODELS update dates data dictionary.csv"
**                               "`model' Viet Nam data dictionary.csv"
**                               "ALL MODLES Viet Nam data dictionary.csv"

** graphs:
** "graph 02 Viet Nam `model' C19 daily deaths all updates.pdf"
** "graph 02 Viet Nam ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Viet Nam absent update_date_block"
	
	qui keep if loc_grand_name == "Viet Nam"
	...
	save "Viet Nam `model' update dates.dta", replace
...
use "Viet Nam DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Viet Nam `model' update dates.dta"
...
save "Viet Nam ALL MODELS update dates.dta", replace
	
export excel using "Viet Nam ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Viet Nam ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Viet Nam"

keep date DayDeaMeSmJOHNVNM
...
save "JOHN Viet Nam.dta", replace 




* graph Viet Nam JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNVNM date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Viet Nam, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Viet Nam JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Viet Nam.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Viet Nam.dta"
...
save "DELP Viet Nam.dta", replace
...
merge m:m date using "JOHN Viet Nam.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Viet Nam data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Viet Nam DELP daily deaths each update 
	...
	subtitle("Viet Nam, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Viet Nam DELP daily deaths update `update'.pdf", replace	
...
* graph Viet Nam DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Viet Nam, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Viet Nam DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Viet Nam IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Viet Nam, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Viet Nam IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Viet Nam.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Viet Nam.dta"
...
save "IHME Viet Nam.dta", replace
...
merge m:1 date using "JOHN Viet Nam.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Viet Nam data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Viet Nam IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Viet Nam, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Viet Nam IHME daily deaths update `update'.pdf", replace	
...
* graph Viet Nam IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Viet Nam, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Viet Nam IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Viet Nam.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Viet Nam.dta"
...
save "IMPE Viet Nam.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Viet Nam data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Viet Nam IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Viet Nam, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Viet Nam IMPE daily deaths update `update'.pdf", replace	
...
* graph Viet Nam IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Viet Nam, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Viet Nam IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Viet Nam.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Viet Nam.dta"
...
save "LANL Viet Nam.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Viet Nam data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Viet Nam LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Viet Nam, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Viet Nam LANL daily deaths update `update'.pdf", replace	
...
* graph Viet Nam LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Viet Nam, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Viet Nam LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Viet Nam.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Viet Nam.dta", force
...
save "ALL MODLES Viet Nam.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Viet Nam data dictionary.csv", replace 
...
* graph Viet Nam ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Viet Nam, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(560 "JOHN" 1 "DELP" 80 "IHME" 16 "IMPE" 22 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Viet Nam ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Viet Nam 1 Process.smcl"
