...
log using "log CovidLongitudinal Austria 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Austria 1 Process.do"
...
**** "do CovidLongitudinal Austria 1 Process.do"
...
** output data files: "`model' Austria.dta"
**                   "ALL MODLES Austria.dta"

** output data dictionary files: "Austria ALL MODELS update dates data dictionary.csv"
**                               "`model' Austria data dictionary.csv"
**                               "ALL MODLES Austria data dictionary.csv"

** graphs:
** "graph 02 Austria `model' C19 daily deaths all updates.pdf"
** "graph 02 Austria ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Austria absent update_date_block"
	
	qui keep if loc_grand_name == "Austria"
	...
	save "Austria `model' update dates.dta", replace
...
use "Austria DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Austria `model' update dates.dta"
...
save "Austria ALL MODELS update dates.dta", replace
	
export excel using "Austria ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Austria ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Austria"

keep date DayDeaMeSmJOHNAUT
...
save "JOHN Austria.dta", replace 




* graph Austria JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNAUT date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Austria, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Austria JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Austria.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Austria.dta"
...
merge m:m date using "JOHN Austria.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Austria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Austria DELP daily deaths each update 
	...
	subtitle("Austria, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Austria DELP daily deaths update `update'.pdf", replace	
...
* graph Austria DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Austria, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Austria DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Austria IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Austria, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Austria IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Austria.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Austria.dta"
...
merge m:1 date using "JOHN Austria.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Austria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Austria IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Austria, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Austria IHME daily deaths update `update'.pdf", replace	
...
* graph Austria IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Austria, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Austria IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Austria.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Austria.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Austria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Austria IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Austria, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Austria IMPE daily deaths update `update'.pdf", replace	
...
* graph Austria IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Austria, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Austria IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Austria.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Austria.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Austria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Austria LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Austria, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Austria LANL daily deaths update `update'.pdf", replace	
...
* graph Austria LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Austria, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Austria LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Austria.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Austria.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Austria.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Austria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Austria YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Austria, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Austria YYGU daily deaths update `update'.pdf", replace	
...
* graph Austria YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Austria, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Austria YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Austria.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Austria.dta", force
...
save "ALL MODLES Austria.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Austria data dictionary.csv", replace 
...
* graph Austria ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Austria, all models, all updates, forecast only", size(medium) color(black)) ///
...
qui graph export "graph 02 Austria ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Austria 1 Process.smcl"
