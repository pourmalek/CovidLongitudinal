...
log using "log CovidLongitudinal Latvia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Latvia 1 Process.do"
...
**** "do CovidLongitudinal Latvia 1 Process.do"
...
** output data files: "`model' Latvia.dta"
**                   "ALL MODLES Latvia.dta"

** output data dictionary files: "Latvia ALL MODELS update dates data dictionary.csv"
**                               "`model' Latvia data dictionary.csv"
**                               "ALL MODLES Latvia data dictionary.csv"

** graphs:
** "graph 02 Latvia `model' C19 daily deaths all updates.pdf"
** "graph 02 Latvia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Latvia absent update_date_block"
	
	qui keep if loc_grand_name == "Latvia"
	...
	save "Latvia `model' update dates.dta", replace
...
use "Latvia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Latvia `model' update dates.dta"
...
save "Latvia ALL MODELS update dates.dta", replace
	
export excel using "Latvia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Latvia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Latvia"

keep date DayDeaMeSmJOHNLVA
...
save "JOHN Latvia.dta", replace 




* graph Latvia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNLVA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Latvia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Latvia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Latvia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Latvia.dta"
...
merge m:m date using "JOHN Latvia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Latvia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Latvia DELP daily deaths each update 
	...
	subtitle("Latvia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Latvia DELP daily deaths update `update'.pdf", replace	
...
* graph Latvia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Latvia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Latvia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Latvia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Latvia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Latvia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Latvia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Latvia.dta"
...
merge m:1 date using "JOHN Latvia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Latvia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Latvia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Latvia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Latvia IHME daily deaths update `update'.pdf", replace	
...
* graph Latvia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Latvia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Latvia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Latvia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Latvia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Latvia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Latvia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Latvia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Latvia IMPE daily deaths update `update'.pdf", replace	
...
* graph Latvia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Latvia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Latvia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Latvia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Latvia.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Latvia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Latvia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Latvia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Latvia LANL daily deaths update `update'.pdf", replace	
...
* graph Latvia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Latvia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Latvia LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Latvia.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Latvia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Latvia.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Latvia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Latvia YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Latvia, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Latvia YYGU daily deaths update `update'.pdf", replace	
...
* graph Latvia YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Latvia, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Latvia YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Latvia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Latvia.dta", force
...
save "ALL MODLES Latvia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Latvia data dictionary.csv", replace 
...
* graph Latvia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Latvia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(827 "JOHN" 15 "DELP" 1 "IHME" 84 "IMPE" 63 "LANL" 2 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Latvia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Latvia 1 Process.smcl"
