...
log using "log CovidLongitudinal Iceland 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Iceland 1 Process.do"
...
**** "do CovidLongitudinal Iceland 1 Process.do"
...
** output data files: "`model' Iceland.dta"
**                   "ALL MODLES Iceland.dta"

** output data dictionary files: "Iceland ALL MODELS update dates data dictionary.csv"
**                               "`model' Iceland data dictionary.csv"
**                               "ALL MODLES Iceland data dictionary.csv"

** graphs:
** "graph 02 Iceland `model' C19 daily deaths all updates.pdf"
** "graph 02 Iceland ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Iceland absent update_date_block"
	
	qui keep if loc_grand_name == "Iceland"
	...
	save "Iceland `model' update dates.dta", replace
...
use "Iceland DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Iceland `model' update dates.dta"
...
save "Iceland ALL MODELS update dates.dta", replace
	
export excel using "Iceland ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Iceland ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Iceland"

keep date DayDeaMeSmJOHNISL
...
save "JOHN Iceland.dta", replace 




* graph Iceland JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNISL date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Iceland, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Iceland JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Iceland.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Iceland.dta"
...
merge m:m date using "JOHN Iceland.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Iceland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Iceland DELP daily deaths each update 
	...
	subtitle("Iceland, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Iceland DELP daily deaths update `update'.pdf", replace	
...
* graph Iceland DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Iceland, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Iceland DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Iceland IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Iceland, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Iceland IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Iceland.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Iceland.dta"
...
merge m:1 date using "JOHN Iceland.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Iceland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Iceland IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Iceland, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Iceland IHME daily deaths update `update'.pdf", replace	
...
* graph Iceland IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Iceland, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Iceland IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Iceland.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Iceland.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Iceland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Iceland IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Iceland, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Iceland IMPE daily deaths update `update'.pdf", replace	
...
* graph Iceland IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Iceland, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Iceland IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Iceland.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Iceland.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Iceland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Iceland LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Iceland, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Iceland LANL daily deaths update `update'.pdf", replace	
...
* graph Iceland LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Iceland, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Iceland LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Iceland.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Iceland.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Iceland.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Iceland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Iceland YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Iceland, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Iceland YYGU daily deaths update `update'.pdf", replace	
...
* graph Iceland YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Iceland, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Iceland YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Iceland.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Iceland.dta", force
...
save "ALL MODLES Iceland.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Iceland data dictionary.csv", replace 
...
* graph Iceland ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Iceland, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(740 "JOHN" 2 "DELP" 1 "IHME" 42 "IMPE" 251 "LANL" 15 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Iceland ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Iceland 1 Process.smcl"
