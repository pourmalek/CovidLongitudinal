...
log using "log CovidLongitudinal Haiti 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Haiti 1 Process.do"
...
**** "do CovidLongitudinal Haiti 1 Process.do"
...
** output data files: "`model' Haiti.dta"
**                   "ALL MODLES Haiti.dta"

** output data dictionary files: "Haiti ALL MODELS update dates data dictionary.csv"
**                               "`model' Haiti data dictionary.csv"
**                               "ALL MODLES Haiti data dictionary.csv"

** graphs:
** "graph 02 Haiti `model' C19 daily deaths all updates.pdf"
** "graph 02 Haiti ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Haiti absent update_date_block"
	
	qui keep if loc_grand_name == "Haiti"
	...
	save "Haiti `model' update dates.dta", replace
...
use "Haiti DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Haiti `model' update dates.dta"
...
save "Haiti ALL MODELS update dates.dta", replace
	
export excel using "Haiti ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Haiti ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Haiti"

keep date DayDeaMeSmJOHNHTI
...
save "JOHN Haiti.dta", replace 




* graph Haiti JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNHTI date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Haiti, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Haiti JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Haiti.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Haiti.dta"
...
merge m:m date using "JOHN Haiti.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Haiti data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Haiti DELP daily deaths each update 
	...
	subtitle("Haiti, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Haiti DELP daily deaths update `update'.pdf", replace	
...
* graph Haiti DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Haiti, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Haiti DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Haiti IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Haiti, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Haiti IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Haiti.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Haiti.dta"
...
merge m:1 date using "JOHN Haiti.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Haiti data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Haiti IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Haiti, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Haiti IHME daily deaths update `update'.pdf", replace	
...
* graph Haiti IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Haiti, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Haiti IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Haiti.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Haiti.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Haiti data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Haiti IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Haiti, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Haiti IMPE daily deaths update `update'.pdf", replace	
...
* graph Haiti IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Haiti, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Haiti IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Haiti.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Haiti.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Haiti data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Haiti LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Haiti, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Haiti LANL daily deaths update `update'.pdf", replace	
...
* graph Haiti LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Haiti, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Haiti LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Haiti.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Haiti.dta", force
...
save "ALL MODLES Haiti.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Haiti data dictionary.csv", replace 
...
* graph Haiti ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Haiti, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(658 "JOHN" 19 "DELP" 62 "IHME" 1 "IMPE" 21 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Haiti ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Haiti 1 Process.smcl"
