...
log using "log CovidLongitudinal Ethiopia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Ethiopia 1 Process.do"
...
**** "do CovidLongitudinal Ethiopia 1 Process.do"
...
** output data files: "`model' Ethiopia.dta"
**                   "ALL MODLES Ethiopia.dta"

** output data dictionary files: "Ethiopia ALL MODELS update dates data dictionary.csv"
**                               "`model' Ethiopia data dictionary.csv"
**                               "ALL MODLES Ethiopia data dictionary.csv"

** graphs:
** "graph 02 Ethiopia `model' C19 daily deaths all updates.pdf"
** "graph 02 Ethiopia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Ethiopia absent update_date_block"
	
	qui keep if loc_grand_name == "Ethiopia"
	...
	save "Ethiopia `model' update dates.dta", replace
...
use "Ethiopia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Ethiopia `model' update dates.dta"
...
save "Ethiopia ALL MODELS update dates.dta", replace
	
export excel using "Ethiopia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Ethiopia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Ethiopia"

keep date DayDeaMeSmJOHNETH
...
save "JOHN Ethiopia.dta", replace 




* graph Ethiopia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNETH date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Ethiopia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ethiopia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Ethiopia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Ethiopia.dta"
...
save "DELP Ethiopia.dta", replace
...
merge m:m date using "JOHN Ethiopia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Ethiopia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ethiopia DELP daily deaths each update 
	...
	subtitle("Ethiopia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ethiopia DELP daily deaths update `update'.pdf", replace	
...
* graph Ethiopia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ethiopia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ethiopia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Ethiopia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Ethiopia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Ethiopia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Ethiopia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Ethiopia.dta"
...
save "IHME Ethiopia.dta", replace
...
merge m:1 date using "JOHN Ethiopia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Ethiopia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ethiopia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ethiopia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ethiopia IHME daily deaths update `update'.pdf", replace	
...
* graph Ethiopia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ethiopia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ethiopia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Ethiopia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Ethiopia.dta"
...
save "IMPE Ethiopia.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Ethiopia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ethiopia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ethiopia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ethiopia IMPE daily deaths update `update'.pdf", replace	
...
* graph Ethiopia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ethiopia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ethiopia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Ethiopia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Ethiopia.dta"
...
save "LANL Ethiopia.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Ethiopia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ethiopia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ethiopia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ethiopia LANL daily deaths update `update'.pdf", replace	
...
* graph Ethiopia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ethiopia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ethiopia LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Ethiopia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Ethiopia.dta", force
...
save "ALL MODLES Ethiopia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Ethiopia data dictionary.csv", replace 
...
* graph Ethiopia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ethiopia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(652 "JOHN" 4 "DELP" 80 "IHME" 1 "IMPE" 45 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ethiopia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Ethiopia 1 Process.smcl"
