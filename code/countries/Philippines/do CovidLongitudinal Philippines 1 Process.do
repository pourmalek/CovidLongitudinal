...
log using "log CovidLongitudinal Philippines 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Philippines 1 Process.do"
...
**** "do CovidLongitudinal Philippines 1 Process.do"
...
** output data files: "`model' Philippines.dta"
**                   "ALL MODLES Philippines.dta"

** output data dictionary files: "Philippines ALL MODELS update dates data dictionary.csv"
**                               "`model' Philippines data dictionary.csv"
**                               "ALL MODLES Philippines data dictionary.csv"

** graphs:
** "graph 02 Philippines `model' C19 daily deaths all updates.pdf"
** "graph 02 Philippines ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Philippines absent update_date_block"
	
	qui keep if loc_grand_name == "Philippines"
	...
	save "Philippines `model' update dates.dta", replace
...
use "Philippines DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Philippines `model' update dates.dta"
...
save "Philippines ALL MODELS update dates.dta", replace
	
export excel using "Philippines ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Philippines ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Philippines"

keep date DayDeaMeSmJOHNPHL
...
save "JOHN Philippines.dta", replace 




* graph Philippines JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNPHL date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Philippines, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Philippines JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Philippines.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Philippines.dta"
...
save "DELP Philippines.dta", replace
...
merge m:m date using "JOHN Philippines.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Philippines data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Philippines DELP daily deaths each update 
	...
	subtitle("Philippines, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Philippines DELP daily deaths update `update'.pdf", replace	
...
* graph Philippines DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Philippines, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Philippines DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Philippines IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Philippines, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Philippines IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Philippines.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Philippines.dta"
...
save "IHME Philippines.dta", replace
...
merge m:1 date using "JOHN Philippines.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Philippines data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Philippines IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Philippines, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Philippines IHME daily deaths update `update'.pdf", replace	
...
* graph Philippines IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Philippines, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Philippines IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Philippines.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Philippines.dta"
...
save "IMPE Philippines.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Philippines data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Philippines IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Philippines, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Philippines IMPE daily deaths update `update'.pdf", replace	
...
* graph Philippines IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Philippines, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Philippines IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Philippines.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Philippines.dta"
...
save "LANL Philippines.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Philippines data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Philippines LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Philippines, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Philippines LANL daily deaths update `update'.pdf", replace	
...
* graph Philippines LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Philippines, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Philippines LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Philippines.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Philippines.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Philippines.dta"
...
save "YYGU Philippines.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Philippines data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Philippines YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Philippines, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Philippines YYGU daily deaths update `update'.pdf", replace	
...
* graph Philippines YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Philippines, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Philippines YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Philippines.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Philippines.dta", force
...
save "ALL MODLES Philippines.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Philippines data dictionary.csv", replace 
...
* graph Philippines ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Philippines, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(861 "JOHN" 11 "DELP" 59 "IHME" 25 "IMPE" 22 "LANL" 1 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Philippines ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Philippines 1 Process.smcl"
