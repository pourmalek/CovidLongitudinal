...
log using "log CovidLongitudinal Solomon Islands 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Solomon Islands 1 Process.do"
...
**** "do CovidLongitudinal Solomon Islands 1 Process.do"
...
** output data files: "`model' Solomon Islands.dta"
**                   "ALL MODLES Solomon Islands.dta"

** output data dictionary files: "Solomon Islands ALL MODELS update dates data dictionary.csv"
**                               "`model' Solomon Islands data dictionary.csv"
**                               "ALL MODLES Solomon Islands data dictionary.csv"

** graphs:
** "graph 02 Solomon Islands `model' C19 daily deaths all updates.pdf"
** "graph 02 Solomon Islands ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Solomon Islands absent update_date_block"
	
	qui keep if loc_grand_name == "Solomon Islands"
	...
	save "Solomon Islands `model' update dates.dta", replace
...
use "Solomon Islands IMPE update dates.dta", clear 

local list IMPE

foreach model of local list {
	
	append using "Solomon Islands `model' update dates.dta"
...
save "Solomon Islands ALL MODELS update dates.dta", replace
	
export excel using "Solomon Islands ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Solomon Islands ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Solomon Islands"

keep date DayDeaMeSmJOHNSLB
...
save "JOHN Solomon Islands.dta", replace 




* graph Solomon Islands JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSLB date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Solomon Islands, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Solomon Islands JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Solomon Islands.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Solomon Islands.dta"
...
save "IMPE Solomon Islands.dta", replace
...
merge m:1 date using "JOHN Solomon Islands.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Solomon Islands data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Solomon Islands IMPE daily deaths each update 
	...
	subtitle("Solomon Islands, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Solomon Islands IMPE daily deaths update `update'.pdf", replace	
...
* graph Solomon Islands IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Solomon Islands, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Solomon Islands IMPE C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Solomon Islands.dta", force
...
save "ALL MODLES Solomon Islands.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Solomon Islands data dictionary.csv", replace 
...
* graph Solomon Islands ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Solomon Islands, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(68 "JOHN" 1 "IMPE") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Solomon Islands ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Solomon Islands 1 Process.smcl"
