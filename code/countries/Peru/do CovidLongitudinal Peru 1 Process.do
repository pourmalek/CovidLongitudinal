...
log using "log CovidLongitudinal Peru 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Peru 1 Process.do"
...
**** "do CovidLongitudinal Peru 1 Process.do"
...
** output data files: "`model' Peru.dta"
**                   "ALL MODLES Peru.dta"

** output data dictionary files: "Peru ALL MODELS update dates data dictionary.csv"
**                               "`model' Peru data dictionary.csv"
**                               "ALL MODLES Peru data dictionary.csv"

** graphs:
** "graph 02 Peru `model' C19 daily deaths all updates.pdf"
** "graph 02 Peru ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Peru absent update_date_block"
	
	qui keep if loc_grand_name == "Peru"
	...
	save "Peru `model' update dates.dta", replace
...
use "Peru DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Peru `model' update dates.dta"
...
save "Peru ALL MODELS update dates.dta", replace
	
export excel using "Peru ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Peru ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Peru"

keep date DayDeaMeSmJOHNPER
...
save "JOHN Peru.dta", replace 




* graph Peru JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNPER date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Peru, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Peru JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Peru.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Peru.dta"
...
merge m:m date using "JOHN Peru.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Peru data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Peru DELP daily deaths each update 
	...
	subtitle("Peru, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Peru DELP daily deaths update `update'.pdf", replace	
...
* graph Peru DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Peru, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Peru DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Peru IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Peru, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Peru IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Peru.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Peru.dta"
...
merge m:1 date using "JOHN Peru.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Peru data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Peru IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Peru, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Peru IHME daily deaths update `update'.pdf", replace	
...
* graph Peru IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Peru, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Peru IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Peru.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Peru.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Peru data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Peru IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Peru, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Peru IMPE daily deaths update `update'.pdf", replace	
...
* graph Peru IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Peru, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Peru IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Peru.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Peru.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Peru data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Peru LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Peru, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Peru LANL daily deaths update `update'.pdf", replace	
...
* graph Peru LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Peru, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Peru LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Peru.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Peru.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Peru.dta"
...
preserve
    ...
    export delimited name varlab using "UCLA Peru data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Peru UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Peru, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Peru UCLA daily deaths update `update'.pdf", replace	

}
*/


* graph Peru UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Peru, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Peru UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Peru.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Peru.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Peru.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Peru data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Peru YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Peru, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Peru YYGU daily deaths update `update'.pdf", replace	
...
* graph Peru YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Peru, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Peru YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Peru.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Peru.dta", force
...
save "ALL MODLES Peru.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Peru data dictionary.csv", replace 
...
* graph Peru ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Peru, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(853 "JOHN" 1 "DELP" 24 "IHME" 4 "IMPE" 3 "LANL" 371 "UCLA" 26 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Peru ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Peru 1 Process.smcl"
