...
log using "log CovidLongitudinal Spain 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Spain 1 Process.do"
...
**** "do CovidLongitudinal Spain 1 Process.do"
...
** output data files: "`model' Spain.dta"
**                   "ALL MODLES Spain.dta"

** output data dictionary files: "Spain ALL MODELS update dates data dictionary.csv"
**                               "`model' Spain data dictionary.csv"
**                               "ALL MODLES Spain data dictionary.csv"

** graphs:
** "graph 02 Spain `model' C19 daily deaths all updates.pdf"
** "graph 02 Spain ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Spain absent update_date_block"
	
	qui keep if loc_grand_name == "Spain"
	...
	save "Spain `model' update dates.dta", replace
...
use "Spain DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Spain `model' update dates.dta"
...
save "Spain ALL MODELS update dates.dta", replace
	
export excel using "Spain ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Spain ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Spain"

keep date DayDeaMeSmJOHNESP
...
save "JOHN Spain.dta", replace 




* graph Spain JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNESP date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Spain, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Spain JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Spain.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Spain.dta"
...
merge m:m date using "JOHN Spain.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Spain data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Spain DELP daily deaths each update 
	...
	subtitle("Spain, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Spain DELP daily deaths update `update'.pdf", replace	
...
* graph Spain DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Spain, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Spain DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Spain IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Spain, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Spain IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Spain.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Spain.dta"
...
merge m:1 date using "JOHN Spain.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Spain data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Spain IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Spain, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Spain IHME daily deaths update `update'.pdf", replace	
...
* graph Spain IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Spain, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Spain IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Spain.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Spain.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Spain data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Spain IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Spain, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Spain IMPE daily deaths update `update'.pdf", replace	
...
* graph Spain IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Spain, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Spain IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Spain.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Spain.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Spain data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Spain LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Spain, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Spain LANL daily deaths update `update'.pdf", replace	
...
* graph Spain LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Spain, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Spain LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Spain.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Spain.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Spain.dta"
...
preserve
    ...
    export delimited name varlab using "UCLA Spain data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Spain UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Spain, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Spain UCLA daily deaths update `update'.pdf", replace	

}
*/


* graph Spain UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Spain, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Spain UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Spain.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Spain.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Spain.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Spain data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Spain YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Spain, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Spain YYGU daily deaths update `update'.pdf", replace	
...
* graph Spain YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Spain, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Spain YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Spain.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Spain.dta", force
...
save "ALL MODLES Spain.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Spain data dictionary.csv", replace 
...
* graph Spain ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Spain, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(863 "JOHN" 19 "DELP" 5 "IHME" 102 "IMPE" 33 "LANL" 378 "UCLA" 1 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Spain ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Spain 1 Process.smcl"
