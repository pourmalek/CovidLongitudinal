...
log using "log CovidLongitudinal Algeria 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Algeria 1 Process.do"
...
**** "do CovidLongitudinal Algeria 1 Process.do"
...
** output data files: "`model' Algeria.dta"
**                   "ALL MODLES Algeria.dta"

** output data dictionary files: "Algeria ALL MODELS update dates data dictionary.csv"
**                               "`model' Algeria data dictionary.csv"
**                               "ALL MODLES Algeria data dictionary.csv"

** graphs:
** "graph 02 Algeria `model' C19 daily deaths all updates.pdf"
** "graph 02 Algeria ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Algeria absent update_date_block"
	
	qui keep if loc_grand_name == "Algeria"
	...
	save "Algeria `model' update dates.dta", replace
...
use "Algeria DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Algeria `model' update dates.dta"
...
save "Algeria ALL MODELS update dates.dta", replace
	
export excel using "Algeria ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Algeria ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Algeria"

keep date DayDeaMeSmJOHNDZA
...
save "JOHN Algeria.dta", replace 




* graph Algeria JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNDZA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Algeria, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Algeria JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Algeria.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Algeria.dta"
...
merge m:m date using "JOHN Algeria.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Algeria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Algeria DELP daily deaths each update 
	...
	subtitle("Algeria, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Algeria DELP daily deaths update `update'.pdf", replace	
...
* graph Algeria DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Algeria, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Algeria DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Algeria IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Algeria, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Algeria IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Algeria.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Algeria.dta"
...
merge m:1 date using "JOHN Algeria.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Algeria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Algeria IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Algeria, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Algeria IHME daily deaths update `update'.pdf", replace	
...
* graph Algeria IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Algeria, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Algeria IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Algeria.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Algeria.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Algeria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Algeria IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Algeria, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Algeria IMPE daily deaths update `update'.pdf", replace	
...
* graph Algeria IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Algeria, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Algeria IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Algeria.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Algeria.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Algeria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Algeria LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Algeria, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Algeria LANL daily deaths update `update'.pdf", replace	
...
* graph Algeria LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Algeria, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Algeria LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Algeria.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Algeria.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Algeria.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Algeria data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Algeria YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Algeria, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Algeria YYGU daily deaths update `update'.pdf", replace	
...
* graph Algeria YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Algeria, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Algeria YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Algeria.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Algeria.dta", force
...
save "ALL MODLES Algeria.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Algeria data dictionary.csv", replace 
...
* graph Algeria ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Algeria, all models, all updates, forecast only", size(medium) color(black)) ///
...
qui graph export "graph 02 Algeria ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Algeria 1 Process.smcl"
