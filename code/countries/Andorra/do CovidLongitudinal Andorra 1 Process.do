...
log using "log CovidLongitudinal Andorra 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Andorra 1 Process.do"
...
**** "do CovidLongitudinal Andorra 1 Process.do"
...
** output data files: "`model' Andorra.dta"
**                   "ALL MODLES Andorra.dta"

** output data dictionary files: "Andorra ALL MODELS update dates data dictionary.csv"
**                               "`model' Andorra data dictionary.csv"
**                               "ALL MODLES Andorra data dictionary.csv"

** graphs:
** "graph 02 Andorra `model' C19 daily deaths all updates.pdf"
** "graph 02 Andorra ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Andorra absent update_date_block"
	
	qui keep if loc_grand_name == "Andorra"
	...
	save "Andorra `model' update dates.dta", replace
...
use "Andorra DELP update dates.dta", clear 

local list DELP IHME LANL

foreach model of local list {
	
	append using "Andorra `model' update dates.dta"
...
save "Andorra ALL MODELS update dates.dta", replace
	
export excel using "Andorra ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Andorra ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Andorra"

keep date DayDeaMeSmJOHNAND
...
save "JOHN Andorra.dta", replace 




* graph Andorra JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNAND date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Andorra, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Andorra JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Andorra.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Andorra.dta"
...
merge m:m date using "JOHN Andorra.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Andorra data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Andorra DELP daily deaths each update 
	...
	subtitle("Andorra, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Andorra DELP daily deaths update `update'.pdf", replace	
...
* graph Andorra DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Andorra, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Andorra DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Andorra IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Andorra, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Andorra IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Andorra.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Andorra.dta"
...
merge m:1 date using "JOHN Andorra.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Andorra data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Andorra IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Andorra, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Andorra IHME daily deaths update `update'.pdf", replace	
...
* graph Andorra IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Andorra, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Andorra IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Andorra.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Andorra.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Andorra data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Andorra LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Andorra, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Andorra LANL daily deaths update `update'.pdf", replace	
...
* graph Andorra LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Andorra, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Andorra LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Andorra.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Andorra.dta", force
...
save "ALL MODLES Andorra.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Andorra data dictionary.csv", replace 
...
* graph Andorra ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Andorra, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(521 "JOHN" 1 "DELP" 28 "IHME" 3 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Andorra ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Andorra 1 Process.smcl"
