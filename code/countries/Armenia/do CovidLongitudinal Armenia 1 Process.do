...
log using "log CovidLongitudinal Armenia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Armenia 1 Process.do"
...
**** "do CovidLongitudinal Armenia 1 Process.do"
...
** output data files: "`model' Armenia.dta"
**                   "ALL MODLES Armenia.dta"

** output data dictionary files: "Armenia ALL MODELS update dates data dictionary.csv"
**                               "`model' Armenia data dictionary.csv"
**                               "ALL MODLES Armenia data dictionary.csv"

** graphs:
** "graph 02 Armenia `model' C19 daily deaths all updates.pdf"
** "graph 02 Armenia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Armenia absent update_date_block"
	
	qui keep if loc_grand_name == "Armenia"
	...
	save "Armenia `model' update dates.dta", replace
...
use "Armenia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Armenia `model' update dates.dta"
...
save "Armenia ALL MODELS update dates.dta", replace
	
export excel using "Armenia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Armenia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Armenia"

keep date DayDeaMeSmJOHNARM
...
save "JOHN Armenia.dta", replace 




* graph Armenia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNARM date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Armenia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Armenia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Armenia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Armenia.dta"
...
merge m:m date using "JOHN Armenia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Armenia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Armenia DELP daily deaths each update 
	...
	subtitle("Armenia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Armenia DELP daily deaths update `update'.pdf", replace	
...
* graph Armenia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Armenia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Armenia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Armenia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Armenia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Armenia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Armenia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Armenia.dta"
...
merge m:1 date using "JOHN Armenia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Armenia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Armenia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Armenia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Armenia IHME daily deaths update `update'.pdf", replace	
...
* graph Armenia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Armenia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Armenia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Armenia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Armenia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Armenia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Armenia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Armenia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Armenia IMPE daily deaths update `update'.pdf", replace	
...
* graph Armenia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Armenia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Armenia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Armenia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Armenia.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Armenia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Armenia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Armenia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Armenia LANL daily deaths update `update'.pdf", replace	
...
* graph Armenia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Armenia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Armenia LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Armenia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Armenia.dta", force
...
save "ALL MODLES Armenia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Armenia data dictionary.csv", replace 
...
* graph Armenia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Armenia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(668 "JOHN" 1 "DELP" 71 "IHME" 4 "IMPE" 3 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Armenia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Armenia 1 Process.smcl"
