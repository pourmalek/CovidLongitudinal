...
log using "log CovidLongitudinal Kosovo 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Kosovo 1 Process.do"
...
**** "do CovidLongitudinal Kosovo 1 Process.do"
...
** output data files: "`model' Kosovo.dta"
**                   "ALL MODLES Kosovo.dta"

** output data dictionary files: "Kosovo ALL MODELS update dates data dictionary.csv"
**                               "`model' Kosovo data dictionary.csv"
**                               "ALL MODLES Kosovo data dictionary.csv"

** graphs:
** "graph 02 Kosovo `model' C19 daily deaths all updates.pdf"
** "graph 02 Kosovo ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Kosovo absent update_date_block"
	
	qui keep if loc_grand_name == "Kosovo"
	...
	save "Kosovo `model' update dates.dta", replace
...
use "Kosovo DELP update dates.dta", clear 

local list DELP LANL

foreach model of local list {
	
	append using "Kosovo `model' update dates.dta"
...
save "Kosovo ALL MODELS update dates.dta", replace
	
export excel using "Kosovo ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Kosovo ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Kosovo"

keep date DayDeaMeSmJOHNKOS
...
save "JOHN Kosovo.dta", replace 




* graph Kosovo JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNKOS date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Kosovo, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kosovo JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Kosovo.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Kosovo.dta"
...
merge m:m date using "JOHN Kosovo.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Kosovo data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kosovo DELP daily deaths each update 
	...
	subtitle("Kosovo, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kosovo DELP daily deaths update `update'.pdf", replace	
...
* graph Kosovo DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kosovo, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kosovo DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Kosovo.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Kosovo.dta"
...
merge m:1 date using "JOHN Kosovo.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Kosovo data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kosovo LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Kosovo, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kosovo LANL daily deaths update `update'.pdf", replace	
...
* graph Kosovo LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kosovo, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kosovo LANL C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Kosovo.dta", force
...
save "ALL MODLES Kosovo.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Kosovo data dictionary.csv", replace 
...
* graph Kosovo ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kosovo, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(442 "JOHN" 1 "DELP" 4 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kosovo ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Kosovo 1 Process.smcl"
