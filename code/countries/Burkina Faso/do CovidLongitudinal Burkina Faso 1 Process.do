...
log using "log CovidLongitudinal Burkina Faso 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Burkina Faso 1 Process.do"
...
**** "do CovidLongitudinal Burkina Faso 1 Process.do"
...
** output data files: "`model' Burkina Faso.dta"
**                   "ALL MODLES Burkina Faso.dta"

** output data dictionary files: "Burkina Faso ALL MODELS update dates data dictionary.csv"
**                               "`model' Burkina Faso data dictionary.csv"
**                               "ALL MODLES Burkina Faso data dictionary.csv"

** graphs:
** "graph 02 Burkina Faso `model' C19 daily deaths all updates.pdf"
** "graph 02 Burkina Faso ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Burkina Faso absent update_date_block"
	
	qui keep if loc_grand_name == "Burkina Faso"
	...
	save "Burkina Faso `model' update dates.dta", replace
...
use "Burkina Faso DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Burkina Faso `model' update dates.dta"
...
save "Burkina Faso ALL MODELS update dates.dta", replace
	
export excel using "Burkina Faso ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Burkina Faso ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Burkina Faso"

keep date DayDeaMeSmJOHNBFA
...
save "JOHN Burkina Faso.dta", replace 




* graph Burkina Faso JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNBFA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Burkina Faso, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Burkina Faso JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Burkina Faso.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Burkina Faso.dta"
...
save "DELP Burkina Faso.dta", replace
...
merge m:m date using "JOHN Burkina Faso.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Burkina Faso data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Burkina Faso DELP daily deaths each update 
	...
	subtitle("Burkina Faso, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Burkina Faso DELP daily deaths update `update'.pdf", replace	
...
* graph Burkina Faso DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Burkina Faso, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Burkina Faso DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Burkina Faso IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Burkina Faso, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Burkina Faso IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Burkina Faso.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Burkina Faso.dta"
...
save "IHME Burkina Faso.dta", replace
...
merge m:1 date using "JOHN Burkina Faso.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Burkina Faso data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Burkina Faso IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Burkina Faso, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Burkina Faso IHME daily deaths update `update'.pdf", replace	
...
* graph Burkina Faso IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Burkina Faso, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Burkina Faso IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Burkina Faso.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Burkina Faso.dta"
...
save "IMPE Burkina Faso.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Burkina Faso data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Burkina Faso IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Burkina Faso, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Burkina Faso IMPE daily deaths update `update'.pdf", replace	
...
* graph Burkina Faso IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Burkina Faso, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Burkina Faso IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Burkina Faso.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Burkina Faso.dta"
...
save "LANL Burkina Faso.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Burkina Faso data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Burkina Faso LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Burkina Faso, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Burkina Faso LANL daily deaths update `update'.pdf", replace	
...
* graph Burkina Faso LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Burkina Faso, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Burkina Faso LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Burkina Faso.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Burkina Faso.dta", force
...
save "ALL MODLES Burkina Faso.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Burkina Faso data dictionary.csv", replace 
...
* graph Burkina Faso ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Burkina Faso, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(669 "JOHN" 1 "DELP" 73 "IHME" 4 "IMPE" 3 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Burkina Faso ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Burkina Faso 1 Process.smcl"
