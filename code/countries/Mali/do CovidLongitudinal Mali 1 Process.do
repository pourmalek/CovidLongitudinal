...
log using "log CovidLongitudinal Mali 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Mali 1 Process.do"
...
**** "do CovidLongitudinal Mali 1 Process.do"
...
** output data files: "`model' Mali.dta"
**                   "ALL MODLES Mali.dta"

** output data dictionary files: "Mali ALL MODELS update dates data dictionary.csv"
**                               "`model' Mali data dictionary.csv"
**                               "ALL MODLES Mali data dictionary.csv"

** graphs:
** "graph 02 Mali `model' C19 daily deaths all updates.pdf"
** "graph 02 Mali ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Mali absent update_date_block"
	
	qui keep if loc_grand_name == "Mali"
	...
	save "Mali `model' update dates.dta", replace
...
use "Mali DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Mali `model' update dates.dta"
...
save "Mali ALL MODELS update dates.dta", replace
	
export excel using "Mali ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Mali ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Mali"

keep date DayDeaMeSmJOHNMLI
...
save "JOHN Mali.dta", replace 




* graph Mali JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMLI date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Mali, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mali JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Mali.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Mali.dta"
...
merge m:m date using "JOHN Mali.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Mali data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mali DELP daily deaths each update 
	...
	subtitle("Mali, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mali DELP daily deaths update `update'.pdf", replace	
...
* graph Mali DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mali, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mali DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Mali IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Mali, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Mali IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Mali.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Mali.dta"
...
merge m:1 date using "JOHN Mali.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Mali data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mali IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mali, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mali IHME daily deaths update `update'.pdf", replace	
...
* graph Mali IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mali, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mali IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Mali.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Mali.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Mali data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mali IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mali, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mali IMPE daily deaths update `update'.pdf", replace	
...
* graph Mali IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mali, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mali IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Mali.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Mali.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Mali data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mali LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mali, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mali LANL daily deaths update `update'.pdf", replace	
...
* graph Mali LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mali, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mali LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Mali.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Mali.dta", force
...
save "ALL MODLES Mali.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Mali data dictionary.csv", replace 
...
* graph Mali ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mali, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(668 "JOHN" 1 "DELP" 72 "IHME" 3 "IMPE" 2 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mali ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Mali 1 Process.smcl"
