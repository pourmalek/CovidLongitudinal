...
log using "log CovidLongitudinal Singapore 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Singapore 1 Process.do"
...
**** "do CovidLongitudinal Singapore 1 Process.do"
...
** output data files: "`model' Singapore.dta"
**                   "ALL MODLES Singapore.dta"

** output data dictionary files: "Singapore ALL MODELS update dates data dictionary.csv"
**                               "`model' Singapore data dictionary.csv"
**                               "ALL MODLES Singapore data dictionary.csv"

** graphs:
** "graph 02 Singapore `model' C19 daily deaths all updates.pdf"
** "graph 02 Singapore ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Singapore absent update_date_block"
	
	qui keep if loc_grand_name == "Singapore"
	...
	save "Singapore `model' update dates.dta", replace
...
use "Singapore DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Singapore `model' update dates.dta"
...
save "Singapore ALL MODELS update dates.dta", replace
	
export excel using "Singapore ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Singapore ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Singapore"

keep date DayDeaMeSmJOHNSGP
...
save "JOHN Singapore.dta", replace 




* graph Singapore JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSGP date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Singapore, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Singapore JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Singapore.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Singapore.dta"
...
save "DELP Singapore.dta", replace
...
merge m:m date using "JOHN Singapore.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Singapore data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Singapore DELP daily deaths each update 
	...
	subtitle("Singapore, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Singapore DELP daily deaths update `update'.pdf", replace	
...
* graph Singapore DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Singapore, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Singapore DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Singapore IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Singapore, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Singapore IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Singapore.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Singapore.dta"
...
save "IHME Singapore.dta", replace
...
merge m:1 date using "JOHN Singapore.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Singapore data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Singapore IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Singapore, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Singapore IHME daily deaths update `update'.pdf", replace	
...
* graph Singapore IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Singapore, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Singapore IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Singapore.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Singapore.dta"
...
save "IMPE Singapore.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Singapore data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Singapore IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Singapore, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Singapore IMPE daily deaths update `update'.pdf", replace	
...
* graph Singapore IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Singapore, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Singapore IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Singapore.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Singapore.dta"
...
save "LANL Singapore.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Singapore data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Singapore LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Singapore, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Singapore LANL daily deaths update `update'.pdf", replace	
...
* graph Singapore LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Singapore, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Singapore LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Singapore.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Singapore.dta", force
...
save "ALL MODLES Singapore.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Singapore data dictionary.csv", replace 
...
* graph Singapore ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Singapore, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(628 "JOHN" 1 "DELP" 33 "IHME" 16 "IMPE" 4 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Singapore ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Singapore 1 Process.smcl"
