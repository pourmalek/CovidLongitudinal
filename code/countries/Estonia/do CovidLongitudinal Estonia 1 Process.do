...
log using "log CovidLongitudinal Estonia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Estonia 1 Process.do"
...
**** "do CovidLongitudinal Estonia 1 Process.do"
...
** output data files: "`model' Estonia.dta"
**                   "ALL MODLES Estonia.dta"

** output data dictionary files: "Estonia ALL MODELS update dates data dictionary.csv"
**                               "`model' Estonia data dictionary.csv"
**                               "ALL MODLES Estonia data dictionary.csv"

** graphs:
** "graph 02 Estonia `model' C19 daily deaths all updates.pdf"
** "graph 02 Estonia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Estonia absent update_date_block"
	
	qui keep if loc_grand_name == "Estonia"
	...
	save "Estonia `model' update dates.dta", replace
...
use "Estonia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Estonia `model' update dates.dta"
...
save "Estonia ALL MODELS update dates.dta", replace
	
export excel using "Estonia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Estonia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Estonia"

keep date DayDeaMeSmJOHNEST
...
save "JOHN Estonia.dta", replace 




* graph Estonia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNEST date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Estonia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Estonia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Estonia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Estonia.dta"
...
merge m:m date using "JOHN Estonia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Estonia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Estonia DELP daily deaths each update 
	...
	subtitle("Estonia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Estonia DELP daily deaths update `update'.pdf", replace	
...
* graph Estonia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Estonia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Estonia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Estonia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Estonia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Estonia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Estonia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Estonia.dta"
...
merge m:1 date using "JOHN Estonia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Estonia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Estonia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Estonia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Estonia IHME daily deaths update `update'.pdf", replace	
...
* graph Estonia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Estonia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Estonia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Estonia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Estonia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Estonia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Estonia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Estonia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Estonia IMPE daily deaths update `update'.pdf", replace	
...
* graph Estonia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Estonia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Estonia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Estonia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Estonia.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Estonia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Estonia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Estonia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Estonia LANL daily deaths update `update'.pdf", replace	
...
* graph Estonia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Estonia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Estonia LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Estonia.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Estonia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Estonia.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Estonia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Estonia YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Estonia, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Estonia YYGU daily deaths update `update'.pdf", replace	
...
* graph Estonia YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Estonia, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Estonia YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Estonia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Estonia.dta", force
...
save "ALL MODLES Estonia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Estonia data dictionary.csv", replace 
...
* graph Estonia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Estonia, all models, all updates, forecast only", size(medium) color(black)) ///
...
qui graph export "graph 02 Estonia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Estonia 1 Process.smcl"
