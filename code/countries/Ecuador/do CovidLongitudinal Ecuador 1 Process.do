...
log using "log CovidLongitudinal Ecuador 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Ecuador 1 Process.do"
...
**** "do CovidLongitudinal Ecuador 1 Process.do"
...
** output data files: "`model' Ecuador.dta"
**                   "ALL MODLES Ecuador.dta"

** output data dictionary files: "Ecuador ALL MODELS update dates data dictionary.csv"
**                               "`model' Ecuador data dictionary.csv"
**                               "ALL MODLES Ecuador data dictionary.csv"

** graphs:
** "graph 02 Ecuador `model' C19 daily deaths all updates.pdf"
** "graph 02 Ecuador ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Ecuador absent update_date_block"
	
	qui keep if loc_grand_name == "Ecuador"
	...
	save "Ecuador `model' update dates.dta", replace
...
use "Ecuador DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Ecuador `model' update dates.dta"
...
save "Ecuador ALL MODELS update dates.dta", replace
	
export excel using "Ecuador ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Ecuador ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Ecuador"

keep date DayDeaMeSmJOHNECU
...
save "JOHN Ecuador.dta", replace 




* graph Ecuador JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNECU date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Ecuador, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ecuador JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Ecuador.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Ecuador.dta"
...
merge m:m date using "JOHN Ecuador.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Ecuador data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ecuador DELP daily deaths each update 
	...
	subtitle("Ecuador, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ecuador DELP daily deaths update `update'.pdf", replace	
...
* graph Ecuador DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ecuador, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ecuador DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Ecuador IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Ecuador, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Ecuador IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Ecuador.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Ecuador.dta"
...
merge m:1 date using "JOHN Ecuador.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Ecuador data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ecuador IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ecuador, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ecuador IHME daily deaths update `update'.pdf", replace	
...
* graph Ecuador IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ecuador, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ecuador IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Ecuador.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Ecuador.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Ecuador data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ecuador IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ecuador, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ecuador IMPE daily deaths update `update'.pdf", replace	
...
* graph Ecuador IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ecuador, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ecuador IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Ecuador.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Ecuador.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Ecuador data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ecuador LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ecuador, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ecuador LANL daily deaths update `update'.pdf", replace	
...
* graph Ecuador LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ecuador, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ecuador LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Ecuador.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Ecuador.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Ecuador.dta"
...
preserve
    ...
    export delimited name varlab using "UCLA Ecuador data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ecuador UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ecuador, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ecuador UCLA daily deaths update `update'.pdf", replace	

}
*/


* graph Ecuador UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ecuador, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ecuador UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Ecuador.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Ecuador.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Ecuador.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Ecuador data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ecuador YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ecuador, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ecuador YYGU daily deaths update `update'.pdf", replace	
...
* graph Ecuador YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ecuador, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ecuador YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Ecuador.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Ecuador.dta", force
...
save "ALL MODLES Ecuador.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Ecuador data dictionary.csv", replace 
...
* graph Ecuador ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ecuador, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(854 "JOHN" 1 "DELP" 23 "IHME" 3 "IMPE" 2 "LANL" 370 "UCLA" 25 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ecuador ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Ecuador 1 Process.smcl"
