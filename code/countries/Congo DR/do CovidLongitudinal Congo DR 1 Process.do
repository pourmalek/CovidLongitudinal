...
log using "log CovidLongitudinal Congo DR 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Congo DR 1 Process.do"
...
**** "do CovidLongitudinal Congo DR 1 Process.do"
...
** output data files: "`model' Congo DR.dta"
**                   "ALL MODLES Congo DR.dta"

** output data dictionary files: "Congo DR ALL MODELS update dates data dictionary.csv"
**                               "`model' Congo DR data dictionary.csv"
**                               "ALL MODLES Congo DR data dictionary.csv"

** graphs:
** "graph 02 Congo DR `model' C19 daily deaths all updates.pdf"
** "graph 02 Congo DR ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Congo DR absent update_date_block"
	
	qui keep if loc_grand_name == "Congo DR"
	...
	save "Congo DR `model' update dates.dta", replace
...
use "Congo DR DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Congo DR `model' update dates.dta"
...
save "Congo DR ALL MODELS update dates.dta", replace
	
export excel using "Congo DR ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Congo DR ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Congo DR"

keep date DayDeaMeSmJOHNCOD
...
save "JOHN Congo DR.dta", replace 




* graph Congo DR JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNCOD date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Congo DR, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Congo DR JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Congo DR.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Congo DR.dta"
...
save "DELP Congo DR.dta", replace
...
merge m:m date using "JOHN Congo DR.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Congo DR data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Congo DR DELP daily deaths each update 
	...
	subtitle("Congo DR, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Congo DR DELP daily deaths update `update'.pdf", replace	
...
* graph Congo DR DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Congo DR, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Congo DR DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Congo DR IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Congo DR, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Congo DR IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Congo DR.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Congo DR.dta"
...
save "IHME Congo DR.dta", replace
...
merge m:1 date using "JOHN Congo DR.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Congo DR data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Congo DR IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Congo DR, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Congo DR IHME daily deaths update `update'.pdf", replace	
...
* graph Congo DR IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Congo DR, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Congo DR IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Congo DR.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Congo DR.dta"
...
save "IMPE Congo DR.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Congo DR data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Congo DR IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Congo DR, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Congo DR IMPE daily deaths update `update'.pdf", replace	
...
* graph Congo DR IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Congo DR, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Congo DR IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Congo DR.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Congo DR.dta"
...
save "LANL Congo DR.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Congo DR data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Congo DR LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Congo DR, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Congo DR LANL daily deaths update `update'.pdf", replace	
...
* graph Congo DR LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Congo DR, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Congo DR LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Congo DR.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Congo DR.dta", force
...
save "ALL MODLES Congo DR.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Congo DR data dictionary.csv", replace 
...
* graph Congo DR ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Congo DR, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(669 "JOHN" 1 "DELP" 72 "IHME" 3 "IMPE" 2 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Congo DR ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Congo DR 1 Process.smcl"
