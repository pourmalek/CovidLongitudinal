...
log using "log CovidLongitudinal Kenya 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Kenya 1 Process.do"
...
**** "do CovidLongitudinal Kenya 1 Process.do"
...
** output data files: "`model' Kenya.dta"
**                   "ALL MODLES Kenya.dta"

** output data dictionary files: "Kenya ALL MODELS update dates data dictionary.csv"
**                               "`model' Kenya data dictionary.csv"
**                               "ALL MODLES Kenya data dictionary.csv"

** graphs:
** "graph 02 Kenya `model' C19 daily deaths all updates.pdf"
** "graph 02 Kenya ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Kenya absent update_date_block"
	
	qui keep if loc_grand_name == "Kenya"
	...
	save "Kenya `model' update dates.dta", replace
...
use "Kenya DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Kenya `model' update dates.dta"
...
save "Kenya ALL MODELS update dates.dta", replace
	
export excel using "Kenya ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Kenya ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Kenya"

keep date DayDeaMeSmJOHNKEN
...
save "JOHN Kenya.dta", replace 




* graph Kenya JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNKEN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Kenya, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kenya JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Kenya.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Kenya.dta"
...
merge m:m date using "JOHN Kenya.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Kenya data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kenya DELP daily deaths each update 
	...
	subtitle("Kenya, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kenya DELP daily deaths update `update'.pdf", replace	
...
* graph Kenya DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kenya, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kenya DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Kenya IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Kenya, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Kenya IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Kenya.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Kenya.dta"
...
merge m:1 date using "JOHN Kenya.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Kenya data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kenya IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Kenya, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kenya IHME daily deaths update `update'.pdf", replace	
...
* graph Kenya IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kenya, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kenya IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Kenya.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Kenya.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Kenya data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kenya IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Kenya, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kenya IMPE daily deaths update `update'.pdf", replace	
...
* graph Kenya IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kenya, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kenya IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Kenya.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Kenya.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Kenya data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kenya LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Kenya, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kenya LANL daily deaths update `update'.pdf", replace	
...
* graph Kenya LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kenya, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kenya LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Kenya.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Kenya.dta", force
...
save "ALL MODLES Kenya.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Kenya data dictionary.csv", replace 
...
* graph Kenya ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kenya, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(664 "JOHN" 1 "DELP" 70 "IHME" 3 "IMPE" 13 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kenya ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Kenya 1 Process.smcl"
