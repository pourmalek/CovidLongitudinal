...
log using "log CovidLongitudinal Denmark 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Denmark 1 Process.do"
...
**** "do CovidLongitudinal Denmark 1 Process.do"
...
** output data files: "`model' Denmark.dta"
**                   "ALL MODLES Denmark.dta"

** output data dictionary files: "Denmark ALL MODELS update dates data dictionary.csv"
**                               "`model' Denmark data dictionary.csv"
**                               "ALL MODLES Denmark data dictionary.csv"

** graphs:
** "graph 02 Denmark `model' C19 daily deaths all updates.pdf"
** "graph 02 Denmark ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Denmark absent update_date_block"
	
	qui keep if loc_grand_name == "Denmark"
	...
	save "Denmark `model' update dates.dta", replace
...
use "Denmark DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Denmark `model' update dates.dta"
...
save "Denmark ALL MODELS update dates.dta", replace
	
export excel using "Denmark ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Denmark ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Denmark"

keep date DayDeaMeSmJOHNDNK
...
save "JOHN Denmark.dta", replace 




* graph Denmark JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNDNK date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Denmark, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Denmark JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Denmark.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Denmark.dta"
...
merge m:m date using "JOHN Denmark.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Denmark data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Denmark DELP daily deaths each update 
	...
	subtitle("Denmark, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Denmark DELP daily deaths update `update'.pdf", replace	
...
* graph Denmark DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Denmark, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Denmark DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Denmark IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Denmark, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Denmark IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Denmark.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Denmark.dta"
...
merge m:1 date using "JOHN Denmark.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Denmark data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Denmark IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Denmark, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Denmark IHME daily deaths update `update'.pdf", replace	
...
* graph Denmark IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Denmark, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Denmark IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Denmark.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Denmark.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Denmark data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Denmark IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Denmark, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Denmark IMPE daily deaths update `update'.pdf", replace	
...
* graph Denmark IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Denmark, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Denmark IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Denmark.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Denmark.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Denmark data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Denmark LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Denmark, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Denmark LANL daily deaths update `update'.pdf", replace	
...
* graph Denmark LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Denmark, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Denmark LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Denmark.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Denmark.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Denmark.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Denmark data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Denmark YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Denmark, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Denmark YYGU daily deaths update `update'.pdf", replace	
...
* graph Denmark YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Denmark, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Denmark YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Denmark.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Denmark.dta", force
...
save "ALL MODLES Denmark.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Denmark data dictionary.csv", replace 
...
* graph Denmark ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Denmark, all models, all updates, forecast only", size(medium) color(black)) ///
...
qui graph export "graph 02 Denmark ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Denmark 1 Process.smcl"
