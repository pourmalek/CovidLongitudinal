...
log using "log CovidLongitudinal China 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal China 1 Process.do"
...
**** "do CovidLongitudinal China 1 Process.do"
...
** output data files: "`model' China.dta"
**                   "ALL MODLES China.dta"

** output data dictionary files: "China ALL MODELS update dates data dictionary.csv"
**                               "`model' China data dictionary.csv"
**                               "ALL MODLES China data dictionary.csv"

** graphs:
** "graph 02 China `model' C19 daily deaths all updates.pdf"
** "graph 02 China ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " China absent update_date_block"
	
	qui keep if loc_grand_name == "China"
	...
	save "China `model' update dates.dta", replace
...
use "China IHME update dates.dta", clear 

local list IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "China `model' update dates.dta"
...
save "China ALL MODELS update dates.dta", replace
	
export excel using "China ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "China ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "China"

keep date DayDeaMeSmJOHNCHN
...
save "JOHN China.dta", replace 




* graph China JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNCHN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, China, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 China JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* graph China IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, China, IHME, update `update'", size(medium) color(black)) ///
	...
	/* qui graph export "graph 00 China IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' China.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' China.dta"
...
merge m:1 date using "JOHN China.dta"
...
preserve
    ...
    export delimited name varlab using "IHME China data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph China IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("China, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 China IHME daily deaths update `update'.pdf", replace	
...
* graph China IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, China, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 China IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' China.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' China.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE China data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph China IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("China, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 China IMPE daily deaths update `update'.pdf", replace	
...
* graph China IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, China, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 China IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' China.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' China.dta"
...
preserve
    ...
    export delimited name varlab using "LANL China data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph China LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("China, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 China LANL daily deaths update `update'.pdf", replace	
...
* graph China LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, China, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 China LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' China.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' China.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' China.dta"
...
merge m:m date using "JOHN China.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU China data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph China YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("China, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 China YYGU daily deaths update `update'.pdf", replace	
...
* graph China YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, China, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 China YYGU C19 daily deaths all updates.pdf", replace
...
use "IHME China.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' China.dta", force
...
save "ALL MODLES China.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES China data dictionary.csv", replace 
...
* graph China ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, China, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(404 "JOHN" 149 "IHME" 204 "IMPE" 1 "LANL" 6 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 China ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal China 1 Process.smcl"
