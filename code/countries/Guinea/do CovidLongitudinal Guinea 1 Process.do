...
log using "log CovidLongitudinal Guinea 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Guinea 1 Process.do"
...
**** "do CovidLongitudinal Guinea 1 Process.do"
...
** output data files: "`model' Guinea.dta"
**                   "ALL MODLES Guinea.dta"

** output data dictionary files: "Guinea ALL MODELS update dates data dictionary.csv"
**                               "`model' Guinea data dictionary.csv"
**                               "ALL MODLES Guinea data dictionary.csv"

** graphs:
** "graph 02 Guinea `model' C19 daily deaths all updates.pdf"
** "graph 02 Guinea ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Guinea absent update_date_block"
	
	qui keep if loc_grand_name == "Guinea"
	...
	save "Guinea `model' update dates.dta", replace
...
use "Guinea DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Guinea `model' update dates.dta"
...
save "Guinea ALL MODELS update dates.dta", replace
	
export excel using "Guinea ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Guinea ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Guinea"

keep date DayDeaMeSmJOHNGIN
...
save "JOHN Guinea.dta", replace 




* graph Guinea JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNGIN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Guinea, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guinea JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Guinea.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Guinea.dta"
...
merge m:m date using "JOHN Guinea.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Guinea data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guinea DELP daily deaths each update 
	...
	subtitle("Guinea, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guinea DELP daily deaths update `update'.pdf", replace	
...
* graph Guinea DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guinea, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guinea DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Guinea IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Guinea, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Guinea IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Guinea.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Guinea.dta"
...
merge m:1 date using "JOHN Guinea.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Guinea data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guinea IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Guinea, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guinea IHME daily deaths update `update'.pdf", replace	
...
* graph Guinea IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guinea, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guinea IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Guinea.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Guinea.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Guinea data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guinea IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Guinea, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guinea IMPE daily deaths update `update'.pdf", replace	
...
* graph Guinea IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guinea, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guinea IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Guinea.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Guinea.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Guinea data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Guinea LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Guinea, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Guinea LANL daily deaths update `update'.pdf", replace	
...
* graph Guinea LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guinea, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guinea LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Guinea.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Guinea.dta", force
...
save "ALL MODLES Guinea.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Guinea data dictionary.csv", replace 
...
* graph Guinea ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Guinea, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(661 "JOHN" 1 "DELP" 64 "IHME" 3 "IMPE" 32 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Guinea ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Guinea 1 Process.smcl"
