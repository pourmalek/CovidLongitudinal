...
log using "log CovidLongitudinal Poland 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Poland 1 Process.do"
...
**** "do CovidLongitudinal Poland 1 Process.do"
...
** output data files: "`model' Poland.dta"
**                   "ALL MODLES Poland.dta"

** output data dictionary files: "Poland ALL MODELS update dates data dictionary.csv"
**                               "`model' Poland data dictionary.csv"
**                               "ALL MODLES Poland data dictionary.csv"

** graphs:
** "graph 02 Poland `model' C19 daily deaths all updates.pdf"
** "graph 02 Poland ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Poland absent update_date_block"
	
	qui keep if loc_grand_name == "Poland"
	...
	save "Poland `model' update dates.dta", replace
...
use "Poland DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Poland `model' update dates.dta"
...
save "Poland ALL MODELS update dates.dta", replace
	
export excel using "Poland ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Poland ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Poland"

keep date DayDeaMeSmJOHNPOL
...
save "JOHN Poland.dta", replace 




* graph Poland JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNPOL date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Poland, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Poland JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Poland.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Poland.dta"
...
merge m:m date using "JOHN Poland.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Poland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Poland DELP daily deaths each update 
	...
	subtitle("Poland, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Poland DELP daily deaths update `update'.pdf", replace	
...
* graph Poland DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Poland, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Poland DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Poland IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Poland, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Poland IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Poland.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Poland.dta"
...
merge m:1 date using "JOHN Poland.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Poland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Poland IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Poland, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Poland IHME daily deaths update `update'.pdf", replace	
...
* graph Poland IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Poland, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Poland IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Poland.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Poland.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Poland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Poland IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Poland, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Poland IMPE daily deaths update `update'.pdf", replace	
...
* graph Poland IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Poland, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Poland IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Poland.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Poland.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Poland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Poland LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Poland, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Poland LANL daily deaths update `update'.pdf", replace	
...
* graph Poland LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Poland, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Poland LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Poland.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Poland.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Poland.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Poland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Poland YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Poland, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Poland YYGU daily deaths update `update'.pdf", replace	
...
* graph Poland YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Poland, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Poland YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Poland.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Poland.dta", force
...
save "ALL MODLES Poland.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Poland data dictionary.csv", replace 
...
* graph Poland ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Poland, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(811 "JOHN" 11 "DELP" 115 "IHME" 74 "IMPE" 22 "LANL" 1 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Poland ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Poland 1 Process.smcl"
