...
log using "log CovidLongitudinal Honduras 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Honduras 1 Process.do"
...
**** "do CovidLongitudinal Honduras 1 Process.do"
...
** output data files: "`model' Honduras.dta"
**                   "ALL MODLES Honduras.dta"

** output data dictionary files: "Honduras ALL MODELS update dates data dictionary.csv"
**                               "`model' Honduras data dictionary.csv"
**                               "ALL MODLES Honduras data dictionary.csv"

** graphs:
** "graph 02 Honduras `model' C19 daily deaths all updates.pdf"
** "graph 02 Honduras ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Honduras absent update_date_block"
	
	qui keep if loc_grand_name == "Honduras"
	...
	save "Honduras `model' update dates.dta", replace
...
use "Honduras DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Honduras `model' update dates.dta"
...
save "Honduras ALL MODELS update dates.dta", replace
	
export excel using "Honduras ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Honduras ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Honduras"

keep date DayDeaMeSmJOHNHND
...
save "JOHN Honduras.dta", replace 




* graph Honduras JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNHND date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Honduras, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Honduras JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Honduras.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Honduras.dta"
...
save "DELP Honduras.dta", replace
...
merge m:m date using "JOHN Honduras.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Honduras data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Honduras DELP daily deaths each update 
	...
	subtitle("Honduras, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Honduras DELP daily deaths update `update'.pdf", replace	
...
* graph Honduras DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Honduras, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Honduras DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Honduras IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Honduras, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Honduras IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Honduras.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Honduras.dta"
...
save "IHME Honduras.dta", replace
...
merge m:1 date using "JOHN Honduras.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Honduras data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Honduras IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Honduras, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Honduras IHME daily deaths update `update'.pdf", replace	
...
* graph Honduras IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Honduras, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Honduras IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Honduras.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Honduras.dta"
...
save "IMPE Honduras.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Honduras data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Honduras IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Honduras, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Honduras IMPE daily deaths update `update'.pdf", replace	
...
* graph Honduras IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Honduras, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Honduras IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Honduras.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Honduras.dta"
...
save "LANL Honduras.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Honduras data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Honduras LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Honduras, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Honduras LANL daily deaths update `update'.pdf", replace	
...
* graph Honduras LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Honduras, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Honduras LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Honduras.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Honduras.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Honduras.dta"
...
save "YYGU Honduras.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Honduras data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Honduras YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Honduras, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Honduras YYGU daily deaths update `update'.pdf", replace	
...
* graph Honduras YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Honduras, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Honduras YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Honduras.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Honduras.dta", force
...
save "ALL MODLES Honduras.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Honduras data dictionary.csv", replace 
...
* graph Honduras ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Honduras, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(810 "JOHN" 1 "DELP" 33 "IHME" 3 "IMPE" 2 "LANL" 46 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Honduras ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Honduras 1 Process.smcl"
