...
log using "log CovidLongitudinal Eswatini 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Eswatini 1 Process.do"
...
**** "do CovidLongitudinal Eswatini 1 Process.do"
...
** output data files: "`model' Eswatini.dta"
**                   "ALL MODLES Eswatini.dta"

** output data dictionary files: "Eswatini ALL MODELS update dates data dictionary.csv"
**                               "`model' Eswatini data dictionary.csv"
**                               "ALL MODLES Eswatini data dictionary.csv"

** graphs:
** "graph 02 Eswatini `model' C19 daily deaths all updates.pdf"
** "graph 02 Eswatini ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Eswatini absent update_date_block"
	
	qui keep if loc_grand_name == "Eswatini"
	...
	save "Eswatini `model' update dates.dta", replace
...
use "Eswatini IHME update dates.dta", clear 

local list IHME IMPE LANL

foreach model of local list {
	
	append using "Eswatini `model' update dates.dta"
...
save "Eswatini ALL MODELS update dates.dta", replace
	
export excel using "Eswatini ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Eswatini ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Eswatini"

keep date DayDeaMeSmJOHNSWZ
...
save "JOHN Eswatini.dta", replace 




* graph Eswatini JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSWZ date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Eswatini, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Eswatini JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* graph Eswatini IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Eswatini, IHME, update `update'", size(medium) color(black)) ///
	...
	*/
	...
	/* qui graph export "graph 00 Eswatini IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Eswatini.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Eswatini.dta"
...
save "IHME Eswatini.dta", replace
...
merge m:1 date using "JOHN Eswatini.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Eswatini data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Eswatini IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Eswatini, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Eswatini IHME daily deaths update `update'.pdf", replace	
...
* graph Eswatini IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Eswatini, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Eswatini IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Eswatini.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Eswatini.dta"
...
save "IMPE Eswatini.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Eswatini data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Eswatini IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Eswatini, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Eswatini IMPE daily deaths update `update'.pdf", replace	
...
* graph Eswatini IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Eswatini, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Eswatini IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Eswatini.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Eswatini.dta"
...
save "LANL Eswatini.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Eswatini data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Eswatini LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Eswatini, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Eswatini LANL daily deaths update `update'.pdf", replace	
...
* graph Eswatini LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Eswatini, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Eswatini LANL C19 daily deaths all updates.pdf", replace
...
use "IHME Eswatini.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Eswatini.dta", force
...
save "ALL MODLES Eswatini.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Eswatini data dictionary.csv", replace 
...
* graph Eswatini ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Eswatini, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(323 "JOHN" 45 "IHME" 1 "IMPE" 55 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Eswatini ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Eswatini 1 Process.smcl"
