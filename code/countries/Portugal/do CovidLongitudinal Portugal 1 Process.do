...
log using "log CovidLongitudinal Portugal 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Portugal 1 Process.do"
...
**** "do CovidLongitudinal Portugal 1 Process.do"
...
** output data files: "`model' Portugal.dta"
**                   "ALL MODLES Portugal.dta"

** output data dictionary files: "Portugal ALL MODELS update dates data dictionary.csv"
**                               "`model' Portugal data dictionary.csv"
**                               "ALL MODLES Portugal data dictionary.csv"

** graphs:
** "graph 02 Portugal `model' C19 daily deaths all updates.pdf"
** "graph 02 Portugal ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Portugal absent update_date_block"
	
	qui keep if loc_grand_name == "Portugal"
	...
	save "Portugal `model' update dates.dta", replace
...
use "Portugal DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Portugal `model' update dates.dta"
...
save "Portugal ALL MODELS update dates.dta", replace
	
export excel using "Portugal ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Portugal ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Portugal"

keep date DayDeaMeSmJOHNPRT
...
save "JOHN Portugal.dta", replace 




* graph Portugal JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNPRT date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Portugal, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Portugal JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Portugal.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Portugal.dta"
...
save "DELP Portugal.dta", replace
...
merge m:m date using "JOHN Portugal.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Portugal data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Portugal DELP daily deaths each update 
	...
	subtitle("Portugal, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Portugal DELP daily deaths update `update'.pdf", replace	
...
* graph Portugal DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Portugal, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Portugal DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Portugal IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Portugal, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Portugal IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Portugal.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Portugal.dta"
...
save "IHME Portugal.dta", replace
...
merge m:1 date using "JOHN Portugal.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Portugal data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Portugal IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Portugal, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Portugal IHME daily deaths update `update'.pdf", replace	
...
* graph Portugal IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Portugal, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Portugal IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Portugal.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Portugal.dta"
...
save "IMPE Portugal.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Portugal data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Portugal IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Portugal, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Portugal IMPE daily deaths update `update'.pdf", replace	
...
* graph Portugal IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Portugal, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Portugal IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Portugal.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Portugal.dta"
...
save "LANL Portugal.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Portugal data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Portugal LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Portugal, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Portugal LANL daily deaths update `update'.pdf", replace	
...
* graph Portugal LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Portugal, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Portugal LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Portugal.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Portugal.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Portugal.dta"
...
save "YYGU Portugal.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Portugal data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Portugal YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Portugal, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Portugal YYGU daily deaths update `update'.pdf", replace	
...
* graph Portugal YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Portugal, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Portugal YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Portugal.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Portugal.dta", force
...
save "ALL MODLES Portugal.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Portugal data dictionary.csv", replace 
...
* graph Portugal ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Portugal, all models, all updates, forecast only", size(medium) color(black)) ///
...
qui graph export "graph 02 Portugal ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Portugal 1 Process.smcl"
