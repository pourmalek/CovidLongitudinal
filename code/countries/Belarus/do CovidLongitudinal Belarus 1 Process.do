...
log using "log CovidLongitudinal Belarus 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Belarus 1 Process.do"
...
**** "do CovidLongitudinal Belarus 1 Process.do"
...
** output data files: "`model' Belarus.dta"
**                   "ALL MODLES Belarus.dta"

** output data dictionary files: "Belarus ALL MODELS update dates data dictionary.csv"
**                               "`model' Belarus data dictionary.csv"
**                               "ALL MODLES Belarus data dictionary.csv"

** graphs:
** "graph 02 Belarus `model' C19 daily deaths all updates.pdf"
** "graph 02 Belarus ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Belarus absent update_date_block"
	
	qui keep if loc_grand_name == "Belarus"
	...
	save "Belarus `model' update dates.dta", replace
...
use "Belarus DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Belarus `model' update dates.dta"
...
save "Belarus ALL MODELS update dates.dta", replace
	
export excel using "Belarus ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Belarus ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Belarus"

keep date DayDeaMeSmJOHNBLR
...
save "JOHN Belarus.dta", replace 




* graph Belarus JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNBLR date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Belarus, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belarus JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Belarus.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Belarus.dta"
...
merge m:m date using "JOHN Belarus.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Belarus data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Belarus DELP daily deaths each update 
	...
	subtitle("Belarus, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Belarus DELP daily deaths update `update'.pdf", replace	
...
* graph Belarus DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belarus, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belarus DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Belarus IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Belarus, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Belarus IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Belarus.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Belarus.dta"
...
merge m:1 date using "JOHN Belarus.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Belarus data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Belarus IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Belarus, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Belarus IHME daily deaths update `update'.pdf", replace	
...
* graph Belarus IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belarus, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belarus IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Belarus.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Belarus.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Belarus data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Belarus IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Belarus, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Belarus IMPE daily deaths update `update'.pdf", replace	
...
* graph Belarus IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belarus, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belarus IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Belarus.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Belarus.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Belarus data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Belarus LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Belarus, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Belarus LANL daily deaths update `update'.pdf", replace	
...
* graph Belarus LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belarus, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belarus LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Belarus.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Belarus.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Belarus.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Belarus data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Belarus YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Belarus, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Belarus YYGU daily deaths update `update'.pdf", replace	
...
* graph Belarus YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belarus, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belarus YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Belarus.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Belarus.dta", force
...
save "ALL MODLES Belarus.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Belarus data dictionary.csv", replace 
...
* graph Belarus ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belarus, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(802 "JOHN" 1 "DELP" 102 "IHME" 4 "IMPE" 3 "LANL" 43 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belarus ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Belarus 1 Process.smcl"
