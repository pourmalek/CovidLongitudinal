...
log using "log CovidLongitudinal Ukraine 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Ukraine 1 Process.do"
...
**** "do CovidLongitudinal Ukraine 1 Process.do"
...
** output data files: "`model' Ukraine.dta"
**                   "ALL MODLES Ukraine.dta"

** output data dictionary files: "Ukraine ALL MODELS update dates data dictionary.csv"
**                               "`model' Ukraine data dictionary.csv"
**                               "ALL MODLES Ukraine data dictionary.csv"

** graphs:
** "graph 02 Ukraine `model' C19 daily deaths all updates.pdf"
** "graph 02 Ukraine ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Ukraine absent update_date_block"
	
	qui keep if loc_grand_name == "Ukraine"
	...
	save "Ukraine `model' update dates.dta", replace
...
use "Ukraine DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Ukraine `model' update dates.dta"
...
save "Ukraine ALL MODELS update dates.dta", replace
	
export excel using "Ukraine ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Ukraine ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Ukraine"

keep date DayDeaMeSmJOHNUKR
...
save "JOHN Ukraine.dta", replace 




* graph Ukraine JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNUKR date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Ukraine, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ukraine JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Ukraine.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Ukraine.dta"
...
merge m:m date using "JOHN Ukraine.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Ukraine data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ukraine DELP daily deaths each update 
	...
	subtitle("Ukraine, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ukraine DELP daily deaths update `update'.pdf", replace	
...
* graph Ukraine DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ukraine, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ukraine DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Ukraine IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Ukraine, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Ukraine IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Ukraine.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Ukraine.dta"
...
merge m:1 date using "JOHN Ukraine.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Ukraine data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ukraine IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ukraine, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ukraine IHME daily deaths update `update'.pdf", replace	
...
* graph Ukraine IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ukraine, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ukraine IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Ukraine.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Ukraine.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Ukraine data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ukraine IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ukraine, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ukraine IMPE daily deaths update `update'.pdf", replace	
...
* graph Ukraine IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ukraine, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ukraine IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Ukraine.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Ukraine.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Ukraine data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ukraine LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ukraine, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ukraine LANL daily deaths update `update'.pdf", replace	
...
* graph Ukraine LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ukraine, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ukraine LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Ukraine.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Ukraine.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Ukraine.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Ukraine data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ukraine YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ukraine, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ukraine YYGU daily deaths update `update'.pdf", replace	
...
* graph Ukraine YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ukraine, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ukraine YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Ukraine.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Ukraine.dta", force
...
save "ALL MODLES Ukraine.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Ukraine data dictionary.csv", replace 
...
* graph Ukraine ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ukraine, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(822 "JOHN" 1 "DELP" 23 "IHME" 4 "IMPE" 3 "LANL" 25 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ukraine ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Ukraine 1 Process.smcl"
