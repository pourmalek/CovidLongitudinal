...
log using "log CovidLongitudinal Malawi 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Malawi 1 Process.do"
...
**** "do CovidLongitudinal Malawi 1 Process.do"
...
** output data files: "`model' Malawi.dta"
**                   "ALL MODLES Malawi.dta"

** output data dictionary files: "Malawi ALL MODELS update dates data dictionary.csv"
**                               "`model' Malawi data dictionary.csv"
**                               "ALL MODLES Malawi data dictionary.csv"

** graphs:
** "graph 02 Malawi `model' C19 daily deaths all updates.pdf"
** "graph 02 Malawi ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Malawi absent update_date_block"
	
	qui keep if loc_grand_name == "Malawi"
	...
	save "Malawi `model' update dates.dta", replace
...
use "Malawi DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Malawi `model' update dates.dta"
...
save "Malawi ALL MODELS update dates.dta", replace
	
export excel using "Malawi ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Malawi ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Malawi"

keep date DayDeaMeSmJOHNMWI
...
save "JOHN Malawi.dta", replace 




* graph Malawi JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMWI date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Malawi, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malawi JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Malawi.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Malawi.dta"
...
merge m:m date using "JOHN Malawi.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Malawi data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Malawi DELP daily deaths each update 
	...
	subtitle("Malawi, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Malawi DELP daily deaths update `update'.pdf", replace	
...
* graph Malawi DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malawi, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malawi DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Malawi IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Malawi, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Malawi IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Malawi.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Malawi.dta"
...
merge m:1 date using "JOHN Malawi.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Malawi data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Malawi IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Malawi, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Malawi IHME daily deaths update `update'.pdf", replace	
...
* graph Malawi IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malawi, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malawi IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Malawi.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Malawi.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Malawi data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Malawi IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Malawi, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Malawi IMPE daily deaths update `update'.pdf", replace	
...
* graph Malawi IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malawi, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malawi IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Malawi.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Malawi.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Malawi data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Malawi LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Malawi, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Malawi LANL daily deaths update `update'.pdf", replace	
...
* graph Malawi LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malawi, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malawi LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Malawi.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Malawi.dta", force
...
save "ALL MODLES Malawi.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Malawi data dictionary.csv", replace 
...
* graph Malawi ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Malawi, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(640 "JOHN" 39 "DELP" 48 "IHME" 1 "IMPE" 59 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Malawi ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Malawi 1 Process.smcl"
