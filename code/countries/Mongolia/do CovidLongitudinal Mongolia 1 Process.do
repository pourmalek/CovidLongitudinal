...
log using "log CovidLongitudinal Mongolia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Mongolia 1 Process.do"
...
**** "do CovidLongitudinal Mongolia 1 Process.do"
...
** output data files: "`model' Mongolia.dta"
**                   "ALL MODLES Mongolia.dta"

** output data dictionary files: "Mongolia ALL MODELS update dates data dictionary.csv"
**                               "`model' Mongolia data dictionary.csv"
**                               "ALL MODLES Mongolia data dictionary.csv"

** graphs:
** "graph 02 Mongolia `model' C19 daily deaths all updates.pdf"
** "graph 02 Mongolia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Mongolia absent update_date_block"
	
	qui keep if loc_grand_name == "Mongolia"
	...
	save "Mongolia `model' update dates.dta", replace
...
use "Mongolia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Mongolia `model' update dates.dta"
...
save "Mongolia ALL MODELS update dates.dta", replace
	
export excel using "Mongolia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Mongolia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Mongolia"

keep date DayDeaMeSmJOHNMNG
...
save "JOHN Mongolia.dta", replace 




* graph Mongolia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMNG date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Mongolia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mongolia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Mongolia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Mongolia.dta"
...
save "DELP Mongolia.dta", replace
...
merge m:m date using "JOHN Mongolia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Mongolia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mongolia DELP daily deaths each update 
	...
	subtitle("Mongolia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mongolia DELP daily deaths update `update'.pdf", replace	
...
* graph Mongolia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mongolia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mongolia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Mongolia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Mongolia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Mongolia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Mongolia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Mongolia.dta"
...
save "IHME Mongolia.dta", replace
...
merge m:1 date using "JOHN Mongolia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Mongolia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mongolia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mongolia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mongolia IHME daily deaths update `update'.pdf", replace	
...
* graph Mongolia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mongolia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mongolia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Mongolia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Mongolia.dta"
...
save "IMPE Mongolia.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Mongolia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mongolia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mongolia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mongolia IMPE daily deaths update `update'.pdf", replace	
...
* graph Mongolia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mongolia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mongolia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Mongolia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Mongolia.dta"
...
save "LANL Mongolia.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Mongolia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mongolia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mongolia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mongolia LANL daily deaths update `update'.pdf", replace	
...
* graph Mongolia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mongolia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mongolia LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Mongolia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Mongolia.dta", force
...
save "ALL MODLES Mongolia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Mongolia data dictionary.csv", replace 
...
* graph Mongolia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mongolia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(418 "JOHN" 1 "DELP" 191 "IHME" 17 "IMPE" 56 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mongolia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Mongolia 1 Process.smcl"
