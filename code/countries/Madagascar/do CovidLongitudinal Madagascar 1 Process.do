...
log using "log CovidLongitudinal Madagascar 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Madagascar 1 Process.do"
...
**** "do CovidLongitudinal Madagascar 1 Process.do"
...
** output data files: "`model' Madagascar.dta"
**                   "ALL MODLES Madagascar.dta"

** output data dictionary files: "Madagascar ALL MODELS update dates data dictionary.csv"
**                               "`model' Madagascar data dictionary.csv"
**                               "ALL MODLES Madagascar data dictionary.csv"

** graphs:
** "graph 02 Madagascar `model' C19 daily deaths all updates.pdf"
** "graph 02 Madagascar ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Madagascar absent update_date_block"
	
	qui keep if loc_grand_name == "Madagascar"
	...
	save "Madagascar `model' update dates.dta", replace
...
use "Madagascar DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Madagascar `model' update dates.dta"
...
save "Madagascar ALL MODELS update dates.dta", replace
	
export excel using "Madagascar ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Madagascar ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Madagascar"

keep date DayDeaMeSmJOHNMDG
...
save "JOHN Madagascar.dta", replace 




* graph Madagascar JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMDG date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Madagascar, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Madagascar JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Madagascar.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Madagascar.dta"
...
save "DELP Madagascar.dta", replace
...
merge m:m date using "JOHN Madagascar.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Madagascar data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Madagascar DELP daily deaths each update 
	...
	subtitle("Madagascar, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Madagascar DELP daily deaths update `update'.pdf", replace	
...
* graph Madagascar DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Madagascar, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Madagascar DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Madagascar IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Madagascar, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Madagascar IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Madagascar.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Madagascar.dta"
...
save "IHME Madagascar.dta", replace
...
merge m:1 date using "JOHN Madagascar.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Madagascar data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Madagascar IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Madagascar, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Madagascar IHME daily deaths update `update'.pdf", replace	
...
* graph Madagascar IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Madagascar, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Madagascar IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Madagascar.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Madagascar.dta"
...
save "IMPE Madagascar.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Madagascar data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Madagascar IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Madagascar, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Madagascar IMPE daily deaths update `update'.pdf", replace	
...
* graph Madagascar IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Madagascar, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Madagascar IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Madagascar.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Madagascar.dta"
...
save "LANL Madagascar.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Madagascar data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Madagascar LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Madagascar, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Madagascar LANL daily deaths update `update'.pdf", replace	
...
* graph Madagascar LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Madagascar, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Madagascar LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Madagascar.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Madagascar.dta", force
...
save "ALL MODLES Madagascar.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Madagascar data dictionary.csv", replace 
...
* graph Madagascar ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Madagascar, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(628 "JOHN" 1 "DELP" 34 "IHME" 5 "IMPE" 39 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Madagascar ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Madagascar 1 Process.smcl"
