...
log using "log CovidLongitudinal Qatar 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Qatar 1 Process.do"
...
**** "do CovidLongitudinal Qatar 1 Process.do"
...
** output data files: "`model' Qatar.dta"
**                   "ALL MODLES Qatar.dta"

** output data dictionary files: "Qatar ALL MODELS update dates data dictionary.csv"
**                               "`model' Qatar data dictionary.csv"
**                               "ALL MODLES Qatar data dictionary.csv"

** graphs:
** "graph 02 Qatar `model' C19 daily deaths all updates.pdf"
** "graph 02 Qatar ALL MODELS C19 daily deaths all updates.pdf"
...
local list DELP IHME IMPE LANL UCLA

foreach model of local list {

	use "$pathCovidLongitudinal/download/`model'/`model' countries long.dta", clear 
	
	di in red "`model'" " Qatar absent update_date_block"
	
	qui keep if loc_grand_name == "Qatar"
	...
	save "Qatar `model' update dates.dta", replace
...
foreach model of local list {
	...
	append using "Qatar `model' update dates.dta"
...
save "Qatar ALL MODELS update dates.dta", replace
	
export excel using "Qatar ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Qatar ALL MODELS update dates data dictionary.csv", replace 
...
save "JOHN Qatar.dta", replace 




* graph Qatar JOHN daily deaths
...
ytitle(Daily deaths) title("C19 daily deaths, Qatar, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Qatar JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Qatar.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Qatar.dta"
...
merge m:m date using "JOHN Qatar.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Qatar data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Qatar DELP daily deaths each update 
	...
	subtitle("Qatar, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Qatar DELP daily deaths update `update'.pdf", replace	
...
* graph Qatar DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Qatar, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Qatar DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Qatar IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Qatar, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Qatar IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Qatar.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Qatar.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Qatar data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Qatar IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Qatar, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Qatar IHME daily deaths update `update'.pdf", replace	
...
* graph Qatar IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Qatar, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Qatar IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Qatar.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Qatar.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Qatar data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Qatar IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Qatar, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Qatar IMPE daily deaths update `update'.pdf", replace	
...
* graph Qatar IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Qatar, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Qatar IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Qatar.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Qatar.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Qatar data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Qatar LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Qatar, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Qatar LANL daily deaths update `update'.pdf", replace	
...
* graph Qatar LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Qatar, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Qatar LANL C19 daily deaths all updates.pdf", replace
...
local list IHME IMPE LANL UCLA

use "DELP Qatar.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Qatar.dta", force
...
save "ALL MODLES Qatar.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Qatar data dictionary.csv", replace 
...
* graph Qatar ALL MODELS daily deaths all updates
...
legend(position(6) order(652 "JOHN" 1 "DELP" 28 "IHME" 11 "IMPE" 7 "LANL" 167 "UCLA") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Qatar ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Qatar 1 Process.smcl"
