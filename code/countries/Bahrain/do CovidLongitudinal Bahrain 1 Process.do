...
log using "log CovidLongitudinal Bahrain 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Bahrain 1 Process.do"
...
**** "do CovidLongitudinal Bahrain 1 Process.do"
...
** output data files: "`model' Bahrain.dta"
**                   "ALL MODLES Bahrain.dta"

** output data dictionary files: "Bahrain ALL MODELS update dates data dictionary.csv"
**                               "`model' Bahrain data dictionary.csv"
**                               "ALL MODLES Bahrain data dictionary.csv"

** graphs:
** "graph 02 Bahrain `model' C19 daily deaths all updates.pdf"
** "graph 02 Bahrain ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Bahrain absent update_date_block"
	
	qui keep if loc_grand_name == "Bahrain"
	...
	save "Bahrain `model' update dates.dta", replace
...
use "Bahrain DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Bahrain `model' update dates.dta"
...
save "Bahrain ALL MODELS update dates.dta", replace
	
export excel using "Bahrain ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Bahrain ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Bahrain"

keep date DayDeaMeSmJOHNBHR
...
save "JOHN Bahrain.dta", replace 




* graph Bahrain JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNBHR date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Bahrain, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bahrain JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Bahrain.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Bahrain.dta"
...
merge m:m date using "JOHN Bahrain.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Bahrain data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bahrain DELP daily deaths each update 
	...
	subtitle("Bahrain, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bahrain DELP daily deaths update `update'.pdf", replace	
...
* graph Bahrain DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bahrain, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bahrain DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Bahrain IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Bahrain, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Bahrain IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Bahrain.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Bahrain.dta"
...
merge m:1 date using "JOHN Bahrain.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Bahrain data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bahrain IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bahrain, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bahrain IHME daily deaths update `update'.pdf", replace	
...
* graph Bahrain IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bahrain, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bahrain IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Bahrain.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Bahrain.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Bahrain data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bahrain IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bahrain, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bahrain IMPE daily deaths update `update'.pdf", replace	
...
* graph Bahrain IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bahrain, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bahrain IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Bahrain.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Bahrain.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Bahrain data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bahrain LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bahrain, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bahrain LANL daily deaths update `update'.pdf", replace	
...
* graph Bahrain LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bahrain, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bahrain LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Bahrain.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Bahrain.dta", force
...
save "ALL MODLES Bahrain.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Bahrain data dictionary.csv", replace 
...
* graph Bahrain ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bahrain, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(618 "JOHN" 1 "DELP" 25 "IHME" 8 "IMPE" 10 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bahrain ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Bahrain 1 Process.smcl"
