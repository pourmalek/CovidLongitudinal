...
log using "log CovidLongitudinal Togo 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Togo 1 Process.do"
...
**** "do CovidLongitudinal Togo 1 Process.do"
...
** output data files: "`model' Togo.dta"
**                   "ALL MODLES Togo.dta"

** output data dictionary files: "Togo ALL MODELS update dates data dictionary.csv"
**                               "`model' Togo data dictionary.csv"
**                               "ALL MODLES Togo data dictionary.csv"

** graphs:
** "graph 02 Togo `model' C19 daily deaths all updates.pdf"
** "graph 02 Togo ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Togo absent update_date_block"
	
	qui keep if loc_grand_name == "Togo"
	...
	save "Togo `model' update dates.dta", replace
...
use "Togo DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Togo `model' update dates.dta"
...
save "Togo ALL MODELS update dates.dta", replace
	
export excel using "Togo ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Togo ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Togo"

keep date DayDeaMeSmJOHNTGO
...
save "JOHN Togo.dta", replace 




* graph Togo JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNTGO date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Togo, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Togo JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Togo.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Togo.dta"
...
merge m:m date using "JOHN Togo.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Togo data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Togo DELP daily deaths each update 
	...
	subtitle("Togo, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Togo DELP daily deaths update `update'.pdf", replace	
...
* graph Togo DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Togo, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Togo DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Togo IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Togo, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Togo IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Togo.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Togo.dta"
...
merge m:1 date using "JOHN Togo.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Togo data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Togo IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Togo, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Togo IHME daily deaths update `update'.pdf", replace	
...
* graph Togo IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Togo, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Togo IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Togo.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Togo.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Togo data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Togo IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Togo, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Togo IMPE daily deaths update `update'.pdf", replace	
...
* graph Togo IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Togo, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Togo IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Togo.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Togo.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Togo data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Togo LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Togo, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Togo LANL daily deaths update `update'.pdf", replace	
...
* graph Togo LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Togo, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Togo LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Togo.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Togo.dta", force
...
save "ALL MODLES Togo.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Togo data dictionary.csv", replace 
...
* graph Togo ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Togo, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(637 "JOHN" 12 "DELP" 51 "IHME" 1 "IMPE" 77 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Togo ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Togo 1 Process.smcl"
