...
log using "log CovidLongitudinal Finland 9 Merge.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Finland 9 Merge.do"
...
** input data files: "`model' Finland error.dta"

** output data files: "ALL MODELS Finland error.dta" with all date
*                     "ALL MODELS Finland error - only models across error measures.dta", replace
*                     "ALL MODELS Finland error - only error measures across models.dta", replace

* output data dictionary files: "ALL MODELS Finland error data dictionary.csv"
*                               "ALL MODELS Finland error - only models across error measures data dictionary.csv"
*                               "ALL MODELS Finland error - only error measures across models data dictionary.csv"


** Graphs:
* "graph 03 Finland ALL MODELS C19 daily deaths error.pdf"
* "graph 04 Finland ALL MODELS C19 daily deaths absolute error.pdf"
* "graph 05 Finland ALL MODELS C19 daily deaths percent error.pdf"
* "graph 06 Finland ALL MODELS C19 daily deaths absolute percent error.pdf"

* "graph 07a1 Finland ALL MODELS C19 daily deaths average median error.pdf"
* "graph 07a2 Finland ALL MODELS C19 daily deaths average median error smoothed.pdf"
* "graph 08a1 Finland ALL MODELS C19 daily deaths average median absolute error.pdf"
* "graph 08a2 Finland ALL MODELS C19 daily deaths average median absolute error smoothed.pdf"
* "graph 09a1 Finland ALL MODELS C19 daily deaths average median percent error.pdf"
* "graph 09a2 Finland ALL MODELS C19 daily deaths average median percent error smoothed.pdf"
* "graph 10a1 Finland ALL MODELS C19 daily deaths average median absolute percent error.pdf"
* "graph 10a2 Finland ALL MODELS C19 daily deaths average median absolute percent error smoothed.pdf"

****** graph each same error measure across models:
* "graph 12a Finland ALL MODELS C19 daily deaths Average Mean Error.pdf"
* "graph 12b Finland ALL MODELS C19 daily deaths Average Absolute Error.pdf"
* "graph 12c Finland ALL MODELS C19 daily deaths Average Percent Error.pdf"
* "graph 12d Finland ALL MODELS C19 daily deaths Average Absolute Percent Error.pdf"

****** graph all models across error measures:
* "graph 13a Finland ALL MODELS C19 daily deaths, models across error measures.pdf"

****** graph all error measures across models:
* "graph 13b Finland ALL MODELS C19 daily deaths, error measures across models.pdf"
...
use "DELP Finland error.dta", clear 

	merge m:m date using "IHME Finland error.dta"

	drop _merge
	
	merge m:m date using "IMPE Finland error.dta"
	
	drop _merge
	
	merge m:m date using "LANL Finland error.dta"	
	
	drop _merge
	
	merge m:m date using "YYGU Finland error.dta", force	
...
subtitle("Finland, all Models, all updates, forecast only", size(small)) ///
legend(position(6) order(15 "DELP" 1 "IHME" 91 "IMPE" 29 "LANL" 2 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 03 Finland ALL MODELS C19 daily deaths error.pdf", replace
...
qui graph export "graph 04 Finland ALL MODELS C19 daily deaths absolute error.pdf", replace
...
qui graph export "graph 05 Finland ALL MODELS C19 daily deaths percent error.pdf", replace
...
qui graph export "graph 06 Finland ALL MODELS C19 daily deaths absolute percent error.pdf", replace
...
subtitle("Finland, all Models, forecast only", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) yscale(titlegap(3)) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL" 5 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 07a1 Finland ALL MODELS C19 daily deaths average median error.pdf", replace 
...
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths error measure) title("C19 daily deaths, average median error", size(medium) color(black)) /// 
subtitle("Finland, all Models, forecast only; smoothed for better viewing", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) yscale(titlegap(3)) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL" 5 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand)

qui graph export "graph 07a2 Finland ALL MODELS C19 daily deaths average median error smoothed.pdf", replace
...
qui graph export "graph 08a1 Finland ALL MODELS C19 daily deaths average median absolute error.pdf", replace 
...
qui graph export "graph 08a2 Finland ALL MODELS C19 daily deaths average median absolute error smoothed.pdf", replace
...
qui graph export "graph 09a1 Finland ALL MODELS C19 daily deaths average median percent error.pdf", replace
...
qui graph export "graph 09a2 Finland ALL MODELS C19 daily deaths average median percent error smoothed.pdf", replace
...
qui graph export "graph 10a1 Finland ALL MODELS C19 daily deaths average median absolute percent error.pdf", replace 
...
qui graph export "graph 10a2 Finland ALL MODELS C19 daily deaths average median absolute percent error smoothed.pdf", replace
...
subtitle("Over updates and calendar months; forecast only; Finland", size(small)) /// 
legend(region(lcolor(none))) legend(bexpand) ///
legend(position(6) order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL" 5 "YYGU") size(small) row(1)) 

qui graph export "graph 12a Finland ALL MODELS C19 daily deaths Average Mean Error.pdf", replace
...
qui graph export "graph 12b Finland ALL MODELS C19 daily deaths Average Absolute Error.pdf", replace
...
qui graph export "graph 12c Finland ALL MODELS C19 daily deaths Average Percent Error.pdf", replace
...
qui graph export "graph 12d Finland ALL MODELS C19 daily deaths Average Absolute Percent Error.pdf", replace
...
save "ALL MODELS Finland error.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Finland error data dictionary.csv", replace 
...
use "ALL MODELS Finland error.dta", clear

save "ALL MODELS Finland error - only models across error measures.dta", replace
...
save "CovidLongitudinal Finland merge temp.dta", replace 
...
qui graph export "graph 13a Finland ALL MODELS C19 daily deaths, models across error measures.pdf", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Finland error - only models across error measures data dictionary.csv", replace 
...
use "CovidLongitudinal Finland merge temp.dta", clear 
...
qui graph export "graph 13b Finland ALL MODELS C19 daily deaths, error measures across models.pdf", replace
...
save "ALL MODELS Finland error - only error measures across models.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODELS Finland error - only error measures across models data dictionary.csv", replace 
...
view "log CovidLongitudinal Finland 9 Merge.smcl"
