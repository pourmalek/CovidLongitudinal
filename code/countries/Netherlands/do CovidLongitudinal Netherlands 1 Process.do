...
log using "log CovidLongitudinal Netherlands 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Netherlands 1 Process.do"
...
**** "do CovidLongitudinal Netherlands 1 Process.do"
...
** output data files: "`model' Netherlands.dta"
**                   "ALL MODLES Netherlands.dta"

** output data dictionary files: "Netherlands ALL MODELS update dates data dictionary.csv"
**                               "`model' Netherlands data dictionary.csv"
**                               "ALL MODLES Netherlands data dictionary.csv"

** graphs:
** "graph 02 Netherlands `model' C19 daily deaths all updates.pdf"
** "graph 02 Netherlands ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Netherlands absent update_date_block"
	
	qui keep if loc_grand_name == "Netherlands"
	...
	save "Netherlands `model' update dates.dta", replace
...
use "Netherlands DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Netherlands `model' update dates.dta"
...
save "Netherlands ALL MODELS update dates.dta", replace
	
export excel using "Netherlands ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Netherlands ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Netherlands"

keep date DayDeaMeSmJOHNNLD
...
save "JOHN Netherlands.dta", replace 




* graph Netherlands JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNNLD date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Netherlands, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Netherlands JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Netherlands.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Netherlands.dta"
...
save "DELP Netherlands.dta", replace
...
merge m:m date using "JOHN Netherlands.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Netherlands data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Netherlands DELP daily deaths each update 
	...
	subtitle("Netherlands, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Netherlands DELP daily deaths update `update'.pdf", replace	
...
* graph Netherlands DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Netherlands, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Netherlands DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Netherlands IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Netherlands, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Netherlands IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Netherlands.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Netherlands.dta"
...
save "IHME Netherlands.dta", replace
...
merge m:1 date using "JOHN Netherlands.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Netherlands data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Netherlands IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Netherlands, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Netherlands IHME daily deaths update `update'.pdf", replace	
...
* graph Netherlands IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Netherlands, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Netherlands IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Netherlands.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Netherlands.dta"
...
save "IMPE Netherlands.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Netherlands data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Netherlands IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Netherlands, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Netherlands IMPE daily deaths update `update'.pdf", replace	
...
* graph Netherlands IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Netherlands, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Netherlands IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Netherlands.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Netherlands.dta"
...
save "LANL Netherlands.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Netherlands data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Netherlands LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Netherlands, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Netherlands LANL daily deaths update `update'.pdf", replace	
...
* graph Netherlands LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Netherlands, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Netherlands LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Netherlands.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Netherlands.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Netherlands.dta"
...
save "YYGU Netherlands.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Netherlands data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Netherlands YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Netherlands, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Netherlands YYGU daily deaths update `update'.pdf", replace	
...
* graph Netherlands YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Netherlands, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Netherlands YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Netherlands.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Netherlands.dta", force
...
save "ALL MODLES Netherlands.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Netherlands data dictionary.csv", replace 
...
* graph Netherlands ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Netherlands, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(838 "JOHN" 19 "DELP" 5 "IHME" 95 "IMPE" 33 "LANL" 1 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Netherlands ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Netherlands 1 Process.smcl"
