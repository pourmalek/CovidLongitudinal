...
log using "log CovidLongitudinal North Macedonia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal North Macedonia 1 Process.do"
...
**** "do CovidLongitudinal North Macedonia 1 Process.do"
...
** output data files: "`model' North Macedonia.dta"
**                   "ALL MODLES North Macedonia.dta"

** output data dictionary files: "North Macedonia ALL MODELS update dates data dictionary.csv"
**                               "`model' North Macedonia data dictionary.csv"
**                               "ALL MODLES North Macedonia data dictionary.csv"

** graphs:
** "graph 02 North Macedonia `model' C19 daily deaths all updates.pdf"
** "graph 02 North Macedonia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " North Macedonia absent update_date_block"
	
	qui keep if loc_grand_name == "North Macedonia"
	...
	save "North Macedonia `model' update dates.dta", replace
...
use "North Macedonia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "North Macedonia `model' update dates.dta"
...
save "North Macedonia ALL MODELS update dates.dta", replace
	
export excel using "North Macedonia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "North Macedonia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "North Macedonia"

keep date DayDeaMeSmJOHNMKD
...
save "JOHN North Macedonia.dta", replace 




* graph North Macedonia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMKD date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, North Macedonia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 North Macedonia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' North Macedonia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' North Macedonia.dta"
...
save "DELP North Macedonia.dta", replace
...
merge m:m date using "JOHN North Macedonia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP North Macedonia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph North Macedonia DELP daily deaths each update 
	...
	subtitle("North Macedonia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 North Macedonia DELP daily deaths update `update'.pdf", replace	
...
* graph North Macedonia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, North Macedonia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 North Macedonia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph North Macedonia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, North Macedonia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 North Macedonia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' North Macedonia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' North Macedonia.dta"
...
save "IHME North Macedonia.dta", replace
...
merge m:1 date using "JOHN North Macedonia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME North Macedonia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph North Macedonia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("North Macedonia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 North Macedonia IHME daily deaths update `update'.pdf", replace	
...
* graph North Macedonia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, North Macedonia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 North Macedonia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' North Macedonia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' North Macedonia.dta"
...
save "IMPE North Macedonia.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE North Macedonia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph North Macedonia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("North Macedonia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 North Macedonia IMPE daily deaths update `update'.pdf", replace	
...
* graph North Macedonia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, North Macedonia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 North Macedonia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' North Macedonia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' North Macedonia.dta"
...
save "LANL North Macedonia.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL North Macedonia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph North Macedonia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("North Macedonia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 North Macedonia LANL daily deaths update `update'.pdf", replace	
...
* graph North Macedonia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, North Macedonia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 North Macedonia LANL C19 daily deaths all updates.pdf", replace
...
use "DELP North Macedonia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' North Macedonia.dta", force
...
save "ALL MODLES North Macedonia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES North Macedonia data dictionary.csv", replace 
...
* graph North Macedonia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, North Macedonia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(668 "JOHN" 1 "DELP" 71 "IHME" 3 "IMPE" 2 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 North Macedonia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal North Macedonia 1 Process.smcl"
