...
log using "log CovidLongitudinal Bangladesh 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Bangladesh 1 Process.do"
...
**** "do CovidLongitudinal Bangladesh 1 Process.do"
...
** output data files: "`model' Bangladesh.dta"
**                   "ALL MODLES Bangladesh.dta"

** output data dictionary files: "Bangladesh ALL MODELS update dates data dictionary.csv"
**                               "`model' Bangladesh data dictionary.csv"
**                               "ALL MODLES Bangladesh data dictionary.csv"

** graphs:
** "graph 02 Bangladesh `model' C19 daily deaths all updates.pdf"
** "graph 02 Bangladesh ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Bangladesh absent update_date_block"
	
	qui keep if loc_grand_name == "Bangladesh"
	...
	save "Bangladesh `model' update dates.dta", replace
...
use "Bangladesh DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Bangladesh `model' update dates.dta"
...
save "Bangladesh ALL MODELS update dates.dta", replace
	
export excel using "Bangladesh ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Bangladesh ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Bangladesh"

keep date DayDeaMeSmJOHNBGD
...
save "JOHN Bangladesh.dta", replace 




* graph Bangladesh JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNBGD date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Bangladesh, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bangladesh JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Bangladesh.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Bangladesh.dta"
...
save "DELP Bangladesh.dta", replace
...
merge m:m date using "JOHN Bangladesh.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Bangladesh data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bangladesh DELP daily deaths each update 
	...
	subtitle("Bangladesh, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bangladesh DELP daily deaths update `update'.pdf", replace	
...
* graph Bangladesh DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bangladesh, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bangladesh DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Bangladesh IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Bangladesh, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Bangladesh IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Bangladesh.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Bangladesh.dta"
...
save "IHME Bangladesh.dta", replace
...
merge m:1 date using "JOHN Bangladesh.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Bangladesh data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bangladesh IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bangladesh, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bangladesh IHME daily deaths update `update'.pdf", replace	
...
* graph Bangladesh IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bangladesh, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bangladesh IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Bangladesh.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Bangladesh.dta"
...
save "IMPE Bangladesh.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Bangladesh data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bangladesh IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bangladesh, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bangladesh IMPE daily deaths update `update'.pdf", replace	
...
* graph Bangladesh IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bangladesh, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bangladesh IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Bangladesh.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Bangladesh.dta"
...
save "LANL Bangladesh.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Bangladesh data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bangladesh LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bangladesh, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bangladesh LANL daily deaths update `update'.pdf", replace	
...
* graph Bangladesh LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bangladesh, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bangladesh LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Bangladesh.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Bangladesh.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Bangladesh.dta"
...
save "YYGU Bangladesh.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Bangladesh data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Bangladesh YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Bangladesh, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Bangladesh YYGU daily deaths update `update'.pdf", replace	
...
* graph Bangladesh YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bangladesh, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bangladesh YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Bangladesh.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Bangladesh.dta", force
...
save "ALL MODLES Bangladesh.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Bangladesh data dictionary.csv", replace 
...
* graph Bangladesh ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Bangladesh, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(816 "JOHN" 1 "DELP" 116 "IHME" 4 "IMPE" 3 "LANL" 24 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Bangladesh ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Bangladesh 1 Process.smcl"
