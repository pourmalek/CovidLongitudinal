...
log using "log CovidLongitudinal Finland 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Finland 1 Process.do"
...
**** "do CovidLongitudinal Finland 1 Process.do"
...
** output data files: "`model' Finland.dta"
**                   "ALL MODLES Finland.dta"

** output data dictionary files: "Finland ALL MODELS update dates data dictionary.csv"
**                               "`model' Finland data dictionary.csv"
**                               "ALL MODLES Finland data dictionary.csv"

** graphs:
** "graph 02 Finland `model' C19 daily deaths all updates.pdf"
** "graph 02 Finland ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Finland absent update_date_block"
	
	qui keep if loc_grand_name == "Finland"
	...
	save "Finland `model' update dates.dta", replace
...
use "Finland DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Finland `model' update dates.dta"
...
save "Finland ALL MODELS update dates.dta", replace
	
export excel using "Finland ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Finland ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Finland"

keep date DayDeaMeSmJOHNFIN
...
save "JOHN Finland.dta", replace 




* graph Finland JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNFIN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Finland, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Finland JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Finland.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Finland.dta"
...
merge m:m date using "JOHN Finland.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Finland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Finland DELP daily deaths each update 
	...
	subtitle("Finland, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Finland DELP daily deaths update `update'.pdf", replace	
...
* graph Finland DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Finland, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Finland DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Finland IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Finland, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Finland IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Finland.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Finland.dta"
...
merge m:1 date using "JOHN Finland.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Finland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Finland IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Finland, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Finland IHME daily deaths update `update'.pdf", replace	
...
* graph Finland IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Finland, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Finland IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Finland.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Finland.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Finland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Finland IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Finland, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Finland IMPE daily deaths update `update'.pdf", replace	
...
* graph Finland IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Finland, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Finland IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Finland.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Finland.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Finland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Finland LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Finland, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Finland LANL daily deaths update `update'.pdf", replace	
...
* graph Finland LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Finland, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Finland LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Finland.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Finland.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Finland.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Finland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Finland YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Finland, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Finland YYGU daily deaths update `update'.pdf", replace	
...
* graph Finland YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Finland, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Finland YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Finland.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Finland.dta", force
...
save "ALL MODLES Finland.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Finland data dictionary.csv", replace 
...
* graph Finland ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Finland, all models, all updates, forecast only", size(medium) color(black)) ///
...
qui graph export "graph 02 Finland ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Finland 1 Process.smcl"
