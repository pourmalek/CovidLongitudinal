...
log using "log CovidLongitudinal Serbia 2 DELP.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Serbia 2 DELP.do"
...
* input data files: "DELP Serbia.dta"
* output data files: "DELP Serbia error.dta" (with error measures saved)

* output data dictionary files: "DELP Serbia error data dictionary.csv"
...
** graphs:
* "graph 03 Serbia DELP C19 daily deaths error.pdf"
* "graph 04 Serbia DELP C19 daily deaths absolute error.pdf"
* "graph 05 Serbia DELP C19 daily deaths percent error.pdf"
* "graph 06 Serbia DELP C19 daily deaths absolute percent error.pdf"

* "graph 07 Serbia DELP C19 daily deaths average median error.pdf"
* "graph 08 Serbia DELP C19 daily deaths average median absolute error.pdf"
* "graph 09 Serbia DELP C19 daily deaths average median percent error.pdf"
* "graph 10 Serbia DELP C19 daily deaths average median absolute percent error.pdf"

* "graph 11 Serbia DELP C19 daily deaths Average MAPE over updates and calendar months.pdf"
...
qui replace DDErrorDELP`update' = . if date < td(17apr2020) // 17apr2020 is the earliest date of DELP forecasts for Serbia
                                                            // <<-- modify 3
...
qui replace DDAbsErrDELP`update' = . if date < td(17apr2020) // 17apr2020 is the earliest date of DELP forecasts for Serbia
...
qui replace DDPerErrDELP`update' = . if date < td(17apr2020) // 17apr2020 is the earliest date of DELP forecasts for Serbia
...
qui replace DDAbPeErDELP`update' = . if date < td(17apr2020) // 17apr2020 is the earliest date of DELP forecasts for Serbia
...
qui replace DDErrorDELP`update'_Med1 = . if date < td(17apr2020) // 17apr2020 is the earliest date of DELP forecasts for Serbia
...
qui replace DDAbsErrDELP`update'_Med1 = . if date < td(17apr2020) // 17apr2020 is the earliest date of DELP forecasts for Serbia
...
qui replace DDPerErrDELP`update'_Med1 = . if date < td(17apr2020) // 17apr2020 is the earliest date of DELP forecasts for Serbia
...
qui replace DDAbPeErDELP`update'_Med1 = . if date < td(17apr2020) // 17apr2020 is the earliest date of DELP forecasts for Serbia
...
qui replace DDErrorDELP_Mean1 = . if date < td(17apr2020) // 17apr2020 is the earliest date of DELP forecasts for Serbia
...
qui replace DDAbsErrDELP_Mean1 = . if date < td(17apr2020) // 17apr2020 is the earliest date of DELP forecasts for Serbia
...
qui replace DDPerErrDELP_Mean1 = . if date < td(17apr2020) // 17apr2020 is the earliest date of DELP forecasts for Serbia
...
qui replace DDAbPeErDELP_Mean1 = . if date < td(17apr2020) // 17apr2020 is the earliest date of DELP forecasts for Serbia
...
subtitle("Serbia, DELP, all updates, forecast only", size(small)) 

qui graph export "graph 03 Serbia DELP C19 daily deaths error.pdf", replace
...
qui graph export "graph 04 Serbia DELP C19 daily deaths absolute error.pdf", replace
...
qui graph export "graph 05 Serbia DELP C19 daily deaths percent error.pdf", replace
...
qui graph export "graph 06 Serbia DELP C19 daily deaths absolute percent error.pdf", replace
...
subtitle("Serbia, DELP, forecast only", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(off) yscale(titlegap(3))

qui graph export "graph 07 Serbia DELP C19 daily deaths average median error.pdf", replace
...
qui graph export "graph 08 Serbia DELP C19 daily deaths average median absolute error.pdf", replace
...
qui graph export "graph 09 Serbia DELP C19 daily deaths average median percent error.pdf", replace
...
qui graph export "graph 10 Serbia DELP C19 daily deaths average median absolute percent error.pdf", replace
...
subtitle("Serbia, DELP, forecast only. MAPE: Median Absolute Percent Error", size(small)) /// 
legend(off) ylabel(, labsize(small) angle(forty_five) format(%30.0fc))

qui graph export "graph 11 Serbia DELP C19 daily deaths Average MAPE over updates and calendar months.pdf", replace
...
save "DELP Serbia error.dta", replace 
...
preserve
    ...
    export delimited name varlab using "DELP Serbia error data dictionary.csv", replace 
	
restore



view "log CovidLongitudinal Serbia 2 DELP.smcl"
