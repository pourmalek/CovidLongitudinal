...
log using "log CovidLongitudinal Italy 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Italy 1 Process.do"
...
**** "do CovidLongitudinal Italy 1 Process.do"
...
** output data files: "`model' Italy.dta"
**                   "ALL MODLES Italy.dta"

** output data dictionary files: "Italy ALL MODELS update dates data dictionary.csv"
**                               "`model' Italy data dictionary.csv"
**                               "ALL MODLES Italy data dictionary.csv"

** graphs:
** "graph 02 Italy `model' C19 daily deaths all updates.pdf"
** "graph 02 Italy ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Italy absent update_date_block"
	
	qui keep if loc_grand_name == "Italy"
	...
	save "Italy `model' update dates.dta", replace
...
use "Italy DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Italy `model' update dates.dta"
...
save "Italy ALL MODELS update dates.dta", replace
	
export excel using "Italy ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Italy ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Italy"

keep date DayDeaMeSmJOHNITA
...
save "JOHN Italy.dta", replace 




* graph Italy JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNITA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Italy, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Italy JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Italy.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Italy.dta"
...
merge m:m date using "JOHN Italy.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Italy data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Italy DELP daily deaths each update 
	...
	subtitle("Italy, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Italy DELP daily deaths update `update'.pdf", replace	
...
* graph Italy DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Italy, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Italy DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Italy IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Italy, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Italy IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Italy.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Italy.dta"
...
merge m:1 date using "JOHN Italy.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Italy data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Italy IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Italy, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Italy IHME daily deaths update `update'.pdf", replace	
...
* graph Italy IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Italy, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Italy IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Italy.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Italy.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Italy data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Italy IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Italy, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Italy IMPE daily deaths update `update'.pdf", replace	
...
* graph Italy IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Italy, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Italy IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Italy.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Italy.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Italy data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Italy LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Italy, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Italy LANL daily deaths update `update'.pdf", replace	
...
* graph Italy LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Italy, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Italy LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Italy.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Italy.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Italy.dta"
...
preserve
    ...
    export delimited name varlab using "UCLA Italy data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Italy UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Italy, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Italy UCLA daily deaths update `update'.pdf", replace	

}
*/


* graph Italy UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Italy, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Italy UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Italy.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Italy.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Italy.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Italy data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Italy YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Italy, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Italy YYGU daily deaths update `update'.pdf", replace	
...
* graph Italy YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Italy, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Italy YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Italy.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Italy.dta", force
...
save "ALL MODLES Italy.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Italy data dictionary.csv", replace 
...
* graph Italy ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Italy, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(867 "JOHN" 20 "DELP" 6 "IHME" 96 "IMPE" 34 "LANL" 384 "UCLA" 1 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Italy ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Italy 1 Process.smcl"
