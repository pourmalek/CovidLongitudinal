...
log using "log CovidLongitudinal Botswana 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Botswana 1 Process.do"
...
**** "do CovidLongitudinal Botswana 1 Process.do"
...
** output data files: "`model' Botswana.dta"
**                   "ALL MODLES Botswana.dta"

** output data dictionary files: "Botswana ALL MODELS update dates data dictionary.csv"
**                               "`model' Botswana data dictionary.csv"
**                               "ALL MODLES Botswana data dictionary.csv"

** graphs:
** "graph 02 Botswana `model' C19 daily deaths all updates.pdf"
** "graph 02 Botswana ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Botswana absent update_date_block"
	
	qui keep if loc_grand_name == "Botswana"
	...
	save "Botswana `model' update dates.dta", replace
...
use "Botswana DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Botswana `model' update dates.dta"
...
save "Botswana ALL MODELS update dates.dta", replace
	
export excel using "Botswana ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Botswana ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Botswana"

keep date DayDeaMeSmJOHNBWA
...
save "JOHN Botswana.dta", replace 




* graph Botswana JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNBWA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Botswana, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Botswana JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Botswana.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Botswana.dta"
...
save "DELP Botswana.dta", replace
...
merge m:m date using "JOHN Botswana.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Botswana data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Botswana DELP daily deaths each update 
	...
	subtitle("Botswana, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Botswana DELP daily deaths update `update'.pdf", replace	
...
* graph Botswana DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Botswana, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Botswana DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Botswana IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Botswana, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Botswana IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Botswana.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Botswana.dta"
...
save "IHME Botswana.dta", replace
...
merge m:1 date using "JOHN Botswana.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Botswana data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Botswana IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Botswana, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Botswana IHME daily deaths update `update'.pdf", replace	
...
* graph Botswana IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Botswana, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Botswana IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Botswana.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Botswana.dta"
...
save "IMPE Botswana.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Botswana data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Botswana IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Botswana, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Botswana IMPE daily deaths update `update'.pdf", replace	
...
* graph Botswana IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Botswana, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Botswana IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Botswana.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Botswana.dta"
...
save "LANL Botswana.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Botswana data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Botswana LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Botswana, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Botswana LANL daily deaths update `update'.pdf", replace	
...
* graph Botswana LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Botswana, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Botswana LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Botswana.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Botswana.dta", force
...
save "ALL MODLES Botswana.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Botswana data dictionary.csv", replace 
...
* graph Botswana ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Botswana, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(542 "JOHN" 56 "DELP" 45 "IHME" 1 "IMPE" 90 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Botswana ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Botswana 1 Process.smcl"
