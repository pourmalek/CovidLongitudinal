...
log using "log CovidLongitudinal Switzerland 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Switzerland 1 Process.do"
...
**** "do CovidLongitudinal Switzerland 1 Process.do"
...
** output data files: "`model' Switzerland.dta"
**                   "ALL MODLES Switzerland.dta"

** output data dictionary files: "Switzerland ALL MODELS update dates data dictionary.csv"
**                               "`model' Switzerland data dictionary.csv"
**                               "ALL MODLES Switzerland data dictionary.csv"

** graphs:
** "graph 02 Switzerland `model' C19 daily deaths all updates.pdf"
** "graph 02 Switzerland ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Switzerland absent update_date_block"
	
	qui keep if loc_grand_name == "Switzerland"
	...
	save "Switzerland `model' update dates.dta", replace
...
use "Switzerland DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Switzerland `model' update dates.dta"
...
save "Switzerland ALL MODELS update dates.dta", replace
	
export excel using "Switzerland ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Switzerland ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Switzerland"

keep date DayDeaMeSmJOHNCHE
...
save "JOHN Switzerland.dta", replace 




* graph Switzerland JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNCHE date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Switzerland, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Switzerland JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Switzerland.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Switzerland.dta"
...
save "DELP Switzerland.dta", replace
...
merge m:m date using "JOHN Switzerland.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Switzerland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Switzerland DELP daily deaths each update 
	...
	subtitle("Switzerland, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Switzerland DELP daily deaths update `update'.pdf", replace	
...
* graph Switzerland DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Switzerland, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Switzerland DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Switzerland IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Switzerland, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Switzerland IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Switzerland.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Switzerland.dta"
...
save "IHME Switzerland.dta", replace
...
merge m:1 date using "JOHN Switzerland.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Switzerland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Switzerland IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Switzerland, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Switzerland IHME daily deaths update `update'.pdf", replace	
...
* graph Switzerland IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Switzerland, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Switzerland IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Switzerland.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Switzerland.dta"
...
save "IMPE Switzerland.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Switzerland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Switzerland IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Switzerland, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Switzerland IMPE daily deaths update `update'.pdf", replace	
...
* graph Switzerland IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Switzerland, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Switzerland IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Switzerland.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Switzerland.dta"
...
save "LANL Switzerland.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Switzerland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Switzerland LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Switzerland, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Switzerland LANL daily deaths update `update'.pdf", replace	
...
* graph Switzerland LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Switzerland, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Switzerland LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Switzerland.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Switzerland.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Switzerland.dta"
...
save "YYGU Switzerland.dta", replace
...
preserve
    ...
    export delimited name varlab using "YYGU Switzerland data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Switzerland YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Switzerland, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Switzerland YYGU daily deaths update `update'.pdf", replace	
...
* graph Switzerland YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Switzerland, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Switzerland YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Switzerland.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Switzerland.dta", force
...
save "ALL MODLES Switzerland.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Switzerland data dictionary.csv", replace 
...
* graph Switzerland ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Switzerland, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(834 "JOHN" 17 "DELP" 8 "IHME" 93 "IMPE" 31 "LANL" 1 "YYGU") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Switzerland ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Switzerland 1 Process.smcl"
