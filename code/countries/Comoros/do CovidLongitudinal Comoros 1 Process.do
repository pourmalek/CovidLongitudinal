...
log using "log CovidLongitudinal Comoros 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Comoros 1 Process.do"
...
**** "do CovidLongitudinal Comoros 1 Process.do"
...
** output data files: "`model' Comoros.dta"
**                   "ALL MODLES Comoros.dta"

** output data dictionary files: "Comoros ALL MODELS update dates data dictionary.csv"
**                               "`model' Comoros data dictionary.csv"
**                               "ALL MODLES Comoros data dictionary.csv"

** graphs:
** "graph 02 Comoros `model' C19 daily deaths all updates.pdf"
** "graph 02 Comoros ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Comoros absent update_date_block"
	
	qui keep if loc_grand_name == "Comoros"
	...
	save "Comoros `model' update dates.dta", replace
...
use "Comoros DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Comoros `model' update dates.dta"
...
save "Comoros ALL MODELS update dates.dta", replace
	
export excel using "Comoros ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Comoros ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Comoros"

keep date DayDeaMeSmJOHNCOM
...
save "JOHN Comoros.dta", replace 




* graph Comoros JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNCOM date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Comoros, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Comoros JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Comoros.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Comoros.dta"
...
merge m:m date using "JOHN Comoros.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Comoros data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Comoros DELP daily deaths each update 
	...
	subtitle("Comoros, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Comoros DELP daily deaths update `update'.pdf", replace	
...
* graph Comoros DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Comoros, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Comoros DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Comoros IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Comoros, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Comoros IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Comoros.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Comoros.dta"
...
merge m:1 date using "JOHN Comoros.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Comoros data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Comoros IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Comoros, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Comoros IHME daily deaths update `update'.pdf", replace	
...
* graph Comoros IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Comoros, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Comoros IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Comoros.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Comoros.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Comoros data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Comoros IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Comoros, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Comoros IMPE daily deaths update `update'.pdf", replace	
...
* graph Comoros IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Comoros, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Comoros IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Comoros.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Comoros.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Comoros data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Comoros LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Comoros, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Comoros LANL daily deaths update `update'.pdf", replace	
...
* graph Comoros LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Comoros, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Comoros LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Comoros.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Comoros.dta", force
...
save "ALL MODLES Comoros.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Comoros data dictionary.csv", replace 
...
* graph Comoros ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Comoros, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(554 "JOHN" 31 "DELP" 40 "IHME" 1 "IMPE" 132 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Comoros ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Comoros 1 Process.smcl"
