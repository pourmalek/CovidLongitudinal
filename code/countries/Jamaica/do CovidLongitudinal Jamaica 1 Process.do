...
log using "log CovidLongitudinal Jamaica 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Jamaica 1 Process.do"
...
**** "do CovidLongitudinal Jamaica 1 Process.do"
...
** output data files: "`model' Jamaica.dta"
**                   "ALL MODLES Jamaica.dta"

** output data dictionary files: "Jamaica ALL MODELS update dates data dictionary.csv"
**                               "`model' Jamaica data dictionary.csv"
**                               "ALL MODLES Jamaica data dictionary.csv"

** graphs:
** "graph 02 Jamaica `model' C19 daily deaths all updates.pdf"
** "graph 02 Jamaica ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Jamaica absent update_date_block"
	
	qui keep if loc_grand_name == "Jamaica"
	...
	save "Jamaica `model' update dates.dta", replace
...
use "Jamaica DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Jamaica `model' update dates.dta"
...
save "Jamaica ALL MODELS update dates.dta", replace
	
export excel using "Jamaica ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Jamaica ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Jamaica"

keep date DayDeaMeSmJOHNJAM
...
save "JOHN Jamaica.dta", replace 




* graph Jamaica JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNJAM date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Jamaica, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Jamaica JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Jamaica.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Jamaica.dta"
...
merge m:m date using "JOHN Jamaica.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Jamaica data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Jamaica DELP daily deaths each update 
	...
	subtitle("Jamaica, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Jamaica DELP daily deaths update `update'.pdf", replace	
...
* graph Jamaica DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Jamaica, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Jamaica DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Jamaica IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Jamaica, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Jamaica IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Jamaica.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Jamaica.dta"
...
merge m:1 date using "JOHN Jamaica.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Jamaica data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Jamaica IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Jamaica, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Jamaica IHME daily deaths update `update'.pdf", replace	
...
* graph Jamaica IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Jamaica, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Jamaica IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Jamaica.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Jamaica.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Jamaica data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Jamaica IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Jamaica, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Jamaica IMPE daily deaths update `update'.pdf", replace	
...
* graph Jamaica IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Jamaica, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Jamaica IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Jamaica.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Jamaica.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Jamaica data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Jamaica LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Jamaica, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Jamaica LANL daily deaths update `update'.pdf", replace	
...
* graph Jamaica LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Jamaica, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Jamaica LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Jamaica.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Jamaica.dta", force
...
save "ALL MODLES Jamaica.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Jamaica data dictionary.csv", replace 
...
* graph Jamaica ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Jamaica, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(633 "JOHN" 1 "DELP" 54 "IHME" 2 "IMPE" 94 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Jamaica ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Jamaica 1 Process.smcl"
