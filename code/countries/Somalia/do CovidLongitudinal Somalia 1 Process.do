...
log using "log CovidLongitudinal Somalia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Somalia 1 Process.do"
...
**** "do CovidLongitudinal Somalia 1 Process.do"
...
** output data files: "`model' Somalia.dta"
**                   "ALL MODLES Somalia.dta"

** output data dictionary files: "Somalia ALL MODELS update dates data dictionary.csv"
**                               "`model' Somalia data dictionary.csv"
**                               "ALL MODLES Somalia data dictionary.csv"

** graphs:
** "graph 02 Somalia `model' C19 daily deaths all updates.pdf"
** "graph 02 Somalia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Somalia absent update_date_block"
	
	qui keep if loc_grand_name == "Somalia"
	...
	save "Somalia `model' update dates.dta", replace
...
use "Somalia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Somalia `model' update dates.dta"
...
save "Somalia ALL MODELS update dates.dta", replace
	
export excel using "Somalia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Somalia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Somalia"

keep date DayDeaMeSmJOHNSOM
...
save "JOHN Somalia.dta", replace 




* graph Somalia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSOM date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Somalia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Somalia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Somalia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Somalia.dta"
...
merge m:m date using "JOHN Somalia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Somalia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Somalia DELP daily deaths each update 
	...
	subtitle("Somalia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Somalia DELP daily deaths update `update'.pdf", replace	
...
* graph Somalia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Somalia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Somalia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Somalia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Somalia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Somalia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Somalia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Somalia.dta"
...
merge m:1 date using "JOHN Somalia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Somalia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Somalia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Somalia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Somalia IHME daily deaths update `update'.pdf", replace	
...
* graph Somalia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Somalia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Somalia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Somalia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Somalia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Somalia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Somalia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Somalia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Somalia IMPE daily deaths update `update'.pdf", replace	
...
* graph Somalia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Somalia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Somalia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Somalia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Somalia.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Somalia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Somalia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Somalia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Somalia LANL daily deaths update `update'.pdf", replace	
...
* graph Somalia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Somalia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Somalia LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Somalia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Somalia.dta", force
...
save "ALL MODLES Somalia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Somalia data dictionary.csv", replace 
...
* graph Somalia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Somalia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(667 "JOHN" 6 "DELP" 71 "IHME" 2 "IMPE" 1 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Somalia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Somalia 1 Process.smcl"
