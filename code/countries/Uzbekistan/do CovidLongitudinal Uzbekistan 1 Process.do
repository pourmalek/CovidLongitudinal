...
log using "log CovidLongitudinal Uzbekistan 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Uzbekistan 1 Process.do"
...
**** "do CovidLongitudinal Uzbekistan 1 Process.do"
...
** output data files: "`model' Uzbekistan.dta"
**                   "ALL MODLES Uzbekistan.dta"

** output data dictionary files: "Uzbekistan ALL MODELS update dates data dictionary.csv"
**                               "`model' Uzbekistan data dictionary.csv"
**                               "ALL MODLES Uzbekistan data dictionary.csv"

** graphs:
** "graph 02 Uzbekistan `model' C19 daily deaths all updates.pdf"
** "graph 02 Uzbekistan ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Uzbekistan absent update_date_block"
	
	qui keep if loc_grand_name == "Uzbekistan"
	...
	save "Uzbekistan `model' update dates.dta", replace
...
use "Uzbekistan DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Uzbekistan `model' update dates.dta"
...
save "Uzbekistan ALL MODELS update dates.dta", replace
	
export excel using "Uzbekistan ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Uzbekistan ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Uzbekistan"

keep date DayDeaMeSmJOHNUZB
...
save "JOHN Uzbekistan.dta", replace 




* graph Uzbekistan JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNUZB date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Uzbekistan, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uzbekistan JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Uzbekistan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Uzbekistan.dta"
...
save "DELP Uzbekistan.dta", replace
...
merge m:m date using "JOHN Uzbekistan.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Uzbekistan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Uzbekistan DELP daily deaths each update 
	...
	subtitle("Uzbekistan, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Uzbekistan DELP daily deaths update `update'.pdf", replace	
...
* graph Uzbekistan DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Uzbekistan, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uzbekistan DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Uzbekistan IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Uzbekistan, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Uzbekistan IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Uzbekistan.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Uzbekistan.dta"
...
save "IHME Uzbekistan.dta", replace
...
merge m:1 date using "JOHN Uzbekistan.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Uzbekistan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Uzbekistan IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Uzbekistan, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Uzbekistan IHME daily deaths update `update'.pdf", replace	
...
* graph Uzbekistan IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Uzbekistan, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uzbekistan IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Uzbekistan.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Uzbekistan.dta"
...
save "IMPE Uzbekistan.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Uzbekistan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Uzbekistan IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Uzbekistan, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Uzbekistan IMPE daily deaths update `update'.pdf", replace	
...
* graph Uzbekistan IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Uzbekistan, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uzbekistan IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Uzbekistan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Uzbekistan.dta"
...
save "LANL Uzbekistan.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Uzbekistan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Uzbekistan LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Uzbekistan, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Uzbekistan LANL daily deaths update `update'.pdf", replace	
...
* graph Uzbekistan LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Uzbekistan, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uzbekistan LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Uzbekistan.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Uzbekistan.dta", force
...
save "ALL MODLES Uzbekistan.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Uzbekistan data dictionary.csv", replace 
...
* graph Uzbekistan ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Uzbekistan, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(652 "JOHN" 1 "DELP" 55 "IHME" 3 "IMPE" 59 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Uzbekistan ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Uzbekistan 1 Process.smcl"
