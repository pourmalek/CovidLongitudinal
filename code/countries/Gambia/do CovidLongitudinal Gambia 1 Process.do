...
log using "log CovidLongitudinal Gambia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Gambia 1 Process.do"
...
**** "do CovidLongitudinal Gambia 1 Process.do"
...
** output data files: "`model' Gambia.dta"
**                   "ALL MODLES Gambia.dta"

** output data dictionary files: "Gambia ALL MODELS update dates data dictionary.csv"
**                               "`model' Gambia data dictionary.csv"
**                               "ALL MODLES Gambia data dictionary.csv"

** graphs:
** "graph 02 Gambia `model' C19 daily deaths all updates.pdf"
** "graph 02 Gambia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Gambia absent update_date_block"
	
	qui keep if loc_grand_name == "Gambia"
	...
	save "Gambia `model' update dates.dta", replace
...
use "Gambia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Gambia `model' update dates.dta"
...
save "Gambia ALL MODELS update dates.dta", replace
	
export excel using "Gambia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Gambia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Gambia"

keep date DayDeaMeSmJOHNGMB
...
save "JOHN Gambia.dta", replace 




* graph Gambia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNGMB date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Gambia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Gambia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Gambia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Gambia.dta"
...
merge m:m date using "JOHN Gambia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Gambia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Gambia DELP daily deaths each update 
	...
	subtitle("Gambia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Gambia DELP daily deaths update `update'.pdf", replace	
...
* graph Gambia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Gambia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Gambia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Gambia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Gambia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Gambia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Gambia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Gambia.dta"
...
merge m:1 date using "JOHN Gambia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Gambia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Gambia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Gambia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Gambia IHME daily deaths update `update'.pdf", replace	
...
* graph Gambia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Gambia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Gambia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Gambia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Gambia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Gambia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Gambia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Gambia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Gambia IMPE daily deaths update `update'.pdf", replace	
...
* graph Gambia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Gambia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Gambia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Gambia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Gambia.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Gambia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Gambia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Gambia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Gambia LANL daily deaths update `update'.pdf", replace	
...
* graph Gambia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Gambia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Gambia LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Gambia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Gambia.dta", force
...
save "ALL MODLES Gambia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Gambia data dictionary.csv", replace 
...
* graph Gambia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Gambia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(624 "JOHN" 64 "DELP" 45 "IHME" 1 "IMPE" 67 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Gambia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Gambia 1 Process.smcl"
