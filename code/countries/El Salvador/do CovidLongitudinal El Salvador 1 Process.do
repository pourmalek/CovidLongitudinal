...
log using "log CovidLongitudinal El Salvador 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal El Salvador 1 Process.do"
...
**** "do CovidLongitudinal El Salvador 1 Process.do"
...
** output data files: "`model' El Salvador.dta"
**                   "ALL MODLES El Salvador.dta"

** output data dictionary files: "El Salvador ALL MODELS update dates data dictionary.csv"
**                               "`model' El Salvador data dictionary.csv"
**                               "ALL MODLES El Salvador data dictionary.csv"

** graphs:
** "graph 02 El Salvador `model' C19 daily deaths all updates.pdf"
** "graph 02 El Salvador ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " El Salvador absent update_date_block"
	
	qui keep if loc_grand_name == "El Salvador"
	...
	save "El Salvador `model' update dates.dta", replace
...
use "El Salvador DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "El Salvador `model' update dates.dta"
...
save "El Salvador ALL MODELS update dates.dta", replace
	
export excel using "El Salvador ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "El Salvador ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "El Salvador"

keep date DayDeaMeSmJOHNSLV
...
save "JOHN El Salvador.dta", replace 




* graph El Salvador JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNSLV date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, El Salvador, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 El Salvador JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' El Salvador.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' El Salvador.dta"
...
save "DELP El Salvador.dta", replace
...
merge m:m date using "JOHN El Salvador.dta"
...
preserve
    ...
    export delimited name varlab using "DELP El Salvador data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph El Salvador DELP daily deaths each update 
	...
	subtitle("El Salvador, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 El Salvador DELP daily deaths update `update'.pdf", replace	
...
* graph El Salvador DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, El Salvador, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 El Salvador DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph El Salvador IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, El Salvador, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 El Salvador IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' El Salvador.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' El Salvador.dta"
...
save "IHME El Salvador.dta", replace
...
merge m:1 date using "JOHN El Salvador.dta"
...
preserve
    ...
    export delimited name varlab using "IHME El Salvador data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph El Salvador IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("El Salvador, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 El Salvador IHME daily deaths update `update'.pdf", replace	
...
* graph El Salvador IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, El Salvador, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 El Salvador IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' El Salvador.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' El Salvador.dta"
...
save "IMPE El Salvador.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE El Salvador data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph El Salvador IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("El Salvador, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 El Salvador IMPE daily deaths update `update'.pdf", replace	
...
* graph El Salvador IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, El Salvador, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 El Salvador IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' El Salvador.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' El Salvador.dta"
...
save "LANL El Salvador.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL El Salvador data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph El Salvador LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("El Salvador, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 El Salvador LANL daily deaths update `update'.pdf", replace	
...
* graph El Salvador LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, El Salvador, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 El Salvador LANL C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' El Salvador.dta", force
...
save "ALL MODLES El Salvador.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES El Salvador data dictionary.csv", replace 
...
* graph El Salvador ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, El Salvador, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(665 "JOHN" 1 "DELP" 68 "IHME" 3 "IMPE" 21 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 El Salvador ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal El Salvador 1 Process.smcl"
