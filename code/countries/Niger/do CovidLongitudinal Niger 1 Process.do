...
log using "log CovidLongitudinal Niger 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Niger 1 Process.do"
...
**** "do CovidLongitudinal Niger 1 Process.do"
...
** output data files: "`model' Niger.dta"
**                   "ALL MODLES Niger.dta"

** output data dictionary files: "Niger ALL MODELS update dates data dictionary.csv"
**                               "`model' Niger data dictionary.csv"
**                               "ALL MODLES Niger data dictionary.csv"

** graphs:
** "graph 02 Niger `model' C19 daily deaths all updates.pdf"
** "graph 02 Niger ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Niger absent update_date_block"
	
	qui keep if loc_grand_name == "Niger"
	...
	save "Niger `model' update dates.dta", replace
...
use "Niger DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Niger `model' update dates.dta"
...
save "Niger ALL MODELS update dates.dta", replace
	
export excel using "Niger ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Niger ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Niger"

keep date DayDeaMeSmJOHNNER
...
save "JOHN Niger.dta", replace 




* graph Niger JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNNER date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Niger, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Niger JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Niger.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Niger.dta"
...
merge m:m date using "JOHN Niger.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Niger data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Niger DELP daily deaths each update 
	...
	subtitle("Niger, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Niger DELP daily deaths update `update'.pdf", replace	
...
* graph Niger DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Niger, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Niger DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Niger IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Niger, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Niger IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Niger.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Niger.dta"
...
merge m:1 date using "JOHN Niger.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Niger data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Niger IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Niger, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Niger IHME daily deaths update `update'.pdf", replace	
...
* graph Niger IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Niger, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Niger IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Niger.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Niger.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Niger data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Niger IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Niger, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Niger IMPE daily deaths update `update'.pdf", replace	
...
* graph Niger IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Niger, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Niger IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Niger.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Niger.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Niger data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Niger LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Niger, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Niger LANL daily deaths update `update'.pdf", replace	
...
* graph Niger LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Niger, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Niger LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Niger.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Niger.dta", force
...
save "ALL MODLES Niger.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Niger data dictionary.csv", replace 
...
* graph Niger ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Niger, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(668 "JOHN" 1 "DELP" 72 "IHME" 4 "IMPE" 3 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Niger ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Niger 1 Process.smcl"
