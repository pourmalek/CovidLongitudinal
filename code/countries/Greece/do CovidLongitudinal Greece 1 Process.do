...
log using "log CovidLongitudinal Greece 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Greece 1 Process.do"
...
**** "do CovidLongitudinal Greece 1 Process.do"
...
** output data files: "`model' Greece.dta"
**                   "ALL MODLES Greece.dta"

** output data dictionary files: "Greece ALL MODELS update dates data dictionary.csv"
**                               "`model' Greece data dictionary.csv"
**                               "ALL MODLES Greece data dictionary.csv"

** graphs:
** "graph 02 Greece `model' C19 daily deaths all updates.pdf"
** "graph 02 Greece ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Greece absent update_date_block"
	
	qui keep if loc_grand_name == "Greece"
	...
	save "Greece `model' update dates.dta", replace
...
use "Greece DELP update dates.dta", clear 

local list DELP IHME IMPE LANL YYGU

foreach model of local list {
	
	append using "Greece `model' update dates.dta"
...
save "Greece ALL MODELS update dates.dta", replace
	
export excel using "Greece ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Greece ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Greece"

keep date DayDeaMeSmJOHNGRC
...
save "JOHN Greece.dta", replace 




* graph Greece JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNGRC date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Greece, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Greece JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Greece.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Greece.dta"
...
merge m:m date using "JOHN Greece.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Greece data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Greece DELP daily deaths each update 
	...
	subtitle("Greece, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Greece DELP daily deaths update `update'.pdf", replace	
...
* graph Greece DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Greece, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Greece DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Greece IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Greece, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Greece IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Greece.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Greece.dta"
...
merge m:1 date using "JOHN Greece.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Greece data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Greece IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Greece, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Greece IHME daily deaths update `update'.pdf", replace	
...
* graph Greece IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Greece, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Greece IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Greece.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Greece.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Greece data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Greece IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Greece, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Greece IMPE daily deaths update `update'.pdf", replace	
...
* graph Greece IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Greece, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Greece IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Greece.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Greece.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Greece data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Greece LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Greece, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Greece LANL daily deaths update `update'.pdf", replace	
...
* graph Greece LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Greece, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Greece LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Greece.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Greece.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Greece.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Greece data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Greece YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Greece, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Greece YYGU daily deaths update `update'.pdf", replace	
...
* graph Greece YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Greece, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Greece YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Greece.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Greece.dta", force
...
save "ALL MODLES Greece.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Greece data dictionary.csv", replace 
...
* graph Greece ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Greece, all models, all updates, forecast only", size(medium) color(black)) ///
...
qui graph export "graph 02 Greece ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Greece 1 Process.smcl"
