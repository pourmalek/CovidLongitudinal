...
log using "log CovidLongitudinal Namibia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Namibia 1 Process.do"
...
**** "do CovidLongitudinal Namibia 1 Process.do"
...
** output data files: "`model' Namibia.dta"
**                   "ALL MODLES Namibia.dta"

** output data dictionary files: "Namibia ALL MODELS update dates data dictionary.csv"
**                               "`model' Namibia data dictionary.csv"
**                               "ALL MODLES Namibia data dictionary.csv"

** graphs:
** "graph 02 Namibia `model' C19 daily deaths all updates.pdf"
** "graph 02 Namibia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Namibia absent update_date_block"
	
	qui keep if loc_grand_name == "Namibia"
	...
	save "Namibia `model' update dates.dta", replace
...
use "Namibia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Namibia `model' update dates.dta"
...
save "Namibia ALL MODELS update dates.dta", replace
	
export excel using "Namibia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Namibia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Namibia"

keep date DayDeaMeSmJOHNNAM
...
save "JOHN Namibia.dta", replace 




* graph Namibia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNNAM date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Namibia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Namibia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Namibia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Namibia.dta"
...
merge m:m date using "JOHN Namibia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Namibia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Namibia DELP daily deaths each update 
	...
	subtitle("Namibia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Namibia DELP daily deaths update `update'.pdf", replace	
...
* graph Namibia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Namibia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Namibia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Namibia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Namibia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Namibia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Namibia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Namibia.dta"
...
merge m:1 date using "JOHN Namibia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Namibia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Namibia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Namibia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Namibia IHME daily deaths update `update'.pdf", replace	
...
* graph Namibia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Namibia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Namibia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Namibia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Namibia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Namibia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Namibia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Namibia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Namibia IMPE daily deaths update `update'.pdf", replace	
...
* graph Namibia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Namibia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Namibia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Namibia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Namibia.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Namibia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Namibia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Namibia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Namibia LANL daily deaths update `update'.pdf", replace	
...
* graph Namibia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Namibia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Namibia LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Namibia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Namibia.dta", force
...
save "ALL MODLES Namibia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Namibia data dictionary.csv", replace 
...
* graph Namibia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Namibia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(570 "JOHN" 1 "DELP" 28 "IHME" 2 "IMPE" 17 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Namibia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Namibia 1 Process.smcl"
