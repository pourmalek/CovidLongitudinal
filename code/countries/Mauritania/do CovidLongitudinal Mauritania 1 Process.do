...
log using "log CovidLongitudinal Mauritania 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Mauritania 1 Process.do"
...
**** "do CovidLongitudinal Mauritania 1 Process.do"
...
** output data files: "`model' Mauritania.dta"
**                   "ALL MODLES Mauritania.dta"

** output data dictionary files: "Mauritania ALL MODELS update dates data dictionary.csv"
**                               "`model' Mauritania data dictionary.csv"
**                               "ALL MODLES Mauritania data dictionary.csv"

** graphs:
** "graph 02 Mauritania `model' C19 daily deaths all updates.pdf"
** "graph 02 Mauritania ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Mauritania absent update_date_block"
	
	qui keep if loc_grand_name == "Mauritania"
	...
	save "Mauritania `model' update dates.dta", replace
...
use "Mauritania DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Mauritania `model' update dates.dta"
...
save "Mauritania ALL MODELS update dates.dta", replace
	
export excel using "Mauritania ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Mauritania ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Mauritania"

keep date DayDeaMeSmJOHNMRT
...
save "JOHN Mauritania.dta", replace 




* graph Mauritania JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMRT date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Mauritania, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mauritania JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Mauritania.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Mauritania.dta"
...
save "DELP Mauritania.dta", replace
...
merge m:m date using "JOHN Mauritania.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Mauritania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mauritania DELP daily deaths each update 
	...
	subtitle("Mauritania, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mauritania DELP daily deaths update `update'.pdf", replace	
...
* graph Mauritania DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mauritania, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mauritania DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Mauritania IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Mauritania, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Mauritania IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Mauritania.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Mauritania.dta"
...
save "IHME Mauritania.dta", replace
...
merge m:1 date using "JOHN Mauritania.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Mauritania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mauritania IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mauritania, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mauritania IHME daily deaths update `update'.pdf", replace	
...
* graph Mauritania IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mauritania, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mauritania IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Mauritania.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Mauritania.dta"
...
save "IMPE Mauritania.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Mauritania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mauritania IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mauritania, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mauritania IMPE daily deaths update `update'.pdf", replace	
...
* graph Mauritania IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mauritania, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mauritania IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Mauritania.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Mauritania.dta"
...
save "LANL Mauritania.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Mauritania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mauritania LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mauritania, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mauritania LANL daily deaths update `update'.pdf", replace	
...
* graph Mauritania LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mauritania, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mauritania LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Mauritania.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Mauritania.dta", force
...
save "ALL MODLES Mauritania.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Mauritania data dictionary.csv", replace 
...
* graph Mauritania ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mauritania, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(653 "JOHN" 33 "DELP" 57 "IHME" 1 "IMPE" 35 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mauritania ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Mauritania 1 Process.smcl"
