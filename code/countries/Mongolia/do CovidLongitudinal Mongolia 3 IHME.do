...
log using "log CovidLongitudinal Mongolia 3 IHME.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Mongolia 3 IHME.do"
...
* input data files: "IHME Mongolia.dta"
* output data files: "IHME Mongolia error.dta" (with error measures saved)

* output data dictionary files: "IHME Mongolia error data dictionary.csv"
...
** graphs:
* "graph 03 Mongolia IHME C19 daily deaths error.pdf"
* "graph 04 Mongolia IHME C19 daily deaths absolute error.pdf"
* "graph 05 Mongolia IHME C19 daily deaths percent error.pdf"
* "graph 06 Mongolia IHME C19 daily deaths absolute percent error.pdf"

* "graph 07 Mongolia IHME C19 daily deaths average median error.pdf"
* "graph 08 Mongolia IHME C19 daily deaths average median absolute error.pdf"
* "graph 09 Mongolia IHME C19 daily deaths average median percent error.pdf"
* "graph 10 Mongolia IHME C19 daily deaths average median absolute percent error.pdf"

* "graph 11 Mongolia IHME C19 daily deaths Average MAPE over updates and calendar months.pdf"
...
qui replace DDErrorIHME`update' = . if date < td(18feb2022) // 18feb2022 is the earliest date of IHME forecasts for Mongolia
                                                            // <<-- modify 3
...
qui replace DDAbsErrIHME`update' = . if date < td(18feb2022) // 18feb2022 is the earliest date of IHME forecasts for Mongolia
...
qui replace DDPerErrIHME`update' = . if date < td(18feb2022) // 18feb2022 is the earliest date of IHME forecasts for Mongolia
...
qui replace DDAbPeErIHME`update' = . if date < td(18feb2022) // 18feb2022 is the earliest date of IHME forecasts for Mongolia
...
qui replace DDErrorIHME`update'_Med1 = . if date < td(18feb2022) // 18feb2022 is the earliest date of IHME forecasts for Mongolia
...
qui replace DDAbsErrIHME`update'_Med1 = . if date < td(18feb2022) // 18feb2022 is the earliest date of IHME forecasts for Mongolia
...
qui replace DDPerErrIHME`update'_Med1 = . if date < td(18feb2022) // 18feb2022 is the earliest date of IHME forecasts for Mongolia
...
qui replace DDAbPeErIHME`update'_Med1 = . if date < td(18feb2022) // 18feb2022 is the earliest date of IHME forecasts for Mongolia
...
qui replace DDErrorIHME_Mean1 = . if date < td(18feb2022) // 18feb2022 is the earliest date of IHME forecasts for Mongolia
...
qui replace DDAbsErrIHME_Mean1 = . if date < td(18feb2022) // 18feb2022 is the earliest date of IHME forecasts for Mongolia
...
qui replace DDPerErrIHME_Mean1 = . if date < td(18feb2022) // 18feb2022 is the earliest date of IHME forecasts for Mongolia
...
qui replace DDAbPeErIHME_Mean1 = . if date < td(18feb2022) // 18feb2022 is the earliest date of IHME forecasts for Mongolia
...
subtitle("Mongolia, IHME, all updates, forecast only", size(small)) 

qui graph export "graph 03 Mongolia IHME C19 daily deaths error.pdf", replace
...
qui graph export "graph 04 Mongolia IHME C19 daily deaths absolute error.pdf", replace
...
qui graph export "graph 05 Mongolia IHME C19 daily deaths percent error.pdf", replace
...
qui graph export "graph 06 Mongolia IHME C19 daily deaths absolute percent error.pdf", replace
...
subtitle("Mongolia, IHME, forecast only", size(small)) xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(off) yscale(titlegap(3))

qui graph export "graph 07 Mongolia IHME C19 daily deaths average median error.pdf", replace
...
qui graph export "graph 08 Mongolia IHME C19 daily deaths average median absolute error.pdf", replace
...
qui graph export "graph 09 Mongolia IHME C19 daily deaths average median percent error.pdf", replace
...
qui graph export "graph 10 Mongolia IHME C19 daily deaths average median absolute percent error.pdf", replace
...
subtitle("Mongolia, IHME, forecast only. MAPE: Median Absolute Percent Error", size(small)) /// 
legend(off) ylabel(, labsize(small) angle(forty_five) format(%30.0fc))

qui graph export "graph 11 Mongolia IHME C19 daily deaths Average MAPE over updates and calendar months.pdf", replace
...
save "IHME Mongolia error.dta", replace 
...
preserve
    ...
    export delimited name varlab using "IHME Mongolia error data dictionary.csv", replace 
...
view "log CovidLongitudinal Mongolia 3 IHME.smcl"
