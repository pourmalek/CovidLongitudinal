...
log using "log CovidLongitudinal Korea North 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Korea North 1 Process.do"
...
**** "do CovidLongitudinal Korea North 1 Process.do"
...
** output data files: "`model' Korea North.dta"
**                   "ALL MODLES Korea North.dta"

** output data dictionary files: "Korea North ALL MODELS update dates data dictionary.csv"
**                               "`model' Korea North data dictionary.csv"
**                               "ALL MODLES Korea North data dictionary.csv"

** graphs:
** "graph 02 Korea North `model' C19 daily deaths all updates.pdf"
** "graph 02 Korea North ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Korea North absent update_date_block"
	
	qui keep if loc_grand_name == "Korea North"
	...
	save "Korea North `model' update dates.dta", replace
...
use "Korea North IMPE update dates.dta", clear 

local list IMPE

foreach model of local list {
	
	append using "Korea North `model' update dates.dta"
...
save "Korea North ALL MODELS update dates.dta", replace
	
export excel using "Korea North ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Korea North ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Korea North"

keep date DayDeaMeSmJOHNPRK
...
save "JOHN Korea North.dta", replace 




* graph Korea North JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNPRK date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Korea North, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Korea North JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Korea North.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Korea North.dta"
...
save "IMPE Korea North.dta", replace
...
merge m:1 date using "JOHN Korea North.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Korea North data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Korea North IMPE daily deaths each update 
	...
	subtitle("Korea North, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Korea North IMPE daily deaths update `update'.pdf", replace	
...
* graph Korea North IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Korea North, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Korea North IMPE C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Korea North.dta", force
...
save "ALL MODLES Korea North.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Korea North data dictionary.csv", replace 
...
* graph Korea North ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Korea North, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(66 "JOHN" 1 "IMPE") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Korea North ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Korea North 1 Process.smcl"
