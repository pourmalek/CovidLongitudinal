...
log using "log CovidLongitudinal Ghana 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Ghana 1 Process.do"
...
**** "do CovidLongitudinal Ghana 1 Process.do"
...
** output data files: "`model' Ghana.dta"
**                   "ALL MODLES Ghana.dta"

** output data dictionary files: "Ghana ALL MODELS update dates data dictionary.csv"
**                               "`model' Ghana data dictionary.csv"
**                               "ALL MODLES Ghana data dictionary.csv"

** graphs:
** "graph 02 Ghana `model' C19 daily deaths all updates.pdf"
** "graph 02 Ghana ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Ghana absent update_date_block"
	
	qui keep if loc_grand_name == "Ghana"
	...
	save "Ghana `model' update dates.dta", replace
...
use "Ghana DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Ghana `model' update dates.dta"
...
save "Ghana ALL MODELS update dates.dta", replace
	
export excel using "Ghana ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Ghana ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Ghana"

keep date DayDeaMeSmJOHNGHA
...
save "JOHN Ghana.dta", replace 




* graph Ghana JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNGHA date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Ghana, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ghana JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Ghana.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Ghana.dta"
...
merge m:m date using "JOHN Ghana.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Ghana data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ghana DELP daily deaths each update 
	...
	subtitle("Ghana, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ghana DELP daily deaths update `update'.pdf", replace	
...
* graph Ghana DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ghana, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ghana DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Ghana IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Ghana, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Ghana IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Ghana.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Ghana.dta"
...
merge m:1 date using "JOHN Ghana.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Ghana data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ghana IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ghana, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ghana IHME daily deaths update `update'.pdf", replace	
...
* graph Ghana IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ghana, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ghana IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Ghana.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Ghana.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Ghana data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ghana IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ghana, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ghana IMPE daily deaths update `update'.pdf", replace	
...
* graph Ghana IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ghana, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ghana IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Ghana.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Ghana.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Ghana data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Ghana LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Ghana, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Ghana LANL daily deaths update `update'.pdf", replace	
...
* graph Ghana LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ghana, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ghana LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Ghana.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Ghana.dta", force
...
save "ALL MODLES Ghana.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Ghana data dictionary.csv", replace 
...
* graph Ghana ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Ghana, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(665 "JOHN" 1 "DELP" 69 "IHME" 3 "IMPE" 18 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Ghana ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Ghana 1 Process.smcl"
