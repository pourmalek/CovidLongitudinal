...
log using "log CovidLongitudinal Seychelles 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Seychelles 1 Process.do"
...
**** "do CovidLongitudinal Seychelles 1 Process.do"
...
** output data files: "`model' Seychelles.dta"
**                   "ALL MODLES Seychelles.dta"

** output data dictionary files: "Seychelles ALL MODELS update dates data dictionary.csv"
**                               "`model' Seychelles data dictionary.csv"
**                               "ALL MODLES Seychelles data dictionary.csv"

** graphs:
** "graph 02 Seychelles `model' C19 daily deaths all updates.pdf"
** "graph 02 Seychelles ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Seychelles absent update_date_block"
	
	qui keep if loc_grand_name == "Seychelles"
	...
	save "Seychelles `model' update dates.dta", replace
...
use "Seychelles DELP update dates.dta", clear 

local list DELP LANL

foreach model of local list {
	
	append using "Seychelles `model' update dates.dta"
...
save "Seychelles ALL MODELS update dates.dta", replace
	
export excel using "Seychelles ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Seychelles ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Seychelles"

keep date DayDeaMeSmJOHNSYC
...
save "JOHN Seychelles.dta", replace 




* graph Seychelles JOHN daily deaths
...
ytitle(Daily deaths) title("C19 daily deaths, Seychelles, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Seychelles JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Seychelles.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Seychelles.dta"
...
save "DELP Seychelles.dta", replace
...
merge m:m date using "JOHN Seychelles.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Seychelles data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Seychelles DELP daily deaths each update 
	...
	qui graph export "graph 01 Seychelles DELP daily deaths update `update'.pdf", replace	
...
* graph Seychelles DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Seychelles, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Seychelles DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Seychelles.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Seychelles.dta"
...
save "LANL Seychelles.dta", replace
...
merge m:1 date using "JOHN Seychelles.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Seychelles data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Seychelles LANL daily deaths each update 
	...
	qui graph export "graph 01 Seychelles LANL daily deaths update `update'.pdf", replace	
...
* graph Seychelles LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.1fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Seychelles, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Seychelles LANL C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Seychelles.dta", force
...
save "ALL MODLES Seychelles.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Seychelles data dictionary.csv", replace 
...
* graph Seychelles ALL MODELS daily deaths all updates
...
qui graph export "graph 02 Seychelles ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Seychelles 1 Process.smcl"
