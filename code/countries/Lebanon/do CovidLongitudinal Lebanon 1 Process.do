...
log using "log CovidLongitudinal Lebanon 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Lebanon 1 Process.do"
...
**** "do CovidLongitudinal Lebanon 1 Process.do"
...
** output data files: "`model' Lebanon.dta"
**                   "ALL MODLES Lebanon.dta"

** output data dictionary files: "Lebanon ALL MODELS update dates data dictionary.csv"
**                               "`model' Lebanon data dictionary.csv"
**                               "ALL MODLES Lebanon data dictionary.csv"

** graphs:
** "graph 02 Lebanon `model' C19 daily deaths all updates.pdf"
** "graph 02 Lebanon ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Lebanon absent update_date_block"
	
	qui keep if loc_grand_name == "Lebanon"
	...
	save "Lebanon `model' update dates.dta", replace
...
use "Lebanon DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Lebanon `model' update dates.dta"
...
save "Lebanon ALL MODELS update dates.dta", replace
	
export excel using "Lebanon ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Lebanon ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Lebanon"

keep date DayDeaMeSmJOHNLBN
...
save "JOHN Lebanon.dta", replace 




* graph Lebanon JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNLBN date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Lebanon, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lebanon JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Lebanon.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Lebanon.dta"
...
merge m:m date using "JOHN Lebanon.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Lebanon data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Lebanon DELP daily deaths each update 
	...
	subtitle("Lebanon, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Lebanon DELP daily deaths update `update'.pdf", replace	
...
* graph Lebanon DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lebanon, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lebanon DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Lebanon IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Lebanon, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Lebanon IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Lebanon.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Lebanon.dta"
...
merge m:1 date using "JOHN Lebanon.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Lebanon data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Lebanon IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Lebanon, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Lebanon IHME daily deaths update `update'.pdf", replace	
...
* graph Lebanon IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lebanon, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lebanon IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Lebanon.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Lebanon.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Lebanon data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Lebanon IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Lebanon, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Lebanon IMPE daily deaths update `update'.pdf", replace	
...
* graph Lebanon IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lebanon, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lebanon IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Lebanon.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Lebanon.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Lebanon data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Lebanon LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Lebanon, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Lebanon LANL daily deaths update `update'.pdf", replace	
...
* graph Lebanon LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lebanon, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lebanon LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Lebanon.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Lebanon.dta", force
...
save "ALL MODLES Lebanon.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Lebanon data dictionary.csv", replace 
...
* graph Lebanon ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Lebanon, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(669 "JOHN" 1 "DELP" 72 "IHME" 4 "IMPE" 3 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Lebanon ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Lebanon 1 Process.smcl"
