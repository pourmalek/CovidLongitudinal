...
log using "log CovidLongitudinal Angola 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Angola 1 Process.do"
...
**** "do CovidLongitudinal Angola 1 Process.do"
...
** output data files: "`model' Angola.dta"
**                   "ALL MODLES Angola.dta"

** output data dictionary files: "Angola ALL MODELS update dates data dictionary.csv"
**                               "`model' Angola data dictionary.csv"
**                               "ALL MODLES Angola data dictionary.csv"

** graphs:
** "graph 02 Angola `model' C19 daily deaths all updates.pdf"
** "graph 02 Angola ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Angola absent update_date_block"
	
	qui keep if loc_grand_name == "Angola"
	...
	save "Angola `model' update dates.dta", replace
...
use "Angola DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Angola `model' update dates.dta"
...
save "Angola ALL MODELS update dates.dta", replace
	
export excel using "Angola ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Angola ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Angola"

keep date DayDeaMeSmJOHNAGO
...
save "JOHN Angola.dta", replace 




* graph Angola JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNAGO date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Angola, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Angola JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Angola.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Angola.dta"
...
merge m:m date using "JOHN Angola.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Angola data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Angola DELP daily deaths each update 
	...
	subtitle("Angola, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Angola DELP daily deaths update `update'.pdf", replace	
...
* graph Angola DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Angola, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Angola DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Angola IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Angola, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Angola IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Angola.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Angola.dta"
...
merge m:1 date using "JOHN Angola.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Angola data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Angola IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Angola, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Angola IHME daily deaths update `update'.pdf", replace	
...
* graph Angola IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Angola, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Angola IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Angola.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Angola.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Angola data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Angola IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Angola, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Angola IMPE daily deaths update `update'.pdf", replace	
...
* graph Angola IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Angola, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Angola IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Angola.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Angola.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Angola data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Angola LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Angola, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Angola LANL daily deaths update `update'.pdf", replace	
...
* graph Angola LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Angola, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Angola LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Angola.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Angola.dta", force
...
save "ALL MODLES Angola.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Angola data dictionary.csv", replace 
...
* graph Angola ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Angola, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(640 "JOHN" 43 "DELP" 46 "IHME" 1 "IMPE" 57 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Angola ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Angola 1 Process.smcl"
