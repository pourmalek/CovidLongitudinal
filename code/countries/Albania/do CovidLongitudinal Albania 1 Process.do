...
log using "log CovidLongitudinal Albania 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Albania 1 Process.do"
...
**** "do CovidLongitudinal Albania 1 Process.do"
...
** output data files: "`model' Albania.dta"
**                   "ALL MODLES Albania.dta"

** output data dictionary files: "Albania ALL MODELS update dates data dictionary.csv"
**                               "`model' Albania data dictionary.csv"
**                               "ALL MODLES Albania data dictionary.csv"

** graphs:
** "graph 02 Albania `model' C19 daily deaths all updates.pdf"
** "graph 02 Albania ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Albania absent update_date_block"
	
	qui keep if loc_grand_name == "Albania"
	...
	save "Albania `model' update dates.dta", replace
...
use "Albania DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Albania `model' update dates.dta"
...
save "Albania ALL MODELS update dates.dta", replace
	
export excel using "Albania ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Albania ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Albania"

keep date DayDeaMeSmJOHNALB
...
save "JOHN Albania.dta", replace 




* graph Albania JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNALB date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Albania, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Albania JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Albania.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Albania.dta"
...
merge m:m date using "JOHN Albania.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Albania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Albania DELP daily deaths each update 
	...
	subtitle("Albania, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Albania DELP daily deaths update `update'.pdf", replace	
...
* graph Albania DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Albania, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Albania DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Albania IHME daily deaths epoch each update 
	...
	*/
	...
	/* qui graph export "graph 00 Albania IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Albania.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Albania.dta"
...
merge m:1 date using "JOHN Albania.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Albania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Albania IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Albania, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Albania IHME daily deaths update `update'.pdf", replace	
...
* graph Albania IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Albania, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Albania IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Albania.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Albania.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Albania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Albania IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Albania, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Albania IMPE daily deaths update `update'.pdf", replace	
...
* graph Albania IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Albania, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Albania IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Albania.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Albania.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Albania data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Albania LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Albania, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Albania LANL daily deaths update `update'.pdf", replace	
...
* graph Albania LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Albania, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Albania LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Albania.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Albania.dta", force
...
save "ALL MODLES Albania.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Albania data dictionary.csv", replace 
...
* graph Albania ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Albania, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(670 "JOHN" 1 "DELP" 73 "IHME" 4 "IMPE" 3 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Albania ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Albania 1 Process.smcl"
