...
log using "log CovidLongitudinal Mauritius 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Mauritius 1 Process.do"
...
**** "do CovidLongitudinal Mauritius 1 Process.do"
...
** output data files: "`model' Mauritius.dta"
**                   "ALL MODLES Mauritius.dta"

** output data dictionary files: "Mauritius ALL MODELS update dates data dictionary.csv"
**                               "`model' Mauritius data dictionary.csv"
**                               "ALL MODLES Mauritius data dictionary.csv"

** graphs:
** "graph 02 Mauritius `model' C19 daily deaths all updates.pdf"
** "graph 02 Mauritius ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Mauritius absent update_date_block"
	
	qui keep if loc_grand_name == "Mauritius"
	...
	save "Mauritius `model' update dates.dta", replace
...
use "Mauritius DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Mauritius `model' update dates.dta"
...
save "Mauritius ALL MODELS update dates.dta", replace
	
export excel using "Mauritius ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Mauritius ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Mauritius"

keep date DayDeaMeSmJOHNMUS
...
save "JOHN Mauritius.dta", replace 




* graph Mauritius JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMUS date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Mauritius, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mauritius JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Mauritius.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Mauritius.dta"
...
save "DELP Mauritius.dta", replace
...
merge m:m date using "JOHN Mauritius.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Mauritius data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mauritius DELP daily deaths each update 
	...
	subtitle("Mauritius, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mauritius DELP daily deaths update `update'.pdf", replace	
...
* graph Mauritius DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mauritius, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mauritius DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Mauritius IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Mauritius, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Mauritius IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Mauritius.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Mauritius.dta"
...
save "IHME Mauritius.dta", replace
...
merge m:1 date using "JOHN Mauritius.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Mauritius data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mauritius IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mauritius, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mauritius IHME daily deaths update `update'.pdf", replace	
...
* graph Mauritius IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mauritius, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mauritius IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Mauritius.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Mauritius.dta"
...
save "IMPE Mauritius.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Mauritius data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mauritius IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mauritius, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mauritius IMPE daily deaths update `update'.pdf", replace	
...
* graph Mauritius IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mauritius, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mauritius IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Mauritius.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Mauritius.dta"
...
save "LANL Mauritius.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Mauritius data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Mauritius LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Mauritius, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Mauritius LANL daily deaths update `update'.pdf", replace	
...
* graph Mauritius LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mauritius, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mauritius LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Mauritius.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Mauritius.dta", force
...
save "ALL MODLES Mauritius.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Mauritius data dictionary.csv", replace 
...
* graph Mauritius ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Mauritius, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(535 "JOHN" 1 "DELP" 55 "IHME" 3 "IMPE" 216 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Mauritius ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Mauritius 1 Process.smcl"
