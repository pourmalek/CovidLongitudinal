...
log using "log CovidLongitudinal Saint Vincent and the Grenadines 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Saint Vincent and the Grenadines 1 Process.do"
...
**** "do CovidLongitudinal Saint Vincent and the Grenadines 1 Process.do"
...
** output data files: "`model' Saint Vincent and the Grenadines.dta"
**                   "ALL MODLES Saint Vincent and the Grenadines.dta"

** output data dictionary files: "Saint Vincent and the Grenadines ALL MODELS update dates data dictionary.csv"
**                               "`model' Saint Vincent and the Grenadines data dictionary.csv"
**                               "ALL MODLES Saint Vincent and the Grenadines data dictionary.csv"

** graphs:
** "graph 02 Saint Vincent and the Grenadines `model' C19 daily deaths all updates.pdf"
** "graph 02 Saint Vincent and the Grenadines ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Saint Vincent and the Grenadines absent update_date_block"
	
	qui keep if loc_grand_name == "Saint Vincent and the Grenadines"
	...
	save "Saint Vincent and the Grenadines `model' update dates.dta", replace
...
use "Saint Vincent and the Grenadines IMPE update dates.dta", clear 

local list IMPE

foreach model of local list {
	
	append using "Saint Vincent and the Grenadines `model' update dates.dta"
...
save "Saint Vincent and the Grenadines ALL MODELS update dates.dta", replace
	
export excel using "Saint Vincent and the Grenadines ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Saint Vincent and the Grenadines ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Saint Vincent and the Grenadines"

keep date DayDeaMeSmJOHNVCT
...
save "JOHN Saint Vincent and the Grenadines.dta", replace 




* graph Saint Vincent and the Grenadines JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNVCT date, sort lwidth(thick) lcolor(cyan)) ///
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Saint Vincent and the Grenadines.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Saint Vincent and the Grenadines.dta"
...
save "IMPE Saint Vincent and the Grenadines.dta", replace
...
merge m:1 date using "JOHN Saint Vincent and the Grenadines.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Saint Vincent and the Grenadines data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Saint Vincent and the Grenadines IMPE daily deaths each update 
	...
	subtitle("Saint Vincent and the Grenadines, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Saint Vincent and the Grenadines IMPE daily deaths update `update'.pdf", replace	
...
* graph Saint Vincent and the Grenadines IMPE daily deaths all updates
...
qui graph export "graph 02 Saint Vincent and the Grenadines IMPE C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Saint Vincent and the Grenadines.dta", force
...
save "ALL MODLES Saint Vincent and the Grenadines.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Saint Vincent and the Grenadines data dictionary.csv", replace 
...
* graph Saint Vincent and the Grenadines ALL MODELS daily deaths all updates
...
qui graph export "graph 02 Saint Vincent and the Grenadines ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Saint Vincent and the Grenadines 1 Process.smcl"
