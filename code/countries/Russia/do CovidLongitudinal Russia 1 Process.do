...
log using "log CovidLongitudinal Russia 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Russia 1 Process.do"
...
**** "do CovidLongitudinal Russia 1 Process.do"
...
** output data files: "`model' Russia.dta"
**                   "ALL MODLES Russia.dta"

** output data dictionary files: "Russia ALL MODELS update dates data dictionary.csv"
**                               "`model' Russia data dictionary.csv"
**                               "ALL MODLES Russia data dictionary.csv"

** graphs:
** "graph 02 Russia `model' C19 daily deaths all updates.pdf"
** "graph 02 Russia ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Russia absent update_date_block"
	
	qui keep if loc_grand_name == "Russia"
	...
	save "Russia `model' update dates.dta", replace
...
use "Russia DELP update dates.dta", clear 

local list DELP IHME IMPE LANL UCLA YYGU

foreach model of local list {
	
	append using "Russia `model' update dates.dta"
...
save "Russia ALL MODELS update dates.dta", replace
	
export excel using "Russia ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Russia ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Russia"

keep date DayDeaMeSmJOHNRUS
...
save "JOHN Russia.dta", replace 




* graph Russia JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNRUS date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Russia, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Russia JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Russia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Russia.dta"
...
merge m:m date using "JOHN Russia.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Russia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Russia DELP daily deaths each update 
	...
	subtitle("Russia, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Russia DELP daily deaths update `update'.pdf", replace	
...
* graph Russia DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Russia, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Russia DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Russia IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Russia, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Russia IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Russia.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Russia.dta"
...
merge m:1 date using "JOHN Russia.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Russia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Russia IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Russia, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Russia IHME daily deaths update `update'.pdf", replace	
...
* graph Russia IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Russia, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Russia IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Russia.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Russia.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Russia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Russia IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Russia, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Russia IMPE daily deaths update `update'.pdf", replace	
...
* graph Russia IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Russia, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Russia IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Russia.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Russia.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Russia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Russia LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Russia, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Russia LANL daily deaths update `update'.pdf", replace	
...
* graph Russia LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Russia, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Russia LANL C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only  
	...
	save "UCLA `update' Russia.dta", replace 
	
	/* line DayDeaMeRaUCLA`update' DayDeaMeSmUCLA`update' DayDeaMeFoUCLA`update' date

	qui graph export "UCLA `update' Russia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "UCLA `update' Russia.dta"
...
preserve
    ...
    export delimited name varlab using "UCLA Russia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Russia UCLA daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Russia, forecast only") legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Russia UCLA daily deaths update `update'.pdf", replace	

}
*/


* graph Russia UCLA daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Russia, UCLA, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "UCLA forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Russia UCLA C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen FORE, that is forecast only 
	...
	save "YYGU `update' Russia.dta", replace 
	
	/* line DayDeaMeRaYYGU`update' DayDeaMeSmYYGU`update' DayDeaMeFoYYGU`update' date

	qui graph export "YYGU `update' Russia.pdf", replace */
...
foreach update of local list {
	...
	merge m:m date using "YYGU `update' Russia.dta"
...
preserve
    ...
    export delimited name varlab using "YYGU Russia data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Russia YYGU daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Russia, forecast only") legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Russia YYGU daily deaths update `update'.pdf", replace	
...
* graph Russia YYGU daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Russia, YYGU, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "YYGU forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Russia YYGU C19 daily deaths all updates.pdf", replace
...
use "DELP Russia.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Russia.dta", force
...
save "ALL MODLES Russia.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Russia data dictionary.csv", replace 
...
* graph Russia ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Russia, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(880 "JOHN" 6 "DELP" 85 "IHME" 20 "IMPE" 83 "LANL" 398 "UCLA" 1 "YYGU") ///
rows(2) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Russia ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Russia 1 Process.smcl"
