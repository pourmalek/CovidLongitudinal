...
log using "log CovidLongitudinal Paraguay 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Paraguay 1 Process.do"
...
**** "do CovidLongitudinal Paraguay 1 Process.do"
...
** output data files: "`model' Paraguay.dta"
**                   "ALL MODLES Paraguay.dta"

** output data dictionary files: "Paraguay ALL MODELS update dates data dictionary.csv"
**                               "`model' Paraguay data dictionary.csv"
**                               "ALL MODLES Paraguay data dictionary.csv"

** graphs:
** "graph 02 Paraguay `model' C19 daily deaths all updates.pdf"
** "graph 02 Paraguay ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Paraguay absent update_date_block"
	
	qui keep if loc_grand_name == "Paraguay"
	...
	save "Paraguay `model' update dates.dta", replace
...
use "Paraguay DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Paraguay `model' update dates.dta"
...
save "Paraguay ALL MODELS update dates.dta", replace
	
export excel using "Paraguay ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Paraguay ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Paraguay"

keep date DayDeaMeSmJOHNPRY
...
save "JOHN Paraguay.dta", replace 




* graph Paraguay JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNPRY date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Paraguay, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Paraguay JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Paraguay.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Paraguay.dta"
...
save "DELP Paraguay.dta", replace
...
merge m:m date using "JOHN Paraguay.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Paraguay data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Paraguay DELP daily deaths each update 
	...
	subtitle("Paraguay, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Paraguay DELP daily deaths update `update'.pdf", replace	
...
* graph Paraguay DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Paraguay, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Paraguay DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Paraguay IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Paraguay, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Paraguay IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Paraguay.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Paraguay.dta"
...
save "IHME Paraguay.dta", replace
...
merge m:1 date using "JOHN Paraguay.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Paraguay data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Paraguay IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Paraguay, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Paraguay IHME daily deaths update `update'.pdf", replace	
...
* graph Paraguay IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Paraguay, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Paraguay IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Paraguay.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Paraguay.dta"
...
save "IMPE Paraguay.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Paraguay data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Paraguay IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Paraguay, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Paraguay IMPE daily deaths update `update'.pdf", replace	
...
* graph Paraguay IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Paraguay, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Paraguay IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Paraguay.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Paraguay.dta"
...
save "LANL Paraguay.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Paraguay data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Paraguay LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Paraguay, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Paraguay LANL daily deaths update `update'.pdf", replace	
...
* graph Paraguay LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Paraguay, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Paraguay LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Paraguay.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Paraguay.dta", force
...
save "ALL MODLES Paraguay.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Paraguay data dictionary.csv", replace 
...
* graph Paraguay ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Paraguay, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(650 "JOHN" 1 "DELP" 55 "IHME" 3 "IMPE" 64 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Paraguay ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Paraguay 1 Process.smcl"
