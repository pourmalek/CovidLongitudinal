...
log using "log CovidLongitudinal Belize 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Belize 1 Process.do"
...
**** "do CovidLongitudinal Belize 1 Process.do"
...
** output data files: "`model' Belize.dta"
**                   "ALL MODLES Belize.dta"

** output data dictionary files: "Belize ALL MODELS update dates data dictionary.csv"
**                               "`model' Belize data dictionary.csv"
**                               "ALL MODLES Belize data dictionary.csv"

** graphs:
** "graph 02 Belize `model' C19 daily deaths all updates.pdf"
** "graph 02 Belize ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Belize absent update_date_block"
	
	qui keep if loc_grand_name == "Belize"
	...
	save "Belize `model' update dates.dta", replace
...
use "Belize DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Belize `model' update dates.dta"
...
save "Belize ALL MODELS update dates.dta", replace
	
export excel using "Belize ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Belize ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Belize"

keep date DayDeaMeSmJOHNBLZ
...
save "JOHN Belize.dta", replace 




* graph Belize JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNBLZ date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Belize, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belize JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Belize.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Belize.dta"
...
merge m:m date using "JOHN Belize.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Belize data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Belize DELP daily deaths each update 
	...
	subtitle("Belize, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Belize DELP daily deaths update `update'.pdf", replace	
...
* graph Belize DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belize, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belize DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Belize IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Belize, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Belize IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Belize.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Belize.dta"
...
merge m:1 date using "JOHN Belize.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Belize data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Belize IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Belize, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Belize IHME daily deaths update `update'.pdf", replace	
...
* graph Belize IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belize, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belize IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Belize.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Belize.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Belize data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Belize IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Belize, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Belize IMPE daily deaths update `update'.pdf", replace	
...
* graph Belize IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belize, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belize IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Belize.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Belize.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Belize data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Belize LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Belize, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Belize LANL daily deaths update `update'.pdf", replace	
...
* graph Belize LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belize, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belize LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Belize.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Belize.dta", force
...
save "ALL MODLES Belize.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Belize data dictionary.csv", replace 
...
* graph Belize ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Belize, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(546 "JOHN" 71 "DELP" 318 "IHME" 1 "IMPE" 80 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Belize ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Belize 1 Process.smcl"
