...
log using "log CovidLongitudinal Montenegro 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Montenegro 1 Process.do"
...
**** "do CovidLongitudinal Montenegro 1 Process.do"
...
** output data files: "`model' Montenegro.dta"
**                   "ALL MODLES Montenegro.dta"

** output data dictionary files: "Montenegro ALL MODELS update dates data dictionary.csv"
**                               "`model' Montenegro data dictionary.csv"
**                               "ALL MODLES Montenegro data dictionary.csv"

** graphs:
** "graph 02 Montenegro `model' C19 daily deaths all updates.pdf"
** "graph 02 Montenegro ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Montenegro absent update_date_block"
	
	qui keep if loc_grand_name == "Montenegro"
	...
	save "Montenegro `model' update dates.dta", replace
...
use "Montenegro DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Montenegro `model' update dates.dta"
...
save "Montenegro ALL MODELS update dates.dta", replace
	
export excel using "Montenegro ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Montenegro ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Montenegro"

keep date DayDeaMeSmJOHNMNE
...
save "JOHN Montenegro.dta", replace 




* graph Montenegro JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNMNE date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Montenegro, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Montenegro JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Montenegro.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Montenegro.dta"
...
save "DELP Montenegro.dta", replace
...
merge m:m date using "JOHN Montenegro.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Montenegro data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Montenegro DELP daily deaths each update 
	...
	subtitle("Montenegro, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Montenegro DELP daily deaths update `update'.pdf", replace	
...
* graph Montenegro DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Montenegro, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Montenegro DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Montenegro IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Montenegro, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Montenegro IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Montenegro.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Montenegro.dta"
...
save "IHME Montenegro.dta", replace
...
merge m:1 date using "JOHN Montenegro.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Montenegro data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Montenegro IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Montenegro, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Montenegro IHME daily deaths update `update'.pdf", replace	
...
* graph Montenegro IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Montenegro, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Montenegro IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Montenegro.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Montenegro.dta"
...
save "IMPE Montenegro.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Montenegro data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Montenegro IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Montenegro, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Montenegro IMPE daily deaths update `update'.pdf", replace	
...
* graph Montenegro IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Montenegro, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Montenegro IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Montenegro.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Montenegro.dta"
...
save "LANL Montenegro.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Montenegro data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Montenegro LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Montenegro, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Montenegro LANL daily deaths update `update'.pdf", replace	
...
* graph Montenegro LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Montenegro, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Montenegro LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Montenegro.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Montenegro.dta", force
...
save "ALL MODLES Montenegro.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Montenegro data dictionary.csv", replace 
...
* graph Montenegro ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Montenegro, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(644 "JOHN" 1 "DELP" 54 "IHME" 3 "IMPE" 65 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Montenegro ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Montenegro 1 Process.smcl"
