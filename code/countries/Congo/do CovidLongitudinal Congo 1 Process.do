...
log using "log CovidLongitudinal Congo 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Congo 1 Process.do"
...
**** "do CovidLongitudinal Congo 1 Process.do"
...
** output data files: "`model' Congo.dta"
**                   "ALL MODLES Congo.dta"

** output data dictionary files: "Congo ALL MODELS update dates data dictionary.csv"
**                               "`model' Congo data dictionary.csv"
**                               "ALL MODLES Congo data dictionary.csv"

** graphs:
** "graph 02 Congo `model' C19 daily deaths all updates.pdf"
** "graph 02 Congo ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Congo absent update_date_block"
	
	qui keep if loc_grand_name == "Congo"
	...
	save "Congo `model' update dates.dta", replace
...
use "Congo DELP update dates.dta", clear 

local list DELP IHME IMPE LANL

foreach model of local list {
	
	append using "Congo `model' update dates.dta"
...
save "Congo ALL MODELS update dates.dta", replace
	
export excel using "Congo ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Congo ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Congo"

keep date DayDeaMeSmJOHNCOG
...
save "JOHN Congo.dta", replace 




* graph Congo JOHN daily deaths
...
ytitle(Daily deaths) title("C19 daily deaths, Congo, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Congo JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "DELP `update' Congo.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "DELP `update' Congo.dta"
...
merge m:m date using "JOHN Congo.dta"
...
preserve
    ...
    export delimited name varlab using "DELP Congo data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Congo DELP daily deaths each update 
	...
	subtitle("Congo, forecast only") legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Congo DELP daily deaths update `update'.pdf", replace	
...
* graph Congo DELP daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Congo, DELP, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "DELP forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Congo DELP C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* graph Congo IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Congo, IHME, update `update'", size(medium) color(black)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	...
	*/
	...
	/* qui graph export "graph 00 Congo IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Congo.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Congo.dta"
...
merge m:1 date using "JOHN Congo.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Congo data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Congo IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Congo, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Congo IHME daily deaths update `update'.pdf", replace	
...
* graph Congo IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Congo, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Congo IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Congo.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Congo.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Congo data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Congo IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Congo, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Congo IMPE daily deaths update `update'.pdf", replace	
...
* graph Congo IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Congo, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Congo IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Congo.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Congo.dta"
...
preserve
    ...
    export delimited name varlab using "LANL Congo data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Congo LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Congo, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Congo LANL daily deaths update `update'.pdf", replace	
...
* graph Congo LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Congo, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Congo LANL C19 daily deaths all updates.pdf", replace
...
use "DELP Congo.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Congo.dta", force
...
save "ALL MODLES Congo.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Congo data dictionary.csv", replace 
...
* graph Congo ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Congo, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(659 "JOHN" 1 "DELP" 62 "IHME" 2 "IMPE" 40 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Congo ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Congo 1 Process.smcl"
