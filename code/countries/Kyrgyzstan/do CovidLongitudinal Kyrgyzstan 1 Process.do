...
log using "log CovidLongitudinal Kyrgyzstan 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Kyrgyzstan 1 Process.do"
...
**** "do CovidLongitudinal Kyrgyzstan 1 Process.do"
...
** output data files: "`model' Kyrgyzstan.dta"
**                   "ALL MODLES Kyrgyzstan.dta"

** output data dictionary files: "Kyrgyzstan ALL MODELS update dates data dictionary.csv"
**                               "`model' Kyrgyzstan data dictionary.csv"
**                               "ALL MODLES Kyrgyzstan data dictionary.csv"

** graphs:
** "graph 02 Kyrgyzstan `model' C19 daily deaths all updates.pdf"
** "graph 02 Kyrgyzstan ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Kyrgyzstan absent update_date_block"
	
	qui keep if loc_grand_name == "Kyrgyzstan"
	...
	save "Kyrgyzstan `model' update dates.dta", replace
...
use "Kyrgyzstan IHME update dates.dta", clear 

local list IHME IMPE LANL

foreach model of local list {
	
	append using "Kyrgyzstan `model' update dates.dta"
...
save "Kyrgyzstan ALL MODELS update dates.dta", replace
	
export excel using "Kyrgyzstan ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Kyrgyzstan ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Kyrgyzstan"

keep date DayDeaMeSmJOHNKGZ
...
save "JOHN Kyrgyzstan.dta", replace 




* graph Kyrgyzstan JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNKGZ date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Kyrgyzstan, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kyrgyzstan JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* graph Kyrgyzstan IHME daily deaths epoch each update 
	...
	ytitle(Daily deaths) title("C19 daily deaths, Kyrgyzstan, IHME, update `update'", size(medium) color(black)) ///
	...
	*/
	...
	/* qui graph export "graph 00 Kyrgyzstan IHME daily deaths epoch `update'.pdf", replace */
	...
	save "IHME `update' Kyrgyzstan.dta", replace emptyok	
...
foreach update of local list {
	...
	merge m:m date using "IHME `update' Kyrgyzstan.dta"
...
save "IHME Kyrgyzstan.dta", replace
...
merge m:1 date using "JOHN Kyrgyzstan.dta"
...
preserve
    ...
    export delimited name varlab using "IHME Kyrgyzstan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kyrgyzstan IHME daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Kyrgyzstan, forecast only") legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kyrgyzstan IHME daily deaths update `update'.pdf", replace	
...
* graph Kyrgyzstan IHME daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kyrgyzstan, IHME, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IHME forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kyrgyzstan IHME C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Kyrgyzstan.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Kyrgyzstan.dta"
...
save "IMPE Kyrgyzstan.dta", replace
...
preserve
    ...
    export delimited name varlab using "IMPE Kyrgyzstan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kyrgyzstan IMPE daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Kyrgyzstan, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kyrgyzstan IMPE daily deaths update `update'.pdf", replace	
...
* graph Kyrgyzstan IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kyrgyzstan, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kyrgyzstan IMPE C19 daily deaths all updates.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only 
	...
	save "LANL `update' Kyrgyzstan.dta", replace 
...
foreach update of local list {
	...
	merge m:m date using "LANL `update' Kyrgyzstan.dta"
...
save "LANL Kyrgyzstan.dta", replace
...
preserve
    ...
    export delimited name varlab using "LANL Kyrgyzstan data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Kyrgyzstan LANL daily deaths each update 
	...
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
	subtitle("Kyrgyzstan, forecast only") legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Kyrgyzstan LANL daily deaths update `update'.pdf", replace	
...
* graph Kyrgyzstan LANL daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kyrgyzstan, LANL, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "LANL forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kyrgyzstan LANL C19 daily deaths all updates.pdf", replace
...
use "IHME Kyrgyzstan.dta", clear

foreach model of local list {

	merge m:m date loc_grand_name using "`model' Kyrgyzstan.dta", force
...
save "ALL MODLES Kyrgyzstan.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Kyrgyzstan data dictionary.csv", replace 
...
* graph Kyrgyzstan ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Kyrgyzstan, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(344 "JOHN" 52 "IHME" 1 "IMPE" 37 "LANL") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Kyrgyzstan ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Kyrgyzstan 1 Process.smcl"
