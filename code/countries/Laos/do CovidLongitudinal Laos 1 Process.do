...
log using "log CovidLongitudinal Laos 1 Process.smcl", replace

***************************************************************************
* This is "do CovidLongitudinal Laos 1 Process.do"
...
**** "do CovidLongitudinal Laos 1 Process.do"
...
** output data files: "`model' Laos.dta"
**                   "ALL MODLES Laos.dta"

** output data dictionary files: "Laos ALL MODELS update dates data dictionary.csv"
**                               "`model' Laos data dictionary.csv"
**                               "ALL MODLES Laos data dictionary.csv"

** graphs:
** "graph 02 Laos `model' C19 daily deaths all updates.pdf"
** "graph 02 Laos ALL MODELS C19 daily deaths all updates.pdf"
...
foreach model of local list {
	...
	di in red "`model'" " Laos absent update_date_block"
	
	qui keep if loc_grand_name == "Laos"
	...
	save "Laos `model' update dates.dta", replace
...
use "Laos IMPE update dates.dta", clear 

local list IMPE

foreach model of local list {
	
	append using "Laos `model' update dates.dta"
...
save "Laos ALL MODELS update dates.dta", replace
	
export excel using "Laos ALL MODELS update dates.xlsx", replace firstrow(variables) 
...
preserve
    ...
    export delimited name varlab using "Laos ALL MODELS update dates data dictionary.csv", replace 
...
keep if loc_grand_name == "Laos"

keep date DayDeaMeSmJOHNLAO
...
save "JOHN Laos.dta", replace 




* graph Laos JOHN daily deaths

twoway ///
(line DayDeaMeSmJOHNLAO date, sort lwidth(thick) lcolor(cyan)) ///
...
ytitle(Daily deaths) title("C19 daily deaths, Laos, JOHN", size(medium) color(black)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Laos JOHN C19 daily deaths reported.pdf", replace
...
foreach update of local list {
	...
	* gen Daily deaths smooth forecast only
	...
	save "IMPE `update' Laos.dta", replace emptyok
...
foreach update of local list {
	...
	merge m:m date using "IMPE `update' Laos.dta"
...
merge m:1 date using "JOHN Laos.dta"
...
preserve
    ...
    export delimited name varlab using "IMPE Laos data dictionary.csv", replace 
...
/* foreach update of local list {
	...
	* graph Laos IMPE daily deaths each update 
	...
	subtitle("Laos, forecast only") legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) ///
	xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) 
	
	qui graph export "graph 01 Laos IMPE daily deaths update `update'.pdf", replace	
...
* graph Laos IMPE daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Laos, IMPE, all updates", size(medium) color(black)) ///
legend(position(6) order(1 "JOHN" 2 "IMPE forecast ") rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Laos IMPE C19 daily deaths all updates.pdf", replace
...
foreach model of local list {
	...
	merge m:m date loc_grand_name using "`model' Laos.dta", force
...
save "ALL MODLES Laos.dta", replace
...
preserve
    ...
    export delimited name varlab using "ALL MODLES Laos data dictionary.csv", replace 
...
* graph Laos ALL MODELS daily deaths all updates
...
, xtitle(Date) xlabel(#12, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, labsize(small) angle(forty_five) format(%30.0fc)) ///
ytitle(Daily deaths) title("C19 daily deaths, Laos, all models, all updates, forecast only", size(medium) color(black)) ///
legend(position(6) order(68 "JOHN" 1 "IMPE") ///
rows(1) size(small)) legend(region(lcolor(none))) legend(bexpand) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2))

qui graph export "graph 02 Laos ALL MODELS C19 daily deaths all updates.pdf", replace
...
view "log CovidLongitudinal Laos 1 Process.smcl"
